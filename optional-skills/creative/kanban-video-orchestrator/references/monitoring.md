...
her kanban watch --tenant <project-slug>

# Snapshot of the board
her kanban list --tenant <project-slug>
her kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
her kanban stats --tenant <project-slug>

# Visual dashboard (browser)
her dashboard

# Inspect a specific task (includes comments + events)
her kanban show <task-id>

# Follow a single task's event stream
her kanban tail <task-id>
```

Verify available subcommands with `her kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `her kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `her kanban block <id>` then `her kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `her kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `her kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `her profile list` (profile exists?), `her status` (gateway/dispatcher up?), and verify tenant. |
...
NEW_TASK_ID=$(her kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
her kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
her kanban block <task-id>
her kanban archive <task-id>

# Diagnose what happened
her kanban show <task-id>      # task body, comments, recent events
her kanban tail <task-id>      # follow the live event stream
her kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `her kanban create` for the director
...
while true; do
    clear
    her kanban list --tenant <slug>
    echo "---"
    her kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `her kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `her profile list` and start
