#!/usr/bin/env python3
"""
Monitor a running video-production kanban. Polls `her kanban list` and
...
def her_available() -> bool:
    return shutil.which("her") is not None


def kanban_list(tenant: str) -> list[dict]:
    """Returns parsed task rows. Falls back to plain stdout parsing if JSON
    output isn't supported by the installed her CLI."""
    try:
        out = subprocess.run(
            ["her", "kanban", "list", "--tenant", tenant, "--json"],
        ...
        pass
    # Fallback: textual parse of `her kanban list`
    out = subprocess.run(
        ["her", "kanban", "list", "--tenant", tenant],
        capture_output=True, text=True, check=False,
    )
    rows = []
...
def main():
    ...
    if not her_available():
        print("ERROR: 'her' CLI not found in PATH", file=sys.stderr)
        sys.exit(1)
