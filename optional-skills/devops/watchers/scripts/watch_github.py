...
Usage (via cron with --no-agent):
    ...
    her cron create her-issues \\
      --schedule "*/5 * * * *" --no-agent \\
      --script "$HER_HOME/skills/devops/watchers/scripts/watch_github.py" \\
      --script-args "--name her-issues --repo NousResearch/her-agent --scope issues"

Set GITHUB_TOKEN (or GH_TOKEN) in ~/.her/.env to avoid the 60 req/hr
