/**
 * her Dashboard Plugin SDK — typed contract (SPIKE)
 * ====================================================
 *
 * This is the public type surface for ``window.__HER_PLUGIN_SDK__`` and
 * ``window.__HER_PLUGINS__``, the globals the dashboard host exposes to
 ...
 * window.__HER_PLUGIN_SDK__`` because:
 ...
 *
 * Versioning: bump ``HerPluginSDK["sdkVersion"]`` (and the
 ...
 *   - Ship as a published ``@her/dashboard-plugin-sdk`` types package, or
 ...
 * of calling ``fetch`` with a hand-read ``window.__HER_SESSION_TOKEN__``.
 */
...
// ---------------------------------------------------------------------------
// Registry surface (window.__HER_PLUGINS__)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// SDK surface (window.__HER_PLUGIN_SDK__)
// ---------------------------------------------------------------------------

export interface HerPluginSDK {
...
declare global {
  interface Window {
    __HER_PLUGIN_SDK__?: HerPluginSDK;
    __HER_PLUGINS__?: PluginRegistry;
  }
}
