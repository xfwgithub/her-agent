...
// (e.g. https://mission-control.tilos.com/her/). The Python backend
// injects ``window.__HER_BASE_PATH__`` into index.html based on the
// incoming ``X-Forwarded-Prefix`` header so the SPA can address its own
// ``/api/...`` and ``/dashboard-plugins/...`` URLs correctly without a
// rebuild. Empty string means "served at root".
function readBasePath(): string {
  if (typeof window === "undefined") return "";
  const raw = window.__HER_BASE_PATH__ ?? "";
  if (!raw) return "";
...
export const HER_BASE_PATH = readBasePath();
const BASE = HER_BASE_PATH;
...
const SESSION_HEADER = "X-Her-Session-Token";
...
export async function fetchJSON<T>(
  ...
  if (res.status === 401) {
    ...
    // (``her update``, ``her gateway restart``, etc.). A tab kept
    // open across the restart holds the OLD token in
    // ``window.__HER_SESSION_TOKEN__`` from the previous HTML render,
    ...
    if (!window.__HER_AUTH_REQUIRED__ && !options?.allowUnauthorized) {
      let alreadyReloaded = false;
      try {
        alreadyReloaded =
          sessionStorage.getItem("her.tokenReloadAttempted") === "1";
      } catch {
        /* SSR / privacy mode — fall through to throw */
      }
      if (!alreadyReloaded) {
        try {
          sessionStorage.setItem("her.tokenReloadAttempted", "1");
        } catch {
  ...
  if (res.ok) {
    ...
    // current ``window.__HER_SESSION_TOKEN__`` is valid, so the next
    // 401 — if any — should be allowed to trigger its own reload cycle.
    try {
      sessionStorage.removeItem("her.tokenReloadAttempted");
    } catch {
      /* SSR / privacy mode — ignore */
    }
  }
...
async function getSessionToken(): Promise<string> {
  ...
  const injected = window.__HER_SESSION_TOKEN__;
  if (injected) {
    _sessionToken = injected;
    return _sessionToken;
  }
  throw new Error("Session token not available — page must be served by the her dashboard server");
}

/**
 * Fetch a single-use ticket for a WebSocket upgrade in gated mode.
 *
 * The dashboard's gated-mode WS auth (``her_cli.web_server._ws_auth_ok``)
...
export async function buildWsAuthParam(): Promise<[string, string]> {
  if (window.__HER_AUTH_REQUIRED__) {
 ...
 *  - loopback / ``--insecure``: attach the ``X-Her-Session-Token`` header.
 *  - gated OAuth: no token header (it's absent by design); the
 *    ``her_session_at`` cookie rides along via ``credentials: 'include'``.
 *
 ...
 * ``window.__HER_SESSION_TOKEN__`` directly, so the gated-mode ticket
...
export const api = {
  ...
  updateher: () =>
    fetchJSON<ActionResponse>("/api/her/update", { method: "POST" }),
  checkherUpdate: (force = false) =>
    fetchJSON<UpdateCheckResponse>(
      `/api/her/update/check${force ? "?force=true" : ""}`,
    ),
...
export interface SkillHubSource {
  ...
  /** her-index only: whether the centralized index loaded. */
  available?: boolean;
}
