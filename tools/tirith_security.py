...
it is automatically downloaded from GitHub releases to $HER_HOME/bin/tirith.
...
def _get_her_home() -> str:
    """Return the Hermes home directory, respecting HER_HOME env var."""
    return str(get_her_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_her_home(), ".tirith-install-failed")
...
def _her_bin_dir() -> str:
    """Return $HER_HOME/bin, creating it if needed."""
    d = os.path.join(_get_her_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $HER_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_her_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $HER_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $HER_HOME/bin/tirith
    ...
    her_bin = os.path.join(_her_bin_dir(), "tirith")
    if os.path.isfile(her_bin) and os.access(her_bin, os.X_OK):
        _resolved_path = her_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return her_bin
