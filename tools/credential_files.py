...
def register_credential_file(
    relative_path: str,
    container_base: str = "/root/.her",
) -> bool:
    """Register a credential file for mounting into remote sandboxes.

    *relative_path* is relative to ``HER_HOME`` (e.g. ``google_token.json``).
    ...
    The resolved host path must remain inside HER_HOME so that a malicious
    skill cannot declare ``required_credential_files: ['../../.ssh/id_rsa']``
    and exfiltrate sensitive host files into a container sandbox.
    """
    her_home = _resolve_her_home()

    # Reject absolute paths — they bypass the HER_HOME sandbox entirely.
    if os.path.isabs(relative_path):
        logger.warning(
            "credential_files: rejected absolute path %r (must be relative to HER_HOME)",
            relative_path,
        )
        return False

    host_path = her_home / relative_path

    # Resolve symlinks and normalise ``..`` before the containment check so
    # that traversal like ``../. ssh/id_rsa`` cannot escape HER_HOME.
    from tools.path_security import validate_within_dir

    containment_error = validate_within_dir(host_path, her_home)
...
def _safe_skills_path(skills_dir: Path) -> str:
    ...
    safe_dir = Path(tempfile.mkdtemp(prefix="her-skills-safe-"))
...
# Each tuple is (new_subpath, old_name) matching her_constants.get_her_dir().
...
def get_cache_directory_mounts(
    ...
    ``get_her_dir()`` for backward compatibility with old directory layouts.
    """
    from her_constants import get_her_dir

    mounts: List[Dict[str, str]] = []
    for new_subpath, old_name in _CACHE_DIRS:
        host_dir = get_her_dir(new_subpath, old_name)
        if host_dir.is_dir():
...
def map_cache_path_to_container(
    ...
    caller decides which ``container_base`` applies (Docker ``/root/.her``,
    SSH ``<remote_home>/.her``, etc.) and whether translation is wanted.
