...
remediation hint pointing at ``her tools`` or the manual pip command.
...
def _is_satisfied(spec: str) -> bool:
    ...
    ``her update`` propagate pin bumps in :data:`LAZY_DEPS` to already-
...
def _venv_pip_install(specs: tuple[str, ...], *, timeout: int = 300) -> _InstallResult:
    ...
    Mirrors the strategy in ``her_cli.tools_config._pip_install`` but
...
def active_features() -> list[str]:
    ...
    Used by ``her update`` to figure out which lazy backends need a
...
def refresh_active_features(*, prompt: bool = False) -> dict[str, str]:
    ...
    Intended for ``her update``. Never raises; lazy-install failures
