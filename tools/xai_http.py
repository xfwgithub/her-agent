...
def has_xai_credentials() -> bool:
    ...
    hot-paint paths (``her tools`` repaint, tool-registration scans,
    ...
    2. ``~/.her/auth.json`` has a non-empty ``providers.xai-oauth.tokens.access_token``
    ...
    try:
        ...
        auth_path = get_her_home() / "auth.json"
...
def get_env_value(name: str, default=None):
    """Read ``name`` from ``~/.her/.env`` first, then ``os.environ``.

    Wraps :func:`her_cli.config.get_env_value` so tests can patch
    ``tools.xai_http.get_env_value`` to inject dotenv-only secrets into the
    xAI credential resolver.
    """
    try:
        from her_cli.config import get_env_value as _her_get_env_value

        value = _her_get_env_value(name)
...
def her_xai_user_agent() -> str:
    """Return a stable her-specific User-Agent for xAI HTTP calls."""
    try:
        from her_cli import __version__
    except Exception:
        __version__ = "unknown"
    return f"her-Agent/{__version__}"


def resolve_xai_http_credentials(*, force_refresh: bool = False) -> Dict[str, str]:
    """Resolve bearer credentials for direct xAI HTTP endpoints.

    Prefers her-managed xAI OAuth credentials when available, then falls back
    to ``XAI_API_KEY`` resolved via ``her_cli.config.get_env_value`` so keys
    stored in ``~/.her/.env`` (the standard her location) are honored —
