...
running under the dispatcher (env var ``HER_KANBAN_TASK`` set) or when
the active profile explicitly enables the ``kanban`` toolset for
orchestrator work. A normal ``her chat`` session still sees **zero**
kanban tools in its schema unless configured.

Why tools instead of just shelling out to ``her kanban``?

1. **Backend portability.** A worker whose terminal tool points at Docker
   / Modal / Singularity / SSH would run ``her kanban complete …``
   inside the container, where ``her`` isn't installed and the DB
   isn't mounted. Tools run in the agent's Python process, so they
   always reach ``~/.her/kanban.db`` regardless of terminal backend.
...
Humans continue to use the CLI (``her kanban …``), the dashboard
(``her dashboard``), and the slash command (``/kanban …``) — all
...
def _check_kanban_mode() -> bool:
    ...
    1. ``HER_KANBAN_TASK`` is set (dispatcher-spawned worker), OR
    2. The current profile has ``kanban`` in its toolsets config
       (orchestrator profiles like techlead that route work via Kanban).

    Humans running ``her chat`` without the kanban toolset see zero
...
def _default_task_id(arg: Optional[str]) -> Optional[str]:
    ...
    if arg:
        return arg
    env_tid = os.environ.get("HER_KANBAN_TASK")
...
def _enforce_worker_task_ownership(tid: str) -> Optional[str]:
    ...
    A process spawned by the dispatcher has ``HER_KANBAN_TASK`` set
    ...
    ``HER_KANBAN_TASK`` in env) aren't subject to this check — their
...
#   - No-op outside dispatcher-spawned worker context (no ``HER_KANBAN_TASK``).
...
def heartbeat_current_worker_from_env() -> bool:
    ...
    tid = os.environ.get("HER_KANBAN_TASK")
    if not tid:
        return False
...
def _handle_show(args: dict, **kw) -> str:
    ...
    if not tid:
        return tool_error(
            "task_id is required (or set HER_KANBAN_TASK in the env)"
        )
...
def _handle_comment(args: dict, **kw) -> str:
    ...
    # an authoritative-looking name like ``her-system`` and poison
    # the future-worker context with what reads as a system directive.
    # Cross-task commenting itself remains unrestricted (see #19713) —
    # comments are the deliberate handoff channel between tasks.
    author = os.environ.get("HER_PROFILE") or "worker"
    board = args.get("board")
    try:
        kb, conn = _connect(board=board)
        try:
...
def _handle_create(args: dict, **kw) -> str:
    ...
    # (kanban toolset, no HER_KANBAN_TASK) and CLI/dashboard callers
    ...
    try:
        ...
        try:
            ...
            if _inherit_workspace:
                _self_tid = os.environ.get("HER_KANBAN_TASK")
                if _self_tid:
            ...
            new_tid = kb.create_task(
                ...
                created_by=os.environ.get("HER_PROFILE") or "worker",
...
_DESC_TASK_ID_DEFAULT = (
    "Task id. If omitted, defaults to HER_KANBAN_TASK from the env "
