...
# All skills live in ~/.her/skills/ (seeded from bundled skills/ on install).
...
def load_env() -> Dict[str, str]:
    """Load profile-scoped environment variables from HER_HOME/.env."""
    env_path = get_her_home() / ".env"
...
def _gateway_setup_hint() -> str:
    try:
        ...
        return f"Secure secret entry is not available. Load this skill in the local CLI to be prompted, or add the key to {display_her_home()}/.env manually."
...
def _get_category_from_path(skill_path: Path) -> Optional[str]:
    ...
    For paths like: ~/.her/skills/mlops/axolotl/SKILL.md -> "mlops"
...
def _find_all_skills(*, skip_disabled: bool = False) -> List[Dict[str, Any]]:
    """Recursively find all skills in ~/.her/skills/ and external dirs.

    Args:
        skip_disabled: If True, return ALL skills regardless of disabled
            state (used by ``her skills`` config UI). Default False
            ...
            return json.dumps(
                {
                    ...
                    "skills": [],
                    "categories": [],
                    "message": f"No skills found. Skills directory created at {display_her_home()}/skills/",
                },
                ensure_ascii=False,
            )
        ...
        return json.dumps(
            {
                ...
                "error": (
                    ...
                    f"Re-enable with: her plugins enable {namespace}"
                ),
            },
            ensure_ascii=False,
        )
    ...
    try:
        ...
        if _outside_skills_dir or _injection_detected:
            ...
            if _outside_skills_dir:
                _warnings.append(f"skill file is outside the trusted skills directory (~/.her/skills/): {skill_md}")
            ...
            return json.dumps(
                {
                    ...
                    "error": (
                        ...
                        "Enable it with `her skills` or inspect the files directly on disk."
                    ),
                },
                ensure_ascii=False,
            )
        ...
        # Check metadata.her.* first (agentskills.io convention), fall back to top-level
        her_meta = {}
        metadata = frontmatter.get("metadata")
        if isinstance(metadata, dict):
            her_meta = metadata.get("her", {}) or {}

        tags = _parse_tags(her_meta.get("tags") or frontmatter.get("tags", ""))
        related_skills = _parse_tags(
            her_meta.get("related_skills") or frontmatter.get("related_skills", "")
        )
