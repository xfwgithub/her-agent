...
_YOLO_MODE_FROZEN: bool = is_truthy_value(os.getenv("HER_YOLO_MODE", ""))
...
def _fire_approval_hook(hook_name: str, **kwargs) -> None:
    ...
    """
    try:
        from her_cli.plugins import invoke_hook
    except Exception:
...
def get_current_session_key(default: str = "default") -> str:
    ...
    return get_session_env("HER_SESSION_KEY", default)
...
def _get_session_platform() -> str:
    ...
    try:
        ...
        return os.getenv("HER_SESSION_PLATFORM", "") or ""


def _is_gateway_approval_context() -> bool:
    """True when this call is inside a gateway/API session.

    Legacy gateway integrations set HER_GATEWAY_SESSION in process env.
    Newer concurrent gateway paths bind HER_SESSION_PLATFORM via
    ...
    from a gateway platform (cron binds HER_SESSION_PLATFORM via
    ...
    """
    if env_var_enabled("HER_CRON_SESSION"):
        return False
...
# via shell expansions like $HOME or $HER_HOME.
...
# ~/.her/config.yaml IS the security policy: approvals.mode, yolo, and the
...
# theater. Mirrors _HER_ENV_PATH; matches the HER_HOME override form as
# well as ~/.her/.
_HER_CONFIG_PATH = (
    r'(?:~\/\.her/|'
    r'(?:\$home|\$\{home\})/\.her/|'
...
DANGEROUS_PATTERNS = [
    ...
    (r'\bher\s+gateway\s+(stop|restart)\b', "stop/restart her gateway (kills running agents)"),
    (r'\bher\s+update\b', "her update (restarts gateway, kills running agents)"),
    # Docker container lifecycle — any user with docker.sock mounted (a common
    # Docker Compose pattern) gives the agent the ability to restart/stop/kill
    # containers without approval.  These are agent-initiated lifecycle operations
    # that should always require user consent, just like `her gateway restart`
    ...
    (r'gateway\s+run\b.*(&\s*$|&\s*;|\bdisown\b|\bsetsid\b)', "start gateway outside systemd (use 'systemctl --user restart her-gateway')"),
    (r'\bnohup\b.*gateway\s+run\b', "start gateway outside systemd (use 'systemctl --user restart her-gateway')"),
    # Self-termination protection: prevent agent from killing its own process
    (r'\b(pkill|killall)\b.*\b(her|gateway|cli\.py)\b', "kill her/gateway process (self-termination)"),
    # Self-termination via kill + command substitution (pgrep/pidof).
    # The name-based pattern above catches `pkill her` but not
    # `kill -9 $(pgrep -f her)` because the substitution is opaque
    ...
    # In-place edit of a her-managed security file (~/.her/config.yaml or
    ...
    (rf'\bsed\s+-[^\s]*i.*(?:{_HER_CONFIG_PATH}|{_HER_ENV_PATH})', "in-place edit of her config/env"),
    (rf'\bsed\s+--in-place\b.*(?:{_HER_CONFIG_PATH}|{_HER_ENV_PATH})', "in-place edit of her config/env (long flag)"),
    ...
    (rf'\b(?:perl|ruby)\b.*(?:^|\s)-[^\s]*i\b.*(?:{_HER_CONFIG_PATH}|{_HER_ENV_PATH})', "in-place edit of her config/env (perl/ruby)"),
...
def check_dangerous_command(command: str, env_type: str,
    ...
    is_cli = env_var_enabled("HER_INTERACTIVE")
    ...
    if not is_cli and not is_gateway:
        ...
        logger.warning(
            ...
            "(pattern: %s): %s — set HER_INTERACTIVE or HER_GATEWAY_SESSION to require approval.",
            description, command[:200],
        )
        return {"approved": True, "message": None}

    if is_gateway or env_var_enabled("HER_EXEC_ASK"):
...
def check_all_command_guards(command: str, env_type: str,
    ...
    is_cli = env_var_enabled("HER_INTERACTIVE")
    is_gateway = _is_gateway_approval_context()
    is_ask = env_var_enabled("HER_EXEC_ASK")
