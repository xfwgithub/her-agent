...
# shellcheck shell=sh
# /opt/her/docker/main-wrapper.sh — wraps the container's CMD with
# the same argument-routing logic the pre-s6 entrypoint.sh used. Runs
# as /init's "main program" (Docker CMD) so it inherits stdin/stdout/
# stderr from the container.
#
# Shebang note: /init scrubs env before invoking CMD, so a plain
# `#!/bin/sh` wrapper sees an empty environ and `ENV HER_HOME=/opt/data`
# from the Dockerfile never reaches `her`. with-contenv repopulates
# the env from /run/s6/container_environment before exec'ing, which is
# what s6-supervised services use too (see main-her/run).
#
# Routing:
#   no args                       → exec `her` (the default)
#   first arg is an executable    → exec it directly (sleep, bash, sh, …)
#   first arg is anything else    → exec `her <args>` (subcommand passthrough)
#
# Drop to her via s6-setuidgid, but skip it when already non-root.
set -e

drop() { [ "$(id -u)" = 0 ] && set -- s6-setuidgid her "$@"; exec "$@"; }
...
# an arbitrary non-root, non-her UID, the bootstrap was skipped and the
# baked image dirs (owned by the her build UID) are unwritable, so fail
...
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u her)" ]; then
    cat >&2 <<EOF
[her] ERROR: container started with --user $cur_uid (an arbitrary, non-her UID) — not supported.
...
# to the her user's home before dropping privileges so libraries that
...
# the user's command so `her chat` starts in the Docker -w
