"""Centralized logging setup for her Agent.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.her/logs/`` (profile-aware via ``get_her_home()``).
...
Component separation:
    ...
    gui.log receives dashboard-side records from ``her_cli.web_server``,
    ``her_cli.pty_bridge``, ``tui_gateway.*``, and ``uvicorn.*``.
...
from her_constants import get_config_path, get_her_home
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_her_session_injector", False):
    ...
    _session_record_factory._her_session_injector = True  # type: ignore[attr-defined]
...
# Used by _ComponentFilter and exposed for ``her logs --component``.
COMPONENT_PREFIXES = {
    "gateway": ("gateway", "her_plugins"),
...
def setup_logging(
    *,
    ...
    """Configure the her logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    her_home
        Override for the her home directory.  Falls back to
        ``get_her_home()`` (profile-aware).
    log_level
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_her_verbose", False):
                return

    handler = logging.StreamHandler(_safe_stderr())
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._her_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    1.  In managed mode (NixOS), the stateDir uses setgid (2770) so new files
        inherit the her group. However, both ``_open()`` (initial creation)
