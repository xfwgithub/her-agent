...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      her-agent = self'.packages.default;
      HerVenv = her-agent.HerVenv;

      configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

      # Auto-generated config key reference — always in sync with Python
      configKeys = pkgs.runCommand "her-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${HerVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "her-cross-eval" { } (
          if failures != [] then
        ...
        build-package = pkgs.runCommand "her-build-package" { } ''
          echo "PASS: package built at ${her-agent}"
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify the devShell builds successfully (cross-platform).
        build-devshell = pkgs.runCommand "her-build-devshell" { } ''
        ...
        package-contents = pkgs.runCommand "her-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${her-agent}/bin/her || (echo "FAIL: her binary missing"; exit 1)
          test -x ${her-agent}/bin/her-agent || (echo "FAIL: her-agent binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${her-agent}/bin/her version 2>&1 | grep -qi "her" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "her-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in her her-agent her-acp; do
            test -x ${her-agent}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "her-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking her --help ==="
          ${her-agent}/bin/her --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${her-agent}/bin/her --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "her-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${her-agent}/share/her-agent/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          SKILL_COUNT=$(find ${her-agent}/share/her-agent/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "HER_BUNDLED_SKILLS" ${her-agent}/bin/her || \
            (echo "FAIL: HER_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: HER_BUNDLED_SKILLS set in wrapper"
        ...
        bundled-plugins = pkgs.runCommand "her-bundled-plugins" { } ''
          set -e
          echo "=== Checking bundled plugins ==="
          test -d ${her-agent}/share/her-agent/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${her-agent}/share/her-agent/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "HER_BUNDLED_PLUGINS" ${her-agent}/bin/her || \
            (echo "FAIL: HER_BUNDLED_PLUGINS not in wrapper"; exit 1)
          echo "PASS: HER_BUNDLED_PLUGINS set in wrapper"
        ...
        bundled-locales = pkgs.runCommand "her-bundled-locales" { } ''
          set -e
          echo "=== Checking bundled locales ==="
          test -d ${her-agent}/share/her-agent/locales || (echo "FAIL: locales directory missing"; exit 1)
          echo "PASS: locales directory exists"

          LOC_COUNT=$(find ${her-agent}/share/her-agent/locales -name "*.yaml" | wc -l)
          test "$LOC_COUNT" -ge 16 || (echo "FAIL: expected >=16 catalogs, found $LOC_COUNT"; exit 1)
          echo "PASS: $LOC_COUNT locale catalogs found"

          test -f ${her-agent}/share/her-agent/locales/en.yaml || (echo "FAIL: en.yaml missing"; exit 1)
          echo "PASS: en.yaml present"

          grep -q "HER_BUNDLED_LOCALES" ${her-agent}/bin/her || \
            (echo "FAIL: HER_BUNDLED_LOCALES not in wrapper"; exit 1)
          echo "PASS: HER_BUNDLED_LOCALES set in wrapper"

          echo "=== Rendering via the wrapper override (HER_BUNDLED_LOCALES) ==="
          export HOME=$(mktemp -d)
          RENDERED=$(cd "$HOME" && HER_BUNDLED_LOCALES=${her-agent}/share/her-agent/locales \
            ${HerVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "rendered: $RENDERED"
          test "$RENDERED" != "gateway.reset.header_default" || (echo "FAIL: i18n returned the raw key with HER_BUNDLED_LOCALES set"; exit 1)
          ...
          BARE_DIR=$(cd "$HOME" && ${HerVenv}/bin/python3 -c "from agent import i18n; print(i18n._locales_dir())")
          BARE=$(cd "$HOME" && ${HerVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "resolved dir (no env var): $BARE_DIR"
          echo "rendered: $BARE"
          test "$BARE" != "gateway.reset.header_default" || \
            (echo "FAIL: sealed venv could not resolve locales without HER_BUNDLED_LOCALES — data-files materialization regressed"; exit 1)
        ...
        bundled-tui = pkgs.runCommand "her-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${her-agent}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${her-agent}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          # self-contained bundle; no runtime node_modules expected

          grep -q "HER_TUI_DIR" ${her-agent}/bin/her || \
            (echo "FAIL: HER_TUI_DIR not in wrapper"; exit 1)
          echo "PASS: HER_TUI_DIR set in wrapper"
        ...
        # Verify HER_NODE is set in wrapper and points to Node 20+
        # (string-width uses the /v regex flag which requires Node 20+)
        her-node = pkgs.runCommand "her-node-version" { } ''
          set -e
          echo "=== Checking HER_NODE in wrapper ==="
          grep -q "HER_NODE" ${her-agent}/bin/her || \
            (echo "FAIL: HER_NODE not set in wrapper"; exit 1)
          echo "PASS: HER_NODE present in wrapper"

          HER_NODE=$(sed -n "s/^export HER_NODE='\(.*\)'/\1/p" ${her-agent}/bin/her)
          test -x "$HER_NODE" || (echo "FAIL: HER_NODE=$HER_NODE not executable"; exit 1)
          echo "PASS: HER_NODE executable at $HER_NODE"

          NODE_MAJOR=$("$HER_NODE" --version | sed 's/^v//' | cut -d. -f1)
          test "$NODE_MAJOR" -ge 20 || \
            (echo "FAIL: Node v$NODE_MAJOR < 20, TUI needs /v regex flag support"; exit 1)
          echo "PASS: Node v$NODE_MAJOR >= 20"

          echo "=== All HER_NODE checks passed ==="
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify HER_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "her-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(HER_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking HER_MANAGED guards ==="
          check_blocked "config set" ${her-agent}/bin/her config set model foo
          check_blocked "config edit" ${her-agent}/bin/her config edit
        ...
        extra-python-packages = let
          ...
          HerWithExtra = her-agent.override {
            extraPythonPackages = [ testPkg ];
          };
        in pkgs.runCommand "her-extra-python-packages" { } ''
          set -e
          echo "=== Checking extraPythonPackages PYTHONPATH injection ==="

          grep -q "PYTHONPATH" ${HerWithExtra}/bin/her || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${HerWithExtra}/bin/her || \
          ...
          if grep -q "PYTHONPATH" ${her-agent}/bin/her; then
        ...
        extra-dependency-groups = let
          HerWithGroups = her-agent.override {
            extraDependencyGroups = [ "honcho" ];
          };
        in pkgs.runCommand "her-extra-dependency-groups" { } ''
          set -e
          ...
          echo "derivation: ${HerWithGroups}"
          echo "venv: ${HerWithGroups.HerVenv}"
        ...
        messaging-variant = pkgs.runCommand "her-messaging-variant" { } ''
          set -e
          echo "=== Checking discord.py importable from messaging variant ==="
          ${self'.packages.messaging.HerVenv}/bin/python3 -c \
        ...
        in pkgs.runCommand "her-config-roundtrip" {
          ...
          merge_and_load() {
            local her_home="$1"
            export HER_HOME="$her_home"
            ${configMergeScript} ${nixSettings} "$her_home/config.yaml"
            ${HerVenv}/bin/python3 -c '
import json, sys
