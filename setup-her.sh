...
# 5. Symlinks the 'her' CLI command into a user-facing bin dir
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/her-uv-install.$$.log")"
...
# ============================================================================
# PATH setup — symlink her into a user-facing bin dir
# ============================================================================
...
echo "Other commands:"
echo "  her status        # Check configuration"
if is_termux; then
    echo "  her gateway       # Run gateway in foreground"
else
    echo "  her gateway install # Install gateway service (messaging + cron)"
fi
echo "  her cron list     # View scheduled jobs"
echo "  her doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m her_cli.main setup
fi
