...
See: https://github.com/NousResearch/her-agent/issues/4426
"""
...
class TestGetSubprocessHome:
    """Unit tests for her_constants.get_subprocess_home()."""

    def test_returns_none_when_her_home_unset(self, monkeypatch):
    ...
    def test_two_profiles_get_different_homes(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("HER_HOME", str(base / "alpha"))
        home_a = get_subprocess_home()

        monkeypatch.setenv("HER_HOME", str(base / "beta"))
    ...
    def test_context_override_is_thread_local(self, tmp_path, monkeypatch):
        ...
        def read_from_other_thread():
            ...
            seen.append(str(get_her_home()))

        thread = threading.Thread(target=read_from_other_thread)
        thread.start()
        assert ready.wait(timeout=5)

        token = set_her_home_override(profile)
        try:
            assert get_her_home() == profile
            release.set()
...
class TestMakeRunEnvHomeInjection:
    ...
    def test_injects_home_when_profile_home_exists(self, tmp_path, monkeypatch):
        ...
        assert result["HOME"] == str(her_home / "home")
    ...
    def test_no_injection_when_her_home_unset(self, monkeypatch):
        monkeypatch.delenv("HER_HOME", raising=False)
    ...
    def test_context_override_bridges_to_subprocess_env(self, tmp_path, monkeypatch):
        ...
        assert result["HER_HOME"] == str(profile)
...
class TestProfileBootstrap:
    ...
    def test_profile_dirs_includes_home(self):
        from her_cli.profiles import _PROFILE_DIRS
