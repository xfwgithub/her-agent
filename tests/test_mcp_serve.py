"""
Tests for mcp_serve — her MCP server.
...
def _isolate_her_home(tmp_path, monkeypatch):
    """Redirect HER_HOME to a temp directory."""
    monkeypatch.setenv("HER_HOME", str(tmp_path))
    try:
        import her_constants
        monkeypatch.setattr(her_constants, "get_her_home", lambda: tmp_path)
...
def _create_test_db(db_path, session_id, messages):
    """Create a minimal SQLite DB mimicking her_state schema."""
