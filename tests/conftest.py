"""Shared fixtures for the her-agent test suite.
...
2. **Isolated HER_HOME.** HER_HOME points to a per-test tempdir so
   code reading ``~/.her/*`` via ``get_her_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".her"`` instead of the canonical
   ``get_her_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No HER_SESSION_* inheritance** — the agent's current gateway
...
# HER_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_HER_BEHAVIORAL_VARS = frozenset({
    "HER_YOLO_MODE",
    "HER_INTERACTIVE",
    "HER_QUIET",
    "HER_TOOL_PROGRESS",
    ...
    "HER_SESSION_CHAT_ID",
    "HER_SESSION_CHAT_NAME",
    "HER_SESSION_THREAD_ID",
    "HER_SESSION_SOURCE",
    "HER_SESSION_KEY",
    "HER_GATEWAY_SESSION",
    "HER_CRON_SESSION",
    ...
    "HER_TIMEZONE",
    "HER_REDACT_SECRETS",
    "HER_BACKGROUND_NOTIFICATIONS",
    "HER_EXEC_ASK",
    "HER_HOME_MODE",
    "HER_AGENT_USE_LEGACY_SESSION_KEYS",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.her/kanban.db instead of the per-test HER_HOME.
    "HER_KANBAN_DB",
    "HER_KANBAN_BOARD",
    "HER_KANBAN_HOME",
    "HER_KANBAN_WORKSPACES_ROOT",
    "HER_KANBAN_LOGS_ROOT",
    "HER_KANBAN_TASK",
    "HER_KANBAN_WORKSPACE",
    "HER_KANBAN_RUN_ID",
    "HER_KANBAN_CLAIM_LOCK",
    "HER_KANBAN_DISPATCH_IN_GATEWAY",
    "HER_TENANT",
    ...
    "HER_DASHBOARD_OAUTH_CLIENT_ID",
    "HER_DASHBOARD_PORTAL_URL",
    "TERMINAL_CWD",
    "TERMINAL_ENV",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and HER_HOME to per-test tempdirs so code that
    reads ``~/.her/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral HER_* vars that could change test semantics.
    for name in _HER_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)

    # 3. Redirect HER_HOME to a per-test tempdir. Code that reads
    #    ``~/.her/*`` via ``get_her_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.her/*`` via
    #    ``Path.home() / ".her"`` instead of ``get_her_home()``
    #    is a bug to fix at the callsite.
    fake_her_home = tmp_path / "her_test"
    fake_her_home.mkdir()
    (fake_her_home / "sessions").mkdir()
    (fake_her_home / "cron").mkdir()
    (fake_her_home / "memories").mkdir()
    (fake_her_home / "skills").mkdir()
    monkeypatch.setenv("HER_HOME", str(fake_her_home))
    ...
    #    ~/.her/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import her_cli.plugins as _plugins_mod
...
@pytest.fixture(autouse=True)
def _isolate_her_home(_hermetic_environment):
...
@pytest.fixture()
def mock_config():
    """Return a minimal her config dict suitable for unit tests."""
    return {
...
# environment and finds the developer's live ``her-gateway`` process
...
# correlated with a ``tests/her_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> her-gateway``
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart her-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting her/python patterns are also blocked.
    """
    ...
    def _matches_her_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
        return any(tok in low for tok in _HER_TOKENS)

    def _is_blocked_systemctl(cmd) -> bool:
        cmd_str = _cmd_to_string(cmd)
        if "systemctl" not in cmd_str:
            return False
        if not _matches_her_gateway(cmd_str):
            return False
        try:
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch her-themed patterns + a
                # plain "python" -f which would catch the live gateway
                # whose cmdline contains "python -m her_cli.main".
                if (
                    "her" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live her-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting her/python could hit the live gateway. "
