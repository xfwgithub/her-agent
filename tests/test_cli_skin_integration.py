...
from cli import HerCLI, _build_compact_banner, _rich_text_from_ansi
...
class TestCompactBannerSkinIntegration:
    def test_default_compact_banner_keeps_legacy_nous_her_branding(self):
        set_active_skin("default")

        with patch("cli.shutil.get_terminal_size", return_value=SimpleNamespace(columns=90)), \
             patch.dict(_build_compact_banner.__globals__, {"format_banner_version_label": lambda: "her Agent v0.1.0 (test)"}):
            banner = _build_compact_banner()

        assert "NOUS HER" in banner

    def test_poseidon_compact_banner_uses_skin_branding_instead_of_nous_her(self):
    ...
    def test_compact_banner_shows_version_label(self):
        ...
        with patch("cli.shutil.get_terminal_size", return_value=SimpleNamespace(columns=90)), \
             patch.dict(_build_compact_banner.__globals__, {"format_banner_version_label": lambda: "her Agent v1.0 (test) · upstream abc12345"}):
            banner = _build_compact_banner()
