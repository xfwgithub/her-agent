...
class TestToolsetConsistency:
    ...
    def test_her_platforms_share_core_tools(self):
        """All her-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on her-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["her-cli", "her-telegram", "her-discord", "her-whatsapp", "her-slack", "her-signal", "her-homeassistant"]
...
class TestDefaultPlatformWebSearchCoverage:
    def test_her_whatsapp_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("her-whatsapp")

    def test_her_api_server_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("her-api-server")
