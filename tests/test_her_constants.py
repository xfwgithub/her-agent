"""Tests for her_constants module."""
...
class TestGetDefaultherRoot:
    ...
    def test_no_her_home_returns_native(self, tmp_path, monkeypatch):
        """When HER_HOME is not set, returns ~/.her."""
        monkeypatch.delenv("HER_HOME", raising=False)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        assert get_default_her_root() == tmp_path / ".her"

    def test_her_home_is_native(self, tmp_path, monkeypatch):
        """When HER_HOME = ~/.her, returns ~/.her."""
        native = tmp_path / ".her"
        native.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("HER_HOME", str(native))
        assert get_default_her_root() == native

    def test_her_home_is_profile(self, tmp_path, monkeypatch):
        """When HER_HOME is a profile under ~/.her, returns ~/.her."""
        native = tmp_path / ".her"
    ...
    def test_her_home_is_docker(self, tmp_path, monkeypatch):
        """When HER_HOME points outside ~/.her (Docker), returns HER_HOME."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("HER_HOME", str(docker_home))
        assert get_default_her_root() == docker_home

    def test_her_home_is_custom_path(self, tmp_path, monkeypatch):
        """Any HER_HOME outside ~/.her is treated as the root."""
        custom = tmp_path / "my-her-data"
        custom.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("HER_HOME", str(custom))
        assert get_default_her_root() == custom

    def test_docker_profile_active(self, tmp_path, monkeypatch):
        """When a Docker profile is active (HER_HOME=<root>/profiles/<name>),
...
class TestIsContainer:
    ...
    def _reset_cache(self, monkeypatch):
        ...
        monkeypatch.setattr(her_constants, "_container_detected", None)
    ...
    def test_caches_result(self, monkeypatch):
        ...
        monkeypatch.setattr(her_constants, "_container_detected", True)
        assert is_container() is True
...
class TestSecureParentDir:
    ...
    def test_safe_path_calls_chmod(self, tmp_path, monkeypatch):
        ...
        safe_dir = tmp_path / "home" / "user" / ".her"
