...
def test_session_search_lazily_opens_db_when_entrypoint_did_not_pass_one(monkeypatch):
    ...
    her_state = ModuleType("her_state")
    her_state.SessionDB = FakeSessionDB
    monkeypatch.setitem(sys.modules, "her_state", her_state)
    ...
    result = json.loads(agent._invoke_tool("session_search", {"query": "her"}, "task-id"))

    assert result["success"] is True
    assert captured["db"] is sentinel_db
    assert captured["query"] == "her"
