...
HER_HOME scoping, and tick() serialization for profile jobs.
"""
...
class TestCronjobToolProfile:
    ...
    def test_schema_advertises_profile(self):
        ...
        assert "her profile" in desc_lower
        assert "context-local" in desc_lower
        assert "subprocess" in desc_lower
        assert "temporarily sets her_home" not in desc_lower
...
class TestRunJobProfileContext:
    ...
    def _install_agent_stubs(monkeypatch, observed: dict):
        ...
        class FakeAgent:
            def __init__(self, **kwargs):
                ...
                observed["env_home_during_init"] = os.environ.get("HER_HOME")
                observed["profile_env_only_during_init"] = os.environ.get(
                    "HER_PROFILE_TEST_ONLY"
                )
                observed["profile_env_shared_during_init"] = os.environ.get(
                    "HER_PROFILE_TEST_SHARED"
                )
                observed["her_home_during_init"] = str(get_her_home())
                observed["scheduler_home_during_init"] = str(sched._get_her_home())
                observed["skip_context_files"] = kwargs.get("skip_context_files")

            def run_conversation(self, *_a, **_kw):
                from her_constants import get_her_home

                observed["env_home_during_run"] = os.environ.get("HER_HOME")
                ...
                observed["her_home_during_run"] = str(get_her_home())
                observed["scheduler_home_during_run"] = str(sched._get_her_home())
        ...
        from her_cli import runtime_provider as runtime_provider
    ...
    def test_run_job_sets_and_restores_profile_home(
        ...
        assert observed["her_home_during_init"] == str(profile_home.resolve())
        assert observed["her_home_during_run"] == str(profile_home.resolve())
    ...
    def test_profile_dotenv_environment_is_restored(
        ...
        observed: dict = {}
        self._install_agent_stubs(monkeypatch, observed)
        monkeypatch.setenv("HER_PROFILE_TEST_SHARED", "outer")
        monkeypatch.delenv("HER_PROFILE_TEST_ONLY", raising=False)
    ...
    def test_run_job_without_profile_leaves_her_home_untouched(
        self, isolated_cron_profile_home, monkeypatch
    ):
        import cron.scheduler as sched
        ...
        assert observed["her_home_during_init"] == str(root)
        assert os.environ["HER_HOME"] == str(root)
