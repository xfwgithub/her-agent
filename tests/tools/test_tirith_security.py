...
class TestEnsureInstalled:
    ...
    def test_not_found_returns_none(self, mock_cfg):
        ...
        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._her_bin_dir", return_value="/nonexistent"), \
...
class TestInstallArchiveMemberValidation:
    ...
    def test_install_extracts_regular_tirith_member(self, mock_target, mock_which,
        ...
        assert reason == ""
        assert path == str(her_home / "bin" / "tirith")
    ...
    def test_install_rejects_non_regular_tirith_member(self, mock_target, mock_which,
        ...
        assert not os.path.lexists(her_home / "bin" / "tirith")
...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_her_bin(self):
        """After _INSTALL_FAILED, manual install in HER_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
        her_bin = os.path.join(tmpdir, "tirith")
        # Create a fake executable
        with open(her_bin, "w") as f:
            f.write("#!/bin/sh\n")
        os.chmod(her_bin, 0o755)

        _tirith_mod._resolved_path = _INSTALL_FAILED

        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._her_bin_dir", return_value=tmpdir), \
             patch("tools.tirith_security._clear_install_failed") as mock_clear:
            result = _resolve_tirith_path("tirith")
            assert result == her_bin
            assert _tirith_mod._resolved_path == her_bin
            mock_clear.assert_called_once()
...
class TestherHomeIsolation:
    def test_her_bin_dir_respects_her_home(self):
        ...
        import tempfile
        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"HER_HOME": tmpdir}):
            result = _her_bin_dir()
        assert result == os.path.join(tmpdir, "bin")
        assert os.path.isdir(result)

    def test_failure_marker_respects_her_home(self):
        """_failure_marker_path must use HER_HOME, not hardcoded ~/.her."""
        from tools.tirith_security import _failure_marker_path
        with patch.dict(os.environ, {"HER_HOME": "/custom/her"}):
            result = _failure_marker_path()
        assert result == "/custom/her/.tirith-install-failed"

    def test_conftest_isolation_prevents_real_home_writes(self):
        """The conftest autouse fixture sets HER_HOME; verify it's active."""
        her_home = os.getenv("HER_HOME")
        assert her_home is not None, "HER_HOME should be set by conftest"
        assert "her_test" in her_home, "Should point to test temp dir"

    def test_get_her_home_fallback(self):
        """Without HER_HOME set, falls back to the active OS home."""
        from tools.tirith_security import _get_her_home
        with patch.dict(os.environ, {}, clear=True):
            # Remove HER_HOME entirely. With HOME also absent, expanduser
            ...
            expected = os.path.join(os.path.expanduser("~"), ".her")
            result = _get_her_home()
