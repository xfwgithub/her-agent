...
def fake_her(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("HER_HOME", str(sec_home))

    import her_constants
    monkeypatch.setattr(her_constants, "get_default_her_root", lambda: root)

    import agent.file_safety as fs
    monkeypatch.setattr(fs, "_her_home_path", lambda: sec_home)
...
class TestWriteFileCrossProfileGuard:
    def test_in_profile_write_allowed(self, fake_her):
        from tools.file_tools import write_file_tool
        target = fake_her["sec_home"] / "skills" / "new-skill" / "SKILL.md"
    ...
    def test_cross_profile_write_blocked_by_default(self, fake_her):
        """The May 2026 incident — security-profile session edits default
        profile's skill. Must be blocked."""
        from tools.file_tools import write_file_tool
        target = fake_her["root"] / "skills" / "shared-skill" / "SKILL.md"
        ...
        assert "her-security" in result["error"]
        # File untouched.
        assert target.read_text() == original

    def test_cross_profile_True_bypass(self, fake_her):
    ...
    def test_non_her_path_unaffected(self, fake_her, tmp_path):
        from tools.file_tools import write_file_tool
...
class TestPatchCrossProfileGuard:
    def test_cross_profile_patch_blocked(self, fake_her):
    ...
    def test_cross_profile_patch_bypass(self, fake_her):
        from tools.file_tools import patch_tool
        target = fake_her["root"] / "skills" / "shared-skill" / "SKILL.md"
        result_json = patch_tool(
            mode="replace",
    ...
    def test_v4a_patch_extracts_path_for_guard(self, fake_her):
...
class TestSkillManageCrossProfileErrorUX:
    ...
    def test_error_names_other_profile_when_skill_lives_there(
        self, fake_her, monkeypatch
    ):
        """The original incident shape — model expects 'foo' in active
        profile, but 'foo' lives in default. Error must point at default."""
        self._make_skill_in_profile(fake_her["root"], "default-only-skill")

        # Re-import the module so SKILLS_DIR picks up HER_HOME (set in
        ...
        assert "not found in active profile 'her-security'" in err
        assert "default" in err
        assert "cross_profile=True" in err

    def test_error_names_multiple_profiles(self, fake_her, monkeypatch):
        """When the skill exists in TWO other profiles, both should be named."""
        self._make_skill_in_profile(fake_her["root"], "everywhere-skill")
        self._make_skill_in_profile(fake_her["coder_home"], "everywhere-skill")
...
class TestSystemPromptActiveProfile:
    def test_default_profile_line_in_prompt(self, tmp_path, monkeypatch):
        ...
        about ~/.her/profiles/<name>/."""
        # Don't set HER_HOME — falls back to default.
        import agent.file_safety as fs
        monkeypatch.setattr(fs, "_her_home_path", lambda: tmp_path / "fake")
        monkeypatch.setattr(fs, "_her_root_path", lambda: tmp_path / "fake")
    ...
    def test_named_profile_line_in_prompt_text(self, fake_her):
        """When active profile is 'her-security', the prompt warns
