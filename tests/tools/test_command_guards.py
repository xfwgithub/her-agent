...
def _clean_state():
    ...
    saved = {}
    for k in ("HER_INTERACTIVE", "HER_GATEWAY_SESSION", "HER_EXEC_ASK", "HER_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["HER_INTERACTIVE"] = "1"
        session_key = os.getenv("HER_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No HER_INTERACTIVE or HER_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
