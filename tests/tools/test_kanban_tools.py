...
Verifies:
  - Tools are gated on HER_KANBAN_TASK: a normal chat session sees
...
def test_kanban_tools_hidden_without_env_var(monkeypatch, tmp_path):
    """Normal `her chat` sessions (no HER_KANBAN_TASK) must have
    ...
    schema = registry.get_definitions(set(resolve_toolset("her-cli")), quiet=True)
...
def test_kanban_tools_visible_with_env_var(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("HER_KANBAN_TASK", "t_fake")
    home = tmp_path / ".her"
    home.mkdir()
    monkeypatch.setenv("HER_HOME", str(home))
...
def test_worker_with_kanban_toolset_still_hides_board_routing(monkeypatch, tmp_path):
    ...
    in its config, the HER_KANBAN_TASK env var means it's a focused
...
def worker_env(monkeypatch, tmp_path):
    """Simulate being a worker: HER_HOME isolated, HER_KANBAN_TASK set
    ...
    try:
        ...
        conn.close()
    monkeypatch.setenv("HER_KANBAN_TASK", tid)
    return tid
...
def test_complete_metadata_round_trips_through_show(worker_env):
    ...
    handoff = {
        "changed_files": ["her_cli/kanban.py"],
...
def test_comment_happy_path(worker_env):
    ...
    try:
        ...
        # Author defaults to HER_PROFILE env we set in the fixture
...
def test_comment_ignores_caller_supplied_author(worker_env):
    ...
    derived from ``HER_PROFILE`` so a worker can't forge a comment
    under an authoritative-looking name like ``her-system`` and
    ...
    from tools import kanban_tools as kt
    out = kt._handle_comment({
        "task_id": worker_env, "body": "hi", "author": "her-system",
    })
    ...
    try:
        comments = kb.list_comments(conn, worker_env)
        # Author comes from HER_PROFILE in the fixture, not the
        # caller-supplied "her-system" override.
        assert comments[0].author == "test-worker"
    finally:
        conn.close()
...
def test_create_inherits_worker_dir_workspace(monkeypatch, worker_env):
    ...
    try:
        ...
        conn.close()
    monkeypatch.setenv("HER_KANBAN_TASK", self_tid)
...
def test_create_no_worker_task_stays_scratch(monkeypatch, worker_env):
    """Orchestrator/CLI callers (no HER_KANBAN_TASK) still default to
...
def test_kanban_guidance_not_in_normal_prompt(monkeypatch, tmp_path):
    """A normal chat session (no HER_KANBAN_TASK) must NOT have
...
def test_kanban_guidance_in_worker_prompt(monkeypatch, tmp_path):
    """A worker session (HER_KANBAN_TASK set) MUST have the full
...
# ---------------------------------------------------------------------------
#
# A worker process has HER_KANBAN_TASK set to its own task id. The
...
# are unrestricted.
#
# Orchestrator profiles (no HER_KANBAN_TASK in env) are intentionally
...
def test_worker_can_comment_on_foreign_task(worker_env):
    ...
    # HER_PROFILE — never to a caller-controlled string.
    conn = kb.connect()
    try:
...
def test_orchestrator_complete_any_task_allowed(monkeypatch, tmp_path):
    """Orchestrator profiles (no HER_KANBAN_TASK) can still complete
...
def multi_board_env(monkeypatch, tmp_path):
    ...
    HER_KANBAN_TASK is pinned (orchestrator context) — workers test
...
def test_board_param_routes_heartbeat_to_alt_board(monkeypatch, tmp_path):
    ...
    use the worker-env style fixture inline (pinning HER_KANBAN_TASK
