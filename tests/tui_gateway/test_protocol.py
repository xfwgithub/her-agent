...
def server():
    with patch.dict("sys.modules", {
        "her_constants": MagicMock(get_her_home=MagicMock(return_value="/tmp/her_test")),
        "her_cli.env_loader": MagicMock(),
        "her_cli.banner": MagicMock(),
        "her_state": MagicMock(),
    }):
        import importlib
...
def test_disable_flush_env_var_actually_wires_to_module_constant(monkeypatch):
    """End-to-end: setting `HER_TUI_GATEWAY_NO_FLUSH=1` and importing
    ...
    monkeypatch.setenv("HER_TUI_GATEWAY_NO_FLUSH", "1")
    ...
    try:
        ...
        monkeypatch.delenv("HER_TUI_GATEWAY_NO_FLUSH", raising=False)
...
def test_slash_exec_rejects_skill_commands(server):
    ...
    fake_skills = {"/her-agent-dev": {"name": "her-agent-dev", "description": "Dev workflow"}}

    with patch("agent.skill_commands.get_skill_commands", return_value=fake_skills):
        resp = server.handle_request({
            "id": "r1",
            "method": "slash.exec",
            "params": {"command": "her-agent-dev", "session_id": sid},
        })
...
def test_slash_exec_handles_plugin_commands_in_live_gateway(server):
    ...
    with patch(
        "her_cli.plugins.get_plugin_command_handler",
...
def test_command_dispatch_returns_skill_payload(server):
    ...
    with patch("agent.skill_commands.scan_skill_commands", return_value=fake_skills), \
        ...
        resp = server.handle_request({
            "id": "r2",
            "method": "command.dispatch",
            "params": {"name": "her-agent-dev", "session_id": sid},
        })
    ...
    assert result["name"] == "her-agent-dev"
