...
def server():
    with patch.dict(
        "sys.modules",
        {
            "her_constants": MagicMock(
                get_her_home=MagicMock(return_value="/tmp/her_test_review_summary")
            ),
...
def test_init_session_attaches_background_review_callback(server, monkeypatch):
    ...
    cb("💾 Self-improvement review: Skill 'her-release' patched")
    ...
    assert sid == "sid-abc"
    assert payload == {
        "text": "💾 Self-improvement review: Skill 'her-release' patched"
    }
