...
class TestMediaDeliveryPathValidation:
    def _patch_roots(self, monkeypatch, *roots):
        ...
        monkeypatch.setenv("HER_MEDIA_DELIVERY_STRICT", "1")
        ...
        monkeypatch.setenv("HER_MEDIA_TRUST_RECENT_FILES", "0")
    ...
    def test_allows_operator_configured_extra_root(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("HER_MEDIA_ALLOW_DIRS", str(extra_root))
    ...
    def test_recency_trust_allows_freshly_produced_file(self, tmp_path, monkeypatch):
        ...
        monkeypatch.delenv("HER_MEDIA_ALLOW_DIRS", raising=False)
        monkeypatch.setenv("HER_MEDIA_TRUST_RECENT_FILES", "1")
        monkeypatch.setenv("HER_MEDIA_TRUST_RECENT_SECONDS", "600")
    ...
    def test_recency_trust_rejects_old_file(self, tmp_path, monkeypatch):
        ...
        monkeypatch.delenv("HER_MEDIA_ALLOW_DIRS", raising=False)
        monkeypatch.setenv("HER_MEDIA_TRUST_RECENT_FILES", "1")
        monkeypatch.setenv("HER_MEDIA_TRUST_RECENT_SECONDS", "60")
...
class TestMediaDeliveryDefaultMode:
    ...
    def _patch_roots(self, monkeypatch, *roots):
        ...
        monkeypatch.delenv("HER_MEDIA_DELIVERY_STRICT", raising=False)
        monkeypatch.delenv("HER_MEDIA_ALLOW_DIRS", raising=False)
    ...
    def test_denylist_blocks_shared_her_root_config_for_profiles(self, tmp_path, monkeypatch):
        """Profile-mode gateways must still block the shared her root config."""
        self._patch_roots(monkeypatch)
        ...
        monkeypatch.setattr(
            "gateway.platforms.base._HER_ROOT",
            her_root,
        )
    ...
    def test_strict_mode_envvar_restores_legacy_behavior(self, tmp_path, monkeypatch):
        """Setting HER_MEDIA_DELIVERY_STRICT=1 reactivates the older
    ...
    def test_strict_mode_truthy_aliases(self, monkeypatch, tmp_path):
        """``HER_MEDIA_DELIVERY_STRICT=true|yes|on|1`` all enable strict mode."""
        self._patch_roots(monkeypatch)
        from gateway.platforms.base import _media_delivery_strict_mode

        for raw in ("1", "true", "TRUE", "yes", "on"):
            monkeypatch.setenv("HER_MEDIA_DELIVERY_STRICT", raw)
...
class TestGetHumanDelay:
    def test_off_mode(self):
        with patch.dict(os.environ, {"HER_HUMAN_DELAY_MODE": "off"}):
            assert BasePlatformAdapter._get_human_delay() == 0.0

    def test_default_is_off(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("HER_HUMAN_DELAY_MODE", None)
            assert BasePlatformAdapter._get_human_delay() == 0.0

    def test_natural_mode_range(self):
        with patch.dict(os.environ, {"HER_HUMAN_DELAY_MODE": "natural"}):
            delay = BasePlatformAdapter._get_human_delay()
            assert 0.8 <= delay <= 2.5

    def test_natural_mode_ignores_malformed_custom_env_vars(self):
        env = {
            "HER_HUMAN_DELAY_MODE": "natural",
            "HER_HUMAN_DELAY_MIN_MS": "oops",
            "HER_HUMAN_DELAY_MAX_MS": "still-bad",
        }
    ...
    def test_custom_mode_uses_env_vars(self):
        env = {
            "HER_HUMAN_DELAY_MODE": "custom",
            "HER_HUMAN_DELAY_MIN_MS": "100",
            "HER_HUMAN_DELAY_MAX_MS": "200",
        }
        with patch.dict(os.environ, env):
            delay = BasePlatformAdapter._get_human_delay()
...
class TestMediaDeliveryDiagnosability:
    ...
    def test_rejected_path_appears_in_log(self, tmp_path, caplog):
        ...
        with patch.dict(os.environ, {"HER_MEDIA_DELIVERY_STRICT": "1",
                                     "HER_MEDIA_TRUST_RECENT_FILES": "0"}), \
    ...
    def test_crafted_null_path_does_not_abort_batch(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("HER_MEDIA_DELIVERY_STRICT", "0")
