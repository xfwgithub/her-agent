...
Guards against the 60-vs-500 bug where a stale `.env HER_MAX_ITERATIONS=60`
...
def _run_gateway_import(her_home: Path, initial_env: dict[str, str]) -> dict[str, str]:
    ...
    script = textwrap.dedent(
        ...
        for k in (
            "HER_MAX_ITERATIONS",
            "HER_AGENT_TIMEOUT",
            "HER_AGENT_TIMEOUT_WARNING",
            "HER_GATEWAY_BUSY_INPUT_MODE",
            "HER_GATEWAY_BUSY_TEXT_MODE",
            "HER_TIMEZONE",
        ):
            v = os.environ.get(k)
            if v is not None:
...
def test_config_max_turns_wins_over_stale_env(her_home: Path) -> None:
    ...
    _write_env(her_home, {"HER_MAX_ITERATIONS": "60"})

    env = _run_gateway_import(her_home, initial_env={})

    assert env.get("HER_MAX_ITERATIONS") == "500", (
        f"expected config.yaml max_turns=500 to win; got {env.get('HER_MAX_ITERATIONS')!r}. "
...
def test_config_gateway_timeout_wins_over_stale_env(her_home: Path) -> None:
    ...
    _write_env(her_home, {
        "HER_AGENT_TIMEOUT": "60",
        "HER_AGENT_TIMEOUT_WARNING": "30",
    })

    env = _run_gateway_import(her_home, initial_env={})

    assert env.get("HER_AGENT_TIMEOUT") == "1800"
    assert env.get("HER_AGENT_TIMEOUT_WARNING") == "900"
...
def test_config_display_busy_input_mode_wins_over_stale_env(her_home: Path) -> None:
    ...
    _write_env(her_home, {"HER_GATEWAY_BUSY_INPUT_MODE": "queue"})

    env = _run_gateway_import(her_home, initial_env={})

    assert env.get("HER_GATEWAY_BUSY_INPUT_MODE") == "interrupt"
...
def test_config_display_busy_text_mode_wins_over_stale_env(her_home: Path) -> None:
    ...
    _write_env(her_home, {"HER_GATEWAY_BUSY_TEXT_MODE": "interrupt"})

    env = _run_gateway_import(her_home, initial_env={})

    assert env.get("HER_GATEWAY_BUSY_TEXT_MODE") == "queue"


def test_config_timezone_wins_over_stale_env(her_home: Path) -> None:
    _write_config(her_home, timezone="America/Los_Angeles")
    _write_env(her_home, {"HER_TIMEZONE": "UTC"})

    env = _run_gateway_import(her_home, initial_env={})

    assert env.get("HER_TIMEZONE") == "America/Los_Angeles"
...
def test_env_value_survives_when_config_omits_key(her_home: Path) -> None:
    ...
    _write_env(her_home, {"HER_MAX_ITERATIONS": "123"})

    env = _run_gateway_import(her_home, initial_env={})

    assert env.get("HER_MAX_ITERATIONS") == "123"
