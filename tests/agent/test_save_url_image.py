...
def http_server(tmp_path, monkeypatch):
    """Spin up a localhost HTTP server and isolate HER_HOME under tmp_path."""
    monkeypatch.setenv("HER_HOME", str(tmp_path / ".her"))
    (tmp_path / ".her").mkdir()

    # Force the constants/image cache helpers to re-read HER_HOME.
    import sys
    for mod in list(sys.modules):
        if mod.startswith("her_constants") or mod.startswith("agent.image_gen_provider"):
...
class TestSaveUrlImage:
    def test_writes_real_bytes_to_her_home_cache(self, http_server):
        base, _ = http_server
        ...
        # The cache directory must be under HER_HOME — gateway cleanup
