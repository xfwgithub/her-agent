...
class TestEnvFileReadBlocking:
    ...
    def test_allowed_her_env(self):
        """her' own .env inside HER_HOME is NOT blocked by this rule
        (it's handled by other mechanisms). Only project-local .env is blocked."""
        # Note: her internal .env is in ~/.her/.env which is NOT a project-local
        # path, but the basename check applies to ANY .env. This is intentional —
        # even ~/.her/.env should not be readable via read_file.
        error = get_read_block_error(os.path.expanduser("~/.her/.env"))
        assert error is not None
...
class TestCacheFileReadBlocking:
    """Internal her cache files must remain blocked."""

    def test_hub_index_cache_blocked(self, tmp_path):
        """Hub index-cache reads are blocked."""
        her_home = tmp_path / ".her"
        cache = her_home / "skills" / ".hub" / "index-cache" / "data.json"
        cache.parent.mkdir(parents=True)
        cache.write_text("{}")

        with patch("agent.file_safety._her_home_path", return_value=her_home):
            error = get_read_block_error(str(cache))
            assert error is not None
            assert "internal her cache" in error

    def test_hub_directory_blocked(self, tmp_path):
        """Hub directory reads are blocked."""
        her_home = tmp_path / ".her"
        hub = her_home / "skills" / ".hub" / "metadata.json"
...
class TestCombinedGuards:
    ...
    def test_env_guard_works_regardless_of_her_home(self, tmp_path):
        """The env basename guard does not depend on HER_HOME resolution."""
        her_home = tmp_path / ".her"
        her_home.mkdir()
    ...
    def test_cache_guard_still_works_with_env_guard(self, tmp_path):
        ...
        her_home = tmp_path / ".her"
        cache = her_home / "skills" / ".hub" / "index-cache" / "x"
        cache.parent.mkdir(parents=True)
        cache.write_text("")
