...
def test_her_client_tag_includes_current_version():
    """The client tag must reflect her_cli.__version__ verbatim."""
    from her_cli import __version__
    from agent.portal_tags import her_client_tag

    assert her_client_tag() == f"client=her-client-v{__version__}"


def test_her_client_tag_format():
    """The client tag has the exact shape Nous Portal expects."""
    from agent.portal_tags import her_client_tag

    tag = her_client_tag()
    assert tag.startswith("client=her-client-v")
...
def test_nous_portal_tags_contains_product_and_client():
    ...
    from agent.portal_tags import her_client_tag, nous_portal_tags

    tags = nous_portal_tags()
    assert "product=her-agent" in tags
    assert her_client_tag() in tags
    assert len(tags) == 2
