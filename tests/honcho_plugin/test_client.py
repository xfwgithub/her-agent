...
from her_cli.profiles import _get_default_her_home
...
class TestHonchoClientConfigDefaults:
    def test_default_values(self):
        ...
        assert config.host == "her"
        assert config.workspace_id == "her"
...
class TestFromGlobalConfig:
    ...
    def test_reads_full_config(self, tmp_path, monkeypatch):
        ...
        # Isolate from real ~/.her/honcho.json
        monkeypatch.setenv("HER_HOME", str(tmp_path / "isolated"))
    ...
    def test_base_url_not_read_from_host_block(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            ...
            "hosts": {"her": {"baseUrl": "http://host-block:9001"}},
        }))
...
class TestResolveSessionName:
    ...
    def test_per_repo_uses_git_root(self):
        ...
        with patch.object(
            HonchoClientConfig, "_git_repo_name", return_value="her-agent"
        ):
            result = config.resolve_session_name("/home/user/her-agent/subdir")
        assert result == "her-agent"
...
class TestResolveConfigPath:
    def test_prefers_her_home_when_exists(self, tmp_path):
        her_home = tmp_path / "her"
        her_home.mkdir()
        local_cfg = her_home / "honcho.json"
    ...
    def test_falls_back_to_default_profile_when_no_local(self, tmp_path, monkeypatch):
        # Profile mode: HER_HOME points at ~/.her/profiles/<name>, so
        # _get_default_her_home() must resolve back to ~/.her — that's
        # the bug the HOME-anchored helper fixes (vs. blindly using Path.home()).
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()
        default_home = fake_home / ".her"
        ...
        assert _get_default_her_home() == default_home
        assert result == default_cfg

    def test_falls_back_to_global_without_her_home_env(self, tmp_path):
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()
    ...
    def test_from_global_config_uses_default_profile_fallback(self, tmp_path, monkeypatch):
        ...
        # via the HOME-anchored helper, not Path.home() / ".her".
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()
        default_home = fake_home / ".her"
        profile_home = default_home / "profiles" / "work"
...
class TestResolveActiveHost:
    def test_profile_host_key_uses_honcho_safe_separator(self):
        assert profile_host_key("coder") == "her_coder"
        assert profile_host_key("default") == "her"

    def test_default_returns_her(self):
        with patch.dict(os.environ, {}, clear=True):
            os.environ.pop("HER_HONCHO_HOST", None)
            os.environ.pop("HER_HOME", None)
            assert resolve_active_host() == "her"

    def test_explicit_env_var_wins(self):
        with patch.dict(os.environ, {"HER_HONCHO_HOST": "her.coder"}):
            assert resolve_active_host() == "her.coder"

    def test_profile_name_derives_host(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("HER_HONCHO_HOST", None)
            with patch("her_cli.profiles.get_active_profile_name", return_value="coder"):
                assert resolve_active_host() == "her_coder"

    def test_default_profile_returns_her(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("HER_HONCHO_HOST", None)
            with patch("her_cli.profiles.get_active_profile_name", return_value="default"):
                assert resolve_active_host() == "her"

    def test_custom_profile_returns_her(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("HER_HONCHO_HOST", None)
            with patch("her_cli.profiles.get_active_profile_name", return_value="custom"):
                assert resolve_active_host() == "her"
    ...
    def test_profiles_import_failure_falls_back(self):
        ...
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("HER_HONCHO_HOST", None)
            # Temporarily remove her_cli.profiles to simulate import failure
            saved = sys.modules.get("her_cli.profiles")
            sys.modules["her_cli.profiles"] = None  # type: ignore
            try:
                assert resolve_active_host() == "her"
            finally:
                if saved is not None:
                    sys.modules["her_cli.profiles"] = saved
                else:
                    sys.modules.pop("her_cli.profiles", None)


class TestProfileScopedConfig:
    def test_from_env_uses_profile_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="her_coder")
        assert config.host == "her_coder"
        assert config.workspace_id == "her"  # shared workspace
        assert config.ai_peer == "her_coder"

    def test_from_env_default_workspace_preserved_for_default_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="her")
        assert config.host == "her"
        assert config.workspace_id == "her"
    ...
    def test_from_global_config_reads_profile_host_block(self, tmp_path):
        ...
        config = HonchoClientConfig.from_global_config(
            host="her_coder", config_path=config_file,
        )
        assert config.host == "her_coder"
    ...
    def test_from_global_config_auto_resolves_host(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "hosts": {
                "her_dreamer": {"peerName": "dreamer-user"},
            },
        }))
        with patch("plugins.memory.honcho.client.resolve_active_host", return_value="her_dreamer"):
            config = HonchoClientConfig.from_global_config(config_path=config_file)
        assert config.host == "her_dreamer"
    ...
    def test_from_global_config_reads_legacy_dot_profile_host_block(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "hosts": {
                "her.dreamer": {"peerName": "dreamer-user"},
            },
        }))
        config = HonchoClientConfig.from_global_config(
            host="her_dreamer",
        ...
        assert config.host == "her_dreamer"
        assert config.peer_name == "dreamer-user"
        assert config.workspace_id == "her_dreamer"
...
class TestObservationModeMigration:
    ...
    def test_existing_config_defaults_to_unified(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"her": {"enabled": True, "aiPeer": "her"}},
        }))
    ...
    def test_explicit_directional_respected(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"her": {"enabled": True, "observationMode": "directional"}},
        }))
        cfg = HonchoClientConfig.from_global_config(config_path=cfg_file)
        assert cfg.observation_mode == "directional"
...
class TestGetHonchoClient:
    ...
    def test_her_config_timeout_override_used_when_config_timeout_missing(self):
        fake_honcho = MagicMock(name="Honcho")
        cfg = HonchoClientConfig(
            api_key="test-key",
            workspace_id="her",
        ...
        with patch("honcho.Honcho", return_value=fake_honcho) as mock_honcho, \
             patch("her_cli.config.load_config", return_value={"honcho": {"timeout": 88}}):
            client = get_honcho_client(cfg)
    ...
    def test_her_request_timeout_alias_used(self):
        fake_honcho = MagicMock(name="Honcho")
        cfg = HonchoClientConfig(
            api_key="test-key",
            workspace_id="her",
        ...
        with patch("honcho.Honcho", return_value=fake_honcho) as mock_honcho, \
             patch("her_cli.config.load_config", return_value={"honcho": {"request_timeout": "77.5"}}):
            client = get_honcho_client(cfg)
