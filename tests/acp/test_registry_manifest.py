"""Tests for ACP Registry metadata shipped with her."""
...
def test_agent_json_matches_official_registry_required_fields():
    ...
    assert data["id"] == "her-agent"
    assert re.fullmatch(r"[a-z][a-z0-9-]*", data["id"])
    assert data["name"] == "her Agent"
    assert data["description"]
    assert data["repository"] == "https://github.com/NousResearch/her-agent"
    assert data["website"].startswith("https://her-agent.nousresearch.com/")
...
def test_agent_json_uses_uvx_distribution_without_local_command_fields():
    ...
    assert uvx["package"] == f"her-agent[acp]=={data['version']}"
    assert uvx["args"] == ["her-acp"]
...
def test_agent_json_pins_uvx_package_to_pyproject_version():
    ...
    assert _manifest()["distribution"]["uvx"]["package"] == (
        f"her-agent[acp]=={_pyproject_version()}"
    )
