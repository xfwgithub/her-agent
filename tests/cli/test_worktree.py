...
class TestWorktreeCreation:
    ...
    def test_creates_worktree(self, git_repo):
        ...
        assert info["branch"].startswith("her/her-")
...
class TestOrphanedBranchPruning:
    """Test cleanup of orphaned her/* and pr-* branches."""

    def test_prunes_orphaned_her_branch(self, git_repo):
        """her/her-* branches with no worktree should be deleted."""
        # Create a branch that looks like a worktree branch but has no worktree
        subprocess.run(
            ["git", "branch", "her/her-deadbeef", "HEAD"],
            cwd=str(git_repo), capture_output=True,
        )

        # Verify it exists
        result = subprocess.run(
            ["git", "branch", "--list", "her/her-deadbeef"],
            capture_output=True, text=True, cwd=str(git_repo),
        )
        assert "her/her-deadbeef" in result.stdout
        ...
        assert "her/her-deadbeef" in orphaned
        ...
        assert "her/her-deadbeef" not in result.stdout
