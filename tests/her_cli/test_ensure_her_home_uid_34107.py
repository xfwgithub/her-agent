...
The fix is a ``_chown_to_her_uid`` helper that reads the env vars and
...
class TestResolveHermesUidGid:
    ...
    def test_invalid_uid_returns_none_for_that_field(self, monkeypatch):
        monkeypatch.setenv("HER_UID", "not-a-number")
        monkeypatch.setenv("HER_GID", "911")
        from her_cli.config import _resolve_her_uid_gid
        uid, gid = _resolve_her_uid_gid()
        assert uid is None
    ...
    def test_empty_string_treated_as_unset(self, monkeypatch):
        monkeypatch.setenv("HER_UID", "")
        monkeypatch.setenv("HER_GID", "")
        from her_cli.config import _resolve_her_uid_gid
        uid, gid = _resolve_her_uid_gid()
        assert uid is None
        assert gid is None
