"""Tests for the ``her hooks`` CLI subcommand."""
...
from her_cli import hooks as hooks_cli


@pytest.fixture(autouse=True)
def _isolated_home(tmp_path, monkeypatch):
    monkeypatch.setenv("HER_HOME", str(tmp_path / "home"))
    monkeypatch.delenv("HER_ACCEPT_HOOKS", raising=False)
...
class TestHooksDoctor:
    ...
    def test_unallowlisted_script_is_not_executed(self, tmp_path):
        """Regression for M4: `her hooks doctor` used to run every
