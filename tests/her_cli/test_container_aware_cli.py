...
def container_env(tmp_path, monkeypatch):
    """Set up a fake HER_HOME with .container-mode file."""
    her_home = tmp_path / ".her"
    her_home.mkdir()
    monkeypatch.setenv("HER_HOME", str(her_home))
    monkeypatch.delenv("HER_DEV", raising=False)

    container_mode = her_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=her-agent\n"
        "exec_user=her\n"
        "her_bin=/data/current-package/bin/her\n"
    )
    return her_home
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("her_constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "her-agent"
    assert info["exec_user"] == "her"
    assert info["her_bin"] == "/data/current-package/bin/her"


def test_get_container_exec_info_none_inside_container(container_env):
    """Returns None when we're already inside a container."""
    with patch("her_constants.is_container", return_value=True):
        info = get_container_exec_info()
...
def test_get_container_exec_info_skipped_when_her_dev(container_env, monkeypatch):
    """Returns None when HER_DEV=1 is set (dev mode bypass)."""
    monkeypatch.setenv("HER_DEV", "1")
...
def test_get_container_exec_info_not_skipped_when_her_dev_zero(container_env, monkeypatch):
    """HER_DEV=0 does NOT trigger bypass — only '1' does."""
    monkeypatch.setenv("HER_DEV", "0")
...
def test_get_container_exec_info_defaults():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        her_home = Path(tmpdir) / ".her"
        her_home.mkdir()
        (her_home / ".container-mode").write_text(
            "# minimal file with no keys\n"
        )

        with patch("her_constants.is_container", return_value=False), \
             patch.dict(get_container_exec_info.__globals__, {"get_her_home": lambda: her_home}), \
             patch.dict(os.environ, {}, clear=False):
            os.environ.pop("HER_DEV", None)
            info = get_container_exec_info()
...
def test_get_container_exec_info_docker_backend(container_env):
    ...
    (container_env / ".container-mode").write_text(
        "backend=docker\n"
        "container_name=her-custom\n"
        "exec_user=myuser\n"
        "her_bin=/opt/her/bin/her\n"
    )
    ...
    assert info["backend"] == "docker"
    assert info["container_name"] == "her-custom"
    assert info["exec_user"] == "myuser"
    assert info["her_bin"] == "/opt/her/bin/her"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from her_cli.main import _exec_in_container
    ...
    assert "-it" in cmd
    idx_u = cmd.index("-u")
    assert cmd[idx_u + 1] == "her"
    e_indices = [i for i, v in enumerate(cmd) if v == "-e"]
    e_values = [cmd[i + 1] for i in e_indices]
    assert "TERM=xterm-256color" in e_values
    assert "LANG=en_US.UTF-8" in e_values
    assert "her-agent" in cmd
    assert "/data/current-package/bin/her" in cmd
    assert "chat" in cmd
