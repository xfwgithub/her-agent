"""Comprehensive tests for her_cli.profiles module.
...
# ---------------------------------------------------------------------------
# Shared fixture: redirect Path.home() and HER_HOME for profile tests
# ---------------------------------------------------------------------------
...
def profile_env(tmp_path, monkeypatch):
    ...
    * Path.home() -> tmp_path  (so _get_profiles_root() = tmp_path/.her/profiles)
    * HER_HOME  -> tmp_path/.her  (so get_her_home() agrees)
    * Creates the bare-minimum ~/.her directory.
    """
...
class TestGetProfileDir:
    ...
    def test_default_returns_her_home(self, profile_env):
    ...
    def test_named_profile_returns_profiles_subdir(self, profile_env):
        ...
        assert result == tmp_path / ".her" / "profiles" / "coder"

    def test_named_profile_matching_is_case_insensitive(self, profile_env):
        tmp_path = profile_env
        assert get_profile_dir("Coder") == tmp_path / ".her" / "profiles" / "coder"
...
class TestCreateProfile:
    ...
    def test_clone_all_excludes_sibling_profiles_tree(self, profile_env):
        """--clone-all from default ~/.her must not copy profiles/* (nested explosion)."""
        tmp_path = profile_env
        default_home = tmp_path / ".her"
    ...
    def test_clone_all_excludes_default_infrastructure(self, profile_env):
        """--clone-all from default profile excludes her-agent, .worktrees,
        ...
        (default_home / "her-agent" / ".git").mkdir(parents=True)
        (default_home / "her-agent" / "venv" / "bin").mkdir(parents=True)
        (default_home / "her-agent" / "README.md").write_text("repo")
        ...
        assert not (profile_dir / "her-agent").exists()
...
class TestNoSkillsOptOut:
    """Tests for `her profile create --no-skills` and the opt-out marker."""
    ...
    def test_seed_profile_skills_respects_marker(self, profile_env):
        ...
        called directly (e.g. by `her update`'s all-profile sync loop)."""
        profile_dir = create_profile("orchestrator", no_alias=True, no_skills=True)
...
class TestDeleteProfile:
    ...
    def test_removes_directory(self, profile_env):
        ...
        with patch("her_cli.profiles._cleanup_gateway_service"):
...
class TestActiveProfile:
    ...
    def test_empty_file_returns_default(self, profile_env):
        tmp_path = profile_env
        active_path = tmp_path / ".her" / "active_profile"
...
class TestGetActiveProfileName:
    ...
    def test_default_her_home_returns_default(self, profile_env):
        # HER_HOME points to tmp_path/.her which is the default
    ...
    def test_custom_path_returns_default(self, profile_env, monkeypatch):
        """A custom HER_HOME (Docker, etc.) IS the default root."""
        tmp_path = profile_env
        custom = tmp_path / "some" / "other" / "path"
        custom.mkdir(parents=True)
        monkeypatch.setenv("HER_HOME", str(custom))
        # With Docker-aware roots, a custom HER_HOME is the default —
...
class TestAliasCollision:
    ...
    def test_reserved_name_returns_message(self, profile_env):
        result = check_alias_collision("her")
        assert result is not None
...
class TestWrapperScript:
    ...
    def test_remove_returns_false_when_absent(self, profile_env):
        from her_cli.profiles import remove_wrapper_script
...
class TestFindAliasForProfile:
    ...
    def test_no_alias_returns_none(self, profile_env, monkeypatch):
        monkeypatch.setattr("sys.platform", "darwin")
        from her_cli.profiles import find_alias_for_profile
    ...
    def test_ignores_unrelated_files(self, profile_env, monkeypatch):
        ...
        monkeypatch.setattr("sys.platform", "darwin")
        from her_cli.profiles import _get_wrapper_dir, find_alias_for_profile
...
class TestRenameProfile:
    ...
    def test_renames_root_honcho_host_without_changing_ai_peer(self, profile_env):
        ...
        assert "her.ssi_health" not in cfg["hosts"]
        assert cfg["hosts"]["her_heimdall"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["her_heimdall"]["peerName"] == "user-peer"
    ...
    def test_pins_ai_peer_when_absent_on_honcho_host_rename(self, profile_env):
        ...
        honcho_path.write_text(json.dumps({
            "hosts": {
                "her.ssi_health": {"workspace": "her", "enabled": True}
            }
        }))
        ...
        assert "her.ssi_health" not in cfg["hosts"]
        assert cfg["hosts"]["her_heimdall"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["her_heimdall"]["workspace"] == "her"
    ...
    def test_does_not_overwrite_existing_honcho_host_on_rename(self, profile_env):
        ...
        honcho_path.write_text(json.dumps({
            "hosts": {
                "her.ssi_health": {"aiPeer": "ssi_health"},
                "her_heimdall": {"aiPeer": "heimdall"},
            }
        }))
        ...
        cfg = json.loads(honcho_path.read_text())
        assert cfg["hosts"]["her.ssi_health"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["her_heimdall"]["aiPeer"] == "heimdall"
...
class TestExportImport:
    ...
    def test_export_default_excludes_infrastructure(self, profile_env, tmp_path):
        ...
        for d in ("her-agent", ".worktrees", "profiles", "bin",
        ...
        for f in ("state.db", "gateway.pid", "gateway_state.json",
                  "processes.json", "errors.log", ".her_history",
        ...
        excluded_prefixes = [
            "default/her-agent", "default/.worktrees", "default/profiles",
        ...
        excluded_files = [
            ...
            "default/errors.log", "default/.her_history",
...
class TestInternalHelpers:
    """Tests for _get_profiles_root() and _get_default_her_home()."""

    def test_profiles_root_under_home(self, profile_env):
        tmp_path = profile_env
        root = _get_profiles_root()
        assert root == tmp_path / ".her" / "profiles"

    def test_default_her_home(self, profile_env):
        tmp_path = profile_env
        home = _get_default_her_home()
        assert home == tmp_path / ".her"

    def test_profiles_root_docker_deployment(self, tmp_path, monkeypatch):
        """In Docker (HER_HOME outside ~/.her), profiles go under HER_HOME."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("HER_HOME", str(docker_home))
        root = _get_profiles_root()
        assert root == docker_home / "profiles"

    def test_default_her_home_docker(self, tmp_path, monkeypatch):
        """In Docker, _get_default_her_home() returns HER_HOME itself."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("HER_HOME", str(docker_home))
    ...
    def test_profiles_root_profile_mode(self, tmp_path, monkeypatch):
        """In profile mode (HER_HOME under ~/.her), profiles root is still ~/.her/profiles."""
        native = tmp_path / ".her"
    ...
    def test_active_profile_path_docker(self, tmp_path, monkeypatch):
        """In Docker, active_profile file lives under HER_HOME."""
        from her_cli.profiles import _get_active_profile_path
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("HER_HOME", str(docker_home))
    ...
    def test_create_profile_docker(self, tmp_path, monkeypatch):
        """Profile created in Docker lands under HER_HOME/profiles/."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("HER_HOME", str(docker_home))
...
class TestEdgeCases:
    ...
    def test_create_profile_returns_correct_path(self, profile_env):
        ...
        expected = tmp_path / ".her" / "profiles" / "mybot"
        assert result == expected
    ...
    def test_gateway_running_check_with_pid_file(self, profile_env):
        ...
        from her_cli.profiles import _check_gateway_running
        tmp_path = profile_env
        default_home = tmp_path / ".her"
