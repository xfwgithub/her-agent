"""Tests for the Kanban DB layer (her_cli.kanban_db)."""
...
def kanban_home(tmp_path, monkeypatch):
    """Isolated HER_HOME with an empty kanban DB."""
    home = tmp_path / ".her"
    home.mkdir()
...
def test_has_spawnable_ready_true_when_real_profile_present(kanban_home, monkeypatch):
    ...
    with kb.connect() as conn:
        ...
        kb.create_task(conn, title="her-task", assignee="daily")
...
def test_scratch_workspace_created_under_her_home(kanban_home):
    with kb.connect() as conn:
        t = kb.create_task(conn, title="x")
        task = kb.get_task(conn, t)
...
# spawned with `her -p <profile>` must read/write the same kanban.db
...
# where `kanban_db_path()` resolved to the active profile's HER_HOME.
# ---------------------------------------------------------------------------

class TestSharedBoardPaths:
    """`kanban_home`/`kanban_db_path`/`workspaces_root`/`worker_log_path`
    must anchor at the **shared root**, not the active profile's HER_HOME."""
    ...
    def test_default_install_anchors_at_home_dot_her(
        ...
        # Standard install: HER_HOME == ~/.her, no profile active.
    ...
    def test_profile_worker_resolves_to_shared_root(
        ...
        # Reproduces the bug: dispatcher uses ~/.her/kanban.db,
        # worker spawned with -p <profile> previously resolved to
        # ~/.her/profiles/<profile>/kanban.db. After the fix both
        # converge on ~/.her/kanban.db.
        default_home = tmp_path / ".her"
        default_home.mkdir()
    ...
    def test_dispatcher_and_profile_worker_converge(
        ...
        # HER_HOME and confirm equality. This is the property the
        ...
        # Worker's perspective (profile activated by `her -p coder`).
    ...
    def test_docker_custom_her_home_uses_env_path_directly(
        self, tmp_path, monkeypatch
    ):
        # Docker / custom deployment: HER_HOME points outside ~/.her.
        # `get_default_her_root()` returns env_home directly when it
        # is not a `<root>/profiles/<name>` shape and not under
        # `Path.home() / ".her"`.
        custom_root = tmp_path / "opt" / "her"
    ...
    def test_docker_profile_layout_uses_grandparent(
        ...
        # Docker profile shape: HER_HOME=/opt/her/profiles/coder;
        # `get_default_her_root()` walks up to /opt/her because
    ...
    def test_dispatcher_and_worker_share_a_real_database(
        ...
        # HER_HOME perspectives via a real SQLite file. Without the
        ...
        # Worker switches to the profile HER_HOME and reads.
        monkeypatch.setenv("HER_HOME", str(profile_home))
        with kb.connect() as conn:
...
# ---------------------------------------------------------------------------
# NFS / network-filesystem fallback (see her_state.apply_wal_with_fallback)
# ---------------------------------------------------------------------------
...
def test_connect_falls_back_to_delete_on_locking_protocol(tmp_path, monkeypatch, caplog):
    ...
    with _patch("her_cli.kanban_db.sqlite3.connect", side_effect=wal_blocking_connect):
        with caplog.at_level("WARNING", logger="her_state"):
            conn = kb.connect()
...
def test_unlink_tasks_triggers_recompute_ready(kanban_home):
    ...
    next dispatcher tick or a manual 'her kanban recompute' would promote it.
    """
    with kb.connect() as conn:
        # A is done.
...
# ---------------------------------------------------------------------------
# Dispatcher spawn invocation — _resolve_her_argv()
#
# Workers spawned by the dispatcher must use a `her` invocation that does
...
# $PATH that doesn't include the venv's bin/, so a bare `["her", ...]`
...
def test_resolve_her_argv_prefers_path_shim(monkeypatch):
    """When `her` is on PATH, use the shim — preserves familiar ps output."""
    import shutil
...
def test_resolve_her_argv_her_bin_bare_name_uses_path(monkeypatch, tmp_path):
    ...
    cwd_her = tmp_path / "her"
    cwd_her.write_text("wrong\n", encoding="utf-8")
    cwd_her.chmod(cwd_her.stat().st_mode | stat.S_IXUSR)
    path_her = tmp_path / "bin" / "her"
    path_her.parent.mkdir()
    path_her.write_text("right\n", encoding="utf-8")
...
def test_resolve_her_argv_falls_back_to_module_form_when_no_path_shim(monkeypatch):
    """When the shim is not on PATH, fall back to `python -m her_cli.main`.

    Pins the correct module name (NOT `her` — there is no top-level
    `her` package). Regression for #23198: the original PR shipped
    `python -m her` which fails with `No module named her` on every
    invocation.
    """
    import shutil
    import sys
...
def test_resolve_her_argv_module_actually_runs():
    """The fallback module name must be importable + runnable.

    A unit test that pins the literal string is necessary but not
    sufficient — if `her_cli.main` ever loses `if __name__ == "__main__"`
    handling or its argparse setup, `python -m her_cli.main --version`
...
def test_maybe_emit_scratch_tip_fires_once_per_install(kanban_home, caplog):
    ...
    with caplog.at_level(logging.WARNING, logger="her_cli.kanban_db"):
        with kb.connect() as conn:
...
def test_write_txn_healthy_commit_no_exception(tmp_path):
    ...
    from her_cli.kanban_db import connect, write_txn
    db = tmp_path / "test.db"
...
def test_write_txn_raises_on_truncated_file(tmp_path):
    ...
    with pytest.raises(sqlite3.DatabaseError, match="torn-extend|page count mismatch"):
        with unittest.mock.patch("her_cli.kanban_db.os.path.getsize", side_effect=fake_getsize):
            with write_txn(conn) as c:
                c.execute(
                    "INSERT INTO tasks (id, title, assignee, status, priority, created_at) "
...
def test_write_txn_post_commit_check_fires_every_call(tmp_path):
    ...
    from her_cli.kanban_db import connect, write_txn
    import her_cli.kanban_db as kanban_db_module
    db = tmp_path / "test.db"
    conn = connect(db_path=db)
    call_count = 0
...
def test_connect_sets_wal_autocheckpoint_100(tmp_path):
    ...
    from her_cli.kanban_db import connect
    db = tmp_path / "test.db"
    conn = connect(db_path=db)
...
def test_write_txn_check_reads_correct_header_fields(tmp_path):
    ...
    import struct
    from her_cli.kanban_db import connect, _check_file_length_invariant
...
def test_reap_worker_zombies_returns_count():
    ...
    with patch("her_cli.kanban_db.os.waitpid", side_effect=fake_waitpid):
        with patch("her_cli.kanban_db._record_worker_exit"):
...
def test_reap_worker_zombies_noop_on_windows(monkeypatch):
    ...
    with patch("her_cli.kanban_db.os.waitpid") as mock_waitpid:
...
def test_reap_worker_zombies_noop_no_children():
    ...
    with patch("her_cli.kanban_db.os.waitpid", side_effect=ChildProcessError):
        result = kb.reap_worker_zombies()
    assert result == []
...
def test_reap_worker_zombies_records_exit_status():
    ...
    with patch("her_cli.kanban_db.os.waitpid", side_effect=fake_waitpid):
        with patch(
            "her_cli.kanban_db._record_worker_exit",
...
def test_reap_worker_zombies_handles_waitpid_os_error():
    ...
    with patch("her_cli.kanban_db.os.waitpid", side_effect=OSError("test error")):
        result = kb.reap_worker_zombies()
    assert result == []
...
def test_zombie_reaper_survives_all_boards_failing():
    ...
    for tick in range(5):
        ...
        with patch(
            "her_cli.kanban_db.os.waitpid", side_effect=make_fake_waitpid(pids)
        ):
            with patch("her_cli.kanban_db._record_worker_exit"):
                pids = kb.reap_worker_zombies()
...
def test_dispatch_once_still_reaps_via_extracted_fn(kanban_home):
    ...
    with patch("her_cli.kanban_db.os.waitpid", side_effect=fake_waitpid):
        with patch("her_cli.kanban_db._record_worker_exit"):
            with patch("her_cli.kanban_db.os.name", "posix"):
                pids = kb.reap_worker_zombies()
