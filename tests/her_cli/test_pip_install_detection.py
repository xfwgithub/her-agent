...
def test_pip_install_detected_when_no_git_dir(tmp_path):
    ...
    with patch("her_cli.config.get_managed_system", return_value=None), \
...
def test_recommended_update_command_pip():
    ...
    from her_cli.config import recommended_update_command_for_method
...
def test_container_without_stamp_is_not_docker(tmp_path):
    ...
    image, so ``her update`` refused to run. With a ``.git`` checkout it
    must resolve to ``git``.
    """
    (tmp_path / ".git").mkdir()
    with patch("her_cli.config.get_managed_system", return_value=None), \
         patch("her_cli.config.get_her_home", return_value=tmp_path), \
         patch("her_constants.is_container", return_value=True):
        from her_cli.config import detect_install_method
...
def test_banner_warns_on_pip_install(tmp_path):
    ...
    with patch("her_cli.config.get_her_home", return_value=hh), \
         patch("her_constants.get_her_home", return_value=hh):
        buf = io.StringIO()
