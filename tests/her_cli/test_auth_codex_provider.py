"""Tests for Codex auth — tokens stored in her auth store (~/.her/auth.json)."""
...
def _setup_her_auth(her_home: Path, *, access_token: str = "access", refresh_token: str = "refresh"):
    """Write Codex tokens into the her auth store."""
...
def test_read_codex_tokens_missing(tmp_path, monkeypatch):
    ...
    her_home.mkdir(parents=True, exist_ok=True)
    # Empty auth store
    (her_home / "auth.json").write_text(json.dumps({"version": 1, "providers": {}}))
    monkeypatch.setenv("HER_HOME", str(her_home))
...
def test_resolve_codex_runtime_credentials_missing_access_token(tmp_path, monkeypatch):
    her_home = tmp_path / "her"
    _setup_her_auth(her_home, access_token="")
    monkeypatch.setenv("HER_HOME", str(her_home))
...
def test_resolve_codex_runtime_credentials_refreshes_expiring_token(tmp_path, monkeypatch):
    ...
    _setup_her_auth(her_home, access_token=expiring_token, refresh_token="refresh-old")
    monkeypatch.setenv("HER_HOME", str(her_home))
    ...
    monkeypatch.setattr("her_cli.auth._refresh_codex_auth_tokens", _fake_refresh)
...
def test_resolve_codex_runtime_credentials_force_refresh(tmp_path, monkeypatch):
    her_home = tmp_path / "her"
    _setup_her_auth(her_home, access_token="access-current", refresh_token="refresh-old")
    monkeypatch.setenv("HER_HOME", str(her_home))
...
def test_resolve_codex_runtime_credentials_falls_back_to_pool_when_singleton_empty(tmp_path, monkeypatch):
    ...
    (her_home / "auth.json").write_text(json.dumps(auth_store))
    monkeypatch.setenv("HER_HOME", str(her_home))
...
def test_save_codex_tokens_syncs_credential_pool(tmp_path, monkeypatch):
    ...
    her_home = tmp_path / "her"
    her_home.mkdir(parents=True, exist_ok=True)
    (her_home / "auth.json").write_text(json.dumps({
        "version": 1,
        "providers": {
            "openai-codex": {
    ...
    auth = json.loads((her_home / "auth.json").read_text())
...
def test_save_codex_tokens_syncs_manual_device_code_entries(tmp_path, monkeypatch):
    ...
    would have run ``her auth add openai-codex`` as a workaround, leaving
...
def test_codex_tokens_not_written_to_shared_file(tmp_path, monkeypatch):
    """Verify _save_codex_tokens writes only to her auth store, not ~/.codex/."""
    her_home = tmp_path / "her"
    codex_home = tmp_path / "codex-cli"
    her_home.mkdir(parents=True, exist_ok=True)
    codex_home.mkdir(parents=True, exist_ok=True)
    ...
    _save_codex_tokens({"access_token": "her-at", "refresh_token": "her-rt"})

    # ~/.codex/auth.json should NOT exist — _save_codex_tokens only touches her store
    assert not (codex_home / "auth.json").exists()

    # her auth store should have the tokens
    data = _read_codex_tokens()
    assert data["tokens"]["access_token"] == "her-at"


def test_resolve_returns_her_auth_store_source(tmp_path, monkeypatch):
    her_home = tmp_path / "her"
    _setup_her_auth(her_home)
    monkeypatch.setenv("HER_HOME", str(her_home))

    creds = resolve_codex_runtime_credentials()
    assert creds["source"] == "her-auth-store"
...
def _patch_httpx(monkeypatch, response):
    ...
    monkeypatch.setattr("her_cli.auth.httpx.Client", _factory)
...
def test_refresh_429_classified_as_quota_not_auth_failure(monkeypatch):
    ...
    than a misleading "run her auth".
    """
    from her_cli.auth import (
    ...
    assert "her auth" not in rendered
...
def test_refresh_429_without_retry_after_header(monkeypatch):
    ...
    from her_cli.auth import CODEX_RATE_LIMITED_CODE
...
def test_is_rate_limited_auth_error_distinguishes_credential_errors():
    ...
    from her_cli.auth import CODEX_RATE_LIMITED_CODE, is_rate_limited_auth_error
...
def test_login_openai_codex_force_new_login_skips_existing_reuse_prompt(monkeypatch):
    ...
    monkeypatch.setattr(
        "her_cli.auth.resolve_codex_runtime_credentials",
    ...
    monkeypatch.setattr(
        "her_cli.auth._codex_device_code_login",
        lambda: {
    ...
    monkeypatch.setattr("her_cli.auth._save_codex_tokens", _fake_save)
    monkeypatch.setattr("her_cli.auth._update_config_for_provider", lambda *args, **kwargs: "/tmp/config.yaml")
    monkeypatch.setattr(
        "builtins.input",
