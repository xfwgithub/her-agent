...
class TestSlackNativeSlashes:
    ...
    def test_her_catchall_is_first(self):
        """``/her`` must be reserved as the first slot so the legacy
        ``/her <subcommand>`` form keeps working after we add new
        commands and hit the 50-slash cap."""
        slashes = slack_native_slashes()
        assert slashes[0][0] == "her"
    ...
    def test_excludes_slack_reserved_commands(self):
        ...
        Users can still reach them via /her <command>."""
        names = {n for n, _d, _h in slack_native_slashes()}
    ...
    def test_includes_aliases_as_first_class_slashes(self):
        ...
        anything dropped remains reachable via ``/her <command>``."""
        names = {n for n, _d, _h in slack_native_slashes()}
        assert "btw" in names
        assert "bg" in names
...
class TestSlackAppManifest:
    """Generated Slack app manifest (used by `her slack manifest`)."""
...
@pytest.mark.skipif(
    ...
    reason=(
        ...
        "in her_cli/commands.py now return empty containers, so these "
...
from her_cli.commands import discord_skill_commands_by_category  # noqa: E402
...
class TestPluginCommandEnumeration:
    ...
    def _patch_plugin_commands(self, monkeypatch, commands):
        """Monkeypatch her_cli.plugins.get_plugin_commands() to a fixed dict."""
        from her_cli import plugins as _plugins_mod
    ...
    def test_plugin_command_appears_in_slack_subcommand_map(self, monkeypatch):
        """/her metricas must route through the Slack subcommand map."""
        self._patch_plugin_commands(monkeypatch, {
            "metricas": {
                "handler": lambda _a: "ok",
                "description": "Metrics",
    ...
    def test_is_gateway_known_command_recognizes_plugin_commands(self, monkeypatch):
        ...
        from her_cli.commands import is_gateway_known_command
