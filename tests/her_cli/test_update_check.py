"""Tests for the update check mechanism in her_cli.banner."""
...
def test_check_for_updates_invalidates_on_version_change(tmp_path, monkeypatch):
    ...
    fake_banner = tmp_path / "her_cli" / "banner.py"
...
def test_check_for_updates_expired_cache(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("HER_HOME", str(tmp_path))
    with patch("her_cli.banner.subprocess.run", return_value=mock_result) as mock_run:
        result = check_for_updates()
...
def test_check_for_updates_no_git_dir(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("HER_HOME", str(tmp_path))
    with patch("her_cli.banner.subprocess.run") as mock_run:
        with patch("her_cli.banner.check_via_pypi", return_value=0):
            result = banner.check_for_updates()
    assert result == 0
    mock_run.assert_not_called()
...
def test_check_for_updates_fallback_to_project_root(tmp_path, monkeypatch):
    """Dev install: falls back to Path(__file__).parent.parent when HER_HOME has no git repo."""
    import her_cli.banner as banner
    ...
    # Point HER_HOME at a temp dir with no her-agent/.git
    monkeypatch.setenv("HER_HOME", str(tmp_path))
    with patch("her_cli.banner.subprocess.run") as mock_run:
...
def test_check_for_updates_docker_returns_none(tmp_path, monkeypatch):
    ...
    container, and `her update` correctly refusing to run there. The guard
    ...
    with patch("her_cli.config.detect_install_method", return_value="docker"), \
         patch("her_cli.banner.subprocess.run") as mock_run, \
         patch("her_cli.banner.check_via_pypi") as mock_pypi:
        result = banner.check_for_updates()
...
def test_check_for_updates_non_docker_still_checks(tmp_path, monkeypatch):
    ...
    with patch("her_cli.config.detect_install_method", return_value="pip"), \
         patch("her_cli.banner.subprocess.run") as mock_run, \
         patch("her_cli.banner.check_via_pypi", return_value=1) as mock_pypi:
        result = banner.check_for_updates()
...
def test_invalidate_update_cache_clears_all_profiles(tmp_path):
    ...
    from her_cli.main import _invalidate_update_cache

    # Build a fake ~/.her with default + two named profiles
    ...
    with patch.object(Path, "home", return_value=tmp_path), \
         patch.dict(os.environ, {"HER_HOME": str(default_home)}):
