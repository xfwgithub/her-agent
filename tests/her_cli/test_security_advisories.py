"""Tests for her_cli.security_advisories.
...
import her_cli.security_advisories as adv
...
def isolated_home(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    """Redirect HER_HOME so banner cache and config writes are sandboxed."""
    home = tmp_path / ".her"
    home.mkdir()
    (home / "cache").mkdir()
...
class TestAck:
    ...
    def test_ack_advisory_persists_id(self, isolated_home, monkeypatch):
        ...
        # don't depend on the full her_cli.config bootstrap.
        store: dict = {"security": {}}
        monkeypatch.setattr(
            "her_cli.config.load_config", lambda: store
        )
        monkeypatch.setattr(
            "her_cli.config.save_config",
