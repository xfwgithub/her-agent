"""Tests for her_cli/skills_config.py and skills_tool disabled filtering."""
...
class TestSaveDisabledSkills:
    @patch("her_cli.skills_config.save_config")
    def test_saves_global_sorted(self, mock_save):
        from her_cli.skills_config import save_disabled_skills
        config = {}
...
class TestIsSkillDisabled:
    @patch("her_cli.config.load_config")
    ...
    @patch("her_cli.config.load_config")
    @patch.dict("os.environ", {"HER_PLATFORM": "discord"})
...
class TestGetDisabledSkillNames:
    ...
    def test_session_platform_env_var(self, tmp_path, monkeypatch):
        """HER_SESSION_PLATFORM should be used when HER_PLATFORM is unset."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  disabled:\n"
    ...
    def test_her_platform_takes_precedence(self, tmp_path, monkeypatch):
        """HER_PLATFORM should win over HER_SESSION_PLATFORM."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  platform_disabled:\n"
    ...
    def test_explicit_param_overrides_env_vars(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("HER_HOME", str(tmp_path))
        monkeypatch.setenv("HER_PLATFORM", "telegram")
        monkeypatch.setenv("HER_SESSION_PLATFORM", "telegram")
...
class TestGetCategories:
    def test_extracts_unique_categories(self):
        from her_cli.skills_config import _get_categories
        skills = [
