"""Tests for her_cli.web_server and related config utilities."""
...
@pytest.fixture
def _install_example_plugin(_isolate_her_home):
    """Drop the example-dashboard fixture into the per-test HER_HOME
    ...
    from her_cli import web_server
    ...
    #      and start serving requests against a torn-down HER_HOME.
    app = web_server.app
...
class TestWebServerEndpoints:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_her_home):
        """Create a TestClient and isolate the state DB under the test HER_HOME."""
        try:
        ...
        import her_state
        from her_constants import get_her_home
        from her_cli.web_server import app, _SESSION_HEADER_NAME, _SESSION_TOKEN

        monkeypatch.setattr(her_state, "DEFAULT_DB_PATH", get_her_home() / "state.db")
    ...
    def test_get_sessions_forwards_min_messages(self, monkeypatch):
        ...
        monkeypatch.setattr("her_state.SessionDB", _FakeDB)
    ...
    def test_update_her_returns_docker_guidance_without_spawning(self, monkeypatch):
        import her_cli.web_server as web_server
        ...
        def fail_spawn(*_args, **_kwargs):
            nonlocal spawned
            spawned = True
            raise AssertionError("docker update guard should not spawn her update")

        monkeypatch.setattr(web_server, "detect_install_method", lambda _root: "docker")
        monkeypatch.setattr(web_server, "_spawn_her_action", fail_spawn)
        web_server._ACTION_PROCS.pop("her-update", None)
        web_server._ACTION_RESULTS.pop("her-update", None)

        resp = self.client.post("/api/her/update")

        assert resp.status_code == 200
        data = resp.json()
        assert data["ok"] is False
        assert data["name"] == "her-update"
        assert data["pid"] is None
        assert data["error"] == "docker_update_unsupported"
        assert "docker pull nousresearch/her-agent:latest" in data["message"]
        assert spawned is False

        status = self.client.get("/api/actions/her-update/status")
        assert status.status_code == 200
        status_data = status.json()
        assert status_data["running"] is False
        assert status_data["exit_code"] == 1
        assert status_data["pid"] is None
        assert any("docker pull nousresearch/her-agent:latest" in line for line in status_data["lines"])

    def test_update_her_spawns_on_non_docker_install(self, monkeypatch):
        import her_cli.web_server as web_server
        ...
        monkeypatch.setattr(web_server, "_spawn_her_action", fake_spawn)
        web_server._ACTION_PROCS.pop("her-update", None)
        web_server._ACTION_RESULTS.pop("her-update", None)
        ...
        assert resp.status_code == 200
        assert resp.json() == {"ok": True, "pid": 12345, "name": "her-update"}
        assert calls == [(["update"], "her-update")]
    ...
    def test_get_env_vars_marks_channel_managed_keys(self):
        from her_cli.web_server import _channel_managed_env_keys
    ...
    def test_platform_scoped_messaging_env_vars_are_channel_managed(self):
        from her_cli.web_server import (
    ...
    def test_reveal_env_var(self, tmp_path):
        ...
        from her_cli.web_server import _SESSION_HEADER_NAME, _SESSION_TOKEN
    ...
    def test_reveal_env_var_bad_token(self, tmp_path):
        ...
        from her_cli.config import save_env_value
        from her_cli.web_server import _SESSION_HEADER_NAME
    ...
    def test_reveal_env_var_legacy_authorization_header_still_works(self, tmp_path):
        ...
        from her_cli.config import save_env_value
        from her_cli.web_server import _SESSION_TOKEN
    ...
    def test_update_messaging_platform_saves_env_and_enablement(self):
        from her_cli.config import load_config, load_env
    ...
    def test_telegram_onboarding_ready_and_apply_never_returns_bot_token(self, monkeypatch):
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            ...
            return {
                "status": "ready",
                "bot_username": "her_pair_ready_bot",
    ...
    def test_path_traversal_dotdot_blocked(self):
        ...
        resp = self.client.get("/%2e%2e/her_cli/web_server.py")
        assert resp.status_code in {200, 404}
        if resp.status_code == 200:
    ...
    def test_set_model_main_nous_applies_gateway_defaults(self, monkeypatch):
        ...
        import her_cli.nous_subscription as ns
        ...
        resp = self.client.post(
            "/api/model/set",
            json={"scope": "main", "provider": "nous", "model": "her-4"},
        )
        assert resp.status_code == 200
        data = resp.json()
    ...
    def test_apply_main_model_assignment_base_url_and_context_reconcile(self):
        ...
        from her_cli.web_server import _apply_main_model_assignment
    ...
    def test_parse_model_ids_handles_openai_and_bare_shapes(self):
        ...
        from her_cli.web_server import _parse_model_ids
    ...
    def test_set_model_main_reports_stale_auxiliary_pins(self):
        ...
        cfg = load_config()
        cfg["model"] = {"provider": "nous", "default": "her-4"}
        cfg["auxiliary"] = {
    ...
    def test_recommended_default_nous_honors_free_tier(self, monkeypatch):
        ...
        model (mirroring `her model`), not the first curated paid entry."""
        import her_cli.models as models_mod
...
class TestBuildSchemaFromConfig:
    ...
    def test_empty_prefix_produces_correct_keys(self):
        from her_cli.web_server import _build_schema_from_config
...
class TestNewEndpoints:
    ...
    def test_profiles_list_includes_default(self):
        from her_constants import get_her_home
        get_her_home().mkdir(parents=True, exist_ok=True)
    ...
    def test_profiles_list_falls_back_when_profile_listing_fails(self, monkeypatch):
        from her_constants import get_her_home
        import her_cli.profiles as profiles_mod
        ...
        named = her_home / "profiles" / "multi-agent"
    ...
    def test_profile_setup_command_uses_her_for_default_profile(self):
        from her_constants import get_her_home
        ...
        assert resp.status_code == 200
        assert resp.json()["command"] == "her setup"
    ...
    def test_profiles_create_with_clone_from_default_copies_default_skills(self, monkeypatch):
        ...
        default_skill = get_her_home() / "skills" / "custom" / "new-skill"
        ...
        assert resp.status_code == 200
        cloned_skill = get_her_home() / "profiles" / "cloned" / "skills" / "custom" / "new-skill" / "SKILL.md"
    ...
    def test_profiles_create_with_clone_from_duplicates_source(self, monkeypatch):
        ...
        source_skill = get_her_home() / "profiles" / "source-prof" / "skills" / "custom" / "src-skill"
        ...
        assert resp.status_code == 200
        cloned_skill = (
            get_her_home() / "profiles" / "source-prof-copy" / "skills" / "custom" / "src-skill" / "SKILL.md"
        )
        assert cloned_skill.exists()
    ...
    def test_profiles_create_without_clone_seeds_bundled_skills(self, monkeypatch):
        ...
        assert resp.status_code == 200
        seeded_skill = get_her_home() / "profiles" / "fresh" / "skills" / "software-development" / "plan" / "SKILL.md"
    ...
    def test_profile_model_round_trip(self, monkeypatch):
        ...
        import yaml
        cfg_path = get_her_home() / "profiles" / "model-prof" / "config.yaml"
    ...
    def test_skills_list_includes_disabled_skills(self, monkeypatch):
        ...
        import her_cli.skills_config as skills_config
        import her_cli.web_server as web_server
...
class TestModelContextLength:
    ...
    def test_normalize_extracts_context_length_from_dict(self):
        ...
        from her_cli.web_server import _normalize_config_for_web
    ...
    def test_denormalize_writes_context_length_into_model_dict(self):
        ...
        from her_cli.web_server import _denormalize_config_from_web
...
class TestNormaliseThemeDefinition:
    ...
    def test_rejects_missing_name(self):
        from her_cli.web_server import _normalise_theme_definition
...
class TestDiscoverUserThemes:
    """Tests for _discover_user_themes() — scans ~/.her/dashboard-themes/."""
...
class TestBulkDeleteSessionsEndpoint:
    ...
    1. Route-ordering: ``/api/sessions/bulk-delete`` must shadow the
       ...
       the block comment in ``her_cli/web_server.py``).
    ...
    def _setup_test_client(self, monkeypatch, _isolate_her_home):
        ...
        monkeypatch.setattr(
            her_state, "DEFAULT_DB_PATH", get_her_home() / "state.db"
        )
    ...
    def test_route_order_not_shadowed_by_session_id(self):
        ...
        assert "deleted" in body, (
            ...
            "registration order in her_cli/web_server.py."
        )
...
class TestDeleteEmptySessionsEndpoint:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_her_home):
        ...
        # Pin the SessionDB to the isolated HER_HOME so each test
...
class TestPluginAPIAuth:
    ...
    @pytest.fixture(autouse=True)
    def _setup_test_client(self, monkeypatch, _isolate_her_home, _install_example_plugin):
        ...
        installs it into the per-test ``HER_HOME``.
        """
        try:
            from starlette.testclient import TestClient
        except ImportError:
    ...
    def test_plugin_route_allows_auth(self):
        ...
        test fixture (installed into HER_HOME by the class-level
    ...
    def test_non_kanban_plugin_route_requires_auth(self):
        ...
        so any plugin's API surface — kanban, her-achievements, future
        plugins — must require the session token. Hit a non-kanban plugin
        path to lock that in.
        """
        # Real plugin path (her-achievements is loaded by default).
        resp = self.client.get("/api/plugins/her-achievements/overview")
        assert resp.status_code == 401
...
# or ``sh -c 'printf …'``) instead of the real ``her --tui`` binary.  The
...
class TestPtyWebSocket:
    ...
    def test_resolve_chat_argv_uses_dashboard_scroll_env(self, monkeypatch):
        ...
        assert env["HER_TUI_INLINE"] == "1"
        assert env["HER_TUI_DISABLE_MOUSE"] == "1"
    ...
    def test_streams_child_stdout_to_client(self, monkeypatch):
        ...
        with self.client.websocket_connect(self._url()) as conn:
            ...
            while time.monotonic() < deadline:
                ...
                if frame:
                    buf += frame
                if b"her-ws-ok" in buf:
                    break
            assert b"her-ws-ok" in buf
    ...
    def test_unavailable_platform_closes_with_message(self, monkeypatch):
        from her_cli.pty_bridge import PtyUnavailableError
        ...
        import her_cli.web_server as ws_mod
    ...
    def test_channel_param_propagates_sidecar_url(self, monkeypatch):
        ...
        HER_TUI_SIDECAR_URL env var pointing back at /api/pub on the
    ...
    def test_pub_broadcasts_to_events_subscribers(self):
        ...
        """
        import asyncio
        from her_cli import web_server as ws_mod
...
def test_resolve_chat_argv_injects_gateway_ws_url(monkeypatch):
    ...
    gateway_url = env.get("HER_TUI_GATEWAY_URL", "")
...
class TestDashboardPluginStaticAssetAllowlist:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_her_home, _install_example_plugin):
        ...
        the per-test ``HER_HOME`` user-plugins dir.
        """
        try:
            from starlette.testclient import TestClient
        except ImportError:
