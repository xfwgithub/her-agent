"""Tests for her_cli.uninstall.remove_node_symlinks.
...
def _make_her_node(her_home: Path) -> Path:
    """Create a fake $HER_HOME/node/bin/{node,npm,npx} tree."""
...
def test_removes_symlinks_pointing_into_her_node(fake_home):
    ...
    removed = uninstall.remove_node_symlinks(her_home)
...
def test_handles_missing_local_bin(fake_home):
    ...
    assert uninstall.remove_node_symlinks(her_home) == []
