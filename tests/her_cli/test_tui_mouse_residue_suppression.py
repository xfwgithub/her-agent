...
from her_cli.main import _suppress_mouse_residue_early
...
class TestEarlyMouseDisable:
    def test_writes_disable_sequence_when_tui_flag_in_argv(self, monkeypatch):
        monkeypatch.setattr(sys, "argv", ["her", "--tui", "-c", "abc"])
        monkeypatch.delenv("HER_TUI", raising=False)
        monkeypatch.delenv("HER_TUI_NO_EARLY_DISABLE", raising=False)
    ...
    def test_writes_disable_sequence_when_her_tui_env_set(self, monkeypatch):
    ...
    def test_respects_diagnostic_escape_hatch(self, monkeypatch):
        ...
        monkeypatch.delenv("HER_TUI", raising=False)
        monkeypatch.setenv("HER_TUI_NO_EARLY_DISABLE", "1")
    ...
    def test_skips_when_stdout_is_not_a_tty(self, monkeypatch):
        # `her --tui … >log` or CI capture: pipe is fd 1, not a TTY. The
