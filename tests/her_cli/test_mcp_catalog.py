"""Tests for her_cli.mcp_catalog and her_cli.mcp_picker.
...
@pytest.fixture(autouse=True)
def _isolate_her_home(tmp_path, monkeypatch):
    """Redirect all config I/O to a temp HER_HOME."""
    hh = tmp_path / "her-home"
    hh.mkdir()
    monkeypatch.setenv("HER_HOME", str(hh))
    monkeypatch.setattr(
        "her_cli.config.get_her_home", lambda: hh
    )
    monkeypatch.setattr(
        "her_cli.config.get_config_path", lambda: hh / "config.yaml"
    )
    monkeypatch.setattr(
        "her_cli.config.get_env_path", lambda: hh / ".env"
    )
    # mcp_catalog grabs get_her_home() lazily through her_constants
    monkeypatch.setattr(
        "her_constants.get_her_home", lambda: hh
    )
    return hh
...
def _entry(name: str):
    ...
    from her_cli.mcp_catalog import get_entry
...
class TestManifestParsing:
    def test_minimal_valid(self, catalog_dir):
        ...
        from her_cli.mcp_catalog import list_catalog
...
class TestInstall:
    def test_install_simple_stdio_writes_config(self, catalog_dir):
        _write_manifest(catalog_dir, "demo", _basic_manifest())
        from her_cli.mcp_catalog import install_entry
    ...
    def test_install_required_env_missing_raises(self, catalog_dir, monkeypatch):
        ...
        from her_cli import mcp_catalog
        from her_cli.mcp_catalog import install_entry, CatalogError
...
class TestUninstall:
    def test_uninstall_removes_server_block(self, catalog_dir):
        _write_manifest(catalog_dir, "demo", _basic_manifest())
        from her_cli.mcp_catalog import install_entry, uninstall_entry
        from her_cli.config import load_config
    ...
    def test_uninstall_missing_returns_false(self):
        from her_cli.mcp_catalog import uninstall_entry
...
class TestCatalogDiagnostics:
    def test_future_manifest_version_skipped_with_diagnostic(self, catalog_dir):
        ...
        from her_cli.mcp_catalog import list_catalog, catalog_diagnostics
    ...
    def test_picker_surfaces_future_manifest_warning(self, catalog_dir, capsys, monkeypatch):
        ...
        entries so users running headless or after `her setup` know to update."""
        body = _basic_manifest()
...
class TestGitInstallShaRef:
    def test_sha_ref_skips_branch_attempt(self, catalog_dir, monkeypatch, tmp_path):
        ...
        from her_cli import mcp_catalog
        from her_cli.mcp_catalog import _do_git_install
    ...
    def test_branch_ref_uses_branch_clone(self, catalog_dir, monkeypatch):
        ...
        from her_cli import mcp_catalog
        from her_cli.mcp_catalog import _do_git_install, get_entry
...
class TestToolsConfigIncludeMode:
    def test_configure_mcp_writes_include_not_exclude(self, monkeypatch, tmp_path):
        ...
        import her_cli.tools_config as tc
        ...
        monkeypatch.setattr(
            "her_cli.curses_ui.curses_checklist",
