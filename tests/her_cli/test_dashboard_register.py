"""Tests for ``her dashboard register``.
...
import her_cli.dashboard_register as dr
...
class TestFastFails:
    def test_not_logged_in_exits_1_with_setup_hint(self, capsys):
        ...
        with patch.object(dr, "cmd_dashboard_register", dr.cmd_dashboard_register):
            with patch(
                "her_cli.auth.resolve_nous_access_token", side_effect=err
...
class TestHappyPath:
    def _run(self, *, args, account_token="tok_abc", portal="https://portal.nousresearch.com",
        ...
        with patch(
            "her_cli.auth.resolve_nous_access_token", return_value=account_token
        ), patch("her_cli.config.is_managed", return_value=False), patch.object(
            dr, "_resolve_portal_base_url", return_value=portal
        ), patch(
            "her_cli.config.get_env_value", return_value=None
        ), patch(
            "her_cli.config.save_env_value", side_effect=fake_save
        ), patch.object(
    ...
    def test_custom_redirect_uri_is_forwarded(self, capsys):
        captured: dict = {}
        self._run(
            args=_ns(redirect_uri="https://her.example.com/auth/callback"),
            captured=captured,
        )
        assert (
            captured["body"]["custom_redirect_uri"]
            == "https://her.example.com/auth/callback"
        )
    ...
    def test_non_default_portal_is_persisted(self, capsys):
        ...
        assert (
            saved["HER_DASHBOARD_PORTAL_URL"]
...
class TestPortalErrors:
    def _run_http_error(self, code, body):
        ...
        with patch(
            "her_cli.auth.resolve_nous_access_token", return_value="tok"
        ), patch("her_cli.config.is_managed", return_value=False), patch.object(
