"""Tests for `her memory setup [provider]` routing.
...
``her memory setup honcho``) without the interactive picker — which matters
because the per-provider ``her <provider>`` subcommand is only registered
...
from her_cli import memory_setup
...
class TestMemorySetupProviderRouting:
    ...
    def test_unknown_provider_reports_and_returns_early(self, capsys):
        ...
        assert "her memory setup" in out
