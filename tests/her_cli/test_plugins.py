...
from her_cli.middleware import (
    VALID_MIDDLEWARE,
...
def _make_plugin_dir(base: Path, name: str, *, register_body: str = "pass",
    ...
    ``<her_home>/config.yaml`` under ``plugins.enabled``. Plugins are
    ...
    *base* is expected to be ``<her_home>/plugins/``; we derive
    ``<her_home>`` from it by walking one level up.
    """
    ...
    if auto_enable:
        # Write/merge plugins.enabled in <HER_HOME>/config.yaml.
        # Config is always read from HER_HOME (not from the project
        # dir for project plugins), so that's where we opt in.
        import os
        her_home_str = os.environ.get("HER_HOME")
        if her_home_str:
            her_home = Path(her_home_str)
        else:
            her_home = base.parent
        her_home.mkdir(parents=True, exist_ok=True)
        cfg_path = her_home / "config.yaml"
        cfg: dict = {}
        if cfg_path.exists():
            try:
...
class TestPluginDiscovery:
    ...
    def test_discover_user_plugins(self, tmp_path, monkeypatch):
        """Plugins in ~/.her/plugins/ are discovered."""
        plugins_dir = tmp_path / "her_test" / "plugins"
    ...
    def test_execution_middleware_does_not_retry_downstream_failure(self, monkeypatch):
        ...
        monkeypatch.setattr("her_cli.plugins.get_plugin_manager", lambda: manager)
...
class TestPluginLoading:
    ...
    def test_load_registers_namespace_module(self, tmp_path, monkeypatch):
        """Directory plugins are importable under her_plugins.<name>."""
        plugins_dir = tmp_path / "her_test" / "plugins"
        _make_plugin_dir(plugins_dir, "ns_plugin")
        monkeypatch.setenv("HER_HOME", str(tmp_path / "her_test"))

        # Clean up any prior namespace module
        sys.modules.pop("her_plugins.ns_plugin", None)

        mgr = PluginManager()
        mgr.discover_and_load()

        assert "her_plugins.ns_plugin" in sys.modules
...
class TestPluginHooks:
    ...
    def test_invalid_hook_name_warns(self, tmp_path, monkeypatch, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="her_cli.plugins"):
            mgr = PluginManager()
            mgr.discover_and_load()
...
class TestPluginCommands:
    ...
    def test_get_plugin_command_handler_found(self):
        ...
        with patch("her_cli.plugins._plugin_manager", mgr):
    ...
    def test_commands_in_list_plugins_output(self, tmp_path, monkeypatch):
        ...
        plugins_dir = tmp_path / "her_test" / "plugins"
        # Set HER_HOME BEFORE _make_plugin_dir so auto-enable targets
...
class TestPluginCommandResultResolution:
    ...
    def test_awaits_async_result_with_running_loop(self, monkeypatch):
        ...
        monkeypatch.setattr("her_cli.plugins.asyncio.get_running_loop", lambda: _Loop())
    ...
    def test_running_loop_timeout_does_not_hang_forever(self, monkeypatch):
        ...
        monkeypatch.setattr("her_cli.plugins.asyncio.get_running_loop", lambda: _Loop())
        monkeypatch.setattr("her_cli.plugins._PLUGIN_COMMAND_AWAIT_TIMEOUT_SECS", 0.1)
...
class TestPluginDispatchTool:
    ...
    def test_dispatch_tool_calls_registry(self):
        ...
        with patch("her_cli.plugins.PluginContext.dispatch_tool.__module__", "her_cli.plugins"):
