...
prefix (e.g. ``mission-control.tilos.com/her/*`` -> local Caddy ->
...
honour it too:
  ...
  1. The gate's ``Location:`` redirect to /login (in
     ``_unauth_response``) needs to be ``/her/login`` so the browser
  ...
  3. ``_redirect_uri`` (the OAuth callback URL handed to the IDP) must
     ...
     ``/her/auth/callback`` and the user gets 404.
  4. Cookies must use ``Path=/her`` when behind a prefix so they
...
from her_cli.dashboard_auth import clear_providers, register_provider
from tests.her_cli.conftest_dashboard_auth import StubAuthProvider
...
class TestGateRedirectsCarryPrefix:
    def test_html_redirect_to_login_carries_prefix(self, gated_app_proxied):
        r = gated_app_proxied.get(
            "/sessions",
            headers={"x-forwarded-prefix": "/her"},
        ...
        assert r.headers["location"].startswith("/her/login"), (
    ...
    def test_api_401_envelope_login_url_carries_prefix(self, gated_app_proxied):
        ...
        # include the prefix.
        assert body["login_url"].startswith("/her/login"), (
...
class TestOAuthRedirectUriRespectsPrefix:
    def test_redirect_uri_includes_prefix_in_authorize_url(
        ...
        ``https://mission-control.tilos.com/her/auth/callback`` — the
        ...
        assert parsed.path == "/her/auth/callback", (
...
class TestPublicUrlOverride:
    ...
    def patch_config(self, monkeypatch):
        """Replace ``her_cli.config.load_config`` with a stub
    ...
    def test_public_url_with_path_prefix_baked_in(
        ...
        (``https://example.com/her``), the OAuth callback URL is
...
class TestCookiePathRespectsPrefix:
    ...
    def test_pkce_cookie_uses_prefix_path(self, gated_app_proxied):
        ...
        pkce = next(c for c in cookies if "her_session_pkce" in c)
        # Browser only sends cookie back if the request path is under
        # the cookie's Path attribute, so we need /her here. Bare
        # /-rooted cookies would still be sent but would also be sent
        # to /billing/... etc.
        assert "Path=/her" in pkce, (
    ...
    def test_pkce_cookie_uses_secure_prefix_when_proxied(
        ...
        pkce_candidates = [
            c for c in cookies
            if c.startswith("__Secure-her_session_pkce=")
        ]
    ...
    def test_pkce_cookie_uses_host_prefix_when_direct(
        ...
        pkce_candidates = [
            c for c in cookies
            if c.startswith("__Host-her_session_pkce=")
        ]
        assert pkce_candidates, (
    ...
    def test_loopback_cookies_unprefixed(self):
        ...
        from her_cli.dashboard_auth.cookies import set_pkce_cookie
        ...
        assert any(c.startswith("her_session_pkce=") for c in cookies), (
    ...
    def test_cookies_read_back_round_trip_through_prefix(
        ...
        __Secure- prefix AND Path=/her, so the next request under
        ...
        (``/her/auth/login``, ``/her/auth/callback``). A cookie
        set with ``Path=/her`` would therefore NOT be sent back on
        the second request through TestClient even though it WOULD be
        sent by a real browser hitting ``/her/auth/callback``. To
        ...
        pkce_set = next(
            ...
            if "her_session_pkce" in c
        )
        # Parse "__Secure-her_session_pkce=...; HttpOnly; ...".
        pkce_kv = pkce_set.split(";", 1)[0]  # "__Secure-her_session_pkce=value"
        state = r1.headers["location"].split("state=")[1]

        # Round-trip the cookie by hand because TestClient's jar won't
        # automatically send a Path=/her cookie to a /auth/callback
        # request path.
        r2 = gated_app_proxied.get(
            f"/auth/callback?code=stub_code&state={state}",
            headers={
                "x-forwarded-prefix": "/her",
                "cookie": pkce_kv,
            },
            follow_redirects=False,
        )
        ...
        at_cookies = [
            c for c in cookies
            if c.startswith("__Secure-her_session_at=")
        ]
        assert at_cookies, (
            f"session_at missing __Secure- prefix: {cookies!r}"
        )
        assert "Path=/her" in at_cookies[0]
