...
import her_cli.doctor as doctor
import her_cli.gateway as gateway_cli
from her_cli import doctor as doctor_mod
from her_cli.doctor import _has_provider_env_config
...
class TestDoctorEnvFileEncoding:
    """Regression for #18637 (bug 3): `her doctor` crashed on Windows
...
class TestDoctorToolAvailabilityOverrides:
    ...
    def test_leaves_kanban_unavailable_when_worker_env_is_set(self, monkeypatch):
        monkeypatch.setenv("HER_KANBAN_TASK", "probe")
...
class TestDoctorMemoryProviderSection:
    ...
    def _make_her_home(self, tmp_path, provider=""):
        """Create a minimal HER_HOME with config.yaml."""
        home = tmp_path / ".her"
    ...
    def _run_doctor_and_capture(self, monkeypatch, tmp_path, provider=""):
        ...
        home = self._make_her_home(tmp_path, provider)
...
def test_run_doctor_accepts_her_provider_ids_that_catalog_aliases(
...
def test_has_healthy_oauth_fallback_returns_false_for_unknown_provider():
    from her_cli.doctor import _has_healthy_oauth_fallback_for_apikey_provider
...
class TestHasHealthyOauthFallbackForXai:
    ...
    def test_returns_false_when_xai_import_unavailable(self, monkeypatch):
        ...
        monkeypatch.delattr("her_cli.auth.get_xai_oauth_auth_status", raising=False)
        # Force doctor module to re-import the function
        monkeypatch.delitem(sys.modules, "her_cli.doctor", raising=False)
        from her_cli.doctor import _has_healthy_oauth_fallback_for_apikey_provider
        assert _has_healthy_oauth_fallback_for_apikey_provider("xai") is False
...
class TestDoctorStaleMaxIterationsDrift:
    ...
    def _run_config_section(self, monkeypatch, tmp_path, *, fix, ghost, cfg_turns,
        ...
        monkeypatch.setattr(doctor_mod, "HER_HOME", her_home)
        monkeypatch.setattr(doctor_mod, "get_her_home", lambda: her_home)
