...
import her_cli.gateway as gateway_cli
...
class TestSystemdServiceRefresh:
    def test_systemd_install_repairs_outdated_unit_without_force(self, tmp_path, monkeypatch):
        unit_path = tmp_path / "her-gateway.service"
    ...
    def test_systemd_stop_timeout_prints_status_guidance(self, monkeypatch, capsys):
        ...
        assert "her gateway status" in output

    def test_systemd_restart_timeout_prints_status_guidance(self, monkeypatch, capsys):
        """`her gateway restart` must not surface a raw TimeoutExpired traceback.

        The dashboard spawns `her gateway restart` in the background; when a
    ...
    def test_run_gateway_refreshes_outdated_unit_on_boot(self, tmp_path, monkeypatch):
        ...
        via exit-code-75 (bypassing `her gateway restart`)."""
        unit_path = tmp_path / "her-gateway.service"
        unit_path.write_text("old unit\n", encoding="utf-8")
    ...
    def test_refresh_refuses_to_bake_pytest_tmpdir_into_real_user_unit(
        ...
        ``generate_systemd_unit()`` bakes ``HER_HOME`` into the unit's
        ...
        ``/tmp/pytest-of-.../her_test`` HER_HOME — silently breaking
        ...
        # Realistic generated unit referencing a pytest tmpdir HER_HOME
        polluted_unit = (
            "[Service]\n"
            'Environment="HER_HOME=/tmp/pytest-of-alice/pytest-42/'
            'popen-gw0/test_x/her_test"\n'
        )
        monkeypatch.setattr(
            gateway_cli,
...
class TestRequireServiceInstalled:
    def test_exits_with_install_hint_when_unit_missing(self, tmp_path, monkeypatch, capsys):
        ...
        assert "her gateway install" in out
...
class TestGeneratedSystemdUnits:
    ...
    def test_system_unit_includes_wsl_windows_interop_paths(self, monkeypatch):
        ...
        monkeypatch.setattr(gateway_cli, "_her_home_for_target_user", lambda home: "/home/alice/.her")
        monkeypatch.setenv("PATH", "/usr/local/bin:/mnt/c/WINDOWS/system32")
        monkeypatch.setattr(gateway_cli.shutil, "which", lambda cmd: None)
...
class TestLaunchdServiceRecovery:
    ...
    def test_launchd_install_repairs_outdated_plist_without_force(self, tmp_path, monkeypatch):
        plist_path = tmp_path / "ai.her.gateway.plist"
    ...
    def test_launchd_fallback_exits_when_spawn_fails(self, monkeypatch, capsys):
        ...
        out = capsys.readouterr().out
        assert "nohup her gateway run" in out
...
class TestGatewaySystemServiceRouting:
    ...
    def test_systemd_status_surfaces_planned_restart_failure(self, monkeypatch, capsys):
        ...
        monkeypatch.setattr(gateway_cli, "has_legacy_her_units", lambda: False)
    ...
    def test_gateway_install_reports_termux_manual_mode(self, monkeypatch, capsys):
        ...
        assert "Run manually: her gateway" in out
    ...
    def test_gateway_status_on_termux_shows_manual_guidance(self, monkeypatch, capsys):
        ...
        assert "nohup her gateway" in out
    ...
    def test_gateway_restart_does_not_fallback_to_foreground_when_launchd_restart_fails(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            lambda: (_ for _ in ()).throw(
                gateway_cli.subprocess.CalledProcessError(5, ["launchctl", "kickstart", "-k", "gui/501/ai.her.gateway"])
            ),
        )
...
class TestSystemUnitHermesHome:
    ...
    def test_system_unit_uses_target_user_home_not_calling_user(self, monkeypatch):
        ...
        assert 'HER_HOME=/home/alice/.her' in unit
        assert '/root/.her' not in unit

    def test_system_unit_remaps_profile_to_target_user(self, monkeypatch):
        # Simulate sudo with a profile: HER_HOME was resolved under root
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.setenv("HER_HOME", "/root/.her/profiles/coder")
        monkeypatch.setattr(
            gateway_cli, "_system_service_identity",
            lambda run_as_user=None: ("alice", "alice", "/home/alice"),
        )
        ...
        assert 'HER_HOME=/home/alice/.her/profiles/coder' in unit
        assert '/root/' not in unit

    def test_system_unit_preserves_custom_her_home(self, monkeypatch):
        # Custom HER_HOME not under any user's home — keep as-is
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.setenv("HER_HOME", "/opt/her-shared")
        monkeypatch.setattr(
            gateway_cli, "_system_service_identity",
            lambda run_as_user=None: ("alice", "alice", "/home/alice"),
        )
        ...
        assert 'HER_HOME=/opt/her-shared' in unit

    def test_user_unit_unaffected_by_change(self):
        # User-scope units should still use the calling user's HER_HOME
        unit = gateway_cli.generate_systemd_unit(system=False)

        her_home = str(gateway_cli.get_her_home().resolve())
...
class TestHermesHomeForTargetUser:
    ...
    def test_remaps_default_home(self, monkeypatch):
        ...
        result = gateway_cli._her_home_for_target_user("/home/alice")
        assert result == "/home/alice/.her"
    ...
    def test_remaps_profile_path(self, monkeypatch):
        ...
        result = gateway_cli._her_home_for_target_user("/home/alice")
        assert result == "/home/alice/.her/profiles/coder"

    def test_keeps_custom_path(self, monkeypatch):
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.setenv("HER_HOME", "/opt/her")

        result = gateway_cli._her_home_for_target_user("/home/alice")
        assert result == "/opt/her"
...
class TestPreflightUserSystemd:
    ...
    def test_raises_when_linger_disabled_and_loginctl_denied(self, monkeypatch):
        ...
        assert "her gateway run" in msg  # foreground fallback mentioned
...
class TestProfileArg:
    ...
    def test_default_her_home_returns_empty(self, tmp_path, monkeypatch):
        """Default ~/.her should not produce a --profile flag."""
        her_home = tmp_path / ".her"
        her_home.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("HER_HOME", str(her_home))
        result = gateway_cli._profile_arg(str(her_home))
        assert result == ""

    def test_named_profile_returns_flag(self, tmp_path, monkeypatch):
        """~/.her/profiles/mybot should return '--profile mybot'."""
        profile_dir = tmp_path / ".her" / "profiles" / "mybot"
    ...
    def test_hash_path_returns_empty(self, tmp_path, monkeypatch):
        """Arbitrary non-profile HER_HOME should return empty string."""
        custom_home = tmp_path / "custom" / "her"
    ...
    def test_nested_profile_path_returns_empty(self, tmp_path, monkeypatch):
        """~/.her/profiles/mybot/subdir should NOT match — too deep."""
        nested = tmp_path / ".her" / "profiles" / "mybot" / "subdir"
    ...
    def test_invalid_profile_name_returns_empty(self, tmp_path, monkeypatch):
        ...
        bad_profile = tmp_path / ".her" / "profiles" / "My Bot!"
    ...
    def test_systemd_unit_includes_profile(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(gateway_cli, "get_her_home", lambda: profile_dir)
        unit = gateway_cli.generate_systemd_unit(system=False)
    ...
    def test_launchd_plist_path_uses_real_user_home_not_profile_home(self, tmp_path, monkeypatch):
        profile_dir = tmp_path / ".her" / "profiles" / "orcha"
        profile_dir.mkdir(parents=True)
        ...
        assert plist_path == machine_home / "Library" / "LaunchAgents" / "ai.her.gateway-orcha.plist"
...
class TestRemapPathForUser:
    ...
    def test_remaps_path_under_current_home(self, monkeypatch, tmp_path):
        ...
        result = gateway_cli._remap_path_for_user(
            str(tmp_path / "root" / ".her" / "her-agent"),
            str(tmp_path / "alice"),
        )
        assert result == str(tmp_path / "alice" / ".her" / "her-agent")

    def test_keeps_system_path_unchanged(self, monkeypatch, tmp_path):
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "root")
        (tmp_path / "root").mkdir()
        result = gateway_cli._remap_path_for_user("/opt/her", str(tmp_path / "alice"))
        assert result == "/opt/her"

    def test_noop_when_same_user(self, monkeypatch, tmp_path):
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "alice")
        (tmp_path / "alice").mkdir()
        original = str(tmp_path / "alice" / ".her" / "her-agent")
...
class TestSystemUnitPathRemapping:
    ...
    def test_system_unit_has_no_root_paths(self, monkeypatch, tmp_path):
        ...
        root_home.mkdir()
        project = root_home / ".her" / "her-agent"
        ...
        monkeypatch.setenv("HER_HOME", str(root_home / ".her"))
        monkeypatch.setattr(gateway_cli, "get_her_home", lambda: root_home / ".her")
        ...
        # WorkingDirectory is anchored at the target user's HER_HOME (stable,
        ...
        assert "WorkingDirectory=/home/alice/.her" in unit
        assert "WorkingDirectory=/home/alice/.her/her-agent" not in unit
...
class TestDockerAwareGateway:
    ...
    def test_run_systemctl_raises_runtimeerror_when_missing(self, monkeypatch):
        ...
        with pytest.raises(RuntimeError, match="systemctl is not available"):
            gateway_cli._run_systemctl(["start", "her-gateway"])
    ...
    def test_run_systemctl_passes_through_on_success(self, monkeypatch):
        ...
        result = gateway_cli._run_systemctl(["status", "her-gateway"])
        assert result.returncode == 0
        assert len(calls) == 1
        assert "status" in calls[0]

    def test_install_in_container_prints_docker_guidance(self, monkeypatch, capsys):
        """'her gateway install' inside Docker exits 0 with container guidance."""
        import pytest
    ...
    def test_uninstall_in_container_prints_docker_guidance(self, monkeypatch, capsys):
        """'her gateway uninstall' inside Docker exits 0 with container guidance."""
        import pytest
    ...
    def test_start_in_container_prints_docker_guidance(self, monkeypatch, capsys):
        """'her gateway start' inside Docker exits 0 with container guidance."""
        import pytest
...
class TestLegacyHermesUnitDetection:
    ...
    older install left a ``her.service`` unit behind when the service was
    renamed to ``her-gateway.service``. After PR #5646 (signal recovery
    via systemd), the two services began SIGTERM-flapping over the same
    Telegram bot token in a 30-second cycle.

    The detector must flag ``her.service`` ONLY when it actually runs our
    gateway, and must NEVER flag profile units
    (``her-gateway-<profile>.service``) or unrelated third-party services.
    """
    ...
    def test_detects_legacy_her_service_in_user_scope(self, tmp_path, monkeypatch):
        user_dir, _ = self._setup_search_paths(tmp_path, monkeypatch)
        legacy = user_dir / "her.service"
        ...
        assert name == "her.service"
        assert path == legacy
        assert is_system is False
        assert gateway_cli.has_legacy_her_units() is True

    def test_detects_legacy_her_service_in_system_scope(self, tmp_path, monkeypatch):
        _, system_dir = self._setup_search_paths(tmp_path, monkeypatch)
        legacy = system_dir / "her.service"
        legacy.write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
    ...
    def test_ignores_profile_unit_her_gateway_coder(self, tmp_path, monkeypatch):
        """CRITICAL: profile units must NOT be flagged as legacy.

        Teknium's concern — ``her-gateway-coder.service`` is our standard
        ...
        for base in (user_dir, system_dir):
            (base / "her-gateway-coder.service").write_text(
                self._OUR_UNIT_TEXT, encoding="utf-8"
            )
            (base / "her-gateway-orcha.service").write_text(
                self._OUR_UNIT_TEXT, encoding="utf-8"
            )
            (base / "her-gateway.service").write_text(
                self._OUR_UNIT_TEXT, encoding="utf-8"
            )
        ...
        assert results == []
        assert gateway_cli.has_legacy_her_units() is False

    def test_ignores_unrelated_her_service(self, tmp_path, monkeypatch):
        """Third-party ``her.service`` that isn't ours stays untouched.

        If a user has some other package named ``her`` installed as a
    ...
    def test_returns_empty_when_no_legacy_files_exist(self, tmp_path, monkeypatch):
        ...
        assert gateway_cli._find_legacy_her_units() == []
        assert gateway_cli.has_legacy_her_units() is False
    ...
    def test_detects_both_scopes_simultaneously(self, tmp_path, monkeypatch):
        ...
        user_dir, system_dir = self._setup_search_paths(tmp_path, monkeypatch)
        (user_dir / "her.service").write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
        (system_dir / "her.service").write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
    ...
    def test_accepts_alternate_execstart_formats(self, tmp_path, monkeypatch):
        ...
        ExecStart variants we've seen in the wild:
          - python -m her_cli.main gateway run
          - python path/to/her_cli/main.py gateway run
          - her gateway run   (direct binary)
          - python path/to/gateway/run.py
        """
        user_dir, _ = self._setup_search_paths(tmp_path, monkeypatch)
        variants = [
            "ExecStart=/venv/bin/python -m her_cli.main gateway run --replace",
            "ExecStart=/venv/bin/python /opt/her/her_cli/main.py gateway run",
            "ExecStart=/usr/local/bin/her gateway run --replace",
            "ExecStart=/venv/bin/python /opt/her/gateway/run.py",
        ]
    ...
    def test_print_legacy_unit_warning_shows_migration_hint(self, tmp_path, monkeypatch, capsys):
        ...
        assert "her.service" in out
        assert "her gateway migrate-legacy" in out
    ...
    def test_handles_unreadable_unit_file_gracefully(self, tmp_path, monkeypatch):
        ...
        user_dir, _ = self._setup_search_paths(tmp_path, monkeypatch)
        unreadable = user_dir / "her.service"
...
class TestRemoveLegacyHermesUnits:
    ...
    def test_returns_zero_when_no_legacy_units(self, tmp_path, monkeypatch, capsys):
        ...
        removed, remaining = gateway_cli.remove_legacy_her_units(interactive=False)
    ...
    def test_dry_run_lists_without_removing(self, tmp_path, monkeypatch, capsys):
        ...
        removed, remaining = gateway_cli.remove_legacy_her_units(
    ...
    def test_removes_user_scope_legacy_unit(self, tmp_path, monkeypatch, capsys):
        ...
        assert any("--user stop her.service" in c for c in cmds_joined)
        assert any("--user disable her.service" in c for c in cmds_joined)
    ...
    def test_system_scope_without_root_defers_removal(self, tmp_path, monkeypatch, capsys):
        ...
        out = capsys.readouterr().out
        assert "sudo her gateway migrate-legacy" in out
    ...
    def test_system_scope_with_root_removes(self, tmp_path, monkeypatch, capsys):
        ...
        assert any(
            c.startswith("systemctl stop her.service") for c in cmds_joined
        )
        assert any(
            c.startswith("systemctl disable her.service") for c in cmds_joined
        )
    ...
    def test_removes_both_scopes_with_root(self, tmp_path, monkeypatch, capsys):
        ...
        user_legacy = user_dir / "her.service"
        system_legacy = system_dir / "her.service"
    ...
    def test_does_not_touch_profile_units_during_migration(
        ...
        """Teknium's constraint: profile units (her-gateway-coder.service)
        ...
        profile_unit = user_dir / "her-gateway-coder.service"
        profile_unit.write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
        default_unit = user_dir / "her-gateway.service"
    ...
    def test_interactive_prompt_no_skips_removal(self, tmp_path, monkeypatch, capsys):
        ...
        removed, remaining = gateway_cli.remove_legacy_her_units(interactive=True)
...
class TestMigrateLegacyCommand:
    """Tests for the `her gateway migrate-legacy` subcommand dispatch."""
    ...
    def test_migrate_legacy_subparser_accepts_dry_run_and_yes(self):
        ...
        import her_cli.main as cli_main
        ...
        # public, just confirm that `her gateway --help` shows it.
        import subprocess
        import sys
        ...
        result = subprocess.run(
            [sys.executable, "-m", "her_cli.main", "gateway", "--help"],
    ...
    def test_gateway_command_migrate_legacy_dispatches(
        ...
        monkeypatch.setattr(gateway_cli, "remove_legacy_her_units", fake_remove)
        monkeypatch.setattr(gateway_cli, "supports_systemd_services", lambda: True)
        monkeypatch.setattr(gateway_cli, "is_macos", lambda: False)
...
class TestGatewayStatusParser:
    def test_gateway_status_subparser_accepts_full_flag(self):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "her_cli.main", "gateway", "status", "-l", "--help"],
...
class TestSystemdInstallOffersLegacyRemoval:
    ...
    def test_install_offers_removal_when_legacy_detected(
        ...
        # has_legacy_her_units must return True
        monkeypatch.setattr(gateway_cli, "has_legacy_her_units", lambda: True)
        monkeypatch.setattr(gateway_cli, "remove_legacy_her_units", fake_remove)
...
class TestSystemScopeWizardPreCheck:
    ...
    def _setup_units(tmp_path, monkeypatch, system_present: bool, user_present: bool):
        ...
        usr_dir.mkdir()
        if system_present:
            (sys_dir / "her-gateway.service").write_text("[Unit]\n")
        if user_present:
            (usr_dir / "her-gateway.service").write_text("[Unit]\n")
        monkeypatch.setattr(
            gateway_cli,
            "get_systemd_unit_path",
            lambda system=False: (sys_dir if system else usr_dir) / "her-gateway.service",
        )
    ...
    def test_non_root_with_explicit_system_arg_returns_true(self, tmp_path, monkeypatch):
        # Caller passed system=True explicitly (e.g. ``her gateway start --system``).
        self._setup_units(tmp_path, monkeypatch, system_present=False, user_present=False)
        monkeypatch.setattr(gateway_cli.os, "geteuid", lambda: 1000)
...
class TestSystemScopeRemediationOutput:
    ...
    def test_start_remediation_mentions_sudo_systemctl_and_uninstall(self, capsys, monkeypatch):
        monkeypatch.setattr(gateway_cli, "get_service_name", lambda: "her-gateway")
        ...
        assert "sudo systemctl start her-gateway" in out
        assert "sudo her gateway uninstall --system" in out
        assert "her gateway install" in out
    ...
    def test_restart_remediation_uses_systemctl_restart(self, capsys, monkeypatch):
        ...
        assert "sudo systemctl restart her-gateway" in out
    ...
    def test_stop_remediation_uses_systemctl_stop(self, capsys, monkeypatch):
        ...
        assert "sudo systemctl stop her-gateway" in out


class TestGatewayCommandCatchesSystemScopeError:
    """The direct CLI path (``her gateway start --system`` etc.) must
...
class TestServiceWorkingDirIsStable:
    ...
    (HER_HOME), never the source checkout / worktree, so a relocated or
    deleted checkout can't crash-loop the unit on CHDIR (status=200).
    """

    def test_stable_working_dir_uses_her_home(self, tmp_path, monkeypatch):
        home = tmp_path / ".her"
        home.mkdir()
        monkeypatch.setattr(gateway_cli, "get_her_home", lambda: home)
    ...
    def test_stable_working_dir_falls_back_to_project_root(self, tmp_path, monkeypatch):
        # HER_HOME points somewhere that does not exist -> fall back.
        missing = tmp_path / "does-not-exist" / ".her"
        monkeypatch.setattr(gateway_cli, "get_her_home", lambda: missing)
        assert gateway_cli._stable_service_working_dir() == str(gateway_cli.PROJECT_ROOT)

    def test_user_unit_workingdirectory_is_her_home_not_checkout(self, tmp_path, monkeypatch):
        home = tmp_path / ".her"
        home.mkdir()
        monkeypatch.setattr(gateway_cli, "get_her_home", lambda: home)
        unit = gateway_cli.generate_systemd_unit(system=False)
    ...
    def test_launchd_workingdirectory_is_her_home(self, tmp_path, monkeypatch):
        import re
