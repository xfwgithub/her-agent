"""Tests for `_can_open_graphical_browser()` in her_cli.auth.

Guards the fix for the May 2026 report where `her auth add xai-oauth`
...
from her_cli.auth import _can_open_graphical_browser
...
def _force_platform_linux(monkeypatch):
    monkeypatch.setattr("her_cli.auth.sys.platform", "linux")
