"""Tests for the top-level `./her` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./her` should use `her_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "her"
    called = []

    fake_main_module = types.ModuleType("her_cli.main")

    def fake_main():
        called.append("her_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "her_cli.main", fake_main_module)
