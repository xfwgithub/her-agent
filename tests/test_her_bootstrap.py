...
def _fresh_import():
    """Return a freshly-imported her_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("her_bootstrap", None)
    import her_bootstrap  # noqa: WPS433
    return her_bootstrap
...
class TestEntryPointsImportBootstrap:
    ...
    ENTRY_POINTS = [
        ...
        "run_agent.py",          # her-agent (console_script)
        "acp_adapter/entry.py",  # her-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import her_bootstrap' and that
        ...
        half-finished ``her update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``her_bootstrap`` as a top-level module) leaves her
        ...
        """
        # Resolve relative to the her-agent repo root.  Tests live
        # at tests/test_her_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # her start even when her_bootstrap hasn't been
        ...
        assert first_import_name == "her_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'her_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import her_bootstrap' line to be the first import."
        )
