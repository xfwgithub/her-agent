...
binary download are exercised manually by `her secrets bitwarden
...
def her_home(tmp_path, monkeypatch):
    ...
    # Some modules cache get_her_home; clear if needed.
    import her_constants
    if hasattr(her_constants, "_HER_HOME_CACHE"):
        her_constants._HER_HOME_CACHE = None  # type: ignore[attr-defined]
    return home
...
def test_install_bws_rejects_malicious_member(her_home, monkeypatch):
...
def test_install_bws_happy_path(her_home, monkeypatch):
...
def test_install_bws_checksum_mismatch(her_home, monkeypatch):
...
def test_install_bws_missing_checksum_entry(her_home, monkeypatch):
...
def test_env_loader_skips_when_disabled(tmp_path, monkeypatch):
    ...
    from her_cli.env_loader import _apply_external_secret_sources
...
def test_disk_cache_written_after_first_fetch(monkeypatch, tmp_path):
    """First fetch hits bws AND writes a 0600 file under her_home/cache/."""
    home = tmp_path / ".her"
    home.mkdir()
    fake_binary = tmp_path / "bws"
    fake_binary.write_text("")
