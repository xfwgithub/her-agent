"""``her dashboard register`` — register a self-hosted dashboard OAuth client.
...
This command:
  1. Resolves a fresh Nous Portal access token from the existing login
     (``~/.her/auth.json``), refreshing it if needed. Fails fast with a
     "run `her setup`" hint when the user isn't logged in.
...
def _resolve_portal_base_url(override: Optional[str] = None) -> str:
    ...
    Precedence:
      1. ``override`` — explicit ``--portal-url`` flag or
         ``HER_DASHBOARD_PORTAL_URL`` env (used for testing against a
    ...
    try:
        from her_cli.auth import DEFAULT_NOUS_PORTAL_URL, get_provider_auth_state
    ...
    try:
        ...
        if exc.code == 401:
            raise RuntimeError(
                ...
                "Try `her auth login nous` to re-authenticate."
            ) from exc
        if exc.code == 403:
            raise RuntimeError(
                detail
...
def _print_post_register_hint(
    *,
    ...
    from her_cli.config import get_env_path
    ...
    print(
        ...
        "  `her dashboard` (localhost) leaves the gate off and serves locally\n"
    ...
    if custom_redirect_uri:
        ...
        print("    her dashboard --host 0.0.0.0")
...
def cmd_dashboard_register(args) -> None:
    ...
    from her_cli.auth import AuthError, resolve_nous_access_token
    ...
    if is_managed():
        print(
            "✗ `her dashboard register` is not available in a managed/hosted "
            "install.\n"
    ...
    try:
        ...
        if getattr(exc, "relogin_required", False):
            ...
            print("  Run `her setup` (or `her auth login nous`) first, then retry.")
        else:
    ...
    # HER_DASHBOARD_PORTAL_URL env var, else the stored login's portal.
    ...
    try:
        existing_portal = get_env_value("HER_DASHBOARD_PORTAL_URL")
    except Exception:
        existing_portal = None
    if not existing_portal and portal_base_url.rstrip("/") != default_portal:
        try:
            save_env_value("HER_DASHBOARD_PORTAL_URL", portal_base_url)
