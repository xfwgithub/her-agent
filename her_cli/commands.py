"""Slash command definitions and autocomplete for the her CLI.
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("rollback", "List or restore filesystem checkpoints", "Session",
               args_hint="[number]"),
    CommandDef("snapshot", "Create or restore state snapshots of her config/state", "Session",
    ...
    CommandDef("steer", "Inject a message after the next tool call without interrupting", "Session",
               args_hint="<prompt>"),
    CommandDef("goal", "Set a standing goal her works on across turns until achieved", "Session",
    ...
    CommandDef("busy", "Control what Enter does while her is working", "Configuration",
    ...
    CommandDef("reload-mcp", "Reload MCP servers from config", "Tools & Skills",
               aliases=("reload_mcp",)),
    CommandDef("reload-skills", "Re-scan ~/.her/skills/ for newly installed or removed skills",
    ...
    CommandDef("update", "Update her Agent to the latest version", "Info"),
    CommandDef("version", "Show her Agent version", "Info", aliases=("v",)),
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`her_cli.plugins.PluginContext.register_command`. They behave
    like ``CommandDef`` entries for gateway surfacing: they appear in the
    Telegram command menu, in Slack's ``/her`` subcommand mapping, and
...
Telegram only displays a small BotCommand menu in practice.  The full her
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``her skills list`` and the agent's ``/skill-name`` dispatch but
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``her skills
...
#: controls; her commands can never collide with them).  Retained as a
...
def slack_app_manifest(request_url: str = "https://her-agent.local/slack/commands") -> dict[str, Any]:
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /her handler.

    Maps both canonical names and aliases so /her bg do stuff works
    the same as /her background do stuff.

    Plugin-registered slash commands are included so ``/her <plugin-cmd>``
