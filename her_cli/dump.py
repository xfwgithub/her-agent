...
from her_cli.config import get_her_home, get_env_path, get_project_root, load_config
...
def _redact(value: str) -> str:
    ...
    ``her dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _cron_summary(her_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = her_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"her_home:      {display_her_home()}")
    ...
    toolsets = config.get("toolsets", ["her-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(her_home)}")
    lines.append(f"  skills:             {_count_skills(her_home)}")
