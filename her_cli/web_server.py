...
try:
    ...
    # running `her dashboard` needs fastapi+uvicorn; lazy install keeps
...
WEB_DIST = Path(os.environ["HER_WEB_DIST"]) if "HER_WEB_DIST" in os.environ else Path(__file__).parent / "web_dist"
...
app = FastAPI(title="her Agent", version=__version__, lifespan=_lifespan)
...
# ---------------------------------------------------------------------------
_SESSION_TOKEN = os.environ.get("HER_DASHBOARD_SESSION_TOKEN") or secrets.token_urlsafe(32)
...
#
# This list is defined in ``her_cli.dashboard_auth.public_paths`` so the
...
# ---------------------------------------------------------------------------
from her_cli.dashboard_auth.public_paths import (
...
async def _dashboard_auth_gate(request: Request, call_next):
    from her_cli.dashboard_auth.middleware import gated_auth_middleware
...
_SCHEMA_OVERRIDES: Dict[str, Dict[str, Any]] = {
    ...
    "updates.non_interactive_local_changes": {
        "type": "select",
        "description": (
            "When the chat app / gateway updates her (no terminal prompt), "
...
async def get_status():
    ...
    # and which providers are registered so ``her status`` and the
    # SPA's StatusPage can show "OAuth gate ON via Nous Research" or
    # "loopback only — no auth gate" with no extra round trips.
    auth_required = bool(getattr(app.state, "auth_required", False))
    auth_providers: list[str] = []
    try:
        from her_cli.dashboard_auth import list_providers as _list_providers
...
async def get_system_stats():
    ...
    and non-sensitive (no env values, no paths beyond the her home root).
    """
    ...
    try:
        ...
        try:
            du = psutil.disk_usage(str(get_her_home()))
            info["disk"] = {
                "total": du.total,
                "used": du.used,
                "free": du.free,
...
async def run_curator():
    ...
    try:
        proc = _spawn_her_action(["curator", "run"], "curator-run")
    except Exception as exc:
...
async def run_prompt_size():
    try:
        proc = _spawn_her_action(["prompt-size"], "prompt-size")
    except Exception as exc:
...
async def run_dump():
    try:
        proc = _spawn_her_action(["dump"], "dump")
    except Exception as exc:
        raise HTTPException(status_code=500, detail=f"Failed: {exc}")
...
async def run_config_migrate():
    try:
        proc = _spawn_her_action(["config", "migrate"], "config-migrate")
    except Exception as exc:
        raise HTTPException(status_code=500, detail=f"Failed: {exc}")
...
async def run_debug_share_endpoint(body: DebugShareRequest | None = None):
    ...
    *synchronous*: the whole point of ``debug share`` is the set of shareable
    ...
    """
    from her_cli.debug import build_debug_share
...
# streamed to a per-action log file under ``~/.her/logs/<action>.log`` so
# the dashboard can tail them back to the user.
# ---------------------------------------------------------------------------

_ACTION_LOG_DIR: Path = get_her_home() / "logs"
...
def _spawn_her_action(subcommand: List[str], name: str) -> subprocess.Popen:
    """Spawn ``her <subcommand>`` detached and record the Popen handle.

    Uses the running interpreter's ``her_cli.main`` module so the action
    ...
    cmd = [sys.executable, "-m", "her_cli.main", *subcommand]
...
async def restart_gateway():
    """Kick off a ``her gateway restart`` in the background."""
    try:
        proc = _spawn_her_action(["gateway", "restart"], "gateway-restart")
    except Exception as exc:
...
@app.post("/api/her/update")
async def update_her():
    """Kick off ``her update`` in the background."""
    install_method = detect_install_method(PROJECT_ROOT)
    if install_method == "docker":
        message = format_docker_update_message()
        _record_completed_action("her-update", message, exit_code=1)
        return {
            "ok": False,
            "pid": None,
            "name": "her-update",
    ...
    try:
        proc = _spawn_her_action(["update"], "her-update")
    except Exception as exc:
        _log.exception("Failed to spawn her update")
...
@app.get("/api/her/update/check")
async def check_her_update(force: bool = False):
    """Report whether a her update is available, without applying it.

    Powers the dashboard's "check before you update" flow: the System page
    shows the commit-behind count and asks the user to confirm before
    ``POST /api/her/update`` actually runs ``her update``.

    Returns:
        install_method: 'git' | 'pip' | 'docker' | 'nixos' | 'homebrew' | ...
        current_version: installed her version string
    ...
    try:
        ...
        if force:
            try:
                (get_her_home() / ".update_check").unlink()
            except OSError:
                pass
...
async def get_profiles_sessions(
    ...
    from her_state import SessionDB
    from her_cli import profiles as profiles_mod
...
async def search_sessions(q: str = "", limit: int = 20):
    ...
    try:
        ...
        try:
            ...
            # logs, or another her surface. FTS can't find those unless the
...
def _normalize_config_for_web(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    her supports ``model`` as either a bare string (``"anthropic/claude-sonnet-4"``)
...
# in her_cli/config.py — listed here for deterministic ordering in the UI.
...
def get_model_options():
    ...
    """
    try:
        from her_cli.inventory import build_models_payload, load_picker_context
        ...
        # onboarding picker) sees the SAME full provider universe `her model`
...
def get_recommended_default_model(provider: str = ""):
    ...
    Mirrors the model-curation `her model` does so GUI onboarding lands on a
    ...
    if slug == "nous":
        try:
            ...
            from her_cli.auth import get_provider_auth_state
...
async def set_model_assignment(body: ModelAssignment):
    ...
    Writes to ``~/.her/config.yaml`` — applies to **new** sessions only.
    ...
    try:
        ...
        if scope == "main":
            ...
            # post-model-selection behaviour (her_cli/main.py
            ...
            if provider.strip().lower() == "nous":
                try:
                    from her_cli.nous_subscription import apply_nous_managed_defaults
...
_PLATFORM_OVERRIDES: dict[str, dict[str, Any]] = {
    "telegram": {
        "name": "Telegram",
        "description": "Run her from Telegram DMs, groups, and topics.",
    ...
    "api_server": {
        "name": "API server",
        "description": "Expose her as an OpenAI-compatible HTTP API for tools like Open WebUI.",
        "docs_url": "https://her-agent.nousresearch.com/docs/user-guide/messaging/",
        "env_vars": (
            "API_SERVER_ENABLED",
            "API_SERVER_KEY",
            "API_SERVER_PORT",
    ...
    "webhook": {
        ...
        "docs_url": "https://her-agent.nousresearch.com/docs/user-guide/messaging/webhooks/",
...
# Display labels for env vars not in OPTIONAL_ENV_VARS.  The legacy
# platform-specific fallbacks (SIGNAL_*, WHATSAPP_*, HASS_*, EMAIL_*,
# TWILIO_*, MSGRAPH_*, MATTERMOST_*, MATRIX_*, DISCORD_*, SLACK_*,
# DINGTALK_*, FEISHU_*, WECOM_*, WEIXIN_*, BLUEBUBBLES_*, QQ_*,
# YUANBAO_*) were removed in the 2026-06-07 platform slim.  Any third-party
# platform that needs an un-OPTIONAL_ENV_VARS label should register it via
# the plugin system (see plugins/platforms/<name>/adapter.py for examples).
...
_TELEGRAM_ONBOARDING_DEFAULT_URL = "https://setup.her-agent.nousresearch.com"
...
async def start_telegram_onboarding(body: TelegramOnboardingStart):
    bot_name = (body.bot_name or "her Agent").strip() or "her Agent"
...
# the canonical ``her auth add <provider>`` command so the dashboard
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    her resolves Anthropic creds in this order at runtime:
    1. ``~/.her/.anthropic_oauth.json`` — her-managed PKCE flow
    ...
    try:
        from agent.anthropic_adapter import (
            read_her_oauth_credentials,
            read_claude_code_credentials,
            _HER_OAUTH_FILE,
        )
    except ImportError:
        read_claude_code_credentials = None  # type: ignore
        read_her_oauth_credentials = None  # type: ignore
        _HER_OAUTH_FILE = None  # type: ignore

    her_creds = None
    if read_her_oauth_credentials:
        try:
            her_creds = read_her_oauth_credentials()
        except Exception:
            her_creds = None
    if her_creds and her_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "her_pkce",
            "source_label": f"her PKCE ({_HER_OAUTH_FILE})",
            "token_preview": _truncate_token(her_creds.get("accessToken")),
            "expires_at": her_creds.get("expiresAt"),
            "has_refresh_token": bool(her_creds.get("refreshToken")),
        }
...
def _claude_code_only_status() -> Dict[str, Any]:
    ...
    Claude Code subscription tokens are actively flowing into her even
    when they also have a separate her-managed PKCE login.
    """
    try:
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "her auth add openai-codex",
    ...
    },
    {
        ...
        "flow": "external",
        "cli_command": "her auth add qwen-oauth",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "her auth add minimax-oauth",
    ...
    },
    {
        ...
        "flow": "loopback",
        "cli_command": "her auth add xai-oauth",
        "docs_url": "https://her-agent.nousresearch.com/docs/guides/xai-grok-oauth",
    ...
    # usage credits on top of a Claude Max plan — see disclaimer in name).
    {
        ...
        "flow": "pkce",
        "cli_command": "her auth add anthropic",
...
async def list_oauth_providers():
    ...
    Response shape (per provider):
        ...
        status:
          ...
          source           short slug ("her_pkce", "claude_code", ...)
...
async def disconnect_oauth_provider(provider_id: str, request: Request):
    ...
    # Anthropic and claude-code clear the same her-managed PKCE file
    # AND forget the Claude Code import. We don't touch ~/.claude/* directly
    # — that's owned by the Claude Code CLI; users can re-auth there if they
    # want to undo a disconnect.
    if provider_id in {"anthropic", "claude-code"}:
        try:
            from agent.anthropic_adapter import _HER_OAUTH_FILE
            if _HER_OAUTH_FILE.exists():
                _HER_OAUTH_FILE.unlink()
        except Exception:
            pass
...
#          → persists to ~/.her/.anthropic_oauth.json AND credential pool
...
# Guarded so her web still starts if anthropic_adapter is unavailable;
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    """Persist Anthropic PKCE creds to both her file AND credential pool.

    Mirrors what auth_commands.add_command does so the dashboard flow leaves
    the system in the same state as ``her auth add anthropic``.
    """
    from agent.anthropic_adapter import _HER_OAUTH_FILE
    payload = {
    ...
    _HER_OAUTH_FILE.parent.mkdir(parents=True, exist_ok=True)
    tmp_path = _HER_OAUTH_FILE.with_name(
        f"{_HER_OAUTH_FILE.name}.tmp.{os.getpid()}.{secrets.token_hex(8)}"
    )
    try:
        ...
        os.replace(tmp_path, _HER_OAUTH_FILE)
        try:
            _HER_OAUTH_FILE.chmod(stat.S_IRUSR | stat.S_IWUSR)
        except OSError:
            pass
    finally:
        try:
...
def _codex_full_login_worker(session_id: str) -> None:
    ...
    try:
        ...
        from her_cli.auth import _save_codex_tokens
...
async def get_session_stats():
    """Session-store statistics for the Sessions page (mirrors `her sessions stats`).
...
async def prune_sessions_endpoint(body: SessionPrune):
    """Delete ended sessions older than N days (mirrors `her sessions prune`)."""
...
async def get_logs(
    ...
    log_path = get_her_home() / "logs" / log_name
...
def _cron_profile_home(profile: Optional[str]) -> Tuple[str, Path]:
    """Resolve a profile query value to (profile_name, HER_HOME)."""
    from her_cli import profiles as profiles_mod
...
def _call_cron_for_profile(profile: Optional[str], func_name: str, *args, **kwargs):
    ...
    from the process HER_HOME at import time. The dashboard is a single
...
#
# Wraps the same config data layer the CLI uses (her_cli.mcp_config), so
# servers managed here show up under `her mcp list` and vice versa.  Secrets
...
async def list_mcp_servers():
    from her_cli.mcp_config import _get_mcp_servers
...
async def add_mcp_server(body: MCPServerCreate):
    from her_cli.mcp_config import _get_mcp_servers, _save_mcp_server
...
async def remove_mcp_server(name: str):
    from her_cli.mcp_config import _remove_mcp_server
...
async def test_mcp_server(name: str):
    ...
    from her_cli.mcp_config import _get_mcp_servers, _probe_single_server
...
async def list_mcp_catalog():
    ...
    `her mcp catalog` / `her mcp install` read.
    """
    try:
        from her_cli import mcp_catalog
    except Exception as exc:
...
async def install_mcp_catalog_entry(body: MCPCatalogInstall):
    ...
    if entry.install is not None:
        try:
            proc = _spawn_her_action(["mcp", "install", name], "mcp-install")
        except Exception as exc:
...
#
# Wraps the same JSON store the CLI uses (her_cli.webhook); the webhook
...
# spawn the real `her gateway <verb>` so behaviour matches the CLI exactly.
...
async def start_gateway():
    try:
        proc = _spawn_her_action(["gateway", "start"], "gateway-start")
    except Exception as exc:
...
async def stop_gateway():
    try:
        proc = _spawn_her_action(["gateway", "stop"], "gateway-stop")
    except Exception as exc:
...
async def list_credential_pool():
    ...
    from her_cli.auth import read_credential_pool
...
# `her memory setup`).  The dashboard covers the common admin actions:
...
async def set_memory_provider(body: MemoryProviderSelect):
    ...
    if provider:
        ...
        if provider not in valid:
            raise HTTPException(
                status_code=400,
                detail=f"Unknown memory provider '{provider}'. Run `her memory setup` to configure a new one.",
            )
...
async def run_doctor():
    try:
        proc = _spawn_her_action(["doctor"], "doctor")
    except Exception as exc:
...
async def run_security_audit():
    try:
        proc = _spawn_her_action(["security", "audit"], "security-audit")
    except Exception as exc:
...
async def run_backup(body: BackupRequest):
    ...
    try:
        proc = _spawn_her_action(args, "backup")
    except Exception as exc:
...
async def run_import(body: ImportRequest):
    ...
    try:
        proc = _spawn_her_action(["import", archive], "import")
    except Exception as exc:
...
async def list_hooks():
    ...
    try:
        from her_cli.plugins import VALID_HOOKS
...
async def list_checkpoints():
    ...
    # Checkpoints live under <her_home>/checkpoints/.  Surface a count +
    ...
    cp_dir = get_her_home() / "checkpoints"
    sessions = []
    total_bytes = 0
    if cp_dir.is_dir():
...
async def prune_checkpoints():
    try:
        proc = _spawn_her_action(["checkpoints", "prune"], "checkpoints-prune")
    except Exception as exc:
...
async def install_skill_hub(body: SkillInstallRequest):
    ...
    try:
        proc = _spawn_her_action(["skills", "install", identifier], "skills-install")
    except Exception as exc:
...
async def uninstall_skill_hub(body: SkillUninstallRequest):
    ...
    try:
        proc = _spawn_her_action(["skills", "uninstall", name, "--yes"], "skills-uninstall")
    except Exception as exc:
...
async def update_skills_hub():
    try:
        proc = _spawn_her_action(["skills", "update"], "skills-update")
    except Exception as exc:
...
# Human-readable labels for each hub source id (matches `her skills search`
...
async def preview_skill_hub(identifier: str = ""):
    ...
    def _run():
        from her_cli.skills_hub import _resolve_source_meta_and_bundle
        from tools.skills_hub import create_source_router
...
def _fallback_profile_dicts(profiles_mod) -> List[Dict[str, Any]]:
    ...
    default_home = profiles_mod._get_default_her_home()
...
def _profile_setup_command(name: str) -> str:
    ...
    return "her setup" if name == "default" else f"{name} setup"
...
def _write_profile_model(profile_dir: Path, provider: str, model: str) -> None:
    ...
    context-local HER_HOME override so the write lands in the target
    ...
    """
    from her_constants import set_her_home_override, reset_her_home_override

    token = set_her_home_override(str(profile_dir))
    try:
        cfg = load_config()
        cfg["model"] = _apply_main_model_assignment(cfg.get("model", {}), provider, model)
        save_config(cfg)
    finally:
        reset_her_home_override(token)
...
async def get_active_profile_endpoint():
    ...
    ``active`` is the sticky default written by ``her profile use`` —
    the profile new CLI invocations pick up. ``current`` is the profile
    the running dashboard/gateway is scoped to (derived from HER_HOME).
    """
    from her_cli import profiles as profiles_mod
    try:
...
async def set_active_profile_endpoint(body: ProfileActiveUpdate):
    """Set the sticky active profile (mirrors ``her profile use``).
...
async def update_profile_model_endpoint(name: str, body: ProfileModelUpdate):
    ...
    to the named profile via the HER_HOME override.
    """
    profile_dir = _resolve_profile_dir(name)
    provider = (body.provider or "").strip()
    model = (body.model or "").strip()
...
async def describe_profile_auto_endpoint(name: str, body: ProfileDescribeAuto):
    ...
    (``auxiliary.profile_describer``). Mirrors ``her profile describe
    <name> --auto``.
    ...
    _resolve_profile_dir(name)
    try:
        from her_cli import profile_describer
...
async def get_toolset_config(name: str):
    ...
    Surfaces the same provider rows the CLI ``her tools`` picker shows
...
async def select_toolset_provider(name: str, body: ToolsetProviderSelect):
    ...
    core extracted from the CLI configurator — so the GUI and ``her tools``
...
async def save_toolset_env(name: str, body: ToolsetEnvUpdate):
    ...
    Writes each ``key: value`` to ``~/.her/.env`` via ``save_env_value`` —
    the same store ``her tools`` writes when it prompts for keys. Keys are
...
async def run_toolset_post_setup(name: str, body: ToolsetPostSetup):
    ...
    ``her tools post-setup <key>`` and the frontend tails the log via
...
#
# The endpoint spawns the same ``her --tui`` binary the CLI uses, behind
...
def _ws_auth_reason(ws: "WebSocket") -> tuple[Optional[str], str]:
    ...
    * ``?internal=<process-credential>`` — the process-lifetime internal
    ...
    if auth_required:
        ...
        from her_cli.dashboard_auth.audit import AuditEvent, audit_log
        from her_cli.dashboard_auth.ws_tickets import (
            TicketInvalid,
...
def _resolve_chat_argv(
    ...
    Default: whatever ``her --tui`` would run.  Tests monkeypatch this
    function to inject a tiny fake command (``cat``, ``sh -c 'printf …'``)
    so nothing has to build Node or the TUI bundle.

    Session resume is propagated via the ``HER_TUI_RESUME`` env var —
    matching what ``her_cli.main._launch_tui`` does for the CLI path.
    Appending ``--resume <id>`` to argv doesn't work because ``ui-tui`` does
    not parse its argv.

    ``HER_TUI_GATEWAY_URL`` is injected so the PTY child can attach to
    this process's in-memory ``tui_gateway`` instance instead of spawning
    its own Python gateway subprocess.

    `sidecar_url` (when set) is forwarded as ``HER_TUI_SIDECAR_URL`` so
    the spawned ``tui_gateway.entry`` can mirror dispatcher emits to the
    dashboard's ``/api/pub`` endpoint (see :func:`pub_ws`).
    """
    from her_cli.main import PROJECT_ROOT, _make_tui_argv
    ...
    env.setdefault("HER_TUI_DISABLE_MOUSE", "1")
    env.setdefault("HER_TUI_INLINE", "1")

    if resume:
        latest_resume, _latest_path = _session_latest_descendant(resume)
        if latest_resume:
            resume = latest_resume
        env["HER_TUI_RESUME"] = resume

    if sidecar_url:
        env["HER_TUI_SIDECAR_URL"] = sidecar_url

    if gateway_ws_url := _build_gateway_ws_url():
        env["HER_TUI_GATEWAY_URL"] = gateway_ws_url
...
def _build_gateway_ws_url() -> Optional[str]:
    ...
    if getattr(app.state, "auth_required", False):
        from her_cli.dashboard_auth.ws_tickets import internal_ws_credential
...
async def pty_ws(ws: WebSocket) -> None:
    ...
    if not _PTY_BRIDGE_AVAILABLE:
        await ws.send_text(
            ...
            "\x1b[33mInstall her inside WSL2 to use the dashboard's /chat "
...
# HER_TUI_SIDECAR_URL set in /api/pty's PTY env) and writes every
...
def _normalise_prefix(raw: Optional[str]) -> str:
    ...
    Thin re-export of :func:`her_cli.dashboard_auth.prefix.normalise_prefix`
    ...
    """
    from her_cli.dashboard_auth.prefix import normalise_prefix
...
def mount_spa(application: FastAPI):
    ...
    ``mission-control.tilos.com/her/*`` -> local Caddy -> :9119), the
    proxy injects ``X-Forwarded-Prefix: /her`` on every request. We
    rewrite the served ``index.html`` so absolute asset URLs (``/assets/...``)
    and the SPA's runtime ``__HER_BASE_PATH__`` honour that prefix
    ...
    def _serve_index(prefix: str = ""):
        ...
        ``prefix`` is the normalised ``X-Forwarded-Prefix`` (e.g. ``/her``)
        ...
        ``__HER_AUTH_REQUIRED__`` flag lets the SPA pick the right
        ...
        if gated:
            bootstrap_script = (
                f"<script>"
                f"window.__HER_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
                f'window.__HER_BASE_PATH__="{prefix}";'
                f"window.__HER_AUTH_REQUIRED__={gated_js};"
                f"</script>"
            )
        else:
            bootstrap_script = (
                f'<script>window.__HER_SESSION_TOKEN__="{_SESSION_TOKEN}";'
                f"window.__HER_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
                f'window.__HER_BASE_PATH__="{prefix}";'
                f"window.__HER_AUTH_REQUIRED__={gated_js};"
                f"</script>"
    ...
    # under ``/her`` they'd hit ``mission-control.tilos.com/fonts/...``
    # (the MC Pages app), not the her backend. Intercept CSS asset
...
_BUILTIN_DASHBOARD_THEMES = [
    {"name": "default",       "label": "her Teal",         "description": "Classic dark teal — the canonical her look"},
    {"name": "default-large", "label": "her Teal (Large)", "description": "her Teal with bigger fonts and roomier spacing"},
...
def _normalise_theme_definition(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    # YAML in ~/.her/, same trust level as the config file itself.
...
def _discover_user_themes() -> list:
    """Scan ~/.her/dashboard-themes/*.yaml for user-created themes.

    Returns a list of fully-normalised theme definitions ready to ship
    to the frontend, so the client can apply them without a secondary
    round-trip or a built-in stub.
    """
    themes_dir = get_her_home() / "dashboard-themes"
...
async def get_dashboard_themes():
    ...
    from `~/.her/dashboard-themes/*.yaml` ship with their full
...
def _discover_dashboard_plugins() -> list:
    ...
    1. User plugins:    ~/.her/plugins/<name>/dashboard/manifest.json
    2. Bundled plugins: <repo>/plugins/<name>/dashboard/manifest.json  (memory/, etc.)
    3. Project plugins: ./.her/plugins/  (only if HER_ENABLE_PROJECT_PLUGINS)
    """
    plugins = []
    seen_names: set = set()

    from her_cli.plugins import get_bundled_plugins_dir
    bundled_root = get_bundled_plugins_dir()
    search_dirs = [
        (get_her_home() / "plugins", "user"),
    ...
    # ``her_cli/plugins.py`` and the documented user contract.
    if env_var_enabled("HER_ENABLE_PROJECT_PLUGINS"):
        search_dirs.append((Path.cwd() / ".her" / "plugins", "project"))
...
def _merged_plugins_hub() -> Dict[str, Any]:
    ...
    from her_cli.plugins_cmd import (
    ...
    plugins_root_resolved = (get_her_home() / "plugins").resolve()
...
async def post_agent_plugin_install(request: Request, body: _AgentPluginInstallBody):
    _require_token(request)
    from her_cli.plugins_cmd import dashboard_install_plugin
...
async def post_agent_plugin_enable(request: Request, name: str):
    ...
    from her_cli.plugins_cmd import dashboard_set_agent_plugin_enabled
...
async def post_agent_plugin_update(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from her_cli.plugins_cmd import dashboard_update_user_plugin
...
async def delete_agent_plugin(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from her_cli.plugins_cmd import dashboard_remove_user_plugin
...
def _mount_plugin_api_routes():
    ...
    Project plugins (``./.her/plugins/``) ship with the CWD and are
    ...
    for plugin in _get_dashboard_plugins():
        ...
        if not api_path.exists():
            ...
            continue
        try:
            module_name = f"her_dashboard_plugin_{plugin['name']}"
...
from her_cli.dashboard_auth.routes import router as _dashboard_auth_router  # noqa: E402
...
def start_server(
    ...
    if app.state.auth_required:
        ...
        from her_cli.dashboard_auth import list_providers
        if not list_providers():
            # Surface the *specific* reason any bundled provider declined
            # to register (e.g. missing HER_DASHBOARD_OAUTH_CLIENT_ID).
            # Each provider plugin that ships with her Agent exposes a
    ...
    print(f"  her Web UI → http://{host}:{port}")
