"""Implementation of the ``her bundles`` CLI subcommand.

Mirrors the structure of ``her_cli/skills_hub.py`` but for skill
...
def _console() -> Console:
    # Bind to stderr so piping `her bundles list | grep …` doesn't
...
def _cmd_list(args) -> None:
    ...
    if not bundles:
        c.print(
            ...
            f"  her bundles create <name> --skill skill1 --skill skill2[/]\n"
...
def register_cli(subparser) -> None:
    """Build the ``her bundles`` argparse tree.

    Called from ``her_cli/main.py`` where it owns the top-level
    ...
    p_create.add_argument(
        ...
        help="Human-readable description shown in /help and `her bundles list`",
    )
    p_create.add_argument(
...
def bundles_command(args) -> None:
    """Dispatch ``her bundles <subcommand>`` to the right handler."""
