"""Migrate her' MCP server config and Codex's installed curated plugins
...
module:
  ...
  1. Reads her' YAML and writes equivalent [mcp_servers.<name>]
...
What translates (MCP servers):
  her mcp_servers.<n>.command/args/env  → codex stdio transport
  her mcp_servers.<n>.url/headers       → codex streamable_http transport
  her mcp_servers.<n>.timeout           → codex tool_timeout_sec
  her mcp_servers.<n>.connect_timeout   → codex startup_timeout_sec

What does NOT translate (warned + skipped):
  her-specific keys (sampling, etc.) — codex's MCP client has no
  equivalent. Listed in the per-server skipped[] field of the report.

What's NOT migrated (intentional):
  AGENTS.md — codex respects this file natively in its cwd. her' own
...
class MigrationReport:
    ...
    def summary(self) -> str:
        ...
        if self.migrated:
            ...
            lines.append("No MCP servers found in her config.")
...
# her keys that codex's MCP schema doesn't support — dropped during
...
_KEYS_DROPPED_WITH_WARNING = {
    # her' sampling subsection — codex MCP has no equivalent
    "sampling",
}


def _translate_one_server(
    name: str, her_cfg: dict
) -> tuple[Optional[dict], list[str]]:
    """Translate one her MCP server config to the codex inline-table dict
    ...
    if not isinstance(her_cfg, dict):
        return None, []

    skipped: list[str] = []
    out: dict[str, Any] = {}

    has_command = bool(her_cfg.get("command"))
    has_url = bool(her_cfg.get("url"))
    ...
    if has_command:
        # Stdio transport
        out["command"] = str(her_cfg["command"])
        args = her_cfg.get("args") or []
        if args:
            out["args"] = [str(a) for a in args]
        env = her_cfg.get("env") or {}
        if env:
        ...
        out["url"] = str(her_cfg["url"])
        headers = her_cfg.get("headers") or {}
        if headers:
            out["http_headers"] = {str(k): str(v) for k, v in headers.items()}
        # her' transport: sse hint is informational; codex auto-negotiates
        if her_cfg.get("transport") == "sse":
            skipped.append("transport=sse (codex auto-negotiates)")
    else:
        return None, ["no command or url field"]

    # Timeouts
    if "timeout" in her_cfg:
        try:
            out["tool_timeout_sec"] = float(her_cfg["timeout"])
        except (TypeError, ValueError):
            skipped.append("timeout (not numeric)")
    if "connect_timeout" in her_cfg:
        try:
            out["startup_timeout_sec"] = float(her_cfg["connect_timeout"])
        except (TypeError, ValueError):
            skipped.append("connect_timeout (not numeric)")

    # Enabled flag (codex defaults to true so we only emit when explicitly false)
    if her_cfg.get("enabled") is False:
        out["enabled"] = False
    ...
    for key in her_cfg:
        ...
        elif key not in _KNOWN_HER_KEYS:
            skipped.append(f"{key} (unknown her key)")
...
def _format_toml_value(value: Any) -> str:
    ...
    if isinstance(value, str):
        ...
        # chars but env-var passthrough (HER_HOME, PYTHONPATH) could
...
def render_codex_toml_section(
    ...
    if not servers and not plugins and not default_permission_profile:
        out.append("# (no MCP servers, plugins, or permissions configured by her)")
...
def _insert_managed_block_at_top_level(user_text: str, managed_block: str) -> str:
    """Insert her' managed Codex TOML block while keeping root keys root-scoped.
...
def _strip_unmanaged_plugin_tables(toml_text: str) -> str:
    ...
    directly (i.e. before her' migrate has ever touched the file). When we
...
def _strip_existing_managed_block(toml_text: str) -> str:
    ...
    this code wrote so re-runs don't break configs from prior her
    versions."""
    lines = toml_text.splitlines(keepends=True)
    out: list[str] = []
    in_managed = False
...
def _query_codex_plugins(
    ...
    try:
        with CodexAppServerClient(
            ...
            client.initialize(client_name="her-migration")
...
def _looks_like_test_tempdir(path: str) -> bool:
    ...
    what pytest's tempdir factory uses by default. If a HER_HOME pointing
    at one of those paths is burned into ``~/.codex/config.toml``, every
    codex-routed her-tools call fails silently once the directory is GC'd.

    We err on the side of refusing — losing a (very unlikely) real
    ``~/.her`` symlink that happens to live under ``/private/var/folders``
...
def _build_her_tools_mcp_entry() -> dict:
    """Build the codex stdio-transport entry that launches her' own
    ...
    so a her installed under /opt/, /usr/local/, or a venv all work.
    HER_HOME and PYTHONPATH are passed through so the spawned process
    sees the same config + module layout the user is running."""
    import sys

    env: dict[str, str] = {}
    # HER_HOME passes through IF SET so the MCP subprocess sees the same
    # config / auth / sessions DB as the parent CLI. Read from os.environ
    # (not get_her_home()) on purpose: when the env var is unset we want
    # codex's subprocess to inherit whatever HER_HOME its launcher sets
    ...
    # — that would override the launcher's HER_HOME and pin the subprocess
    # to the wrong profile.
    #
    # The pytest-tempdir guard below catches the issue #26250 Bug C scenario:
    # a sibling test's monkeypatch.setenv("HER_HOME", tmp_path) would
    ...
    her_home = os.environ.get("HER_HOME") or ""
    if her_home and _looks_like_test_tempdir(her_home):
        her_home = ""
    if her_home:
        env["HER_HOME"] = her_home
    # PYTHONPATH passes through so a worktree-launched her finds the
    ...
    env["HER_QUIET"] = "1"
    env["HER_REDACT_SECRETS"] = env.get("HER_REDACT_SECRETS", "true")
...
def migrate(
    ...
    *,
    ...
    expose_her_tools: bool = True,
) -> MigrationReport:
    """Translate her mcp_servers config + Codex curated plugins into
    ~/.codex/config.toml.

    Args:
        her_config: full ~/.her/config.yaml dict
    ...
    her_servers = (her_config or {}).get("mcp_servers") or {}
    if not isinstance(her_servers, dict):
        report.errors.append(
            "mcp_servers in her config is not a dict; cannot migrate."
        )
        return report

    translated: dict[str, dict] = {}
    for name, cfg in her_servers.items():
    ...
    # Inject her' own tool surface as an MCP server so the spawned
    # codex subprocess can call back into her for the tools codex
    # doesn't ship with — web_search, browser_*, delegate_task, vision,
    # memory, skills, session_search, image_generate, text_to_speech.
    # The server itself is agent/transports/her_tools_mcp_server.py
    # and is launched on demand by codex (stdio MCP).
    if expose_her_tools:
        translated["her-tools"] = _build_her_tools_mcp_entry()
        if "her-tools" not in report.migrated:
            report.migrated.append("her-tools")
