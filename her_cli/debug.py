...
Currently supports:
    her debug share    Upload debug report (system info + logs) to a
                          ...
                          ``~/.her/logs/*.log`` are not leaked into
...
_REDACTION_BANNER = (
    "[her debug share: log content redacted at upload time. "
...
def _pending_file() -> Path:
    """Path to ``~/.her/pastes/pending.json``.
...
def _save_pending(entries: list[dict]) -> None:
    ...
    try:
        ...
        # Non-fatal — worst case the user has to run ``her debug delete``
        # manually.
        pass
...
def _sweep_expired_pastes(now: Optional[float] = None) -> tuple[int, int]:
    ...
    intended to be called from every ``her debug`` invocation with
    minimal noise.
    """
    entries = _load_pending()
    if not entries:
...
_GATEWAY_PRIVACY_NOTICE = (
    ...
    "Full logs are NOT included from the gateway — use `her debug share` "
...
def delete_paste(url: str) -> bool:
    ...
    req = urllib.request.Request(
        ...
        headers={"User-Agent": "her-agent/debug-share"},
    )
...
def _schedule_auto_delete(urls: list[str], delay_seconds: int = _AUTO_DELETE_SECONDS):
    ...
    every ``her debug share`` invocation added ~20 MB of resident Python
    interpreters that never exited until the sleep completed.

    The replacement is stateless: we append to ``~/.her/pastes/pending.json``
    and the gateway's cron ticker sweeps expired entries once per hour.
    ``her debug share`` also runs an opportunistic sweep as a fallback
...
def _upload_paste_rs(content: str) -> str:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "her-agent/debug-share",
        },
    )
    with urllib.request.urlopen(req, timeout=30) as resp:
...
def _primary_log_path(log_name: str) -> Optional[Path]:
    ...
    return (get_her_home() / "logs" / filename) if filename else None
...
def _capture_dump() -> str:
    """Run ``her dump`` and return its stdout as a string."""
...
def collect_debug_report(
    *,
    ...
    dump_text
        Pre-captured dump output.  If empty, ``her dump`` is run
        internally.
...
def build_debug_share(
    *,
    ...
    This is the shared core behind ``her debug share`` (CLI) and the
    ...
    if redact:
        logger.info(
            "her debug share: applied force-mode redaction to log snapshots before upload"
        )
...
def run_debug_share(args):
    ...
    try:
        ...
        print("\nRun `her debug share --local` to print the report instead.\n")
        sys.exit(1)
...
def run_debug_delete(args):
    ...
    if not urls:
        print("Usage: her debug delete <url> [<url> ...]")
        print("  Deletes paste.rs pastes uploaded by 'her debug share'.")
        return
...
def run_debug(args):
    ...
    # Opportunistic sweep of expired pastes on every ``her debug`` call.
    ...
    # best-effort — any failure is swallowed so ``her debug`` stays
