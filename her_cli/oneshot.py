...
def _validate_explicit_toolsets(toolsets: object = None) -> tuple[list[str] | None, str | None]:
    ...
    try:
        ...
        return None, f"her -z: failed to validate --toolsets: {exc}\n"
    ...
    if any(name in {"all", "*"} for name in built_in):
        ...
        if ignored:
            sys.stderr.write(
                "her -z: --toolsets all enables every toolset; "
    ...
    if unknown:
        sys.stderr.write(f"her -z: ignoring unknown --toolsets entries: {', '.join(unknown)}\n")
    if disabled:
        sys.stderr.write(
            "her -z: ignoring disabled MCP servers (set enabled: true in config.yaml to use): "
            f"{', '.join(disabled)}\n"
        )

    if not valid:
        return None, "her -z: --toolsets did not contain any valid toolsets.\n"
...
def run_oneshot(
    ...
    if failure is not None:
        ...
        if isinstance(failure, (KeyboardInterrupt, SystemExit)):
            raise failure
        real_stderr.write(f"her -z: agent failed: {failure}\n")
        real_stderr.flush()
        return 1

    if not (response or "").strip():
        real_stderr.write("her -z: no final response was produced; treating the run as failed.\n")
        real_stderr.flush()
        return 1
...
def _run_agent(
    ...
    # Imports are local so they don't run when her is invoked for
    # other commands (keeps top-level CLI startup cheap).
    from her_cli.config import load_config
    from her_cli.models import detect_provider_for_model
    ...
    if effective_provider is None and (model or env_model):
        ...
        if explicit_model:
            ...
            try:
                from her_cli import model_switch as _ms
