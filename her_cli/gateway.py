"""
Gateway subcommand for her CLI.

Handles: her gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
...
# display_her_home is imported lazily at call sites to avoid ImportError
# when her_constants is cached from a pre-update version during `her update`.
...
def _get_ancestor_pids() -> set[int]:
    ...
    ``her gateway status`` from falsely counting the ``her`` CLI that
...
def _scan_gateway_pids(exclude_pids: set[int], all_profiles: bool = False) -> list[int]:
    ...
    # scan (e.g. ``her gateway status``) is never mistaken for a running
    # gateway.  See #13242.
    ...
    patterns = [
        ...
        # ``her.exe --profile`` / ``her.exe -p`` would also match
        # ``her.exe --profile foo dashboard`` and other CLI subcommands,
        # producing false-positive gateway PIDs (Copilot review).
        "her.exe gateway",
        "her-gateway.exe",
        "gateway/run.py",
    ]
    current_home = str(get_her_home().resolve())
    ...
    def _matches_current_profile(command: str) -> bool:
        ...
        if current_profile_name:
            return (
                ...
                or f"her_home={current_home_lc}" in command_lc
            )

        # Default-profile case: no profile flag in argv. Accept as long as
        # the command doesn't advertise *some other* profile. HER_HOME
        ...
        if "--profile " in command_lc or " -p " in command_lc:
            return False
        if (
            "her_home=" in command_lc
            and f"her_home={current_home_lc}" not in command_lc
        ):
            return False
        return True
...
def _gateway_run_args_for_profile(profile: str) -> list[str]:
    args = [get_python_path(), "-m", "her_cli.main"]
    if profile != "default":
...
def _sync_her_home_from_systemd_unit(system: bool) -> None:
    """When acting on a system-scope unit, adopt its ``HER_HOME``.

    Under ``sudo``, ``HER_HOME`` is stripped and ``HOME=/root``, so
    :func:`get_her_home` falls back to ``/root/.her`` — the wrong
    profile. The unit file pins ``HER_HOME`` for the actual gateway
    ...
    unit_home = env.get("HER_HOME", "").strip()
    if not unit_home:
        return
    current = os.environ.get("HER_HOME", "").strip()
    if current == unit_home:
        return
    os.environ["HER_HOME"] = unit_home
...
def _wait_for_systemd_service_restart(
    *,
    ...
    print(
        ...
        f"  Check status: {'sudo ' if system else ''}her gateway status\n"
...
def _print_systemd_start_limit_wait(system: bool = False) -> None:
    ...
    print(
        f"  Wait for the start-limit window to expire, then run: {'sudo ' if system else ''}her gateway restart{scope_flag}"
    )
...
def get_gateway_runtime_snapshot(system: bool = False) -> GatewayRuntimeSnapshot:
    ...
    if is_linux() and is_container():
        ...
        try:
            from her_cli.service_manager import detect_service_manager
...
def _print_gateway_process_mismatch(snapshot: GatewayRuntimeSnapshot) -> None:
    ...
    print("  This is usually a manual foreground/tmux/nohup run, so `her gateway`")
...
def _print_other_profiles_gateway_status() -> None:
    ...
    Shown at the bottom of ``her gateway status`` output so users with
...
def _gateway_list() -> None:
    ...
    """
    try:
        from her_cli.profiles import list_profiles, get_active_profile_name
    except Exception:
...
def stop_profile_gateway() -> bool:
    """Stop only the gateway for the current profile (HER_HOME-scoped).
...
def _profile_suffix() -> str:
    """Derive a service-name suffix from the current HER_HOME.

    Returns ``""`` for the default root, the profile name for
    ``<root>/profiles/<name>``, or a short hash for any other path.
    Works correctly in Docker (HER_HOME=/opt/data) and standard deployments.
    """
    import hashlib
    import re
    from her_constants import get_default_her_root

    home = get_her_home().resolve()
    default = get_default_her_root().resolve()
    if home == default:
        return ""
    ...
    try:
        ...
        pass
    # Fallback: short hash for arbitrary HER_HOME paths
    return hashlib.sha256(str(home).encode()).hexdigest()[:8]


def _profile_arg(her_home: str | None = None) -> str:
    """Return ``--profile <name>`` only when HER_HOME is a named profile.

    For ``~/.her/profiles/<name>``, returns ``"--profile <name>"``.
    For the default profile or hash-based custom paths, returns the empty string.

    Args:
        her_home: Optional explicit HER_HOME path. Defaults to the current
            ``get_her_home()`` value. Should be passed when generating a
            service definition for a different user (e.g. system service).
    """
    import re
    from her_constants import get_default_her_root

    home = Path(her_home or str(get_her_home())).resolve()
    default = get_default_her_root().resolve()
    if home == default:
        return ""
    profiles_root = (default / "profiles").resolve()
...
def get_service_name() -> str:
    """Derive a systemd service name scoped to this HER_HOME.

    Default ``~/.her`` returns ``her-gateway`` (backward compatible).
    Profile ``~/.her/profiles/coder`` returns ``her-gateway-coder``.
    Any other HER_HOME appends a short hash for uniqueness.
    """
...
_LEGACY_SERVICE_NAMES: tuple[str, ...] = ("her.service",)
...
def _find_legacy_her_units() -> list[tuple[str, Path, bool]]:
    ...
    * Explicit allowlist of legacy names (no globbing). Profile units such
      as ``her-gateway-coder.service`` and unrelated third-party
      ``her-*`` services are never matched.
    * ExecStart content check — only flag units that invoke our gateway
      entrypoint. A user-created ``her.service`` running an unrelated
...
def print_legacy_unit_warning() -> None:
    ...
    print_info("  These run alongside the current her-gateway service and")
    print_info("  cause SIGTERM flap loops — both try to use the same bot token.")
    print_info("  Remove them with:")
    print_info("    her gateway migrate-legacy")


def remove_legacy_her_units(
    ...
    if interactive and not prompt_yes_no("Remove these legacy units?", True):
        print("Skipped. Run again with: her gateway migrate-legacy")
        return 0, [p for _, p, _ in legacy]
    ...
    if system_units:
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd removal path, guarded by `if system == "Linux"` / systemd-only branch
            ...
            print_info("  Re-run with: sudo her gateway migrate-legacy")
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    her gateway uninstall")
    print_info("    sudo her gateway uninstall --system")
...
def install_linux_gateway_from_setup(force: bool = False, enable_on_startup: bool = True) -> tuple[str | None, bool]:
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd install wizard, never invoked on Windows
            ...
            if run_as_user:
                print_info(
                    f"  After setup, run: sudo her gateway install --system --run-as-user {run_as_user}"
                )
            else:
                print_info(
                    "  After setup, run: sudo her gateway install --system --run-as-user <your-user>"
                )
            print_info("  Then start it with: sudo her gateway start --system")
            return scope, False
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.her`` → ``ai.her.gateway.plist`` (backward compatible).
    Profile ``~/.her/profiles/coder`` → ``ai.her.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
    name = f"ai.her.gateway-{suffix}" if suffix else "ai.her.gateway"
...
def _remap_path_for_user(path: str, target_home_dir: str) -> str:
    ...
    to *target_home_dir*; otherwise the path is returned unchanged.
      ...
      /root/.her/her-agent  -> /home/alice/.her/her-agent
      /opt/her                 -> /opt/her  (kept as-is)
...
def _her_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current HER_HOME to the equivalent under a target user's home.

    When installing a system service via sudo, get_her_home() resolves to
    root's home.  This translates it to the target user's equivalent path:
      /root/.her                    → /home/alice/.her
      /root/.her/profiles/coder     → /home/alice/.her/profiles/coder
      /opt/custom-her               → /opt/custom-her  (kept as-is)
    """
    current_her = get_her_home().resolve()
    current_default = (Path.home() / ".her").resolve()
    target_default = Path(target_home_dir) / ".her"

    # Default ~/.her → remap to target user's default
    if current_her == current_default:
        return str(target_default)

    # Profile or subdir of ~/.her → preserve the relative structure
    try:
        relative = current_her.relative_to(current_default)
        return str(target_default / relative)
    except ValueError:
        # Completely custom path (not under ~/.her) — keep as-is
        return str(current_her)
...
def _build_service_path_dirs(project_root: Path | None = None) -> list[str]:
    ...
    her_node = her_home / "node" / "bin"
    if _is_dir(her_node):
        candidates.append(str(her_node))
    her_nm = her_home / "node_modules" / ".bin"
    if _is_dir(her_nm):
        candidates.append(str(her_nm))
...
def _stable_service_working_dir() -> str:
    ...
    uses an absolute python interpreter and ``-m her_cli.main``, so module
    ...
    dir, or a clone that ``her update`` later relocates/removes — the path
    ...
    ``HER_HOME`` is the stable anchor: it is where config/state/logs live,
    it never moves, and it is guaranteed to exist whenever the gateway is
    meaningfully installed. Fall back to ``PROJECT_ROOT`` only if HER_HOME
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if system:
        ...
        her_home = _her_home_for_target_user(home_dir)
        profile_arg = _profile_arg(her_home)
        # Remap all paths that may resolve under the calling user's home
        # (e.g. /root/) to the target user's home so the service can
        # actually access them.
        python_path = _remap_path_for_user(python_path, home_dir)
        # Anchor cwd to the target user's HER_HOME (stable, always exists)
        # rather than a remapped source-checkout path that can rot. See
        # _stable_service_working_dir() for the full rationale.
        working_dir = str(her_home) if her_home else _remap_path_for_user(working_dir, home_dir)
...
"""
    ...
    her_home = str(get_her_home().resolve())
    profile_arg = _profile_arg(her_home)
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    # sandboxed by the test conftest (only HER_HOME is). If a test
    # exercises ``run_gateway()`` with a pytest-tmp HER_HOME, the freshly
    # generated unit bakes that ``/tmp/pytest-of-.../her_test`` path into
    # ``Environment="HER_HOME=..."``. Writing that to the developer's
...
def _print_system_scope_remediation(action: str) -> None:
    ...
    print_info("         sudo her gateway uninstall --system")
    print_info("         her gateway install")
    print_info("         her gateway start")
...
def systemd_install(
    ...
    # Offer to remove legacy units (her.service from pre-rename installs)
    # before installing the new her-gateway.service. If both remain, they
    ...
    print("Next steps:")
    print(
        f"  {'sudo ' if system else ''}her gateway start{scope_flag}              # Start the service"
    )
    print(
        f"  {'sudo ' if system else ''}her gateway status{scope_flag}             # Check status"
    )
    print(
...
def _require_service_installed(action: str, system: bool = False) -> None:
    ...
    if not unit_path.exists():
        ...
        print(f"  Run: {'sudo ' if system else ''}her gateway install{scope_flag}")
        sys.exit(1)
...
def systemd_stop(system: bool = False):
    ...
    _sync_her_home_from_systemd_unit(system=system)
    try:
    ...
    try:
        ...
        print(
            ...
            "check `her gateway status` or logs for final shutdown state."
        )
        return
    ...
    if pid is not None:
        ...
        try:
            ...
            print(
                ...
                "check `her gateway status` or logs for final state."
            )
            return
...
def systemd_status(deep: bool = False, system: bool = False, full: bool = False):
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(
            f"  Run: {'sudo ' if system else ''}her gateway restart{scope_flag}  # auto-refreshes the unit"
        )
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}her gateway start{scope_flag}")
    ...
    if active_state == "activating" and sub_state == "auto-restart":
        ...
        print(
            f"  Run after the start-limit window expires: {'sudo ' if system else ''}her gateway restart{scope_flag}"
        )
        print(
        ...
        print(
            f"  Run: systemctl {'--user ' if not system else ''}reset-failed {get_service_name()} && {'sudo ' if system else ''}her gateway start{scope_flag}"
        )
...
def get_launchd_label() -> str:
    ...
    suffix = _profile_suffix()
    return f"ai.her.gateway-{suffix}" if suffix else "ai.her.gateway"
...
# (the documented `nohup her gateway run` workaround). See #23387.
...
def _gateway_run_command() -> list[str]:
    """Build the `python -m her_cli.main [--profile X] gateway run --replace` argv.

    Profile-aware: honors the active HER_HOME via `_profile_arg()` so the
    detached fallback launches into the same profile as the CLI invocation.
    """
    cmd = [get_python_path(), "-m", "her_cli.main"]
...
def _spawn_detached_gateway() -> bool:
    ...
    macOS 26+ (issue #23387). Mirrors the `nohup her gateway run --replace`
...
def _launchd_fallback_to_detached(reason: str, *, exit_on_failure: bool = True) -> bool:
    ...
    if _spawn_detached_gateway():
        ...
        print("  Stop it with: her gateway stop")
        return True
    print_error("Failed to start the gateway as a background process.")
    print(
        f"  Try manually: nohup her gateway run --replace "
...
def launchd_install(force: bool = False):
    ...
    print()
    print("Next steps:")
    print("  her gateway status             # Check status")
    from her_constants import display_her_home as _dhh
...
def launchd_stop():
    ...
    # `her gateway start` re-bootstraps when it detects the job is unloaded.
    try:
...
def launchd_status(deep: bool = False):
    ...
    if deep:
        log_file = get_her_home() / "logs" / "gateway.log"
        if log_file.exists():
            print()
            print("Recent logs:")
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False):
    ...
    # /restart) rather than through `her gateway restart` which already
    ...
    # `her gateway start/restart` — leaving the gateway vulnerable to
...
def _all_platforms() -> list[dict]:
    ...
    ``her setup gateway`` without needing the gateway to be running.
    ...
    every host. Currently:
      - Matrix is hidden on Windows. The [matrix] extra pulls
        ...
        picker. Users who want Matrix on Windows can run her under
        WSL.
    """
    ...
    # User-installed platform plugins under ~/.her/plugins/ still require
...
def _platform_status(platform: dict) -> str:
    ...
    if token_var == "WHATSAPP_ENABLED":
        if val and val.lower() == "true":
            session_file = get_her_home() / "whatsapp" / "session" / "creds.json"
...
def _setup_standard_platform(platform: dict):
    ...
    if platform.get("key") == "telegram":
        ...
        if choice.strip() == "1":
            try:
                from her_cli.telegram_managed_bot import (
        ...
        if var.get("is_allowlist"):
            ...
            if value:
                ...
                access_choices = [
                    ...
                    "Use DM pairing (unknown users request access, you approve with 'her pairing approve')",
                ...
                if access_idx == 0:
                    ...
                    print_info(
                        "  Approve with: her pairing approve <platform> <code>"
                    )
                else:
                    print_info(
                        "  Skipped — configure later with 'her gateway setup'"
                    )
            continue
...
def _builtin_setup_fn(key: str):
    ...
    Late-bound to avoid a circular import with ``her_cli.setup`` (which
    imports from this module for the remaining bespoke flows).
    """
    from her_cli import setup as _s

    return {
        "telegram": _s._setup_telegram,
        # All other platforms (discord, slack, matrix, wecom, weixin,
        # signal, whatsapp, bluebubbles, dingtalk, feishu, qqbot, ...)
        # were moved out of the core in the 2026-06-07 platform slim.
        # They register setup_fn through gateway.platform_registry
        # when their plugin is installed, dispatched via the plugin
        # path in _configure_platform().
...
def _configure_platform(platform: dict) -> None:
    ...
    is needed here. User-installed platform plugins under ~/.her/plugins/
    ...
    if required:
        print_info(f"  Set these env vars in ~/.her/.env: {', '.join(required)}")
    else:
        print_info(
...
def gateway_setup():
    ...
    if any_configured:
            ...
            if supports_systemd_services() and _system_scope_wizard_would_need_root():
                ...
                try:
                    if supports_systemd_services():
                        ...
                        print_info("Start manually: her gateway")
                except UserSystemdUnavailableError as e:
            ...
            if supports_systemd_services() or is_macos() or is_windows():
                ...
                if start_now or start_on_login:
                    try:
                        ...
                        print_info("  You can try manually: her gateway install")
                else:
                    print_info("  Skipped start and auto-start setup.")
                    print_info("  You can install later: her gateway install")
                    if supports_systemd_services():
                        print_info(
                            "  Or as a boot-time service: sudo her gateway install --system"
                        )
                    print_info("  Or run in foreground:  her gateway run")
            elif is_wsl():
                print_info("  WSL detected but systemd is not running.")
                print_info("  Run in foreground: her gateway run")
                print_info(
                    "  For persistence:   tmux new -s her 'her gateway run'"
                )
                print_info(
                ...
                print_info("  Run in foreground: her gateway run")
                print_info(
                    f"  Or start it manually in the background (best effort): nohup her gateway run >{_dhh()}/logs/gateway.log 2>&1 &"
                )
            else:
        ...
        print()
        print_info("No platforms configured. Run 'her gateway setup' when ready.")
...
def _dispatch_via_service_manager_if_s6(
    ...
    :mod:`her_cli.service_manager` are caught and surfaced as
    actionable CLI messages (no raw ``CalledProcessError`` traceback).
    """
    from her_cli.service_manager import (
    ...
    if profile is None:
        ...
        # HER_HOME=<root>/profiles/<name>, "" for the default root,
...
def _dispatch_all_via_service_manager_if_s6(action: str) -> bool:
    ...
    Without this, ``her gateway stop --all`` and ``... restart --all``
...
def gateway_command(args):
    ...
    try:
        ...
        # The direct ``her gateway install|uninstall|start|stop|restart``
...
def _maybe_redirect_run_to_s6_supervision(args) -> bool:
    ...
    # `docker logs` and ${HER_HOME}/logs/gateways/<profile>/current,
    ...
    #
    # Prefer `sleep infinity` (matches the static main-her service's
    # pattern in docker/s6-rc.d/main-her/run, and frees the Python
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if is_termux():
            ...
            print("Run manually: her gateway")
            sys.exit(1)
        if supports_systemd_services():
            if is_wsl():
                print_warning(
                ...
                print_info(
                    "  Consider running in foreground instead: her gateway run"
                )
                print_info(
                    "  Or use tmux/screen for persistence: tmux new -s her 'her gateway run'"
                )
                print()
            ...
            print()
            print(
                "  her gateway run                              # direct foreground"
            )
            print(
                "  tmux new -s her 'her gateway run'         # persistent via tmux"
            )
            print(
                "  nohup her gateway run > ~/.her/logs/gateway.log 2>&1 &  # background"
            )
            sys.exit(1)
        elif is_container():
            ...
            if detect_service_manager() == "s6":
                ...
                print()
                print("  her profile create <name>     # creates the s6 service slot")
                print("  her -p <name> gateway start   # bring it up via s6")
                print("  her status                    # see currently-supervised gateways")
                return
            ...
            print()
            print("To run the gateway: her gateway run")
            sys.exit(0)
        else:
            print("Service installation not supported on this platform.")
            print("Run manually: her gateway run")
            sys.exit(1)
        ...
        if is_termux():
            ...
            print("Stop manual runs with: her gateway stop")
            sys.exit(1)
        if supports_systemd_services():
            ...
            if detect_service_manager() == "s6":
                ...
                print()
                print("  her profile delete <name>     # tears down the s6 service slot")
                print("  her -p <name> gateway stop    # stop without deleting the profile")
                return
        ...
        # slot — start them individually via `her -p <name> gateway
        ...
        if supports_systemd_services():
            ...
            print()
            print("Or run the gateway directly: her gateway run")
            sys.exit(0)
        else:
            print("Not supported on this platform.")
            sys.exit(1)
        ...
        if os.getenv("_HERMES_GATEWAY") == "1":
            print_error(
                ...
                "Use `her gateway stop` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if os.getenv("_HERMES_GATEWAY") == "1":
            print_error(
                ...
                "This command was blocked to prevent restart loops.\n"
                "Use `her gateway restart` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if not service_available:
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: her gateway start")
                sys.exit(1)
        ...
        if supports_systemd_services() and (
            ...
            if pids:
                ...
                if is_termux():
                    ...
                    print("  her gateway install")
                else:
                    print("To install as a service:")
                    print("  her gateway install")
                    print("  sudo her gateway install --system")
            else:
                ...
                print()
                print("To start:")
                print("  her gateway run      # Run in foreground")
                if is_termux():
                    print(
                        "  nohup her gateway run > ~/.her/logs/gateway.log 2>&1 &  # Best-effort background start"
                    )
                elif is_wsl():
                    print(
                        "  tmux new -s her 'her gateway run'         # persistent via tmux"
                    ...
                    print(
                        "  her gateway install  # Install as Windows Scheduled Task (auto-start on login)"
                    )
                else:
                    print("  her gateway install  # Install as user service")
                    print(
                        "  sudo her gateway install --system  # Install as boot-time system service"
                    )
        ...
        if not supports_systemd_services() and not is_macos():
            ...
            return
        remove_legacy_her_units(interactive=not yes, dry_run=dry_run)
