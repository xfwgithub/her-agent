...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or her -c.",
    ...
    # --- CLI Flags ---
    "her -c resumes your most recent CLI session. her -c \"project name\" resumes by title.",
    "her -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "her -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "her chat -t web,terminal enables only specific toolsets for a focused session.",
    "her chat -s github-pr-workflow preloads a skill at launch.",
    "her chat -q \"query\" runs a single non-interactive query and exits.",
    "her chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "her chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "her --yolo bypasses all dangerous command approval prompts for the entire session.",
    "her chat --source telegram tags the session for filtering in her sessions list.",
    "her -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "her doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "her dump outputs a compact setup summary — great for bug reports.",
    "her config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "her config edit opens config.yaml in your default editor.",
    "her config check scans for missing or stale configuration options.",
    "her sessions browse opens an interactive session picker with search.",
    "her sessions stats shows session counts by platform and database size.",
    "her sessions prune --older-than 30 cleans up old sessions.",
    "her skills search react --source skills-sh searches the skills.sh public directory.",
    "her skills check scans installed hub skills for upstream updates.",
    ...
    "her models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    ...
    "Profile names become shell commands — 'her profile create coder' creates the 'coder' command.",
    "her profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "her skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .her/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: her cron add --skill blogwatcher \"Check for new posts\".",
    ...
    "Context files (.her.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    # --- MCP ---
    "her mcp opens an interactive picker of Nous-approved MCPs you can install in one keystroke.",
    "her mcp catalog lists Nous-approved MCP servers shipped with the repo.",
    "her mcp install <name> installs a catalog entry, prompts for credentials, and lets you pick which of its tools to enable.",
    ...
    "MCP servers auto-generate toolsets at runtime — her tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.her/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "her chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "her plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "her status --deep runs deeper diagnostic checks across all components.",
    ...
    "The skills audit log at ~/.her/skills/.hub/audit.log tracks every install and removal operation.",
    "Stale git worktrees are auto-cleaned: 24-72h old with no unpushed commits get pruned on startup.",
    "Each profile gets its own subprocess HOME at HER_HOME/home/ — isolated git, ssh, npm, gh configs.",
    "HER_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in HER_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.her/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'her profile create ls' would be rejected.",
    "her profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "her claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "The skills quarantine at ~/.her/skills/.hub/quarantine/ holds skills pending security review.",
    ...
    '/reload reloads ~/.her/.env into the running session — pick up new API keys without restarting.',
    ...
    'Drop a ~/.her/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',

    # --- Curator ---
    'her curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "her curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'her curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',

    # --- Credential Pools & Routing ---
    'her auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    ...
    # --- TUI & Dashboard ---
    'HER_TUI_RESUME=1 auto-re-attaches to the most recent TUI session on launch — handy after SSH drops.',
    "HER_TUI_THEME=light|dark|<hex> forces the TUI theme on terminals that don't set COLORFGBG.",
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.her/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.her/skills/ so drop-in skills appear without restarting the session.',
    ...
    'her -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'her chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'her chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'her chat --ignore-user-config skips the active user config — reproducible bug reports and CI runs.',
    "her chat --source tool tags programmatic chats so they don't clutter her sessions list.",
    'her dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'her sessions rename <ID> "new title" renames any past session; her sessions delete <ID> removes one.',
    'her import restores a session export or profile archive produced by sessions export or profile export.',
    'her fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'her pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'her setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    ...
    '--source tool chats are excluded from her sessions list by default — set --source explicitly to see them.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.her/dashboard-plugins/.',
]
