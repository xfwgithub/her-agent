...
``$HER_HOME/profiles/<name>/`` live on the persistent VOLUME, and
...
Dockerfile (Phase 4 Task 4.0). Runs as root after 01-her-setup
(the stage2 hook) has chowned the volume and seeded $HER_HOME, but
...
def reconcile_profile_gateways(
    *,
    ...
    (the implicit profile that lives at the top of ``$HER_HOME``,
    not under ``profiles/``). The dispatcher in ``her_cli.gateway``
    maps an empty profile suffix to ``gateway-default``, so this slot
    is what ``her gateway start`` (no ``-p``) targets. Without it,
    bare ``her gateway start`` inside the container would land on
    ...
    ``$HER_HOME/gateway_state.json`` (sibling to the profile root,
    not under ``profiles/``); stale runtime files there are swept the
    same way as for named profiles.

    Args:
        her_home: The container's HER_HOME (typically /opt/data).
            Profiles live under ``<her_home>/profiles/<name>/``;
            the default profile lives at ``<her_home>`` itself.
    ...
    # ``her gateway start`` (no ``-p``) has somewhere to land;
    ...
    default_prior_state = legacy_default_state or _read_prior_state(her_home)
    default_should_start = default_prior_state in _AUTOSTART_STATES
    if not dry_run:
        _cleanup_stale_runtime_files(her_home)
    ...
    profiles_root = her_home / "profiles"
    if profiles_root.is_dir():
        for entry in sorted(profiles_root.iterdir()):
            if not entry.is_dir():
                continue
            # SOUL.md is always seeded by `her profile create` (config.yaml
            # is not — that comes later via `her setup`). Use it as the
            ...
            # via ``her -p default-named gateway start`` if they
    ...
    if not dry_run:
        _write_reconcile_log(her_home, actions)
    return actions
...
def _maybe_migrate_legacy_gateway_run_state(
    ...
    *,
    ...
    """
    state_file = her_home / "gateway_state.json"
...
def _is_legacy_gateway_run_request(argv: Sequence[str]) -> bool:
    ...
    if args and args[0].endswith("main-wrapper.sh"):
        args = args[1:]
    if args and Path(args[0]).name == "her":
        args = args[1:]
...
def _register_service(scandir: Path, profile: str, *, start: bool) -> None:
    ...
    try:
        ...
        # it up explicitly with `her -p <profile> gateway start`
        ...
        # spawns will EEXIST our dirs/FIFOs and inherit her
        # ownership, so runtime s6-svc / s6-svstat / s6-svwait calls
        # (all dispatched as the her user) won't hit EACCES. See
...
def _write_reconcile_log(
    her_home: Path, actions: list[ReconcileAction],
) -> None:
    """Append one line per profile to $HER_HOME/logs/container-boot.log.
...
def main() -> int:
    ...
    her_home = Path(os.environ.get("HER_HOME", "/opt/data"))
    scandir = Path(os.environ.get("S6_PROFILE_GATEWAY_SCANDIR", "/run/service"))
    actions = reconcile_profile_gateways(
        her_home=her_home, scandir=scandir,
    )
    for a in actions:
        print(
