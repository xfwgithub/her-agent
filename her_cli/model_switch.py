...
_HER_MODEL_WARNING = (
    "Nous Research her 3 & 4 models are NOT agentic and are not designed "
    "for use with her Agent. They lack the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(Claude, GPT, Gemini, DeepSeek, etc.)."
)

# Match only the real Nous Research her 3 / her 4 chat families.
# The previous substring check (`"her" in name.lower()`) false-positived on
# unrelated local Modelfiles like ``her-brain:qwen3-14b-ctx16k`` that just
# happen to carry "her" in their tag but are fully tool-capable.
#
# Positive examples the regex must match:
#   NousResearch/her-3-Llama-3.1-70B, her-4-405b, openrouter/her3:70b
# Negative examples it must NOT match:
#   her-brain:qwen3-14b-ctx16k, qwen3:14b, claude-opus-4-6
_NOUS_HER_NON_AGENTIC_RE = re.compile(
    r"(?:^|[/:])her[-_ ]?[34](?:[-_.:]|$)",
    re.IGNORECASE,
)


def is_nous_her_non_agentic(model_name: str) -> bool:
    """Return True if *model_name* is a real Nous her 3/4 chat model.
    ...
    if not model_name:
        return False
    return bool(_NOUS_HER_NON_AGENTIC_RE.search(model_name))


def _check_her_model_warning(model_name: str) -> str:
    """Return a warning string if *model_name* is a Nous her 3/4 chat model."""
    if is_nous_her_non_agentic(model_name):
        return _HER_MODEL_WARNING
    return ""
...
def _load_direct_aliases() -> dict[str, DirectAlias]:
    ...
    Also reads ``model.aliases`` (set by ``her config set model.aliases.xxx``)
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from her_cli.model_switch import
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'her model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'her doctor' — config issues detected:"
        ...
        from her_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
        from her_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    ...
    if provider_changed or explicit_provider:
        ...
        if explicit_provider and user_providers:
            from her_cli.providers import resolve_user_provider as _ruser
    ...
    her_warn = _check_her_model_warning(new_model)
    if her_warn:
        warnings.append(her_warn)
...
def prewarm_picker_cache_async() -> Optional["_threading.Thread"]:
    ...
    def _warm() -> None:
        try:
            from her_cli.inventory import load_picker_context
...
def list_authenticated_providers(
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from her_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    # Build curated model lists keyed by her provider ID
    curated: dict[str, list[str]] = dict(_PROVIDER_MODELS)
    curated["openrouter"] = [mid for mid, _ in OPENROUTER_MODELS]
    # "nous" pulls from the remote model-catalog manifest published at
    # https://her-agent.nousresearch.com/docs/api/model-catalog.json so
    # newly added Portal models surface in the /model picker without
    # requiring a her release. Falls back to the in-repo
    ...
    # --- 1. Check her-mapped providers ---
    from her_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
    from her_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    for her_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        _alias_target = _PROVIDER_ALIAS_TABLE.get(her_id)
        if (
            _alias_target
            and _alias_target != her_id
        ...
        pconfig = PROVIDER_REGISTRY.get(her_id)
        # Skip non-API-key auth providers here — they are handled in
        # section 2 (HER_OVERLAYS) with proper auth store checking.
        ...
        if not has_creds:
            try:
                ...
                if store and store.get("credential_pool", {}).get(her_id):
                    has_creds = True
            except Exception:
                pass
        if not has_creds:
        ...
        # /model picker sees the SAME list `her model` would build, with
        # disk caching to keep the picker open snappy. Falls back to the
        # curated static list when the live fetcher returns nothing.
        model_ids = cached_provider_model_ids(her_id)
        if not model_ids:
            model_ids = curated.get(her_id, [])
            if her_id in _MODELS_DEV_PREFERRED:
                model_ids = _merge_with_models_dev(her_id, model_ids)
        total = len(model_ids)
    ...
    # --- 2. Check her-only providers (nous, openai-codex, copilot, opencode-go) ---
    from her_cli.providers import HER_OVERLAYS
    from her_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → her provider ID.
    # HER_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use her IDs ("copilot").
    _mdev_to_her = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}

    for pid, overlay in HER_OVERLAYS.items():
        if pid.lower() in seen_slugs:
            continue

        # Resolve her slug — e.g. "github-copilot" → "copilot"
        her_slug = _mdev_to_her.get(pid, pid)
        if her_slug.lower() in seen_slugs:
            continue

        # Check if credentials exist
        has_creds = False
        if overlay.auth_type == "aws_sdk":
            has_creds = _has_aws_sdk_creds_for_listing(her_slug)
        elif overlay.extra_env_vars:
            has_creds = any(os.environ.get(ev) for ev in overlay.extra_env_vars)
        # Also check api_key_env_vars from PROVIDER_REGISTRY for api_key auth_type
        if not has_creds and overlay.auth_type == "api_key":
            for _key in (pid, her_slug):
        ...
        if not has_creds:
            try:
                ...
                if store and (pid in providers_store or her_slug in providers_store):
                    has_creds = True
        ...
        if not has_creds:
            try:
                ...
                pool = load_pool(her_slug)
                if pool.has_credentials():
                    has_creds = True
            except Exception as exc:
                logger.debug("Credential pool check failed for %s: %s", her_slug, exc)
        ...
        # configured.
        if not has_creds and her_slug == "anthropic":
            try:
                ...
                if (her_creds and her_creds.get("accessToken")) or \
        ...
        if her_slug in {"openai-codex", "copilot", "copilot-acp"}:
            ...
            model_ids = cached_provider_model_ids(her_slug)
        # For aws_sdk providers (bedrock), use live discovery so the list
        # reflects the active region (eu.*, ap.*) not the static us.* list.
        elif overlay.auth_type == "aws_sdk":
            try:
                _ids = cached_provider_model_ids(her_slug)
                model_ids = _ids if _ids else (curated.get(her_slug, []) or curated.get(pid, []))
            except Exception:
                model_ids = curated.get(her_slug, []) or curated.get(pid, [])
        elif her_slug == "nous":
            # Nous serves a large live /v1/models catalog (vendor-prefixed
            # models from many providers, returned alphabetically). The
            # `her model` picker deliberately shows ONLY the curated agentic
            ...
            try:
                ...
                from her_cli.auth import get_provider_auth_state as _nous_state
            ...
            if not model_ids:
                model_ids = curated.get(her_slug, []) or curated.get(pid, [])
                if her_slug in _MODELS_DEV_PREFERRED:
                    model_ids = _merge_with_models_dev(her_slug, model_ids)
        total = len(model_ids)
        top = model_ids[:max_models]

        results.append({
            "slug": her_slug,
            "name": get_label(her_slug),
            "is_current": her_slug == current_provider or pid == current_provider,
            "is_user_defined": False,
            "models": top,
            "total_models": total,
            "source": "her",
        ...
        seen_slugs.add(her_slug.lower())
        _record_builtin_endpoint(her_slug)

    # --- 2b. Cross-check canonical provider list ---
    # Catches providers that are in CANONICAL_PROVIDERS but weren't found
    # in PROVIDER_TO_MODELS_DEV or HER_OVERLAYS (keeps /model in sync
    # with `her model`).
    try:
    ...
    if user_providers and isinstance(user_providers, dict):
        for ep_name, ep_cfg in user_providers.items():
            ...
            # ``base_url`` is her's canonical write key (matches
            ...
            # her writes ``models:`` as a dict keyed by model id
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            if group_key not in groups:
                ...
                # her's own writer uses; a hyphen variant is accepted
            ...
            # active model. her's own writer (main.py::_save_custom_provider)
...
def list_picker_providers(
    ...
    - OpenRouter's model list is replaced with the output of
      :func:`her_cli.models.fetch_openrouter_models`, which filters the
    ...
    """
    from her_cli.models import fetch_openrouter_models
