...
supports runtime registration (for per-profile gateways) — host backends
raise NotImplementedError from those methods, and callers MUST check
supports_runtime_registration() before invoking them.

Host-side call sites (setup wizard, uninstall, status) continue to use
the existing module-level functions in her_cli.gateway directly. This
protocol is a thin facade used by new code that needs to be
backend-agnostic — specifically the profile create/delete hooks (Phase 4)
and the s6 dispatch path in ``her gateway start/stop/restart`` when
...
def detect_service_manager() -> ServiceManagerKind:
    ...
    in ``her gateway start/stop/restart``).
    """
...
def _s6_running() -> bool:
    ...
    Detection has to work for **both** root and the unprivileged her
...
# These adapters are thin facades over the existing module-level functions
    # in ``her_cli.gateway`` (systemd/launchd). The protocol's ``name`` parameter
    # is currently unused for host backends — they operate on whichever profile
    # is currently active (set via the ``her -p <profile>`` flag before the
    # call). This matches existing host-side semantics; the parameter shape is
    # designed for s6 where each profile maps to a distinct service directory.
    # ---------------------------------------------------------------------------
...
class SystemdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``systemd_*`` functions in her_cli.gateway.
    ...
    def start(self, name: str) -> None:
        from her_cli.gateway import systemd_start
        systemd_start()

    def stop(self, name: str) -> None:
        from her_cli.gateway import systemd_stop
        systemd_stop()

    def restart(self, name: str) -> None:
        from her_cli.gateway import systemd_restart
        systemd_restart()

    def is_running(self, name: str) -> bool:
        from her_cli.gateway import _probe_systemd_service_running
        _, running = _probe_systemd_service_running()
        return running


class LaunchdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``launchd_*`` functions in her_cli.gateway."""

    kind: ServiceManagerKind = "launchd"

    def start(self, name: str) -> None:
        from her_cli.gateway import launchd_start
        launchd_start()

    def stop(self, name: str) -> None:
        from her_cli.gateway import launchd_stop
        launchd_stop()

    def restart(self, name: str) -> None:
        from her_cli.gateway import launchd_restart
        launchd_restart()

    def is_running(self, name: str) -> bool:
        from her_cli.gateway import _probe_launchd_service_running
...
#
# Per-profile gateways are registered dynamically when `her profile create`
# runs inside the container (Phase 4). Static services (main-her, dashboard)
...
# UID/GID of the in-image ``her`` user. Hardcoded to match what
...
def _seed_supervise_skeleton(svc_dir: Path) -> None:
    ...
    inside a service directory, owned by the her user.
    ...
    with her ownership before triggering ``s6-svscanctl -a``,
    s6-supervise inherits our layout and never touches it.

    Layout produced
    ---------------
    ``svc_dir/``                           her:her, 0755 (parent must already exist)
    ``svc_dir/event/``                     her:her, 03730   (setgid + g+rwx + sticky)
    ``svc_dir/supervise/``                 her:her, 0755
    ``svc_dir/supervise/event/``           her:her, 03730
    ``svc_dir/supervise/control``          her:her, 0660    (FIFO)
    ...
    slot's supervise/ was her-owned.
    ...
    def _mkdir_owned(path: Path, mode: int) -> None:
        ...
        path.chmod(mode)
        try:
            os.chown(path, _HER_UID, _HER_GID)
        except PermissionError:
            # Running as the her user already — directory is her-
    ...
    if not control.exists():
        ...
        control.chmod(0o660)
        try:
            os.chown(control, _HER_UID, _HER_GID)
        except PermissionError:
            pass
    ...
    # when the parent slot's supervise/ is her-owned.
    log_dir = svc_dir / "log"
    if log_dir.is_dir():
        ...
        if not log_control.exists():
            ...
            try:
                os.chown(log_control, _HER_UID, _HER_GID)
            except PermissionError:
                pass
...
class GatewayNotRegisteredError(S6Error):
    ...
    Most commonly: ``her -p typo gateway start`` when no profile
    ...
    def __init__(self, profile: str) -> None:
        ...
        super().__init__(
            ...
            f"`her profile create {profile}` first, or pass "
...
class S6ServiceManager:
    ...
    ``S6_DYNAMIC_SCANDIR``. Static services (main-her, dashboard)
    ...
    def _render_run_script(
        ...
        The script:
          1. Sources HER_HOME (and any extra env) via with-contenv —
             so e.g. ``-e HER_HOME=/data/her`` is honored at run
          ...
          4. Drops to the her user and exec's
             ``her -p <profile> gateway run`` (or just ``her
             gateway run`` for the default profile — see below).

        Special case: ``profile == "default"`` emits ``her gateway
        run`` with **no** ``-p`` flag. This is the sentinel for "the
        root HER_HOME profile" (the implicit profile that exists at
        the top of $HER_HOME, not under profiles/). It must be
        spelled this way because ``_profile_suffix()`` returns the
        empty string for the root profile, and the dispatcher in
        ``her_cli.gateway`` maps that empty string to the
        ``gateway-default`` service slot. Passing ``-p default`` here
        would instead look up ``$HER_HOME/profiles/default/`` — a
        ...
        ``her_cli.profiles._allocate_gateway_port``). PR #30136
    ...
    def _render_log_run(profile: str) -> str:
        ...
        OQ8-C: persist to ``${HER_HOME}/logs/gateways/<profile>/``.
        CRITICAL: the HER_HOME path is sourced from the runtime env
        via with-contenv — NOT Python-substituted at registration time
        — so a container started with ``-e HER_HOME=/data/her``
        gets its logs under /data/her/logs/..., not the build-time
        default.
        ...
        per line, in order:
          ...
          2. ``T <log_dir>`` — also write a timestamped copy to the
             ...
             files). This is what ``her logs`` reads and what
        ...
        return (
            ...
            f': "${{HER_HOME:=/opt/data}}"\n'
            f'log_dir="$HER_HOME/logs/gateways/{prof}"\n'
            f'mkdir -p "$log_dir"\n'
            f'chown -R her:her "$log_dir" 2>/dev/null || true\n'
            # Skip the drop when already non-root (CAP_SETGID).
            f'[ "$(id -u)" = 0 ] || exec s6-log 1 n10 s1000000 T "$log_dir"\n'
            f'exec s6-setuidgid her s6-log 1 n10 s1000000 T "$log_dir"\n'
        )
    ...
    def register_profile_gateway(
        ...
        *,
        ...
        try:
            ...
            # Pre-create the supervise/ skeleton with her ownership
            ...
            # the her user) won't hit EACCES on root-owned 0700
    ...
    def unregister_profile_gateway(self, profile: str) -> None:
        ...
        **before** ``rmtree`` so s6-svscan reaps the supervise child
        ...
        # is her-owned (see ``_seed_supervise_skeleton``), and on
