"""CLI subcommand: ``her send`` — pipe text from shell scripts to any
...
def _read_message_body(
    ...
    if file_path:
        ...
        try:
            ...
            print(f"her send: cannot read {file_path}: {exc}", file=sys.stderr)
    ...
    *,
    ...
    if json_mode:
        ...
        if payload.get("error"):
            print(f"her send: {payload['error']}", file=sys.stderr)
...
def _list_targets(platform_filter: Optional[str], *, json_mode: bool) -> int:
    ...
    try:
        ...
        print(f"her send: failed to load channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT

    try:
        raw = load_directory()
    except Exception as exc:
        print(f"her send: failed to read channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT
    ...
    if platform_filter:
        ...
        if not filtered:
            print(
                f"her send: no targets found for platform '{platform_filter}'. "
    ...
    if not any(platforms.values()):
        ...
        print("Set one up with `her gateway setup`, or run the gateway once so")
        print("channel discovery can populate ~/.her/channel_directory.json.")
        return _SUCCESS_EXIT
...
def _load_her_env() -> None:
    """Populate ``os.environ`` from ``~/.her/.env`` AND bridge top-level
    ...
    startup that ``her send`` must replicate when invoked standalone:

    1. ``load_dotenv(~/.her/.env)`` — brings bot tokens into the env.
    2. Bridge top-level simple values from ``~/.her/config.yaml`` into
    ...
    intentionally reimplement the minimum needed here so ``her send``
...
def cmd_send(args: argparse.Namespace) -> None:
    ...
    # Bridge ~/.her/.env and ~/.her/config.yaml into os.environ so the
    ...
    if not target:
        print(
            "her send: --to PLATFORM[:channel[:thread]] is required\n"
            "Examples:\n"
            "  her send --to telegram \"hello\"\n"
            "  her send --to discord:#ops --file report.md\n"
            "  her send --list      # list available targets",
            file=sys.stderr,
        )
        sys.exit(_USAGE_EXIT)
    ...
    if message is None or not message.strip():
        print(
            "her send: no message provided. Pass text as a positional "
    ...
    # Import lazily so `her send --help` stays fast and does not pull in
...
def register_send_subparser(subparsers) -> argparse.ArgumentParser:
    ...
    parser = subparsers.add_parser(
        ...
        description=(
            "Pipe text from any shell script to any messaging platform her "
            "is already configured for. Reuses the gateway's platform "
            "credentials (~/.her/.env + ~/.her/config.yaml) — no LLM, "
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="List available targets. Optional positional filter: `her send --list telegram`.",
    )
