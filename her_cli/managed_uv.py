...
class _UvResult(str):
    ...
    ``(path, fresh_bootstrap)`` tuple across releases. ``her update`` runs
    the call site from the *old*, already-imported ``her_cli.main`` against
...
def update_managed_uv() -> Optional[str]:
    ...
    Call this during ``her update`` so the managed copy stays current.
