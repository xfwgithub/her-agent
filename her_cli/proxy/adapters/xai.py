...
# xAI's public API is OpenAI-compatible for the endpoints her commonly
# uses. The Responses endpoint is included because her' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via her-managed OAuth credentials."""

    auth_hint = "her auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`her auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`her auth reset xai-oauth` or re-authenticate with "
                    "`her auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `her auth add xai-oauth --type oauth`."
            )
