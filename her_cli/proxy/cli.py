"""CLI handlers for the ``her proxy`` subcommand."""
...
from her_cli.proxy.server import (
    AIOHTTP_AVAILABLE,
    DEFAULT_HOST,
    DEFAULT_PORT,
    run_server,
...
def _print_aiohttp_missing() -> None:
    print(
        "her proxy requires aiohttp. Install one of:\n"
...
def cmd_proxy_start(args: Any) -> int:
    ...
    if not adapter.is_authenticated():
        auth_hint = getattr(adapter, "auth_hint", f"her auth add {adapter.name}")
        print(
    ...
    print(
        f"Starting her proxy for {adapter.display_name}\n"
...
def cmd_proxy_status(args: Any) -> int:
    ...
    print("her proxy upstream adapters\n")
    ...
    print(
        "\nStart the proxy with: her proxy start [--provider <name>]"
    )
    return 0
...
def cmd_proxy(args: Any) -> int:
    ...
    print(
        "her proxy — local OpenAI-compatible proxy that attaches your\n"
        "OAuth-authenticated provider credentials to outbound requests.\n"
        "\n"
        "Subcommands:\n"
        "  her proxy start [--provider nous|xai] [--host 127.0.0.1] [--port 23454]\n"
