...
Saves per-platform tool configuration to ~/.her/config.yaml under
...
from her_cli.nous_account import format_nous_portal_entitlement_message
...
# who want it opt in via `her tools` → Video Generation, which walks
...
# `her tools` → X (Twitter) Search setup walks users through credential
...
def _xai_credentials_present() -> bool:
    ...
    """
    try:
        from her_cli.auth import _read_xai_oauth_tokens
...
# Platform-scoped toolsets: only appear in the `her tools` checklist for
...
def _get_effective_configurable_toolsets():
    ...
    Without the dedupe, ``her tools`` → "reconfigure existing" would
...
def _checklist_toolset_keys(platform: str) -> Set[str]:
    """Return the toolset keys the ``her tools`` checklist actually offers
    for ``platform``.
    ...
    so ``her tools`` never claims to add or remove a toolset the user was
...
def install_cua_driver(upgrade: bool = False) -> bool:
    ...
    * ``upgrade=True`` — always re-run the installer (or call ``cua-driver
      update`` if the binary supports it). Used by ``her update`` and
      by ``her computer-use install --upgrade``.
    ...
    if _plat.system() != "Darwin":
        if upgrade:
            # Silent on non-macOS — `her update` calls this for every
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        if not node_modules.exists() and npm_bin:
            ...
            if result.returncode == 0:
                ...
                _print_warning(f"    npm install failed - run manually: cd {display_her_home()}/her-agent && npm install --workspaces=false")
                if result.stderr:
                    _print_info(f"      {result.stderr.strip()[:200]}")
        elif not node_modules.exists():
            _print_warning("    Node.js not found - browser tools require: npm install (in her-agent directory)")
            return
        ...
        if _running_in_docker():
            ...
            _print_info(
                "      docker pull ghcr.io/nousresearch/her-agent:latest"
            )
            return
        ...
        _print_info("    Switch voices by setting tts.piper.voice in ~/.her/config.yaml")
        ...
        # Run the full `her auth spotify` flow — if the user has no
        # client_id yet, this drops them into the interactive wizard
        # (opens the Spotify dashboard, prompts for client_id, persists
        # to ~/.her/.env), then continues straight into PKCE. If they
        # already have an app, it skips the wizard and just does OAuth.
        from types import SimpleNamespace
        try:
            from her_cli.auth import login_spotify_command
        except Exception as exc:
            _print_warning(f"    Could not load Spotify auth: {exc}")
            _print_info("    Run manually: her auth spotify")
            return
        ...
        try:
            ...
            # toolset enable; they can retry with `her auth spotify`.
            _print_warning(f"    Spotify login did not complete: {exc}")
            _print_info("    Run later: her auth spotify")
        except Exception as exc:
        ...
        try:
            from her_cli.plugins_cmd import _get_enabled_set, _save_enabled_set
        ...
        try:
            ...
            _print_info("    Run later: her auth add xai-oauth   (or set XAI_API_KEY)")
            return
        ...
        idx = prompt_choice(
            ...
            choices=[
                ...
                "Skip — configure later via `her auth add xai-oauth`",
            ],
            default=0,
        )
        if idx == 0:
            if _run_xai_oauth_login_from_setup():
                ...
                _print_warning(
                    ...
                    "Run later: her auth add xai-oauth"
                )
        elif idx == 1:
            ...
            if api_key:
                ...
                _print_warning(
                    "    No API key provided. Run later: her auth add xai-oauth"
                )
        else:
...
def valid_post_setup_keys() -> Set[str]:
    ...
    ``post_setup``). This is the allowlist the ``her tools post-setup``
...
def run_post_setup_command(args) -> int:
    """``her tools post-setup <key>`` — non-interactive post-setup runner.
    ...
    if not key:
        _print_error("Usage: her tools post-setup <key>")
        return 2
...
def _get_platform_tools(
    ...
    *,
    ...
    if has_explicit_config:
        ...
        # ``[her-cli, spotify]`` after enabling Spotify via ``her
        ...
        # (e.g. "her-cli") to individual tool names and reverse-mapping.
        all_tool_names = set()
        for ts_name in toolset_names:
        ...
        # through ``her tools`` to flip the toolset on. Only fires when
    ...
    # otherwise saving via `her tools` (which flips has_explicit_config
    # to True) silently drops them.
    _plat_info = PLATFORMS.get(platform)
    _default_ts = _plat_info["default_toolset"] if _plat_info else f"her-{platform}"
    ...
    skip |= {k for k in TOOLSETS if k.startswith("her-")}
    ...
    # shipped as a bundled plugin but user must opt in via `her tools`
    # so we don't ship 7 Spotify tool schemas to users who don't use it).
    # A plugin toolset is "known" for a platform once `her tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by her tools — default enabled
                enabled_toolsets.add(pts)
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Also exclude platform default toolsets (her-cli, her-telegram, etc.)
    ...
    # Opening `her tools` is the user's opt-in to reconfigure tools, so treat
...
_POST_SETUP_INSTALLED: dict = {
    ...
    # who toggles the toolset on via `her tools` gets a silent no-op
...
def _configure_provider(
    ...
    *,
    ...
    # "enable all tools" prompt (that lives in `her model`).
    if managed_feature:
        from her_cli.nous_subscription import (
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `her tools` and `her setup tools`.
    ...
    print()
    from her_constants import display_her_home
    print(color(f"  Tool configuration saved to {display_her_home()}/config.yaml", Colors.DIM))
    print(color("  Changes take effect on next 'her' or gateway restart.", Colors.DIM))
    print()
...
def _configure_mcp_tools_interactive(config: dict):
    ...
    for server_name, tools in server_tools.items():
        ...
        # tools.include across the codebase (catalog installs, her mcp
