"""
Status command for her CLI.

Shows the status of all her Agent components.
"""
...
from her_cli.config import get_env_path, get_env_value, get_her_home, load_config
...
def redact_key(key: str) -> str:
    ...
    "(not set)" placeholder in dim color to match ``her config``'s
...
def show_status(args):
    """Show status of all her Agent components."""
    show_all = getattr(args, 'all', False)
    deep = getattr(args, 'deep', False)

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.CYAN))
    print(color("│                 ⚕ her Agent Status                  │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    if nous_logged_in:
        ...
        nous_label = "not logged in (run: her portal)"
    print(
    ...
    print(
        ...
        f"{'logged in' if codex_logged_in else 'not logged in (run: her model)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if minimax_logged_in else 'not logged in (run: her auth add minimax-oauth)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if xai_oauth_logged_in else 'not logged in (run: her auth add xai-oauth)'}"
    )
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: her model)"
    ...
    if _effective_provider_label() == "LM Studio":
        from her_cli.models import probe_lmstudio_models
        model_cfg = config.get("model")
    ...
    jobs_file = get_her_home() / "cron" / "jobs.json"
    if jobs_file.exists():
        import json
        try:
    ...
    sessions_file = get_her_home() / "sessions" / "sessions.json"
    ...
    print(color("  Run 'her doctor' for detailed diagnostics", Colors.DIM))
    print(color("  Run 'her setup' to configure", Colors.DIM))
    print()
