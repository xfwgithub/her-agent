"""Profile distributions — shareable, packaged her profiles via git.

A distribution is a her profile published as a git repository (or
...
* ``her profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``her skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``her profile``, not a parallel tree):

    her profile install <source> [--name N] [--alias] [--force] [--yes]
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "backups", "cache",
    # Infrastructure
    "her-agent", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            her_requires=str(data.get("her_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.her_requires:
            out["her_requires"] = self.her_requires
        if self.author:
...
def check_her_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires her {op}{target}, "
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``her profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a her profile distribution."
            )
...
def plan_install(
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a her distribution."
        )

    # Version check up-front so we fail fast
    check_her_requires(manifest.her_requires, her_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.her).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="her_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `her profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`her profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="her_dist_update_") as tmp:
        plan = plan_install(
