...
from her_cli.config import get_project_root, get_her_home, get_env_path
from her_cli.env_loader import load_her_dotenv
from her_constants import display_her_home

PROJECT_ROOT = get_project_root()
HER_HOME = get_her_home()
_DHH = display_her_home()  # user-facing display path (e.g. ~/.her or ~/.her/profiles/coder)

# Load environment variables from ~/.her/.env so API key checks work
...
def _has_provider_env_config(content: str) -> bool:
    """Return True when ~/.her/.env contains provider auth/base URL settings."""
...
def _doctor_tool_availability_detail(toolset: str) -> str:
    ...
    if toolset == "kanban" and not os.environ.get("HER_KANBAN_TASK"):
...
def _has_healthy_oauth_fallback_for_apikey_provider(provider_label: str) -> bool:
    ...
    if normalized in {"google / gemini", "gemini"}:
        try:
            from her_cli.auth import get_gemini_oauth_auth_status
            return bool((get_gemini_oauth_auth_status() or {}).get("logged_in"))
        except Exception:
            return False
    if normalized == "minimax":
        try:
            from her_cli.auth import get_minimax_oauth_auth_status
            return bool((get_minimax_oauth_auth_status() or {}).get("logged_in"))
        except Exception:
            return False
    if normalized == "xai":
        try:
            from her_cli.auth import get_xai_oauth_auth_status
...
def _check_version_consistency(issues: list[str]) -> None:
    """Verify pyproject.toml version matches her_cli.__version__.

    A git conflict resolution (reset/merge) can revert one file without the
    other, leaving ``her --version`` reporting a stale version while
    ...
    """
    try:
        from her_cli import __version__ as init_version
    except Exception:
        return
    ...
    if pyproject_version == init_version:
        ...
        _fail_and_issue(
            ...
            f"(pyproject.toml {pyproject_version} != her_cli/__init__.py {init_version})",
            "Re-sync version files (e.g. run 'her update', or set "
            "her_cli/__init__.py __version__ to match pyproject.toml)",
            issues,
        )
...
def _check_s6_supervision(issues: list[str]) -> None:
    ...
    Reports:
      - Whether the main-her and dashboard static services are up
    ...
    for static in ("main-her", "dashboard"):
    ...
    if not profiles:
        check_info("No per-profile gateways registered yet — create one with `her profile create <name>`")
        return
...
def _build_apikey_providers_list() -> list:
    ...
    try:
        ...
        try:
            from her_cli.providers import normalize_provider as _normalize_provider
...
def run_doctor(args):
    ...
    if ack_target:
        ...
        if ack_advisory(ack_target):
            ...
            print(color(
                ...
                f"Check ~/.her/config.yaml is writable.",
                Colors.RED,
            ))
            sys.exit(1)
        return
        ...
        if fresh_hits:
            for hit in fresh_hits:
                ...
                manual_issues.append(
                    ...
                    f"`her doctor --ack {hit.advisory.id}`."
                )
    ...
    # Detect drift between pyproject.toml and her_cli/__init__.py versions
    ...
    # Check ~/.her/.env (primary location for user config)
    env_path = HER_HOME / '.env'
    if env_path.exists():
        ...
        if _has_provider_env_config(content):
            ...
            issues.append("Run 'her setup' to configure API keys")
    else:
        ...
        if fallback_env.exists():
            ...
            if should_fix:
                ...
                check_info("Run 'her setup' to configure API keys")
                fixed_count += 1
            else:
                check_info("Run 'her setup' to create one")
                issues.append("Run 'her setup' to create .env")
    
    # Check ~/.her/config.yaml (primary) or project cli-config.yaml (fallback)
    ...
    if config_path.exists():
        ...
        try:
            ...
            try:
                ...
                pass
            try:
                from her_cli.config import get_compatible_custom_providers as _compatible_custom_providers
                ...
                if catalog_provider is None or (
                    ...
                    _fail_and_issue(
                        ...
                        (
                            ...
                            f"Fix: run 'her config set model.provider <valid_provider>'"
                        ),
                        issues,
                    )
                ...
                try:
                    ...
                    if not configured:
                        _fail_and_issue(
                            ...
                            "(check ~/.her/.env or run 'her setup')",
                            (
                                f"No credentials found for provider '{runtime_provider}'. "
                                f"Run 'her setup' or set the provider's API key in {_DHH}/.env, "
                                f"or switch providers with 'her config set model.provider <name>'"
                            ),
                            issues,
                        )
                except Exception:
        ...
        if fallback_config.exists():
            ...
            if should_fix:
                ...
                if example_config.exists():
                    ...
                    from her_cli.config import DEFAULT_CONFIG, save_config
    ...
    if config_path.exists():
            ...
            if current_ver < latest_ver:
                ...
                if should_fix:
                    try:
                        ...
                        issues.append("Run 'her setup' to migrate config")
                else:
                    issues.append("Run 'her doctor --fix' or 'her setup' to migrate config")
            else:
        ...
        try:
            ...
            if stale_root_keys:
                ...
                if should_fix:
                    ...
                    issues.append("Stale root-level provider/base_url in config.yaml — run 'her doctor --fix'")
        except Exception:
            pass
        ...
        try:
            import yaml
            from her_cli.config import load_env, remove_env_value
            with open(config_path, encoding="utf-8") as f:
                raw_config = yaml.safe_load(f) or {}
    ...
    for subdir_name in expected_subdirs:
        subdir_path = her_home / subdir_name
    ...
    memories_dir = her_home / "memories"
    ...
    state_db_path = her_home / "state.db"
    ...
    wal_path = her_home / "state.db-wal"
    if wal_path.exists():
        try:
            ...
            if wal_size > 50 * 1024 * 1024:  # 50 MB
                ...
                if should_fix:
                    ...
                    issues.append("Large WAL file — run 'her doctor --fix' to checkpoint")
    ...
    try:
        from her_constants import is_container as _is_container
    ...
    if _safe_which("node"):
        ...
        if agent_browser_ok and not _is_termux():
            try:
                ...
                # to eagerly load in every `her doctor` invocation.
            ...
            if r.status_code == 402:
                return _ConnectivityResult(
                    ...
                    ["OpenRouter account has insufficient credits. "
                     "Fix: run 'her config set model.provider <provider>' "
    ...
    def _probe_apikey_provider(pname, env_vars, default_url, base_env,
        ...
        try:
            ...
            # GOOGLE_API_KEY/GEMINI_API_KEY always shows red in ``her doctor``.
    ...
    # never the legitimate source for `her doctor`.
    ...
    try:
        ...
        if api_disabled:
            issues.append("Run 'her setup' to configure missing API keys for full tool access")
    except Exception as e:
        check_warn("Could not check tool availability", f"({e})")
    
    _section("Skills Hub")
    hub_dir = HER_HOME / "skills" / ".hub"
    if hub_dir.exists():
        ...
        check_warn("Skills Hub directory not initialized", "(run: her skills list)")
    ...
    try:
        import yaml as _yaml
        _mem_cfg_path = HER_HOME / "config.yaml"
    ...
    if not _active_memory_provider:
        ...
        try:
            ...
            if not _honcho_cfg_path.exists():
                check_warn("Honcho config not found", "run: her memory setup")
            elif not hcfg.enabled:
                check_info(f"Honcho disabled (set enabled: true in {_honcho_cfg_path} to activate)")
            elif not (hcfg.api_key or hcfg.base_url):
                _fail_and_issue(
                    "Honcho API key or base URL not set",
                    "run: her memory setup",
                    "No Honcho API key — run 'her memory setup'",
                    issues,
                )
            else:
        ...
        try:
            ...
            if mem0_key:
                ...
                _fail_and_issue(
                    ...
                    "(set MEM0_API_KEY in .env or run her memory setup)",
        ...
        try:
            ...
            if _provider and _provider.is_available():
                ...
                check_warn(f"{_active_memory_provider} configured but not available", "run: her memory status")
            else:
                check_warn(f"{_active_memory_provider} plugin not found", "run: her memory setup")
        except Exception as _e:
            ...
            if wrapper_dir.is_dir():
                for wrapper in wrapper_dir.iterdir():
                    ...
                    try:
                        ...
                        if "her -p" in content:
                            _m = _re.search(r"her -p (\S+)", content)
    ...
    if should_fix and fixed_count > 0:
        ...
        print()
        if not should_fix:
            print(color("  Tip: run 'her doctor --fix' to auto-fix what's possible.", Colors.DIM))
    else:
        print(color("─" * 60, Colors.GREEN))
