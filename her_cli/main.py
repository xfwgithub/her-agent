...
Usage:
    ...
    her chat                # Interactive chat
    her gateway             # Run gateway in foreground
    her gateway start       # Start gateway as service
    her gateway stop        # Stop gateway service
    her gateway status      # Show gateway status
    her gateway install     # Install gateway service
    her gateway uninstall   # Uninstall gateway service
    her setup               # Interactive setup wizard
    her logout              # Clear stored authentication
    her status              # Show status of all components
    her cron                # Manage cron jobs
    her cron list           # List cron jobs
    her cron status         # Check if cron scheduler is running
    her doctor              # Check configuration and dependencies
    her honcho setup                    # Configure Honcho AI memory integration
    her honcho status                   # Show Honcho config and connection status
    her honcho sessions                 # List directory → session name mappings
    her honcho map <name>               # Map current directory to a session name
    her honcho peer                     # Show peer names and dialectic settings
    her honcho peer --user NAME         # Set user peer name
    her honcho peer --ai NAME           # Set AI peer name
    her honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    her honcho mode                     # Show current memory mode
    her honcho mode [hybrid|honcho|local]  # Set memory mode
    her honcho tokens                   # Show token budget settings
    her honcho tokens --context N       # Set session.context() token cap
    ...
    her claw migrate --dry-run  # Preview migration without changes
"""

# IMPORTANT: her_bootstrap must be the very first import — it sets up
# UTF-8 stdio on Windows so print()/subprocess children don't hit
# UnicodeEncodeError with non-ASCII characters.  No-op on POSIX.
#
# Guarded against ModuleNotFoundError because ``her_bootstrap`` is a
# top-level module registered via pyproject.toml's ``py-modules`` list.
# When the user upgrades code via ``git pull`` (or ``her update``
# crashes between ``git reset --hard`` and ``uv pip install -e .``), the
# new code references ``her_bootstrap`` but the editable install's
# ``.pth`` file still points at the old set of top-level modules.  Without
# this guard, her crashes on import and the user can't run
# ``her update`` to recover.  Missing the bootstrap means UTF-8 stdio
...
def _set_process_title() -> None:
    """Set the process title to 'her' so tools like 'ps', 'top', and
    'htop' show the app name instead of 'python3.xx'.

    Purely cosmetic — non-fatal on any platform.

    Strategy (try in order):
      1. ``setproctitle`` (opt-in dep — installed via ``her tools`` or
    ...
    try:
        ...
        if system == "Linux":
            ...
            libc.prctl(15, b"her", 0, 0, 0)  # PR_SET_NAME = 15
        elif system == "Darwin":
            libc = ctypes.CDLL("libc.dylib", use_errno=True)
            libc.pthread_setname_np(b"her")
        # Windows: the .exe name is already ``her.exe`` — nothing to do.
    except Exception:
        pass
...
# that run *before* her_cli.config is importable. Mirrors the explicit
...
def _config_default_interface_early() -> str:
    ...
    value = "cli"
    try:
        home = os.environ.get("HER_HOME")
        if home:
            cfg_path = os.path.join(home, "config.yaml")
        else:
            cfg_path = os.path.join(os.path.expanduser("~"), ".her", "config.yaml")
...
def _wants_tui_early(argv: "list[str] | None" = None) -> bool:
    ...
    ``--tui``/``HER_TUI=1``, then ``display.interface`` in config.
    """
    if argv is None:
        argv = sys.argv[1:]
    if "--cli" in argv:
        return False
    if os.environ.get("HER_TUI") == "1" or "--tui" in argv:
        return True
...
# `entry.tsx`; this is just the earlier cousin. ``HER_TUI_NO_EARLY_DISABLE``
# escapes the behaviour for diagnostics.
def _suppress_mouse_residue_early() -> None:
    if os.environ.get("HER_TUI_NO_EARLY_DISABLE") == "1":
        return
    if not _wants_tui_early():
        return
    try:
        # Skip when stdout is redirected (`her --tui … >log`, CI capture):
...
def _require_tty(command_name: str) -> None:
    ...
    Interactive TUI commands (her tools, her setup, her model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'her {command_name}' requires an interactive terminal.\n"
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any her module import.
#
# Many modules cache HER_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("HER_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.her/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set HER_HOME before module imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    ...
    # Mirrors her_cli.profiles._PROFILE_ID_RE so we never call
    ...
    # 1.5 If HER_HOME is already set and no explicit flag was given, trust it
    # only when it already points to a specific profile directory.  The
    # distinguishing heuristic: a profile path has "profiles" as its immediate
    # parent directory name (e.g. ~/.her/profiles/coder or
    # /opt/data/profiles/coder).  If HER_HOME points to the her root
    # instead (e.g. systemd hardcodes HER_HOME=/root/.her), we must
    # still read active_profile — the user may have switched profiles via
    # `her profile use` and the gateway should honour that choice.
    # See issue #22502.
    her_home_env = os.environ.get("HER_HOME", "")
    if profile_name is None and her_home_env:
        if Path(her_home_env).parent.name == "profiles":
            return

    # 2. If no flag, check active_profile in the her root
    if profile_name is None:
        try:
            from her_constants import get_default_her_root

            active_path = get_default_her_root() / "active_profile"
    ...
    # 3. If we found a profile, resolve and set HER_HOME
    if profile_name is not None:
        try:
            from her_cli.profiles import resolve_profile_env

            her_home = resolve_profile_env(profile_name)
        except (ValueError, FileNotFoundError) as exc:
            print(f"Error: {exc}", file=sys.stderr)
            sys.exit(1)
        except Exception as exc:
            # A bug in profiles.py must NEVER prevent her from starting
            print(
                f"Warning: profile override failed ({exc}), using default",
                file=sys.stderr,
            )
            return
        os.environ["HER_HOME"] = her_home
...
# Load .env from ~/.her/.env first, then project root as dev fallback.
...
try:
    ...
    _cfg_path = get_her_home() / "config.yaml"
    if _cfg_path.exists():
...
# Initialize centralized file logging early — all `her` subcommands
...
if _FORCE_IPV4_EARLY:
    try:
        ...
        pass  # best-effort — don't crash if her_constants not importable yet
...
def _termux_bundled_skills_stamp_path() -> Path:
    return get_her_home() / "skills" / ".termux_bundled_sync_stamp"
...
def _has_any_provider_configured() -> bool:
    ...
    if isinstance(model_cfg, dict):
        ...
        _model_name = ""
    _has_her_config = _model_name and _model_name != _DEFAULT_MODEL
    ...
    auth_file = get_her_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        container_info: dict with backend, container_name, exec_user, her_bin
        cli_args: the original CLI arguments (everything after 'her')
    """

    backend = container_info["backend"]
    container_name = container_info["container_name"]
    exec_user = container_info["exec_user"]
    her_bin = container_info["her_bin"]
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    ...
                    f"  }}];\n"
                    f"\n"
                    f"Or run: sudo her {' '.join(cli_args)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
            print(
                f"Error: container '{container_name}' not found via {backend}.\n"
                f"The container may be running under root. Try: sudo her {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def _print_tui_exit_summary(
    ...
    print(f"  her --tui --resume {target}")
    if title:
        print(f'  her --tui -c "{title}"')
    print()
...
def _tui_need_npm_install(root: Path) -> bool:
    """True when @her/ink is missing or node_modules is behind package-lock.json.
    ...
    *root* (``ui-tui/dist/entry.js``).
    ...
    ink = ws_root / "node_modules" / "@her" / "ink" / "package.json"
    if not ink.is_file():
        return True
    if not lock.is_file():
        return False
...
_TUI_BUILD_INPUT_FILES = (
    ...
    "scripts/build.mjs",
    "packages/her-ink/package.json",
    "packages/her-ink/index.js",
    "packages/her-ink/text-input.js",
)
...
def _tui_need_rebuild(root: Path) -> bool:
    ...
    or local edits. Set ``HER_TUI_FORCE_BUILD=1`` to force the old behaviour.
    """
    force = (os.environ.get("HER_TUI_FORCE_BUILD") or "").strip().lower()
...
def _ensure_tui_node() -> None:
    ...
    her_home = os.environ.get("HER_HOME") or str(Path.home() / ".her")
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "HER_HOME": her_home},
            capture_output=True,
            text=True,
            check=False,
        )
    ...
    extras.extend([Path(her_home) / "node" / "bin", Path.home() / ".local" / "bin"])
...
def _find_bundled_tui(her_cli_dir: Path | None = None) -> Path | None:
    """Find a pre-built TUI entry.js bundled in the wheel."""
    if her_cli_dir is None:
        her_cli_dir = Path(__file__).parent
    bundled = her_cli_dir / "tui_dist" / "entry.js"
    return bundled if bundled.is_file() else None


def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    """TUI: --dev → tsx src; else node dist (HER_TUI_DIR prebuilt or esbuild)."""
    _ensure_tui_node()
    ...
    ext_dir = os.environ.get("HER_TUI_DIR")
    if tui_dev and ext_dir:
        print(
            f"Error: --dev is incompatible with HER_TUI_DIR={ext_dir}\n"
            f"The prebuilt TUI has no source code to hot-reload.\n"
            f"Unset HER_TUI_DIR (e.g. `unset HER_TUI_DIR`) to use --dev from a checkout.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if tui_dev:
        # Keep the local @her/ink package exports in sync with source.
        # --dev runs src/entry.tsx directly, but @her/ink resolves through
        # packages/her-ink/dist/entry-exports.js. If that dist bundle is
        ...
        npm = _node_bin("npm")
        ink_dir = tui_dir / "packages" / "her-ink"
        result = subprocess.run(
            [npm, "run", "build"],
            cwd=str(ink_dir),
            capture_output=True,
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from her_cli.oneshot import _normalize_toolsets
...
def _launch_tui(
    ...
    active_session_fd, active_session_file = tempfile.mkstemp(
        prefix="her-tui-active-session-", suffix=".json"
    )
    ...
    if skills:
        if isinstance(skills, (list, tuple)):
            ...
            if flattened:
                env["HER_TUI_SKILLS"] = ",".join(flattened)
        else:
            value = str(skills).strip()
            if value:
                env["HER_TUI_SKILLS"] = value
    if query:
        env["HER_TUI_QUERY"] = query
    if image:
        env["HER_TUI_IMAGE"] = image
    if checkpoints:
        env["HER_TUI_CHECKPOINTS"] = "1"
    if pass_session_id:
        env["HER_TUI_PASS_SESSION_ID"] = "1"
    if max_turns is not None:
        env["HER_TUI_MAX_TURNS"] = str(max_turns)
    if verbose:
        env["HER_TUI_TOOL_PROGRESS"] = "verbose"
    elif quiet:
    ...
    # HER_TUI_RESUME is an internal hand-off from the Python wrapper to the
    # Ink app.  Because we start from os.environ.copy(), an exported/stale value
    # in the user's shell would otherwise make a plain `her --tui` try to
    ...
    # still set HER_TUI_RESUME themselves.
    env.pop("HER_TUI_RESUME", None)
    if resume_session_id:
        env["HER_TUI_RESUME"] = resume_session_id
    ...
    # Exit code 42 = TUI requested an update. Relaunch as `her update` so
    ...
    if code == 42:
        from her_cli.relaunch import relaunch
...
def _pin_kanban_board_env() -> None:
    """Pin the active kanban board into ``HER_KANBAN_BOARD`` for the chat session.

    Without this, in-process tools (``kanban_*``) and shelled-out CLI calls
    (``her kanban …``) resolve the board on different paths: the env-pin if
    set, otherwise the global ``<root>/kanban/current`` file. A concurrent
    ``her kanban boards switch`` from another session can flip the file
    ...
    """
    if os.environ.get("HER_KANBAN_BOARD"):
        return
    try:
        from her_cli.kanban_db import get_current_board

        os.environ["HER_KANBAN_BOARD"] = get_current_board()
    except Exception:
        pass
...
def _resolve_use_tui(args) -> bool:
    ...
    Precedence (highest first):
      ...
      2. ``--tui`` flag / ``HER_TUI=1`` → always TUI
    ...
    if getattr(args, "cli", False):
        return False
    if getattr(args, "tui", False) or os.environ.get("HER_TUI") == "1":
        return True
    try:
...
def cmd_chat(args):
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'her sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    try:
        from her_cli.xai_retirement import (
            MIGRATION_GUIDE_URL,
            RETIREMENT_DATE,
            find_retired_xai_refs,
            format_issue,
        )
        from her_cli.config import load_config as _load_config_for_xai_check
        ...
        if _retired_xai_refs:
            ...
            sys.stderr.write("  \033[2mRun 'her doctor' for details.\033[0m\n\n")
    except Exception:
        pass
    ...
    if not _has_any_provider_configured():
        ...
        if reply in {"", "y", "yes"}:
            ...
            return
        print()
        print("You can run 'her setup' at any time to configure.")
        sys.exit(1)
...
def cmd_proxy(args):
    ...
    from her_cli.proxy.cli import cmd_proxy as _cmd_proxy
...
def cmd_whatsapp(args):
    ...
    # has no creds.json.  Every subsequent `her gateway` then paid a 30s
    ...
    session_dir = get_her_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in {"y", "yes"}:
            ...
            print("  Start the gateway with: her gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        # and `her gateway` skips it cleanly instead of paying a 30s
        ...
        print("⚠ Pairing may not have completed. Run 'her whatsapp' to try again.")
...
def cmd_postinstall(args):
    ...
    from her_cli.config import stamp_install_method
    from her_cli.dep_ensure import ensure_dependency
...
def cmd_model(args):
    ...
    if getattr(args, "refresh", False):
        try:
            from her_cli.models import clear_provider_models_cache
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``her model``) and the setup wizard
    ...
    from her_cli.providers import resolve_provider_full
    ...
    if not active and effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            warning = (
                f"Unknown provider '{effective_provider}'. Check 'her model' for "
                "available providers, or run 'her doctor' to diagnose config "
                "issues."
            )
    ...
    # PROVIDER_GROUPS in her_cli/models.py). A multi-member group shows one
    ...
    # a leftover OPENAI_BASE_URL in ~/.her/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.her/.env if the active provider is not 'custom'.
...
# `her model` provider picker. It does NOT re-run credential setup — it
# only routes already-authenticated providers to specific aux tasks. Users
# configure new providers through the normal `her model` flow first.
# ─────────────────────────────────────────────────────────────────────────────
...
def _all_aux_tasks() -> list[tuple[str, str, str]]:
    ...
    :meth:`her_cli.plugins.PluginContext.register_auxiliary_task`) appear
    in the same surfaces as built-in ones without core knowing about them.
    """
    tasks = list(_AUX_TASKS)
    try:
        from her_cli.plugins import get_plugin_auxiliary_tasks
...
def _aux_select_for_task(task: str) -> None:
    ...
    ``her model`` flow, then route aux tasks to them here.
    """
    from her_cli.config import load_config
...
def _aux_flow_custom_endpoint(task: str, task_cfg: dict) -> None:
    ...
    from her_cli.secret_prompt import masked_secret_prompt
...
def _model_flow_openrouter(config, current_model=""):
    ...
    # in-flow (K/R/C) instead of having to edit ~/.her/.env by hand. The
    # previous bypass-when-key-exists branch left no way to recover from a
    # bad paste short of re-running `her setup` from scratch. OpenRouter
...
def _model_flow_custom(config):
    ...
    if model_name:
        ...
        print("Endpoint saved. Use `/model` in chat or `her model` to set a model.")
...
def _prompt_custom_api_mode_selection(base_url: str, current_api_mode: str = "") -> Optional[str]:
    ...
    """
    from her_cli.runtime_provider import _detect_api_mode_for_url
...
def _model_flow_azure_foundry(config, current_model=""):
    ...
    * Manual entry when every probe fails (private endpoints, etc.).
    ...
    """
    from her_cli.auth import _save_model_choice, deactivate_provider  # noqa: F401
    from her_cli.config import (
        get_env_value,
        save_env_value,
        load_config,
    ...
    from her_cli import azure_detect
...
# downstream call sites read `her_cli.main._PROVIDER_MODELS` directly,
# so the symbol needs to be reachable as a module attribute. But importing
# the catalog eagerly costs ~55ms on every `her` invocation — including
# fast paths like `her --version` and slash-command dispatch that never
...
def _model_flow_copilot(config, current_model=""):
    ...
    if not api_key:
        ...
        if source in {"GITHUB_TOKEN", "GH_TOKEN"}:
            from her_cli.env_loader import format_secret_source_suffix
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "") -> tuple:
    """Shared API-key entry point for ``her setup`` / ``her model``.
    ...
    recover from a malformed paste without editing ``~/.her/.env`` by hand.

    Returns ``(resolved_key, abort)``.  ``abort=True`` means the caller should
    ``return`` immediately — the user cancelled entry, declined to replace, or
    cleared the key and is now unconfigured.
    """
    from her_cli.auth import LMSTUDIO_NOAUTH_PLACEHOLDER
    ...
    if choice.startswith("c"):
        ...
        print(
            f"  API key cleared.  Re-run `her setup` to configure {pconfig.name} again."
        )
        return "", True
...
def _model_flow_stepfun(config, current_model=""):
    ...
    from her_cli.models import _PROVIDER_MODELS, fetch_api_models
...
def _model_flow_bedrock_api_key(config, region, current_model=""):
    ...
    if selected:
        ...
        # Save the API key env var name so her knows where to find it
...
def _model_flow_api_key_provider(config, provider_id, current_model=""):
    ...
    if provider_id == "lmstudio":
        ...
        from her_cli.models import fetch_lmstudio_models
...
def _model_flow_anthropic(config, current_model=""):
    ...
    if selected:
        ...
        # providers if the user switches without running 'her model'.
        cfg = load_config()
        model = cfg.get("model")
        if not isinstance(model, dict):
            model = {"default": model} if model else {}
...
def cmd_slack(args):
    ...
    Dispatches ``her slack <subcommand>``. Currently supports:
    ...
    if sub in {None, ""}:
        ...
        print(
            "usage: her slack <subcommand>\n"
            "\n"
            "subcommands:\n"
            "  manifest   Generate a Slack app manifest with every gateway\n"
            "             command registered as a native slash\n"
            "\n"
            "Run `her slack manifest -h` for details.",
            file=sys.stderr,
        )
        return 1

    if sub == "manifest":
        from her_cli.slack_cli import slack_manifest_command
...
def cmd_kanban(args):
    ...
    from her_cli.kanban import kanban_command
...
def cmd_hooks(args):
    ...
    from her_cli.hooks import hooks_command
...
def cmd_security(args):
    """Dispatch `her security <subcmd>`."""
    sub = getattr(args, "security_command", None)
    if sub in ("audit", None):
        from her_cli.security_audit import cmd_security_audit
...
# Critical files that every ``her`` invocation imports at startup. If any
# of these fail to parse after a pull, the CLI is bricked — the user can't
# even run ``her update`` again to roll forward. The post-pull syntax
...
def _validate_critical_files_syntax(root) -> tuple[bool, str | None, str | None]:
    ...
    These are the files imported on every ``her`` startup; if any of them
    ...
    root = Path(root)
    with tempfile.TemporaryDirectory(prefix="her-syntax-check-") as tmpdir:
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``her update --gateway`` so interactive prompts (stash restore,
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    still outputs to ``her_cli/web_dist/`` (per vite.config.ts
    outDir: "../her_cli/web_dist"), NOT to ``web/dist/``, so Python
    ...
    dist_dir = project_root / "her_cli" / "web_dist"
...
def _run_with_idle_timeout(
    ...
    *,
    ...
    ``her`` launcher present but ``her_cli`` not importable.
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    the working tree dirty and causes the next ``her update`` to stash the
    lockfile — repeatedly.
    """
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    Args:
        ...
        fatal: If True, print error guidance and return False on failure
               instead of a soft warning (used by ``her web``).
    ...
    # build path usable on a stock `py -m her_cli.main web` invocation.
    ...
    if r1.returncode != 0:
        _say(
            ...
            + ("" if fatal else " (her web will not be available)")
        )
        _relay(r1)
        if fatal:
...
def _find_stale_dashboard_pids(
    *,
    ...
    """Return PIDs of ``her dashboard`` processes other than ourselves.

    ``her dashboard`` is a long-lived server process commonly started and
    forgotten.  When ``her update`` replaces files on disk, the running
    ...
    *exclude_pids* is an optional set of PIDs that must never be returned.
    ...
    patterns = [
        "her dashboard",
        "her_cli.main dashboard",
        "her_cli/main.py dashboard",
    ]
    self_pid = os.getpid()
    ...
    try:
            ...
            # with `her_cli.gateway._scan_gateway_pids` and avoids the
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `her update`.
    ...
    print("  Preview now:  her curator run --dry-run")
    print("  Pause it:     her curator pause")
    print(
        "  Docs:         https://her-agent.nousresearch.com/docs/user-guide/features/curator"
    )
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``her update`` is a high-attention surface — surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent ``her update`` invocations skip the block until a newer
    ...
    print(
        ...
        "View anytime: her curator status)"
    )
...
def _kill_stale_dashboard_processes(
    ...
    """Kill running ``her dashboard`` processes.

    Called at the end of ``her update`` (default ``reason``) and also
    from ``her dashboard --stop`` (which overrides ``reason``).  The
...
def _update_via_zip(args):
    ...
    if branch != "main":
        ...
        print(
            ...
            "or NTFS filter holding files open) and rerun `her update "
            f"--branch {branch}`, or update against main with `her update`."
        )
        sys.exit(1)
    zip_url = (
        f"https://github.com/NousResearch/her-agent/archive/refs/heads/{branch}.zip"
    )

    print("→ Downloading latest version...")
    tmp_dir = tempfile.mkdtemp(prefix="her-update-")
    try:
        zip_path = os.path.join(tmp_dir, f"her-agent-{branch}.zip")
        ...
        with zipfile.ZipFile(zip_path, "r") as zf:
            ...
            # symlink members. A GitHub source ZIP for her-agent itself
        ...
        # GitHub ZIPs extract to her-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"her-agent-{branch}")
    ...
    from her_cli.managed_uv import ensure_uv, update_managed_uv
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime(
        "her-update-autostash-%Y%m%d-%H%M%S"
    )
...
def _restore_stashed_changes(
    ...
    if restore.returncode != 0 or has_conflicts:
        ...
        # files makes her completely unrunnable (SyntaxError on import).
...
# =========================================================================
# Fork detection and upstream management for `her update`
# =========================================================================

OFFICIAL_REPO_URLS = {
    "https://github.com/NousResearch/her-agent.git",
    "git@github.com:NousResearch/her-agent.git",
    "https://github.com/NousResearch/her-agent",
    "git@github.com:NousResearch/her-agent",
}
OFFICIAL_REPO_URL = "https://github.com/NousResearch/her-agent.git"
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_her_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_her_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
    ...
    if not has_upstream:
        ...
        if response in {"", "y", "yes"}:
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/NousResearch/her-agent.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/NousResearch/her-agent.git' to add later."
            )
...
def _invalidate_update_cache():
    ...
    ``her update``, every profile is now current.
    """
    homes = []
    # Default profile home (Docker-aware — uses /opt/data in Docker)
    from her_constants import get_default_her_root

    default_home = get_default_her_root()
...
def _run_install_with_heartbeat(
    ...
    *,
    ...
    know ``her update`` is still progressing even if pip/uv itself is silent.
    """
...
def _her_exe_shims(scripts_dir: Path) -> list[Path]:
    ...
    return [
        scripts_dir / "her.exe",
        scripts_dir / "her-gateway.exe",
    ]


def _detect_concurrent_her_instances(
    ...
    chain are excluded so the running ``her update`` invocation never
    reports itself — this matters on Windows where the setuptools .exe
    launcher (``her.exe``) is a separate process from the Python
    ...
    for shim in _her_exe_shims(scripts_dir):
        try:
    ...
    # setuptools-generated her.exe launcher is a separate native process
    ...
    # file lock) is the parent. Without excluding it, every ``her update``
...
def _format_concurrent_instances_message(
    ...
    shim = scripts_dir / "her.exe"
    lines = ["✗ Another her.exe is running:"]
    ...
    if matches:
        ...
        lines.append("")
    lines.append("  Override with `her update --force` if you've already")
    lines.append("  confirmed those processes will not write to the venv.")
    return "\n".join(lines)


def _quarantine_running_her_exe(
    scripts_dir: Path, *, max_attempts: int = 4
) -> list[tuple[Path, Path]]:
    """Pre-empt Windows file lock on the running ``her.exe``.
    ...
    when ``her update`` runs, ``her.exe`` IS the live process, and uv
    fails with ``Access is denied. (os error 5)``.

    We rename live shims to ``her.exe.old.<unix-ms>`` first. uv then writes
    fresh shims at the original paths. The ``.old`` files are cleaned up on
    the next her invocation by ``_cleanup_quarantined_exes``.
...
def _restore_quarantined_exes(moved: list[tuple[Path, Path]]) -> None:
    """Roll back ``_quarantine_running_her_exe`` if uv didn't write replacements."""
...
def _run_quarantined_install(
    ...
    *,
    ...
    """Run an editable install, quarantining the running ``her.exe`` first.

    Any ``pip install -e .`` (or ``--reinstall``) rewrites the entry-point
    shims, and on Windows the live ``her.exe`` is the running process —
    pip can neither delete nor overwrite it, so without quarantine the shim
    is left missing and ``her`` drops off PATH. This wraps
    ...
    if scripts_dir is not None:
        moved = _quarantine_running_her_exe(scripts_dir)
    try:
...
def _cleanup_quarantined_exes(scripts_dir: Path | None = None) -> None:
    """Sweep ``her.exe.old.*`` left by prior updates.

    Called early on every her invocation. The .old files are unlocked once
...
def _refresh_active_lazy_features() -> None:
    ...
    first use. ``her update`` runs ``uv pip install -e .[all]`` which
    ...
    if failed:
        ...
        print("  `her update` once the upstream issue is resolved.")
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    On Windows, pre-renames live ``her.exe`` / ``her-gateway.exe`` shims
    in the venv Scripts dir before each install attempt so uv can write fresh
    copies (Windows blocks REPLACE on a running .exe but allows RENAME). See
    ``_quarantine_running_her_exe`` for the rationale.
    """
...
def _verify_core_dependencies_installed(
    ...
    *,
    ...
    # outer Python that drove ``her update``, not the venv we just wrote
    ...
    #
    # Quarantine the running ``her.exe`` first: ``--reinstall -e .``
    # rewrites the entry-point shims, and on Windows pip can't overwrite the
    # live launcher, which would leave ``her`` off PATH.
    scripts_dir = _venv_scripts_dir() if _is_windows() else None
    ...
    try:
        ...
        print("  ⚠ Repair install failed; check `her update` output above.")
        return
    ...
    try:
        ...
        print(
            ...
            "Run `her update --force` after closing other her processes."
        )
        return
...
def _install_psutil_android_compat(
    ...
    *,
    ...
    from her_cli.psutil_android import PSUTIL_URL, prepare_patched_psutil_sdist
...
def _ensure_uv_for_termux(pip_cmd: list[str]) -> str | None:
    ...
    standalone uv into ``$HER_HOME/bin/uv``, but on Termux the official
...
class _UpdateOutputStream:
    """Stream wrapper used during ``her update`` to survive terminal loss.

    Wraps the process's original stdout/stderr so that:

    * Every write is also mirrored to an append-only log file
      (``~/.her/logs/update.log``) that users can inspect after the
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``her update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``her update`` in an SSH session or a terminal
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.her/logs/update.log`` and to silently absorb
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In gateway mode (``her update --gateway``) the update is already
    ...
    try:
        ...
        # her_cli.config.get_her_home to simulate setup failure.
        from her_cli.config import get_her_home as _get_her_home

        logs_dir = _get_her_home() / "logs"
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    """Implement ``her update --check``: fetch and report without installing.
    ...
    if method == "docker":
        ...
        # same long-form ``docker pull`` guidance ``her update`` (apply
        # path) uses — telling the user to "reinstall via curl" or that
        # ".git is missing" would point them at the wrong remediation.
        from her_cli.config import format_docker_update_message
        print(format_docker_update_message())
        sys.exit(1)
    if method == "pip":
        from her_cli.config import recommended_update_command
        from her_cli.banner import check_via_pypi
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``her update`` without requiring a reinstall.  The
    ...
    ``her`` prints ``command not found`` even though the symlink lives
    at /usr/local/bin/her.

    Silent no-op on: non-Linux, non-root, non-FHS installs, and any system
    where ``bash -i -c 'command -v her'`` already resolves.  Idempotent.
    """
    ...
    # /usr/local/bin/her, code at /usr/local/lib/her-agent).
    fhs_link = Path("/usr/local/bin/her")
...
def _run_pre_update_backup(args) -> None:
    """Create a full zip backup of HER_HOME before running the update.
    ...
    HER_HOME directories.  The ``--backup`` flag on ``her update``
    ...
    # Render path using display_her_home so the user sees ~/.her/...
    try:
        from her_constants import get_her_home, display_her_home

        home = get_her_home()
        try:
            display_path = f"{display_her_home()}/{out_path.relative_to(home)}"
        except ValueError:
    ...
    print(f"  Restore:  her import {out_path}")
...
def _discard_lockfile_churn(git_cmd, repo_root):
    ...
    ``her update`` sees a clean tree instead of autostashing every run.
...
def cmd_update(args):
    ...
    if getattr(args, "check", False):
        ...
        # what a subsequent `her update --branch=<x>` would actually pull.
        branch = _resolve_update_branch(args)
        _cmd_update_check(
            branch=branch,
...
def _cmd_update_pip(args):
    ...
    if is_uv_tool_install():
        ...
        cmd = [pipx, "upgrade", "her-agent"]
    elif uv:
        cmd = [uv, "pip", "install", "--upgrade", "her-agent"]
        if in_venv:
        ...
        cmd = [sys.executable, "-m", "pip", "install", "--upgrade", "her-agent"]
    ...
    print("✓ Update complete! Restart her to use the new version.")
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    if _is_windows() and not getattr(args, "force", False):
        scripts_dir = _venv_scripts_dir()
        if scripts_dir is not None:
            concurrent = _detect_concurrent_her_instances(scripts_dir)
            if concurrent:
    ...
    try:
        ...
        # though `git pull` can't touch $HER_HOME, this is cheap
        ...
        # orphan merge-conflict markers in her_cli/config.py bricked
        # every user who ran ``her update`` for the 7 minutes between
        ...
        try:
            ...
            # so the CLI stays bootable. The user can then retry ``her
            ...
            if not syntax_ok:
                ...
                if pre_pull_sha:
                    ...
                    if rollback_result.returncode == 0:
                        ...
                        print("  Try ``her update`` again later once a fix lands.")
                    else:
        ...
        # the old bytecode (e.g. get_her_home added to her_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # attributes like display_her_home() added since the last release.
        try:
            import importlib
        ...
        # seed_profile_skills() uses subprocess with an explicit HER_HOME so
        # it is not affected by sync_skills()'s module-level HER_HOME cache,
        # which means the active profile is reliably synced regardless of whether
        # the caller's HER_HOME env var points at the default or a named profile.
        try:
        ...
        if version_bump_only:
            ...
            try:
                ...
                print("     Run 'her config migrate' to retry.")
            ...
            if response in {"", "y", "yes", "auto"}:
                ...
                if (gateway_mode or assume_yes or response == "auto") and missing_env:
                    print("  ℹ API keys require manual entry: her config migrate")
            else:
                print()
                print("Skipped. Run 'her config migrate' later to configure.")
        else:
        ...
        # check `her curator status`. Self-stamps after printing so it
        ...
        # Tying the refresh to ``her update`` gives users a predictable
        ...
        try:
            if sys.platform == "darwin" and shutil.which("cua-driver"):
                from her_cli.tools_config import install_cua_driver
        ...
        # When running as ``her update --gateway`` (spawned by the gateway's
        ...
        if gateway_mode:
            _exit_code_path = get_her_home() / ".update_exit_code"
            try:
        ...
        try:
            ...
            # Discover all her-gateway* units (default + profiles)
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            unit = parts[
                                0
                            ]  # e.g. her-gateway.service or her-gateway-coder.service
            ...
            if restarted_services or killed_pids:
                ...
                if unmapped_count:
                    ...
                    print("    Restart manually: her gateway run")
                    if unmapped_count > 1:
                        print(
                            "    (or: her -p <profile> gateway run  for each profile)"
                        )
        ...
        # every `her update` surfaces the issue until the user migrates.
        try:
            from her_cli.gateway import (
                has_legacy_her_units,
                _find_legacy_her_units,
                supports_systemd_services,
            )
            ...
            if supports_systemd_services() and has_legacy_her_units():
                ...
                print()
                print("  These pre-rename units (her.service) fight the current")
                print("  her-gateway.service for the bot token and cause SIGTERM")
                print("  flap loops. Remove them with:")
                print()
                print("    her gateway migrate-legacy")
                print()
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``her -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    from her_constants import display_her_home

    action = getattr(args, "profile_action", None)

    if action is None:
        # Bare `her profile` — show current profile status
        profile_name = get_active_profile_name()
        dhh = display_her_home()
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    ...
                    print(f"Alias:          {alias_display} → her -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print(f"Switched to: default (~/.her)")
            else:
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(
                        f"  Choose a custom alias:  her profile alias {name} --name <custom>"
                    )
                    print(f"  Or access via flag:     her -p {name} chat")
                else:
        ...
        from her_cli import profiles as _profiles_mod
        ...
        if name and not text_value and not auto_flag:
            try:
                if _profiles_mod.normalize_profile_name(name) == "default":
                    from her_constants import get_her_home as _hh
        ...
        from her_cli import profile_describer as _pd
        ...
        import tempfile
        from her_cli.profile_distribution import (
            plan_install,
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="her_dist_preview_") as tmp:
                plan = plan_install(
                    args.source,
                    Path(tmp),
            ...
            if plan.has_cron:
                print(
                    ...
                    f"  Review them with:  her -p {plan.manifest.name} cron list"
                )
            print(f"\n  Use with:      her -p {plan.manifest.name} chat")
        ...
        from her_cli.profiles import get_profile_dir, normalize_profile_name
        ...
        try:
            ...
            if current is None:
                print(
                    ...
                    "Only profiles installed via `her profile install` can be updated."
                )
                sys.exit(1)
            ...
            if plan.has_cron:
                print(
                    ...
                    f"her -p {plan.manifest.name} cron list"
                )
        except (DistributionError, ValueError) as e:
            print(f"Error: {e}")
            sys.exit(1)
        ...
        from her_cli.profile_distribution import describe_distribution, DistributionError
...
def _render_distribution_plan(plan) -> None:
    ...
    from her_cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
...
def _report_dashboard_status() -> int:
    """Print ``her dashboard`` PIDs and return the count.

    Uses the same detection logic as ``_find_stale_dashboard_pids`` (the
    current process is excluded, but since ``her dashboard --status``
    runs in a short-lived CLI process that never matches the pattern,
    the exclusion is irrelevant here).
    """
    pids = _find_stale_dashboard_pids()
    if not pids:
        print("No her dashboard processes running.")
        return 0

    print(f"{len(pids)} her dashboard process(es) running:")
    for pid in pids:
...
def cmd_dashboard(args):
    ...
    if getattr(args, "stop", False):
        ...
        if not pids:
            print("No her dashboard processes running.")
            sys.exit(0)
        # Reuse the same SIGTERM-grace-SIGKILL path used after `her update`.
    ...
    try:
        from her_cli.plugins import discover_plugins
        discover_plugins()
    except Exception as exc:
...
def cmd_dashboard_register(args):
    ...
    from her_cli.dashboard_register import cmd_dashboard_register as _impl
...
def cmd_prompt_size(args):
    ...
    from her_cli.prompt_size import cmd_prompt_size as _impl
...
# so that in ``her -m gpt5 chat``, ``gpt5`` is correctly skipped as a
# flag value rather than misclassified as a subcommand. Kept in sync with
# the top-level flags declared in ``her_cli/_parser.py``.
#
...
def _first_positional_argv() -> str | None:
    ...
    ``her -m gpt5 --provider openai chat "msg"`` by skipping the
...
def _plugin_cli_discovery_needed() -> bool:
    ...
    if first is None:
        # Bare ``her`` or only flags → defaults to ``chat``.
        return False
...
def _is_tui_chat_launch(args) -> bool:
    return bool(getattr(args, "tui", False) or os.environ.get("HER_TUI") == "1")
...
def _prepare_agent_startup(args) -> None:
    ...
    if _is_tui_chat_launch(args):
        ...
        try:
            from her_cli.mcp_startup import start_background_mcp_discovery
...
def _try_termux_fast_cli_launch() -> bool:
    ...
    from her_cli._parser import build_top_level_parser
    ...
    if getattr(args, "oneshot", None):
        ...
        from her_cli.oneshot import run_oneshot
...
def _try_termux_fast_tui_launch() -> bool:
    ...
    `her --tui` is the hot path on phones. The full parser setup imports
...
def main():
    """Main entry point for her CLI."""
    # Cosmetic: make the process show up as 'her' instead of 'python3.11'
    ...
    # Sweep stale ``her.exe.old.*`` quarantine files left by previous
    # ``her update`` runs on Windows. Silent no-op on non-Windows or when
    # there's nothing to clean. See ``_quarantine_running_her_exe``.
    try:
    ...
    model_parser.add_argument(
        "--client-id",
        default=None,
        help="OAuth client id to use for Nous login (default: her-cli)",
    )
    model_parser.add_argument(
    ...
    # =========================================================================
    from her_cli.fallback_cmd import cmd_fallback
    ...
    fallback_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://her-agent.nousresearch.com/docs/user-guide/features/fallback-providers"
        ),
    )
    ...
    fallback_subparsers.add_parser(
        "add",
        help="Pick a provider + model (same picker as `her model`) and append to the chain",
    )
    fallback_subparsers.add_parser(
        "remove",
        aliases=["rm"],
    ...
    secrets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "instead of storing them in ~/.her/.env.  Currently supports "
            "Bitwarden Secrets Manager.  See: "
            "https://her-agent.nousresearch.com/docs/user-guide/secrets/bitwarden"
        ),
    )
    ...
    from her_cli import secrets_cli as _secrets_cli
    ...
    # migrate command
    # =========================================================================
    from her_cli.migrate import cmd_migrate, cmd_migrate_xai
    ...
    proxy_start.add_argument(
        "--provider",
        help="Upstream provider: nous or xai (default: nous). See `her proxy providers`.",
    )
    proxy_start.add_argument(
        "--host",
        default=None,
    ...
    setup_parser.add_argument(
        ...
        "compatibility — a bare 'her setup' now does this.",
    )
    setup_parser.add_argument(
        "--quick",
        action="store_true",
    ...
    # =========================================================================
    from her_cli.send_cmd import register_send_subparser
    ...
    login_parser.add_argument(
        "--client-id", default=None, help="OAuth client id to use (default: her-cli)"
    )
    ...
    cron_create.add_argument(
        "--script",
        help=(
            "Path to a script under ~/.her/scripts/. Default mode: "
    ...
    cron_edit.add_argument(
        "--script",
        help=(
            "Path to a script under ~/.her/scripts/. Pass empty string to clear. "
    ...
    # =========================================================================
    from her_cli.kanban import build_parser as _build_kanban_parser
    ...
    hooks_parser = subparsers.add_parser(
        ...
        description=(
            "Inspect shell-script hooks declared in ~/.her/config.yaml, "
            "test them against synthetic payloads, and manage the first-use "
            "consent allowlist at ~/.her/shell-hooks-allowlist.json."
        ),
    )
    ...
    doctor_parser.add_argument(
        ...
        help=(
            ...
            "advisory will no longer trigger startup banners. Run `her "
    ...
    debug_parser = subparsers.add_parser(
        ...
        epilog="""\
Examples:
    her debug share              Upload debug report and print URL
    her debug share --lines 500  Include more log lines
    her debug share --expire 30  Keep paste for 30 days
    her debug share --local      Print report locally (no upload)
    her debug share --no-redact  Disable upload-time secret redaction
    her debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'her debug share'",
    )
    ...
    backup_parser.add_argument(
        "-o",
        "--output",
        help="Output path for the zip file (default: ~/her-backup-<timestamp>.zip)",
    )
    backup_parser.add_argument(
        "-q",
        "--quick",
    ...
    checkpoints_parser = subparsers.add_parser(
        "checkpoints",
        help="Inspect / prune / clear ~/.her/checkpoints/",
        description="Manage the filesystem checkpoint store — the shadow git "
        "repo her uses to snapshot working directories before "
        "write_file/patch/terminal calls. Lets you see how much "
        "space checkpoints occupy, force a prune, or wipe the base.",
    )
    from her_cli.checkpoints import register_cli as _register_checkpoints_cli
    ...
    skills_reset = skills_subparsers.add_parser(
        ...
        description=(
            "Clear a bundled skill's entry from the sync manifest (~/.her/skills/.bundled_manifest) "
            "so future 'her update' runs stop marking it as user-modified. Pass --restore to also "
    ...
    skills_opt_out = skills_subparsers.add_parser(
        ...
        description=(
            ...
            "`her update`, and any direct sync stop seeding bundled skills "
    ...
    skills_opt_in = skills_subparsers.add_parser(
        ...
        description=(
            ...
            "again on the next `her update`. Pass --sync to re-seed now."
        ),
    )
    ...
    from her_cli.bundles import register_cli as _bundles_register, bundles_command
    ...
    plugins_install.add_argument(
        "identifier",
        help="Git URL or owner/repo shorthand (e.g. anpicasso/her-plugin-chrome-profiles)",
    )
    plugins_install.add_argument(
        "--force",
        "-f",
    ...
    _install_enable_group.add_argument(
        "--no-enable",
        action="store_true",
        help="Install disabled (skip confirmation prompt); enable later with `her plugins enable <name>`",
    )
    ...
    # subcommand — ``her --help``, ``her version``, ``her logs``,
    ...
    try:
        from her_cli.curator import register_cli as _register_curator_cli
    ...
    def cmd_memory(args):
        ...
        if sub == "off":
            ...
            if not existing:
                print(
                    f"\n  Nothing to reset — no memory files found in {display_her_home()}/memories/\n"
                )
                return
            ...
            print(f"  Files were in: {display_her_home()}/memories/\n")
        else:
    ...
    tools_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Run 'her tools' with no subcommand for the interactive configuration UI."
        ),
    )
    ...
    # her tools disable <name...> [--platform cli]
    ...
    # her tools enable <name...> [--platform cli]
    ...
    tools_postsetup_p = tools_sub.add_parser(
        ...
        description=(
            ...
            "same step `her tools` runs after you pick a provider that\n"
    ...
    def cmd_tools(args):
        ...
        if action in {"list", "disable", "enable"}:
            ...
            from her_cli.tools_config import run_post_setup_command
    ...
    computer_use_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Use `her computer-use install` to fetch and run the\n"
            "upstream cua-driver installer. This is equivalent to the\n"
            "post-setup hook that `her tools` runs when you first\n"
    ...
    def cmd_computer_use(args):
        ...
        if action == "status":
            ...
            if path:
                ...
                print("  Refresh to latest: her computer-use install --upgrade")
                return
            print("cua-driver: not installed")
            print("  Run: her computer-use install")
            return
    ...
    # flag is omitted, causing `her mcp add ...` to fall through to
    # interactive chat.
    ...
    mcp_sub.add_parser(
        "picker",
        help="Interactive catalog picker (also the default for `her mcp`)",
    )
    mcp_sub.add_parser(
        "catalog",
    ...
    mcp_install_p = mcp_sub.add_parser(
        "install",
        help="Install a catalog MCP by name (e.g. `her mcp install n8n`)",
    )
        ...
        if action == "list":
            ...
            if not args.yes:
                if not _confirm_prompt(
                    ...
                    print("Cancelled.")
                    return
            sessions_dir = get_her_home() / "sessions"
            ...
            # Launch her --resume <id> by replacing the current process
    ...
    claw_migrate.add_argument(
        "--no-backup",
        action="store_true",
        help="Skip the pre-migration zip snapshot of ~/.her/ (by default a "
        "single restore-point archive is written to ~/.her/backups/ "
        "before apply; restorable with 'her import').",
    )
    claw_migrate.add_argument(
    ...
    update_parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Assume yes for interactive prompts (config migration, stash restore). API-key entry is skipped; run 'her config migrate' separately for those.",
    )
    update_parser.add_argument(
        "--branch",
        default=None,
        ...
        help=(
            ...
            "If the local checkout is on a different branch, her will "
    ...
    update_parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Windows: proceed with the update even when another her.exe is detected. The concurrent process will likely cause WinError 32 warnings and may leave a reboot-deferred .exe replacement.",
    )
    ...
    acp_parser.add_argument(
        "--setup-browser",
        action="store_true",
        help="Install agent-browser + Playwright Chromium into ~/.her/node/ "
    ...
    profile_create.add_argument(
        "--no-skills",
        action="store_true",
        help="Create an empty profile with no bundled skills (opts out of `her update` skill sync)",
    )
    profile_create.add_argument(
        "--description",
        default=None,
        help="One- or two-sentence description of what this profile is good at. "
             ...
             "of profile name alone. Skip and add later via `her profile describe`.",
    )
    ...
    # for `her dashboard` cmdlines and SIGTERM them directly — the same
    # path `her update` uses to clean up stale dashboards.
    dashboard_parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running her dashboard processes and exit",
    )
    dashboard_parser.add_argument(
        "--status",
        action="store_true",
    ...
    # client with Nous Portal and write the client_id into ~/.her/.env.
    # Nested subparser so bare `her dashboard` keeps launching the server
    ...
    dashboard_register_parser.add_argument(
        ...
        help=(
            ...
            "https://her.example.com/auth/callback. Omit for localhost-only use."
        ),
    )
    dashboard_register_parser.add_argument(
        "--portal-url",
        ...
        help=(
            ...
            "portal. Also settable via HER_DASHBOARD_PORTAL_URL. Mainly for "
    ...
    logs_parser = subparsers.add_parser(
        ...
        epilog="""\
Examples:
    her logs                    Show last 50 lines of agent.log
    ...
    her logs list               List available log files with sizes
""",
    )
    ...
    # e.g. ``her -c Pokemon Agent Dev`` → ``her -c 'Pokemon Agent Dev'``
    ...
    # fails (e.g. 'her -c model' where 'model' is consumed as the
    ...
    # so introspection/management commands (her hooks list, cron
