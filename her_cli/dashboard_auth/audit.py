...
Profile-aware location: ``$HER_HOME/logs/dashboard-auth.log``.
...
from ``her_constants`` or other her_cli modules — so it can be
...
def _resolve_log_path() -> Path:
    """``$HER_HOME/logs/dashboard-auth.log`` with the standard fallback.

    Mirrors ``her_constants.get_her_home`` semantics: env var wins,
    else ``~/.her``. A local copy avoids an import cycle with the
    middleware which lives below ``her_cli``.
    """
    home = os.environ.get("HER_HOME") or str(Path.home() / ".her")
