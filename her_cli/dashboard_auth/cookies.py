...
Three cookies in play:
  ...
  - her_session_pkce: short-lived PKCE state + CSRF nonce + provider
...
setter wrote ``__Secure-her_session_at`` would never find the value.
...
SESSION_AT_COOKIE = "her_session_at"
SESSION_RT_COOKIE = "her_session_rt"
PKCE_COOKIE = "her_session_pkce"
...
def _cookie_path(prefix: str) -> str:
    ...
    Under ``X-Forwarded-Prefix: /her`` we want ``Path=/her`` so:
...
def set_session_cookies(
    ...
    *,
    ...
    issues a 24h rotating refresh token (her #37247); a provider that
    ...
    ``prefix`` is the normalised X-Forwarded-Prefix value (e.g. ``/her``)
