...
prefix (e.g. ``mission-control.tilos.com/her/*`` -> dashboard on
:9119), injecting ``X-Forwarded-Prefix: /her`` so the backend can
...
def normalise_prefix(raw: Optional[str]) -> str:
    ...
    Returns a string like ``"/her"`` (no trailing slash) or ``""``
