"""MCP picker — interactive `her mcp picker` (also the default `her mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``her mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `her plugin` picker UX: arrow keys to navigate, ENTER on a row
...
from her_cli.cli_output import prompt_yes_no
from her_cli.curses_ui import curses_single_select
from her_cli.mcp_catalog import (
    CatalogEntry,
    CatalogError,
    catalog_diagnostics,
    install_entry,
...
def _configure_tools(name: str) -> None:
    ...
    """
    import argparse
    from her_cli.mcp_config import cmd_mcp_configure
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `her mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: her mcp install <name>    Picker: her mcp",
        Colors.DIM,
    ))
...
def show_catalog() -> None:
    """`her mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`her mcp picker` (and default `her mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`her mcp install <name>` — non-interactive entry-point.
    ...
    if entry is None:
        print(color(
            ...
            "Run `her mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:
