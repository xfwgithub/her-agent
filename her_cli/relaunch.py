...
def resolve_her_bin() -> Optional[str]:
    ...
    ``her.exe`` on PATH or the ``sys.executable -m her_cli.main``
    fallback.
    """
    argv0 = sys.argv[0]
...
def relaunch(
    ...
    *,
    ...
    relaunch contract wants: "run her again as if the user had typed
    the new argv".
    ...
    *emulates* exec by spawning the child and exiting the parent, but
    ...
    is usually ``her.exe`` (a Python console-script shim that wraps
    ``python -m her_cli.main``) or a ``.cmd`` batch file, and both
    ...
    That's functionally equivalent — the user sees "her exited, then
    new her started" — just with two PIDs in play instead of one.
    """
