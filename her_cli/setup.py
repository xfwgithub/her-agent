"""
Interactive setup wizard for her Agent.
...
_DOCS_BASE = "https://her-agent.nousresearch.com/docs"
...
# display_her_home imported lazily at call sites (stale-module safety during her update)
...
from her_cli.secret_prompt import masked_secret_prompt  # noqa: E402
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ her Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure her using environment variables or config commands:")
    print_info("  her config set model.provider custom")
    print_info("  her config set model.base_url http://localhost:8080/v1")
    print_info("  her config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'her setup' in an interactive terminal to use the full wizard.")
    print()
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'her setup')")


def _print_setup_summary(config: dict, her_home):
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'her setup' to configure"))
    ...
    # Video generation — opt-in via `her tools` → Video Generation.
    ...
    if subscription_features.video_gen.managed_by_nous:
        ...
        try:
            ...
            from her_cli.plugins import _ensure_plugins_discovered as _ensure_plugins
            _ensure_plugins()
            _video_backend = None
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'her setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'her setup tts'"))
    else:
    ...
    if subscription_features.modal.managed_by_nous:
        ...
        if subscription_features.modal.direct_override:
            ...
            tool_status.append(("Modal Execution", False, "run 'her setup terminal'"))
    ...
    # Spotify (OAuth via her auth spotify — check auth.json, not env vars)
    try:
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'her setup tools' to configure them,"
        )
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {her_home}/cron/, sessions/, logs/"
    )
    print()
    ...
    print()
    print(f"   {color('her setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('her setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('her setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('her setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('her setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('her config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('her config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('her config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('her', Colors.GREEN)}              Start chatting")
    print(f"   {color('her gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('her doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `her tools` and `her setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``her model``)
    ...
    provider added to ``her model`` is automatically available here.
    ...
    # Delegate to the shared her model flow — handles provider picker,
    ...
    try:
        ...
        print_info("You can try again later with: her model")
    ...
    # on demand via `her auth add`, `her setup` vision, and
    # `her setup tts`. This keeps both quick and full setup thin.
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "nous-openai":
        ...
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.her/.env."
            )
        ...
        if not existing_api_key:
            ...
            if api_key:
                ...
                print_warning(
                    ...
                    f"via her setup model or {_dhh()}/.env to use xAI TTS. "
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'her setup tts')."""
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where her runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        # configurable later via `her setup terminal` / config.yaml.
        ...
        # Image and resource limits use defaults; tune via `her setup terminal`.
...
def _apply_default_agent_settings(config: dict):
    ...
    # bridges it into HER_MAX_ITERATIONS at startup. We no longer write
    ...
    remove_env_value("HER_MAX_ITERATIONS")
    ...
    print_info("  Run `her setup agent` later to customize.")
...
def setup_agent_settings(config: dict):
    ...
    try:
        ...
        if max_iter > 0:
            ...
            # HER_MAX_ITERATIONS from agent.max_turns at startup.
...
def _setup_telegram_auto_result():
    ...
    try:
        from her_cli.telegram_managed_bot import auto_setup_telegram_bot_result
    except ImportError:
        return None
    ...
    try:
        ...
        if "/profiles/" in her_home:
            profile_name = her_home.rstrip("/").rsplit("/", 1)[-1]
    except Exception:
        pass
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where her delivers cron job results,")
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https://her-agent.nousresearch.com/docs/user-guide/messaging/webhooks/")
    print()
    ...
    print_info("   https://her-agent.nousresearch.com/docs/user-guide/messaging/webhooks/#configuring-routes")
    print()
    print_info("   Open config in your editor:  her config edit")
    print_info("   Open config in your editor:  her config edit")


def setup_gateway(config: dict):
    """Configure messaging platform integrations."""
    from her_cli.gateway import _all_platforms, _platform_status, _configure_platform

    print_header("Messaging Platforms")
    print_info("Connect to messaging platforms to chat with her from anywhere.")
    ...
    if not selected:
        print_info("No platforms selected. Run 'her setup gateway' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     her config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if supports_systemd and has_legacy_her_units():
            print_legacy_unit_warning()
            print()
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                if supports_systemd:
                    print_info("  Or as a boot-time service: sudo her gateway install --system")
                print_info("  Or run in foreground:  her gateway")
        else:
            from her_constants import is_container
            if is_container():
                print_info("Start the gateway to bring your bots online:")
                print_info("   her gateway run          # Run as container main process")
                print_info("")
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   her gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `her setup tools` and `her tools` use the same flow:
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    Sources of truth:
      * ``PROVIDER_REGISTRY`` in ``her_cli.auth`` — lists every supported
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    if section_key == "model":
        ...
        from her_cli.gateway import _all_platforms, _platform_status
...
def _load_openclaw_migration_module():
    """Load the openclaw_to_her migration script as a module.
    ...
    spec = importlib.util.spec_from_file_location(
        "openclaw_to_her", _OPENCLAW_SCRIPT
    )
...
# Config values may have different semantics between OpenClaw and her.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure her to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point her at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point her at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point her at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point her at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to her equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing her config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in her.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ her's yolo mode.", Colors.YELLOW))
        print(color("  Instruction files (.md) from OpenClaw may contain incompatible procedures.", Colors.YELLOW))
        print()


def _offer_openclaw_migration(her_home: Path) -> bool:
    ...
    print_info("her can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: her claw migrate --dry-run"
        )
        return False
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: her claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing her configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing her config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in her (use her claw migrate --overwrite to force).")
    if skipped:
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    Wired into ``her setup --portal`` and ``her portal``. This is the
    ...
    her session — model selected, provider set, and web/image/tts/browser
    tools routed via their Portal sub — without being told to run
    ``her setup`` and hunt for the quick-setup option.
    ...
    (``_run_first_time_quick_setup``) and the same one ``her model`` runs
    when you pick Nous. Routing through it (instead of hand-rolling the auth +
    provider write here) means ``her portal`` always offers a model picker,
    ...
    print(color("│     ⚕ her Setup — Nous Portal (one-shot)             │", Colors.MAGENTA))
    print(
        color(
            "└─────────────────────────────────────────────────────────┘",
            Colors.MAGENTA,
    ...
    # setup calls, so `her portal` == quick setup's Nous step.
    try:
        from her_cli.main import _model_flow_nous
        ...
        print_info("  You can retry later with `her portal`.")
        return
    except Exception as exc:
        logger.debug("_model_flow_nous error during `her portal`: %s", exc)
        print()
    ...
    print_info("  Run `her portal info` to inspect routing.")
    print_info("  Run `her` to start chatting.")
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      her setup           — full or quick (auto-detected)
      her setup model     — just model/provider
      her setup tts       — just text-to-speech
      her setup terminal  — just terminal backend
      her setup gateway   — just messaging platforms
      her setup tools     — just tool configuration
      her setup agent     — just agent settings
    """
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚕ her Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ⚕ her Agent Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your her Agent installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_quick_setup(config, her_home)
            return

        print()
        print_header("Reconfigure")
        print_success("You already have her configured.")
        print_info("Running the full wizard — each prompt shows your current value.")
        print_info("Press Enter to keep it, or type a new value to change it.")
        print_info("")
        print_info("Tip: jump straight to a section with 'her setup model|terminal|")
        ...
        migration_ran = _offer_openclaw_migration(her_home)
        if migration_ran:
            config = load_config()

        setup_mode = prompt_choice(
            "How would you like to set up her?",
            [
        ...
        if setup_mode == 0:
            _run_first_time_quick_setup(config, her_home, is_existing)
            return
    ...
    print_info(f"Data folder:  {her_home}")
    print_info(f"Install dir:  {PROJECT_ROOT}")
    print()
    print_info("You can edit these files directly or use 'her config edit'")
    ...
    # Tune later with `her setup agent`.
    if not is_existing:
    ...
    _print_setup_summary(config, her_home)


def _run_first_time_quick_setup(config: dict, her_home, is_existing: bool):
    ...
    settings, tools); the user can customize later via ``her setup <section>``
    or switch providers with ``her model``.
    """
    from her_cli.config import load_config
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'her setup gateway'",
        ],
        0,
    )
    ...
    print()
    print_info("  Configure all settings:    her setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  her setup gateway")
    print()

    _print_setup_summary(config, her_home)


def _run_quick_setup(config: dict, her_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'her setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect her to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'her setup gateway'.")
