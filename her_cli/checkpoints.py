"""`her checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.her/checkpoints/``.  Actions:

    her checkpoints               # same as `status`
    her checkpoints status        # total size, project count, breakdown
    her checkpoints list          # per-project checkpoint counts + workdir
    her checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    her checkpoints clear [-f]    # nuke the entire base (asks first)
    her checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    her checkpoints
    her checkpoints prune --retention-days 3 --max-size-mb 200
    her checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: her checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``her checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `her checkpoints` → status
