...
* We write two files: a shared ``gateway.cmd`` wrapper script (cwd + env + the
  actual ``python -m her_cli.main gateway run --replace`` invocation) and
...
def _launch_elevated_gateway_command(command: str, extra_args: list[str] | None = None) -> bool:
    ...
    """
    _assert_windows()
    args = ["-m", "her_cli.main", *_current_profile_cli_args(), "gateway", command]
    if extra_args:
...
def get_task_name() -> str:
    ...
    from her_cli.gateway import _profile_suffix
...
def get_task_script_path() -> Path:
    ...
    script_dir = Path(get_her_home()) / "gateway-service"
...
def _stable_gateway_working_dir(project_root: Path) -> str:
    ...
    Mirror the POSIX service invariant: anchor at ``HER_HOME`` whenever it
    ...
    source checkout only if ``HER_HOME`` cannot be resolved yet.
    """
    from her_cli.config import get_her_home
...
def _build_gateway_cmd_script(
    ...
    The script:
      ...
      - exports HER_HOME, PYTHONIOENCODING, VIRTUAL_ENV
      - invokes ``pythonw -m her_cli.main [--profile X] gateway run``
    ...
    prog_args = [pythonw_path, "-m", "her_cli.main"]
    if profile_arg:
...
def _write_task_script() -> Path:
    ...
    her_home = str(Path(get_her_home()).resolve())
    profile_arg = _profile_arg(her_home)

    content = _build_gateway_cmd_script(python_path, working_dir, her_home, profile_arg)
...
def _build_gateway_argv() -> tuple[list[str], str, dict[str, str]]:
    ...
    argv = [python_exe, "-m", "her_cli.main"]
    if profile_arg:
...
def _spawn_detached(script_path: Path | None = None) -> int:
    ...
    We spawn ``pythonw.exe -m her_cli.main gateway run``
    ...
    log_dir = Path(get_her_home()) / "logs"
...
def _prompt_install_choices(
    ...
    from her_cli.setup import prompt_yes_no
...
def _install_startup_fallback(script_path: Path, start_now: bool, detail: str) -> None:
    ...
    # Re-running `her -p <profile> gateway install` must be safe.
    ...
    from her_cli.gateway import find_gateway_pids, _profile_arg
    ...
    if running_pids:
        ...
        profile_arg = _profile_arg()
        start_cmd = f"her {profile_arg} gateway start" if profile_arg else "her gateway start"
    ...
    *,
    ...
    if not start_on_login:
        ...
        if start_now:
            ...
            print("  Run later with: her gateway start")
        return
    ...
    if ok:
        ...
        if start_now:
            ...
            print("  Start manually with: her gateway start")
        _print_next_steps()
        return
...
def _report_gateway_start(via: str) -> None:
    ...
    if pids:
        ...
        from her_cli.config import get_her_home
        print(f"    type {Path(get_her_home()).resolve()}\\logs\\gateway.log")
        print(f"    type {Path(get_her_home()).resolve()}\\logs\\gateway-stdio.log")


def _print_next_steps() -> None:
    from her_cli.config import get_her_home

    her_home = Path(get_her_home()).resolve()
    print()
    print("Next steps:")
    print("  her gateway status                      # Check status")
    print(f"  type {her_home}\\logs\\gateway.log       # View logs")
...
def _print_deep_probes() -> None:
    ...
    home = Path(get_her_home()).resolve()
...
def start() -> None:
    ...
    if not task_installed and not startup_installed:
        ...
        if not task_installed and not startup_installed:
            ...
            print("  If a UAC prompt opened, approve it, then run: her gateway start")
            return
...
def stop() -> None:
    ...
    """
    _assert_windows()
    from her_cli.gateway import kill_gateway_processes, _get_restart_drain_timeout
    from gateway.status import get_running_pid
