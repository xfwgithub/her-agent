"""``her logs`` — view and filter Hermes log files.
...
Usage examples::
    ...
    her logs                    # last 50 lines of agent.log
    ...
    her logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_her_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print(f"(Logs are created when Hermes runs — try 'her chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_her_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_her_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_her_home()}/logs/")
        return

    print(f"Log files in {display_her_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'her chat' to generate logs)")
