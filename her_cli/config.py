...
- her config          - Show current configuration
- her config edit     - Open config in editor
- her config set      - Set a specific value
- her config wizard   - Re-run setup wizard
"""
...
def _backup_corrupt_config(config_path: Path) -> Optional[Path]:
    ...
    they re-run the setup wizard or ``her config set`` (which rewrites
    ...
    ``config.yaml`` in place: her never silently mutates the user's config,
...
def _warn_config_parse_failure(config_path: Path, exc: Exception) -> None:
    ...
    A YAML parse error in ``~/.her/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``her logs``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  her config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
#   on every plugin install / ``her update``.
...
def get_managed_system() -> Optional[str]:
    ...
    managed_marker = get_her_home() / ".managed"
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    ...
    container detection here:
      ...
      - the published ``nousresearch/her-agent`` image stamps ``docker``
    ...
    so ``her update`` bailed with "doesn't apply inside the Docker
    ...
    """
    stamp = get_her_home() / ".install_method"
    try:
...
def stamp_install_method(method: str) -> None:
    """Write the install method to ~/.her/.install_method."""
    stamp = get_her_home() / ".install_method"
    try:
...
def is_uv_tool_install() -> bool:
    ...
    def _has_uv_tool_marker(path: str) -> bool:
        ...
        return "/uv/tools/her-agent/" in norm + "/"
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method == "homebrew":
        return "brew upgrade her-agent"
    if method == "docker":
        return "docker pull nousresearch/her-agent:latest"
    if method == "pip":
        if is_uv_tool_install():
            return "uv tool upgrade her-agent"
        import shutil
        if shutil.which("uv"):
            return "uv pip install --upgrade her-agent"
        return "pip install --upgrade her-agent"
    return "her update"
...
# Long-form text for ``her update`` / ``--check`` when running inside the
# Docker image.  Surfaced by ``cmd_update`` and ``_cmd_update_check`` in
# her_cli/main.py; lives here so the wording stays consistent and we
...
pulling a fresh image and restarting your container instead:
  ...
  docker compose up -d --force-recreate her-agent
  # or, for ad-hoc runs, exit the current container and `docker run` again

Verify the new version after restart:
  docker run --rm nousresearch/her-agent:latest --version

Notes:
  • If you pinned a specific tag (e.g. ``:v0.14.0``) the ``:latest`` tag
    won't move your container — pull the newer tag you actually want, or
    switch to ``:latest`` / ``:main`` for rolling updates.  See available
    tags at https://hub.docker.com/r/nousresearch/her-agent/tags
  • Your config and session history live under ``$HER_HOME`` (``/opt/data``
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``her update`` inside Docker.
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from HER_HOME/.container-mode.
    ...
    container_mode_file = get_her_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "her-agent")
    exec_user = info.get("exec_user", "her")
    her_bin = info.get("her_bin", "/data/current-package/bin/her")
...
from her_constants import get_her_home  # noqa: F811,E402
from utils import atomic_replace

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_her_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_her_home() / ".env"
...
def _secure_dir(path):
    ...
    The mode can be overridden via the HER_HOME_MODE environment variable
    (e.g. HER_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse HER_HOME to reach a served subdirectory.
    The execute-only bit on a directory permits cd-through without exposing
    directory listings.

    Also applies ``HER_UID``/``HER_GID``-based ownership when those env
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("HER_HOME_MODE", "").strip()
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into HER_HOME if the user doesn't have one yet."""
...
def ensure_her_home():
    """Ensure ~/.her directory structure exists with secure permissions.
...
def _ensure_her_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"HER_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    # Opt in via ``her chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.her/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: her sweeps the checkpoint base at startup
    ...
    "auxiliary": {
        ...
        # Invoked by ``her kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``her kanban decompose`` and the kanban
        ...
        # ``her profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `her model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # Which interface bare `her` (and `her chat`) launches by default:
        ...
        # REPL and `--tui` (or HER_TUI=1) forces the TUI regardless of config.
        "interface": "cli",
        # When true, `her --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `her -c` muscle memory.  Default off so existing
        # users aren't surprised.  HER_TUI_RESUME=<id> always wins.
        "tui_auto_resume_recent": False,
        # When true (default), `her --tui` drops a one-time hint
        ...
        # e.g. `model · 68% · ~/projects/her`. Per-platform overrides go under
    ...
    "tts": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.her/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.her/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    # cron daemon.
    #
    # See `her curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # normally restored on every `her update`, so pruning them only
        ...
        # skipped), snapshot ~/.her/skills/ into
        # ~/.her/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `her curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for her-specific overrides; everything else
    ...
    "approvals": {
        ...
        # false.  TUI has its own modal overlay (HER_TUI_NO_CONFIRM=1 to
        # opt out there).
    ...
    # stored approval from ~/.her/shell-hooks-allowlist.json.
    ...
    "security": {
        ...
        # longer trigger the startup banner. Add via `her doctor --ack
    ...
    # todos to ready, and fires `her -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # one `her` launches with no -p flag). This does not control the
        ...
        # decomposition is manual via `her kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     her-agent's own python (sys.executable). Maximum isolation
    ...
    # Logging — controls file logging to ~/.her/logs/.
    ...
    # update model picker lists without shipping a her-agent release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://her-agent.nousresearch.com/docs/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `her model` invocation; network failures
    ...
    "gateway": {
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.her/.env,
    ...
    # Session storage — controls automatic cleanup of ~/.her/state.db.
    ...
    "sessions": {
        ...
        # rewrites ``~/.her/sessions/session_{sid}.json`` on every turn
    ...
    # ``her update`` behaviour.
    "updates": {
        # Run a full ``her backup``-style zip of HER_HOME before every
        # ``her update``.  Backups land in ``<HER_HOME>/backups/`` and
        # can be restored with ``her import <path>``.  Off by default —
        # on large HER_HOME directories the zip can add minutes to every
        ...
        "backup_keep": 5,
        # What `her update` does with uncommitted local changes to the
    ...
    # `her tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.her/.env.
    "secrets": {
        "bitwarden": {
            ...
            # in ~/.her/.env (or your shell) and never in config.yaml.
            ...
            # ~/.her/bin/ on first use.  When False you must install
...
OPTIONAL_ENV_VARS = {
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'her model' for endpoint-specific config)",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @her:example.org)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #her)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: her-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'her-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.her.config`` entries, then checks
    ...
    try:
        ...
        # should never break `her update`.  Skill-config prompting is a
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``her_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'her setup' to create a valid config")]
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'her doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("HER_HOME", "~/.her")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$HER_HOME/plugins/`` for currently installed user plugins.
            grandfathered: List[str] = []
            try:
                user_plugins_dir = get_her_home() / "plugins"
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `her plugins enable <name>` to activate."
                    )
    ...
    # settings in their `config.yaml`, and `her curator status` has no
    ...
    #   3. Creates `~/.her/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_her_home() — old profiles that predate this
    #      migration still benefit).
    if current_ver < 23:
        try:
            curator_dir = get_her_home() / "logs" / "curator"
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: her config set <key> <value>")
    ...
    # metadata.her.config in their SKILL.md frontmatter.
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _her_home / "config.yaml"`` local variable
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.her/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.her/config.yaml.
    ...
    (which change ``HER_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
#   openai-codex (OAuth — her auth) — OpenAI Codex
#   nous         (OAuth — her auth) — Nous Portal
...
def save_config(config: Dict[str, Any]):
    """Save configuration to ~/.her/config.yaml."""
    with _CONFIG_LOCK:
        if is_managed():
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.her/.env.
    ...
    interactive menu render (`her tools`, `her setup`, status
    panels). Sanitisation is O(lines × known-keys), so re-parsing the
    same file on every call was burning ~300ms of CPU per `her tools`
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.her/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.her/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.her/.env and os.environ.
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.her/.env or environment."""
...
def show_config():
    ...
    print(color("  her config edit     # Edit config file", Colors.DIM))
    print(color("  her config set <key> <value>", Colors.DIM))
    print(color("  her setup           # Run setup wizard", Colors.DIM))
    print()
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: her config set <key> <value>")
            print()
            print("Examples:")
            print("  her config set model anthropic/claude-sonnet-4")
            print("  her config set terminal.backend docker")
            print("  her config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'her config migrate' to add them")
        ...
        print("  her config           Show current configuration")
        print("  her config edit      Open config in editor")
        print("  her config set <key> <value>   Set a config value")
        print("  her config check     Check for missing/outdated config")
        print("  her config migrate   Update config with new options")
        print("  her config path      Show config file path")
        print("  her config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``her config`` UI so users
