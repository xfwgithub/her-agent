...
Profiles live under ``~/.her/profiles/<name>/`` by default.

The "default" profile is ``~/.her`` itself — backward compatible,
zero migration needed.

Usage::

    her profile create coder          # fresh profile + bundled skills
    her profile create coder --clone  # also copy config, .env, SOUL.md, skills
    her profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    her -p coder chat                 # or via flag
    her profile use coder             # set as sticky default
    her profile delete coder          # remove profile + alias + service
"""
...
# Marker file written by `her profile create --no-skills`.  When present in
# a profile's root, callers of seed_profile_skills() (fresh-create, `her
...
# `her skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    is_default_source = source_resolved == _get_default_her_home().resolve()
...
# Directories/files to exclude when exporting the default (~/.her) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "her-agent",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the her root, NOT to the current HER_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where HER_HOME points outside
    ``~/.her``, profiles live under ``HER_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_her_home() / "profiles"


def _get_default_her_home() -> Path:
    """Return the default (pre-profile) HER_HOME path.

    In standard deployments this is ``~/.her``.
    In Docker/custom deployments where HER_HOME is outside ``~/.her``
    (e.g. ``/opt/data``), returns HER_HOME directly.
    """
...
def _get_active_profile_path() -> Path:
    ...
    return _get_default_her_home() / "active_profile"
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its HER_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_her_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: reserved names, her subcommands, existing binaries in PATH.
    """
    canon = normalize_profile_name(name)
...
def find_alias_for_profile(profile_name: str) -> Optional[str]:
    ...
    alias whose body invokes ``her -p <profile>``. When the alias name equals
    the profile name this is trivial, but a custom alias (``her profile alias
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``her profile list`` for an unrelated profile.
    """
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_her_home()
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``her update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.her)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.her/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
    ...
    # Write the opt-out marker so seed_profile_skills() and `her update`'s
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `her profile describe`
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches HER_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``her profile create
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "HER_HOME": str(profile_dir)},
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.her).\n"
            "To remove everything, use: her uninstall"
        )
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    try:
        ...
        from her_cli.service_manager import get_service_manager
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set HER_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("HER_HOME")
    try:
        os.environ["HER_HOME"] = str(profile_dir)
        ...
        if old_home is not None:
            os.environ["HER_HOME"] = old_home
        elif "HER_HOME" in os.environ:
            del os.environ["HER_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.her/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: her profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from HER_HOME.

    Returns ``"default"`` if HER_HOME is not set or points to ``~/.her``.
    Returns the profile name if HER_HOME points into ``~/.her/profiles/<name>``.
    Returns ``"custom"`` if HER_HOME is set to an unrecognized path.
    """
    from her_constants import get_her_home
    her_home = get_her_home()
    resolved = her_home.resolve()

    default_resolved = _get_default_her_home().resolve()
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.her itself — its parent is ~/ and its
        # directory name is ".her", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: her profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.her itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.her). "
            "Specify a different name: her profile import <archive> --name <name>"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="her_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"her_{old_name}"
    legacy_old_host = f"her.{old_name}"
    new_host = f"her_{new_name}"

    candidates = [
        new_dir / "honcho.json",
        _get_default_her_home() / "honcho.json",
    ...
    for path in candidates:
        ...
        if isinstance(block, dict) and "aiPeer" not in block:
            if source_host.startswith("her_"):
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a HER_HOME path string.

    Called early in the CLI entry point, before any her modules
    are imported, to set the HER_HOME environment variable.
    """
    canon = normalize_profile_name(profile_name)
    validate_profile_name(canon)
    profile_dir = get_profile_dir(canon)
