...
Add, remove, or reorder entries here — both `her setup` and
`her` provider-selection will pick up the change automatically.
"""
...
def _codex_curated_models() -> list[str]:
    ...
    This keeps the gateway /model picker in sync with the CLI `her model`
...
# $HER_HOME/models_dev_cache.json as of 2026-04-28. Whenever xAI renames
...
# this list:  her model, /model, list_authenticated_providers.
#
# Fields:
#   slug        — internal provider ID (used in config.yaml, --provider flag)
#   label       — short display name
#   tui_desc    — longer description for the `her model` interactive picker
# ---------------------------------------------------------------------------

class ProviderEntry(NamedTuple):
    slug: str
    label: str
    tui_desc: str   # detailed description for `her model` TUI
...
def group_providers(slugs):
    ...
    DISPLAY ONLY. Used by every interactive picker (``her model``, the
...
# path. The *interactive* default (GUI onboarding / ``her model``) uses the
# richer free/paid-tier-aware resolver — see ``get_recommended_default_model``
# in her_cli/web_server.py and ``partition_nous_models_by_tier`` — which can
...
def get_default_model_for_provider(provider: str) -> str:
    ...
    model was ever selected (e.g. ``her auth add openai-codex`` without
    ``her model``, or a profile that sets ``provider`` with no ``model``).

    For most providers this is the first entry in ``_PROVIDER_MODELS`` — the
    same model the ``her model`` picker offers first. For metered aggregators
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    ...
    her-agent is tool-calling-first — every provider path assumes the model
...
def fetch_openrouter_models(
    ...
    *,
    ...
    try:
        from her_cli.model_catalog import get_curated_openrouter_models
    ...
    for preferred_id in preferred_ids:
        ...
        if live_item is None:
            continue
        # Hide models that don't advertise tool-calling support — her-agent
...
def get_curated_nous_model_ids() -> list[str]:
    ...
    """
    try:
        from her_cli.model_catalog import get_curated_nous_models
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    source of truth shared with ``her model``, ``/model``, etc.).
    """
...
def _resolve_static_model_alias(
    ...
    try:
        from her_cli.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    Resolution order:
      ...
      2. ``read_credential_pool("copilot")`` — a token (typically a
         ...
         populated by ``her auth add copilot`` and by ``_seed_from_env``
         when the env var is set in ``~/.her/.env``.
    ...
    try:
        ...
        from her_cli.copilot_auth import (
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    if normalized in ("openai", "openai-api"):
        ...
        if api_key:
            ...
            # agentic catalog so ``/model`` matches what ``her model`` shows.
...
#
# Cache strategy:
#   - One JSON file at $HER_HOME/provider_models_cache.json
...
def _provider_models_cache_path() -> Path:
    ...
    return get_her_home() / "provider_models_cache.json"
...
def _credential_fingerprint(provider: str) -> str:
    ...
    relevant tokens live in ``$HER_HOME/auth.json`` and external
...
def _ollama_cloud_cache_path() -> Path:
    ...
    from her_constants import get_her_home
    return get_her_home() / "ollama_cloud_models_cache.json"
