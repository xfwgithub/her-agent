"""
her fallback — manage the fallback provider chain.
...
Subcommands:
  her fallback [list]   Show the current fallback chain (default when no subcommand)
  her fallback add      Pick provider + model via the same picker as `her model`,
                           then append the selection to the chain
  her fallback remove   Pick an entry to delete from the chain
  her fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.her/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        from her_cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store
        ...
        # `her model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  her fallback add")
        print()
        return
    ...
    print("  Docs: https://her-agent.nousresearch.com/docs/user-guide/features/fallback-providers")
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `her model`, then append the selection to the chain."""
    from her_cli.main import _require_tty, select_provider_and_model
    ...
    print("  `her model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `her fallback list` to view, or `her fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``her fallback [subcommand]``."""
