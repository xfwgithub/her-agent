"""her hooks — inspect and manage shell-script hooks.

Usage::

    her hooks list
    her hooks test <event> [--for-tool X] [--payload-file F]
    her hooks revoke <command>
    her hooks doctor

Consent records live under ``~/.her/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.her/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``her hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: her hooks {list|test|revoke|doctor}")
        print("Run 'her hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.her/config.yaml.")
        print("See `her hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `her hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `her hooks test` and `her hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (her wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `her hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `her hooks doctor` would execute
