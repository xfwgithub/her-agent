"""
Backup and import commands for her CLI.

`her backup` creates a zip archive of the entire ~/.her/ directory
(excluding the her-agent repo and transient files).

`her import` restores from a backup zip, overlaying onto the current
HER_HOME root.
"""
...
from her_constants import get_default_her_root, get_her_home, display_her_home
...
_EXCLUDED_DIRS = {
    "her-agent",     # the codebase repo — re-clone instead
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to her root) should be skipped."""
...
def _should_skip_backup_file(abs_path: Path, rel_path: Path, out_path: Path) -> bool:
    ...
    # write can copy data from outside HER_HOME.
...
def run_backup(args) -> None:
    """Create a zip backup of the her home directory."""
    her_root = get_default_her_root()

    if not her_root.is_dir():
        print(f"Error: her home directory not found at {her_root}")
        sys.exit(1)
    ...
    if args.output:
        ...
        if out_path.is_dir():
            ...
            out_path = out_path / f"her-backup-{stamp}.zip"
    else:
        stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
        out_path = Path.home() / f"her-backup-{stamp}.zip"
    ...
    # Collect files
    print(f"Scanning {display_her_home()} ...")
    files_to_add: list[tuple[Path, Path]] = []  # (absolute, relative)
    skipped_dirs = set()

    for dirpath, dirnames, filenames in os.walk(her_root, followlinks=False):
        dp = Path(dirpath)
        rel_dir = dp.relative_to(her_root)
        ...
        for fname in filenames:
            fpath = dp / fname
            rel = fpath.relative_to(her_root)
    ...
    print(f"\nRestore with: her import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a her backup.
    ...
    # Look for telltale files that a her home would have
    ...
    if not found:
        return False, (
            "zip does not appear to be a her backup "
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.her/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a her dir name
        if prefix in {".her", "her"}:
            return prefix + "/"
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_her_home()}")

        if prefix:
            print(f"Detected archive prefix: {prefix!r} (will be stripped)")

        # Check for existing installation
        has_config = (her_root / "config.yaml").exists()
        has_env = (her_root / ".env").exists()

        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has her configuration.")
        ...
        for member in members:
            ...
            try:
                target.resolve().relative_to(her_root.resolve())
            except ValueError:
        ...
        profiles_dir = her_root / "profiles"
        restored_profiles = []
        if profiles_dir.is_dir():
            try:
                ...
                if any(profiles_dir.iterdir()):
                    ...
                    print(f"  Run: her profile list  (after installing her)")

        # Guidance
        print()
        if not (her_root / "her-agent").is_dir():
            print("Note: The her-agent codebase was not included in the backup.")
            print("  If this is a fresh install, run: her update")

        if restored_profiles:
            gw_profiles = [n for n, _ in restored_profiles]
            print("\nTo re-enable gateway services for profiles:")
            for pname in gw_profiles:
                print(f"  her -p {pname} gateway install")

        print("Done. Your her configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and her backup --quick)
# ---------------------------------------------------------------------------

# Critical state files to include in quick snapshots (relative to HER_HOME).
...
# — `her update` snapshots this set before pulling so approved-user lists
...
def _quick_snapshot_root(her_home: Optional[Path] = None) -> Path:
    home = her_home or get_her_home()
    return home / _QUICK_SNAPSHOTS_DIR


def create_quick_snapshot(
    label: Optional[str] = None,
    her_home: Optional[Path] = None,
...
def list_quick_snapshots(
    ...
    root = _quick_snapshot_root(her_home)
    if not root.exists():
        return []
...
# Relative path of the cron job database inside HER_HOME. Kept in sync with
...
def restore_cron_jobs_if_emptied(
    ...
    """Safety net for silent cron-job loss across ``her update``.
    ...
    Args:
        ...
        her_home: Override for the her home directory (tests).
...
def prune_quick_snapshots(
    ...
    return _prune_quick_snapshots(_quick_snapshot_root(her_home), keep=keep)


def run_quick_backup(args) -> None:
    """CLI entry point for her backup --quick."""
    label = getattr(args, "label", None)
    snap_id = create_quick_snapshot(label=label)
    if snap_id:
        print(f"State snapshot created: {snap_id}")
        snaps = list_quick_snapshots()
        print(f"  {len(snaps)} snapshot(s) stored in {display_her_home()}/state-snapshots/")
...
def _write_full_zip_backup(out_path: Path, her_root: Path) -> Optional[Path]:
    """Write a full zip snapshot of ``her_root`` to ``out_path``.
...
def _pre_update_backup_dir(her_home: Optional[Path] = None) -> Path:
    home = her_home or get_her_home()
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of HER_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<HER_HOME>/backups/pre-update-<timestamp>.zip`` and
    auto-prunes old pre-update backups.

    Returns the path to the created zip, or ``None`` if no files were
    found or the backup could not be created.  Never raises — the caller
    (``her update``) should continue even if the backup fails.
    """
    her_root = her_home or get_default_her_root()
    if not her_root.is_dir():
        return None

    backup_dir = _pre_update_backup_dir(her_root)
    try:
    ...
    result = _write_full_zip_backup(out_path, her_root)
    if result is None:
        return None
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `her claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of HER_HOME under ``backups/`` before a
    ``her claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``her import <archive>``.  Writes to
    ``<HER_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
    ...
    # Reuses the shared backups/ directory so `her import` and the
