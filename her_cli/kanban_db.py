...
``<root>`` is the **shared her root** (the parent of any active
...
``her -p <profile>`` joins the same board as the dispatcher that
...
the profile's her home but are otherwise isolated: a worker spawned
...
* ``board=`` argument passed directly to :func:`connect` / :func:`init_db`
  ...
  ``?board=...`` query param).
* ``HER_KANBAN_BOARD`` env var (used by the dispatcher to pin workers
  to the board their task lives on — workers cannot see other boards).
* ``HER_KANBAN_DB`` env var (pins the DB file path directly — legacy
...
* ``<root>/kanban/current`` — a one-line text file holding the slug of
  the "currently selected" board. Written by ``her kanban boards
  switch <slug>``. When absent, the active board is ``default``.

In standard installs ``<root>`` is ``~/.her``. In Docker / custom
deployments where ``HER_HOME`` points outside ``~/.her`` (e.g.
``/opt/her``), ``<root>`` is ``HER_HOME``. Legacy env-var
overrides still work:

* ``HER_KANBAN_DB`` — pin the database file path directly.
* ``HER_KANBAN_WORKSPACES_ROOT`` — pin the workspaces root directly.
* ``HER_KANBAN_HOME`` — pin the umbrella root that anchors kanban
  paths. Useful for tests and unusual deployments.

The dispatcher injects ``HER_KANBAN_DB``,
``HER_KANBAN_WORKSPACES_ROOT``, and ``HER_KANBAN_BOARD`` into
...
coding workloads.  See ``docs/her-kanban-v1-spec.pdf`` for the full
...
# ``HER_KANBAN_CLAIM_TTL_SECONDS`` to raise the default claim window for
...
def _resolve_claim_ttl_seconds(ttl_seconds: Optional[int] = None) -> int:
    ...
    ``HER_KANBAN_CLAIM_TTL_SECONDS`` overrides the built-in default.
    ...
    raw = os.environ.get("HER_KANBAN_CLAIM_TTL_SECONDS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _resolve_crash_grace_seconds() -> int:
    ...
    Reads ``HER_KANBAN_CRASH_GRACE_SECONDS`` from the environment;
    ...
    """
    raw = os.environ.get("HER_KANBAN_CRASH_GRACE_SECONDS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _resolve_rate_limit_cooldown_seconds() -> int:
    ...
    Reads ``HER_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS`` from the environment;
    ...
    """
    raw = os.environ.get(
        "HER_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS", ""
    ).strip()
    if raw:
        try:
            parsed = int(raw)
...
_CURRENT_BOARD_OVERRIDE: ContextVar[str | None] = ContextVar(
    "her_kanban_current_board_override",
    default=None,
)
...
# enough that kebab-case names like ``atm10-server`` or ``her-agent``
...
def kanban_home() -> Path:
    """Return the shared her root that anchors the kanban board.

    Resolution order:

    1. ``HER_KANBAN_HOME`` env var when set and non-empty (explicit
       override for tests and unusual deployments).
    2. ``get_default_her_root()``, which already returns ``<root>``
       when ``HER_HOME`` is ``<root>/profiles/<name>``, and returns
       ``HER_HOME`` directly for Docker / custom deployments.

    The kanban board is shared across profiles **by design** (see the
    module docstring). Resolving the kanban paths through the active
    profile's ``HER_HOME`` would silently fork the board per profile,
    which breaks the dispatcher / worker handoff.
    """
    override = os.environ.get("HER_KANBAN_HOME", "").strip()
    if override:
        return Path(override).expanduser()
    from her_constants import get_default_her_root
    return get_default_her_root()
...
def current_board_path() -> Path:
    ...
    One-line text file written by ``her kanban boards switch <slug>``
...
def get_current_board() -> str:
    ...
    1. ``HER_KANBAN_BOARD`` env var (set by the dispatcher on worker
       spawn, or manually for ad-hoc overrides).
    2. ``<root>/kanban/current`` on disk (set by ``her kanban boards
    ...
    env = os.environ.get("HER_KANBAN_BOARD", "").strip()
    if env:
        try:
...
def set_current_board(slug: str) -> Path:
    ...
    so that ``her kanban boards switch <typo>`` returns an error
...
def kanban_db_path(board: Optional[str] = None) -> Path:
    ...
    1. ``HER_KANBAN_DB`` env var — pins the path directly. Honoured for
    ...
    """
    override = os.environ.get("HER_KANBAN_DB", "").strip()
    if override:
        return Path(override).expanduser()
...
def workspaces_root(board: Optional[str] = None) -> Path:
    ...
    ``HER_KANBAN_WORKSPACES_ROOT`` pins the path directly (highest
    ...
    """
    override = os.environ.get("HER_KANBAN_WORKSPACES_ROOT", "").strip()
    if override:
        return Path(override).expanduser()
    slug = _normalize_board_slug(board)
    if slug is None:
...
def attachments_root(board: Optional[str] = None) -> Path:
    ...
    ``HER_KANBAN_ATTACHMENTS_ROOT`` pins the path directly (highest
    ...
    see the kanban docs.
    """
    override = os.environ.get("HER_KANBAN_ATTACHMENTS_ROOT", "").strip()
    if override:
        return Path(override).expanduser()
    slug = _normalize_board_slug(board)
    if slug is None:
...
def worker_logs_dir(board: Optional[str] = None) -> Path:
    ...
    board — makes ``her kanban log`` unambiguous even when multiple
...
class Task:
    ...
    # within an agent loop that propagated ``HER_SESSION_ID``. NULL for
...
CREATE TABLE IF NOT EXISTS tasks (
    ...
    -- Originating chat/agent session id when the task was created from
    -- inside an agent loop that propagated ``HER_SESSION_ID``. NULL
...
def _resolve_busy_timeout_ms() -> int:
    ...
    """
    raw = os.environ.get("HER_KANBAN_BUSY_TIMEOUT_MS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _guard_existing_db_is_healthy(path: Path) -> None:
    ...
    all sources her treats as user-controlled-but-trusted on the
...
def connect(
    ...
    * Neither → :func:`kanban_db_path` resolves via
      ``HER_KANBAN_DB`` env → ``HER_KANBAN_BOARD`` env →
    ...
    with _cross_process_init_lock(path):
        ...
        try:
            ...
            with _INIT_LOCK:
                ...
                # See her_state._WAL_INCOMPAT_MARKERS for detection logic.
                from her_state import apply_wal_with_fallback
...
def init_db(
    ...
    *,
    ...
    Kept as a public entry point so CLI ``her kanban init`` and the
...
def _migrate_add_optional_columns(conn: sqlite3.Connection) -> None:
    ...
    if "session_id" not in cols:
        ...
        # ``HER_SESSION_ID`` (e.g. ACP). NULL on legacy rows and on any
...
def _canonical_assignee(assignee: Optional[str]) -> Optional[str]:
    ...
    if assignee is None:
        return None
    from her_cli.profiles import normalize_profile_name
...
def create_task(
    ...
    *,
    ...
    each name to ``her --skills ...`` alongside the built-in
    ...
    # `her --skills X,Y` comma syntax is handled in the dispatcher,
    # not here.
...
# Canonical sort-order mappings for ``her kanban list --sort``.
...
def unlink_tasks(conn: sqlite3.Connection, parent_id: str, child_id: str) -> bool:
    ...
    if removed:
        ...
        # next dispatcher tick or a manual `her kanban recompute` (issue #22459).
...
def _end_run(
    ...
    *,
    ...
    existed (e.g. a CLI user calling ``her kanban complete`` on a
...
def _synthesize_ended_run(
    ...
    *,
    ...
    claimed (CLI user calling ``her kanban complete <ready-task>
...
def _has_sticky_block(conn: sqlite3.Connection, task_id: str) -> bool:
    ...
    * **Worker- or operator-initiated** — a worker called
      ...
      ``her kanban block <id>``).  This is a deliberate handoff that
...
def complete_task(
    ...
    *,
    ...
    completion (``her kanban complete <id>``) works without requiring
...
def _is_managed_scratch_path(p: Path) -> bool:
    ...
    * ``HER_KANBAN_WORKSPACES_ROOT`` when set (worker-side override
    ...
    * ``<kanban_home>/kanban/boards/<slug>/workspaces`` for each board slug
    ...
    subtrees hold her' own DB, metadata, and logs, not task workspaces.

    Used by :func:`_cleanup_workspace` to refuse to ``shutil.rmtree`` paths
    outside her-managed storage. A board ``default_workdir`` pointing at a
...
#      via ``her kanban show <id>`` and the dashboard.
...
# without thrashing. Overridable via ``HER_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS``
...
class DispatchResult:
    ...
    lane (a Claude Code terminal like ``orion-cc``) rather than a her
...
def _set_worker_pid(conn: sqlite3.Connection, task_id: str, pid: int) -> None:
    ...
    The event's payload carries the pid so a human reading ``her kanban
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    ...
    whose assignee maps to a real her profile.
    ...
    if not rows:
        return False
    try:
        from her_cli.profiles import profile_exists  # local import: avoids cycle
    except Exception:
...
def dispatch_once(
    ...
    *,
    ...
    if _default_assignee:
        try:
            from her_cli.profiles import profile_exists as _pe
    ...
    for row in ready_rows:
        ...
        if not row_assignee:
            ...
            if _default_assignee and _default_assignee_resolved:
                ...
                result.skipped_unassigned.append(row["id"])
                continue
        # Skip ready tasks whose assignee is not a real her profile.
        # `_default_spawn` invokes ``her -p <assignee>`` which fails
        # with "Profile 'X' does not exist" when the assignee names a
        # control-plane lane (e.g. an interactive Claude Code terminal
        # like ``orion-cc`` / ``orion-research``) rather than a her
        ...
        if guard_reason is not None:
            ...
            # skipped when reading `her kanban tail` — without
...
def _module_her_argv() -> list[str]:
    """Return the interpreter-bound her CLI invocation."""
    # ``her_cli.main`` is the console-script target declared in
    # pyproject.toml, NOT a top-level ``her`` package — there is no
    # ``her`` package to import.
    return [sys.executable, "-m", "her_cli.main"]


def _absolute_her_path(path: str) -> str:
    """Return an absolute filesystem path for a resolved her shim."""
...
def _her_path_argv(path: str) -> list[str]:
    """Return argv for a resolved her executable path.
...
def _resolve_her_argv() -> list[str]:
    """Resolve the ``her`` invocation as argv parts for ``Popen``.

    Tries in order:

    1. ``$HER_BIN`` — explicit operator override. Path-like values are
    ...
    2. ``shutil.which("her")`` — the console-script shim, normalized to
       an absolute path. On Windows, ``which`` can return a relative
       ``.\\her.CMD`` when the current directory is on ``PATH``; directly
    ...
    3. ``sys.executable -m her_cli.main`` — fallback for setups where
       her is launched from a venv and the ``her`` shim is not on
       the dispatcher's ``$PATH`` (cron, systemd ``User=`` services,
       launchd jobs, detached processes, etc.). Goes through the running
       interpreter so the result is independent of ``$PATH``.

    Mirrors ``gateway.run._resolve_her_bin`` for the same reason. Kept
    local (not imported from gateway) because ``her_cli`` sits below
    ``gateway`` in the dependency order.
    """
    import shutil

    env_bin = os.environ.get("HER_BIN", "").strip()
    if env_bin:
        if _looks_like_path(env_bin):
            return _her_path_argv(env_bin)
...
def _kanban_worker_skill_available(her_home: Optional[str]) -> bool:
    """True if the bundled ``kanban-worker`` skill resolves for the home the
    spawned worker will run under.

    The dispatcher injects ``--skills kanban-worker`` into every worker. When
    the worker activates a profile (``her -p <name>``), its ``SKILLS_DIR``
    ...
    # An unset HER_HOME means the worker falls back to the default root
    # home (``~/.her``), which ships the bundled skill.
    base = _Path(her_home) if her_home else (_Path.home() / ".her")
...
def _default_spawn(
    ...
    *,
    ...
    """Fire-and-forget ``her -p <profile> chat -q ...`` subprocess.
    ...
    ``HER_KANBAN_DB`` / ``HER_KANBAN_BOARD`` / workspaces_root env
    ...
    # Inject HER_HOME so the worker reads the profile-scoped config.yaml
    # (fallback_providers, toolsets, agent settings, etc.) instead of the root
    # config.  Without this, `env = dict(os.environ)` copies only the parent's
    # env, and when the child process starts `her -p <name>` the
    # _apply_profile_override() runs *before* her_constants is imported.
    # If HER_HOME is absent from the child's env, get_her_home() falls
    # back to Path.home() / ".her" (the DEFAULT profile root), ignoring the
    # profile-specific config entirely.  Fixes profile-scoped fallback_providers
    # being invisible to kanban workers.
    from her_cli.profiles import resolve_profile_env
    try:
        env["HER_HOME"] = resolve_profile_env(profile_arg)
    except FileNotFoundError:
        # Profile dir doesn't exist — defer resolution to the CLI's
        # _apply_profile_override() via HER_PROFILE (set below).
        # This only happens in test fixtures where the isolated
        # HER_HOME never had profiles created.
        pass
    if task.tenant:
        env["HER_TENANT"] = task.tenant
    env["HER_KANBAN_TASK"] = task.id
    env["HER_KANBAN_WORKSPACE"] = workspace
    if task.branch_name:
        env["HER_KANBAN_BRANCH"] = task.branch_name
    if task.current_run_id is not None:
        env["HER_KANBAN_RUN_ID"] = str(task.current_run_id)
    if task.claim_lock:
        env["HER_KANBAN_CLAIM_LOCK"] = task.claim_lock
    # Goal-loop mode: the worker reads these and wraps its run in the
    # Ralph-style /goal judge loop (see cli.py quiet-mode path). Only set
    # when enabled so non-goal tasks keep a clean env.
    if task.goal_mode:
        env["HER_KANBAN_GOAL_MODE"] = "1"
        if task.goal_max_turns is not None:
            env["HER_KANBAN_GOAL_MAX_TURNS"] = str(int(task.goal_max_turns))
    ...
    # that even when the worker activates a profile (`her -p <name>`
    # rewrites HER_HOME), its kanban paths still match the
    # dispatcher's. Belt-and-braces with the `get_default_her_root()`
    # resolution in `kanban_home()` — symmetric resolution is the norm,
    # but unusual symlink / Docker layouts are caught here too.
    env["HER_KANBAN_DB"] = str(kanban_db_path(board=board))
    env["HER_KANBAN_WORKSPACES_ROOT"] = str(workspaces_root(board=board))
    # Board slug — the final defense-in-depth pin. If the worker ever
    # resolves kanban paths without the DB / workspaces env vars, the
    # board slug still forces it to the right directory.
    resolved_board = _normalize_board_slug(board) or get_current_board()
    env["HER_KANBAN_BOARD"] = resolved_board
    # HER_PROFILE is the author the kanban_comment tool defaults to.
    # `her -p <assignee>` activates the profile, but the env var is
    # what the tool reads — set it explicitly here so comments are
    # attributed correctly regardless of how the child loads config.
    env["HER_PROFILE"] = profile_arg

    cmd = [
        *_resolve_her_argv(),
        "-p", profile_arg,
        # Worker subprocesses switch to a profile-scoped HER_HOME above,
    ...
    if _kanban_worker_skill_available(env.get("HER_HOME")):
    ...
    # `her kanban log` on a specific board reads its own file and
    ...
    try:
        ...
        log_f.close()
        raise RuntimeError(
            "`her` executable not found on PATH. "
            "Install her Agent or activate its venv before running the kanban dispatcher."
        )
...
def run_daemon(
    *,
    ...
    on SIGINT / SIGTERM so ``her kanban daemon`` is systemd-friendly.
...
def build_worker_context(conn: sqlite3.Connection, task_id: str) -> str:
    ...
    if shown_c:
        ...
        for c in shown_c:
            ...
            # operator-controlled HER_PROFILE values like "her-system"
...
def list_profiles_on_disk() -> list[str]:
    ...
    - the implicit ``default`` profile when the default her root exists

    Reads profile paths directly so this module has no import dependency on
    ``her_cli.profiles`` (which pulls in a large chunk of the CLI startup
    path).
    """
    try:
        from her_constants import get_default_her_root
        default_root = get_default_her_root()
...
def known_assignees(conn: sqlite3.Connection) -> list[dict]:
    ...
    - ``her kanban assignees`` for the terminal.
