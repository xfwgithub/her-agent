...
Pure display functions with no HerCLI state dependency.
"""
...
# (e.g. nix-built her — no local git history to count against).
UPDATE_AVAILABLE_NO_COUNT = -1

_UPSTREAM_REPO_URL = "https://github.com/NousResearch/her-agent.git"
...
def _fetch_pypi_latest(package: str = "her-agent") -> Optional[str]:
...
def check_for_updates() -> Optional[int]:
    """Check whether a her update is available.

    Two paths: if ``HER_REVISION`` is set (nix builds embed it), compare
    ...
    cache_file = her_home / ".update_check"
    embedded_rev = os.environ.get("HER_REVISION") or None

    # Docker images have no working tree to count commits against — the
    # published image excludes `.git` (see .dockerignore) and sets no
    # HER_REVISION (that's nix-only). Without this guard the checks below
    ...
    # and `her update` correctly refuses to run in-place inside the
    # container anyway. The dashboard's REST `/api/her/update/check`
    ...
    if embedded_rev:
        ...
        # $HER_HOME/her-agent/ may be a stale copy from --clone-all;
        # Path(__file__) always resolves to the actual installed checkout.
        repo_dir = Path(__file__).parent.parent.resolve()
        if not (repo_dir / ".git").exists():
            repo_dir = her_home / "her-agent"
        if not (repo_dir / ".git").exists():
...
def _resolve_repo_dir() -> Optional[Path]:
    """Return the active her git checkout, or None if this isn't a git install.

    Prefers the running code's location over the profile-scoped path
    because ``$HER_HOME/her-agent/`` may be a stale copy carried
    over by ``--clone-all``.
    """
    repo_dir = Path(__file__).parent.parent.resolve()
    if not (repo_dir / ".git").exists():
...
def get_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    ``her_cli/build_info.py``) and return it as a frozen
    ...
    if repo_dir is None:
        ...
        try:
            from her_cli.build_info import get_build_sha
...
_RELEASE_URL_BASE = "https://github.com/NousResearch/her-agent/releases/tag"
...
def get_latest_release_tag(repo_dir: Optional[Path] = None) -> Optional[tuple]:
    ...
    her checkout. Cached per-process. Release URL always points at the
    canonical NousResearch/her-agent repo (forks don't get a link).
    """
...
def format_banner_version_label() -> str:
    ...
    base = f"her Agent v{VERSION} ({RELEASE_DATE})"
...
def build_welcome_banner(console: "Console", model: str, cwd: str,
    ...
    try:
        ...
        _hero = _bskin.banner_hero if hasattr(_bskin, 'banner_hero') and _bskin.banner_hero else HER_CADUCEUS
    except Exception:
        _bskin = None
        _hero = HER_CADUCEUS
    left_lines = ["", _hero, ""]
    ...
    try:
        from her_cli.codex_runtime_switch import get_current_runtime
        from her_cli.config import load_config as _load_cfg
    ...
    try:
        ...
        if behind is not None and behind != 0:
            from her_cli.config import get_managed_update_command, recommended_update_command
            if behind > 0:
                ...
                # UPDATE_AVAILABLE_NO_COUNT: nix-built her; we know an update
    ...
    # Pip-install warning — `pip install her-agent` is not the supported
    ...
    if term_width >= 95:
        _logo = _bskin.banner_logo if _bskin and hasattr(_bskin, 'banner_logo') and _bskin.banner_logo else HER_AGENT_LOGO
        console.print(_logo)
        console.print()
