...
the Python agent or the user's config/data:
  ...
  1. Source-built GUI (``her desktop`` / ``her gui``)
     Built inside the agent checkout under ``$HER_HOME/her-agent/``:
       ...
       - ``$HER_HOME/desktop-build-stamp.json`` (the build freshness stamp)
...
``her uninstall --gui`` (and friends) without paying for the full CLI.
"""
...
def source_built_gui_artifacts(her_home: Path) -> "list[Path]":
    """GUI build artifacts produced by ``her desktop`` inside the checkout.

    These are removable on a GUI uninstall without harming the agent: the
    Python agent runs from ``her-agent/`` source + ``venv/`` and never
    needs the Electron build output or node_modules.
    """
    agent_root = _agent_root(her_home)
    ...
    return [
        ...
        her_home / "desktop-build-stamp.json",
    ]
...
def agent_is_installed(her_home: Path) -> bool:
    """Return True when a usable Python agent install exists under HER_HOME.
...
def gui_is_installed(her_home: Path) -> bool:
    """Return True when any desktop GUI artifact exists (built or packaged)."""
    for p in source_built_gui_artifacts(her_home):
        if p.exists():
            return True
...
def gui_install_summary(her_home: "Path | None" = None) -> dict:
    ...
    """
    home: Path = her_home if her_home is not None else get_her_home()
...
def uninstall_gui(her_home: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    Never touches ``her-agent/her_cli`` (agent source), ``venv/``, or any
    config / sessions / .env under ``$HER_HOME``.
    ...
    if sys.platform.startswith("linux"):
        log_info(
            ...
            "with your package manager (e.g. 'sudo apt remove her' or "
            "'sudo dnf remove her'). AppImage builds are a single file you "
