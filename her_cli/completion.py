"""Shell completion script generation for her CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_her_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# her Agent bash completion
# Add to ~/.bashrc:
#   eval "$(her completion bash)"

_her_profiles() {{
    local profiles_dir="$HOME/.her/profiles"
...
_her_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_her_profiles)" -- "$cur"))
        return
    fi
...
complete -F _her_completion her
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef her
# her Agent zsh completion
# Add to ~/.zshrc:
...
_her_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.her/profiles" ]]; then
        profiles+=($HOME/.her/profiles/*(N/:t))
    fi
...
_her() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_her_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# her Agent fish completion",
        "# Add to your config:",
        "#   her completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __her_profiles",
        "    echo default",
        "    if test -d $HOME/.her/profiles",
        "        for d in $HOME/.her/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c her -f -s p -l profile"
        " -d 'Profile name' -xa '(__her_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__her_profiles)' -d 'Profile name'"
                )
