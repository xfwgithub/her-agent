"""
MCP Server Management CLI — ``her mcp`` subcommand.

Implements ``her mcp add/remove/list/test/configure`` for interactive
MCP server lifecycle management (issue #690 Phase 2).

Relies on tools/mcp_tool.py for connection/discovery and keeps
configuration in ~/.her/config.yaml under the ``mcp_servers`` key.
"""
...
from her_cli.config import (
    ...
    get_env_value,
    save_env_value,
    get_her_home,  # noqa: F401 — used by test mocks
)
...
def _resolve_mcp_server_config(config: dict) -> dict:
    ...
    ``~/.her/.env`` into ``os.environ`` and recursively interpolate any
...
def _oauth_tokens_present(name: str) -> bool:
    ...
    Used after ``her mcp login`` to distinguish a genuine authentication
    ...
    try:
        ...
        return HerTokenStorage(name).has_cached_tokens()
...
def cmd_mcp_add(args):
    ...
    # her_cli/main.py for why the dest is renamed.
    ...
    if not url and not command:
        ...
        _info("Examples:")
        _info('  her mcp add ink --url "https://mcp.ml.ink/mcp"')
        _info('  her mcp add github --command npx --args @modelcontextprotocol/server-github')
        _info('  her mcp add myserver --preset mypreset')
        return
            ...
            if auth_type == "header" or not auth_type:
                ...
                if existing_key:
                    ...
                    if api_key:
                        ...
                        _success(f"Saved to {display_her_home()}/.env as {env_key}")
    ...
    try:
        ...
        if _confirm("Save config anyway (you can test later)?", default=False):
            ...
            _info("Fix the issue, then: her mcp test " + name)
        return
    ...
    print()
    _success(f"Saved '{name}' to {display_her_home()}/config.yaml ({tool_count}/{total} tools enabled)")
...
def cmd_mcp_remove(args):
    ...
    # earlier `her mcp test` in the same session) is evicted too.
    try:
        from tools.mcp_oauth_manager import get_manager
...
def cmd_mcp_list(args=None):
    ...
    if not servers:
        ...
        print()
        _info("Add one with:")
        _info('  her mcp add <name> --url <endpoint>')
        _info('  her mcp add <name> --command <cmd> --args <args...>')
        print()
        return
...
def cmd_mcp_login(args):
    ...
    if server_config.get("auth") != "oauth":
        ...
        _info("Use `her mcp remove` + `her mcp add` to reconfigure auth.")
        return
    ...
    try:
        ...
        if not _oauth_tokens_present(name):
            ...
            print()
            _info("Then re-run `her mcp login " + name + "`.")
            return
        if tools:
...
def cmd_mcp_configure(args):
    ...
    if not _sys.stdin.isatty():
        print("Error: 'her mcp configure' requires an interactive terminal.", file=_sys.stderr)
        _sys.exit(1)
    name = args.name
    servers = _get_mcp_servers()
...
def mcp_command(args):
    """Main dispatcher for ``her mcp`` subcommands."""
    ...
    if action == "picker":
        from her_cli.mcp_picker import run_picker
        run_picker()
        return
    if action == "catalog":
        from her_cli.mcp_picker import show_catalog
        show_catalog()
        return
    if action == "install":
        from her_cli.mcp_picker import install_by_name
        import sys as _sys
    ...
    if handler:
        ...
        # "try enabling and it flows you into setup" UX matching `her plugin`.
        from her_cli.mcp_picker import run_picker
        run_picker()
        print(color("  Commands:", Colors.CYAN))
        _info("her mcp                                    Open the catalog picker (default)")
        _info("her mcp catalog                            List Nous-approved MCPs")
        _info("her mcp install <name>                     Install a catalog MCP")
        _info("her mcp serve                              Run as MCP server")
        _info("her mcp add <name> --url <endpoint>        Add a custom MCP server")
        _info("her mcp add <name> --command <cmd>         Add a stdio server")
        _info("her mcp add <name> --preset <preset>       Add from a known preset")
        _info("her mcp remove <name>                      Remove a server")
        _info("her mcp list                               List configured servers")
        _info("her mcp test <name>                        Test connection")
        _info("her mcp configure <name>                   Toggle tools")
        _info("her mcp login <name>                       Re-authenticate OAuth")
        print()
