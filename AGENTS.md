...
`$HOME/.her/her-agent/venv` (for worktrees that share a venv with the
main checkout).
...
├── her_cli/           # CLI subcommands, setup wizard, plugins loader, skin engine
...
│   ├── her-achievements/  # Gamified achievement tracking
...
├── ui-tui/               # Ink (React) terminal UI — `her --tui`
...
**User config:** `~/.her/config.yaml` (settings), `~/.her/.env` (API keys only).
**Logs:** `~/.her/logs/` — `agent.log` (INFO+), `errors.log` (WARNING+),
`gateway.log` when running the gateway. Profile-aware via `get_her_home()`.
Browse with `her logs [--follow] [--level ...] [--session ...]`.
...
- **Slack** — `slack_subcommand_map()` generates `/her` subcommand routing
- **Autocomplete** — `COMMANDS` flat dict feeds `SlashCommandCompleter`
- **CLI help** — `COMMANDS_BY_CATEGORY` dict feeds `show_help()`

### Adding a Slash Command

1. Add a `CommandDef` entry to `COMMAND_REGISTRY` in `her_cli/commands.py`:
```python
...
The TUI is a full replacement for the classic (prompt_toolkit) CLI, activated via `her --tui` or `HER_TUI=1`.
...
npm run dev       # watch mode (rebuilds her-ink + tsx --watch)
npm start         # production
npm run build     # full build (her-ink + tsc)
...
### TUI in the Dashboard (`her dashboard` → `/chat`)

The dashboard embeds the real `her --tui` — **not** a rewrite.  See `her_cli/pty_bridge.py` + the `@app.websocket("/api/pty")` endpoint in `her_cli/web_server.py`.
...
A **separate** chat surface from both the classic CLI and the dashboard's embedded TUI. It is an Electron + React + nanostore renderer (`@assistant-ui/react`) that talks to a `tui_gateway` backend over JSON-RPC (`requestGateway(method, params)`). It does NOT embed `her --tui` — it has its own composer, transcript, and slash-command pipeline. Route desktop bugs to the `her-desktop-app-work` skill, not `her-dashboard-work`.
...
**Path references in tool schemas**: If the schema description mentions file paths (e.g. default output directories), use `display_her_home()` to make them profile-aware. The schema is generated at import time, which is after `_apply_profile_override()` sets `HER_HOME`.

**State files**: If a tool stores persistent state (caches, logs, checkpoints), use `get_her_home()` for the base directory — never `Path.home() / ".her"`. This ensures each profile gets its own state.
...
### config.yaml options:
1. Add to `DEFAULT_CONFIG` in `her_cli/config.py`
...
1. Add to `OPTIONAL_ENV_VARS` in `her_cli/config.py` with metadata:
```python
"NEW_API_KEY": {
...
| `load_config()` | `her tools`, `her setup`, most CLI subcommands | `her_cli/config.py` — merges `DEFAULT_CONFIG` + user YAML |
...
The skin engine (`her_cli/skin_engine.py`) provides data-driven CLI visual customization. Skins are **pure data** — no code changes needed to add a new skin.

### Architecture

```
her_cli/skin_engine.py    # SkinConfig dataclass, built-in skins, YAML loader
~/.her/skins/*.yaml       # User-installed custom skins (drop-in)
```
...
Add to `_BUILTIN_SKINS` dict in `her_cli/skin_engine.py`:
...
Users create `~/.her/skins/<name>.yaml`:
...
### General plugins (`her_cli/plugins.py` + `plugins/<name>/`)

`PluginManager` discovers plugins from `~/.her/plugins/`, `./.her/plugins/`,
...
- Register CLI subcommands via `ctx.register_cli_command(...)` — the
  plugin's argparse tree is wired into `her` at startup so
  `her <pluginname> <subcmd>` works with no change to `main.py`
...
`post_setup(her_home, config)` for setup-wizard integration.

**CLI commands via `plugins/memory/<name>/cli.py`:** if a memory plugin
defines `register_cli(subparser)`, `discover_plugin_cli_commands()` finds
it at argparse setup time and wires it into `her <plugin>`. The
framework only exposes CLI commands for the **currently active** memory
provider (read from `memory.provider` in config.yaml), so disabled
providers don't clutter `her --help`.

**Rule (Teknium, May 2026):** plugins MUST NOT modify core files
(`run_agent.py`, `cli.py`, `gateway/run.py`, `her_cli/main.py`, etc.).
...
into `~/.her/plugins/` (or via pip entry points) — they implement
the same `MemoryProvider` ABC, register through the same discovery
path, and integrate via `her memory setup` / `post_setup()` without
...
2. User: `$HER_HOME/plugins/model-providers/<name>/`
...
[`her-example-plugins`](https://github.com/NousResearch/her-example-plugins)
...
- **`optional-skills/`** — heavier or niche skills shipped with the repo but
  ...
  `her skills install official/<category>/<skill>`. Adapter lives in
...
`metadata.her.tags`, `metadata.her.category`,
`metadata.her.related_skills`, `metadata.her.config` (config.yaml
settings the skill needs — stored under `skills.config.<key>`, prompted
during setup, injected at load time).

Top-level `tags:` and `category:` are also accepted and mirrored from
`metadata.her.*` by the loader.
...
lives in the `her-agent-dev` skill at
...
Enable/disable per platform via `her tools` (the curses UI) or the
...
go to `~/.her/skills/.archive/` and are restorable.

- **Core:** `agent/curator.py` (review loop, auto-transitions, LLM review
  prompt) + `agent/curator_backup.py` (pre-run tar.gz snapshots).
- **CLI:** `her_cli/curator.py` wires `her curator <verb>` where
  verbs are: `status`, `run`, `pause`, `resume`, `pin`, `unpin`,
  `archive`, `restore`, `prune`, `backup`, `rollback`.
- **Telemetry:** `tools/skill_usage.py` owns the sidecar
  `~/.her/skills/.usage.json` — per-skill `use_count`, `view_count`,
...
schedule jobs via the `cronjob` tool; users via `her cron <verb>`
...
- File lock at `~/.her/cron/.tick.lock` prevents duplicate ticks
  across processes.
...
collaborate on shared tasks. Users drive it via `her kanban <verb>`;
workers spawned by the dispatcher drive it via a dedicated `kanban_*`
toolset so their schema footprint is zero when they're not inside a
kanban task.

- **CLI:** `her_cli/kanban.py` wires `her kanban` with verbs
...
- **Plugin assets:** `plugins/kanban/dashboard/` (web UI) +
  `plugins/kanban/systemd/` (`her-kanban-dispatcher.service` for
  standalone dispatcher deployment).

Isolation model:
- **Board** is the hard boundary — workers are spawned with
  `HER_KANBAN_BOARD` pinned in their env so they can't see other
  boards.
...
in config.yaml (or `HER_BACKGROUND_NOTIFICATIONS` env var):
...
`HER_HOME` before any module imports. All `get_her_home()` references
automatically scope to the active profile.

### Rules for profile-safe code

1. **Use `get_her_home()` for all HER_HOME paths.** Import from `her_constants`.
   NEVER hardcode `~/.her` or `Path.home() / ".her"` in code that reads/writes state.
   ```python
   # GOOD
   from her_constants import get_her_home
   config_path = get_her_home() / "config.yaml"

   # BAD — breaks profiles
   config_path = Path.home() / ".her" / "config.yaml"
   ```

2. **Use `display_her_home()` for user-facing messages.** Import from `her_constants`.
   This returns `~/.her` for default or `~/.her/profiles/<name>` for profiles.
   ```python
   # GOOD
   from her_constants import display_her_home
   print(f"Config saved to {display_her_home()}/config.yaml")

   # BAD — shows wrong path for profiles
   print("Config saved to ~/.her/config.yaml")
   ```

3. **Module-level constants are fine** — they cache `get_her_home()` at import time,
   which is AFTER `_apply_profile_override()` sets the env var. Just use `get_her_home()`,
   not `Path.home() / ".her"`.

4. **Tests that mock `Path.home()` must also set `HER_HOME`** — since code now uses
   `get_her_home()` (reads env var), not `Path.home() / ".her"`:
   ```python
   with patch.object(Path, "home", return_value=tmp_path), \
        patch.dict(os.environ, {"HER_HOME": str(tmp_path / ".her")}):
       ...
   ```
...
6. **Profile operations are HOME-anchored, not HER_HOME-anchored** — `_get_profiles_root()`
   returns `Path.home() / ".her" / "profiles"`, NOT `get_her_home() / "profiles"`.
   This is intentional — it lets `her -p coder profile list` see all profiles regardless
   of which one is active.

## Known Pitfalls

### DO NOT hardcode `~/.her` paths
Use `get_her_home()` from `her_constants` for code paths. Use `display_her_home()`
for user-facing print/log messages. Hardcoding `~/.her` breaks profiles — each profile
has its own `HER_HOME` directory. This was the source of 5 bugs fixed in PR #3575.

### DO NOT introduce new `simple_term_menu` usage
Existing call sites in `her_cli/main.py` remain for legacy fallback only;
the preferred UI is curses (stdlib) because `simple_term_menu` has
ghost-duplication rendering bugs in tmux/iTerm2 with arrow keys. New
interactive menus must use `her_cli/curses_ui.py` — see
`her_cli/tools_config.py` for the canonical pattern.
...
with actual imports (not mocks) against a temp `HER_HOME`.

### Tests must not write to `~/.her/`
The `_isolate_her_home` autouse fixture in `tests/conftest.py` redirects `HER_HOME` to a temp dir. Never hardcode `~/.her/` paths in tests.

**Profile tests**: When testing profile features, also mock `Path.home()` so that
`_get_profiles_root()` and `_get_default_her_home()` resolve within the temp dir.
Use the pattern from `tests/her_cli/test_profiles.py`:
```python
@pytest.fixture
...
- The plugin disables itself in child processes (sentinel envvar
  `HER_ISOLATE_CHILD=1`), so there's no fork-bomb risk.
...
| HOME / `~/.her/` | Your real config+auth.json | Temp dir per test |
