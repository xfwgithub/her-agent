"""
her MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (her-specific extra)

Usage:
    her mcp serve
    her mcp serve --verbose
...
logger = logging.getLogger("her.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using HER_HOME."""
    try:
        from her_constants import get_her_home
        return get_her_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("HER_HOME", Path.home() / ".her")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from her_constants import get_her_home
        directory_file = get_her_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("HER_HOME", Path.home() / ".her")
...
class EventBridge:
    ...
    This is the her equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        try:
            from her_constants import get_her_home
            db_file = get_her_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("HER_HOME", Path.home() / ".her")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the her MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "her",
        instructions=(
            "her Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the her MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
