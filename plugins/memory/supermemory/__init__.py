...
def _load_supermemory_config(her_home: str) -> dict:
    config = _default_config()
    config_path = Path(her_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, her_home: str) -> None:
    config_path = Path(her_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class _SupermemoryClient:
    def __init__(self, api_key: str, timeout: float, container_tag: str, search_mode: str = "hybrid"):
        ...
        self._client = Supermemory(
            ...
            timeout=timeout,
            max_retries=0,
            default_headers={"x-sm-source": "her"},
        )
    ...
    def _merge_metadata(self, metadata: Optional[dict]) -> dict:
        ...
        merged = {"sm_source": "her", **(metadata or {})}
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `her memory setup`.
        # All other options are documented for $HER_HOME/supermemory.json
    ...
    def save_config(self, values, her_home):
        ...
        _save_supermemory_config(sanitized, her_home)

    def initialize(self, session_id: str, **kwargs) -> None:
        from her_constants import get_her_home
        self._her_home = kwargs.get("her_home") or str(get_her_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._her_home)
