...
Config via environment variables:
  ...
  HINDSIGHT_BANK_ID                — memory bank identifier (default: her)
...
Or via $HER_HOME/hindsight/config.json (profile-scoped), falling back to
...
def _load_config() -> dict:
    ...
    Resolution order:
      1. $HER_HOME/hindsight/config.json  (profile-scoped)
    ...
    profile_path = get_her_home() / "hindsight" / "config.json"
    ...
    return {
        ...
        "banks": {
            "her": {
                "bankId": os.environ.get("HINDSIGHT_BANK_ID", "her"),
...
def _resolve_bank_id_template(template: str, fallback: str, **placeholders: str) -> str:
    ...
    collapsed — e.g. ``her-{user}`` with no user becomes ``her``.
...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, her_home):
        """Write config to $HER_HOME/hindsight/config.json."""
        import json
        from pathlib import Path
        config_dir = Path(her_home) / "hindsight"
    ...
    def post_setup(self, her_home: str, config: dict) -> None:
        ...
        from her_cli.memory_setup import _curses_select
        ...
        provider_config.setdefault("bank_id", "her")
        ...
        self.save_config(provider_config, her_home)
        ...
        if mode == "local_embedded":
            ...
            config_path = Path(her_home) / "hindsight" / "config.json"
            try:
            ...
            if not llm_api_key:
                llm_api_key = _load_simple_env(Path(her_home) / ".env").get("HINDSIGHT_LLM_API_KEY", "")
            if not llm_api_key:
    ...
    def get_config_schema(self):
        return [
            ...
            {"key": "bank_id", "description": "Memory bank name (static fallback when bank_id_template is unset)", "default": "her"},
            {"key": "bank_id_template", "description": "Optional template to derive bank_id dynamically. Placeholders: {profile}, {workspace}, {platform}, {user}, {session}. Example: her-{profile}", "default": ""},
        ...
        if self._client is None:
            if self._mode == "local_embedded":
                ...
                logger.debug("Creating HindsightEmbedded client (profile=%s, provider=%s)",
                             self._config.get("profile", "her"), llm_provider)
                kwargs = dict(
                    profile=self._config.get("profile", "her"),
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        banks = cfg_get(self._config, "banks", "her", default={})
        static_bank_id = self._config.get("bank_id") or banks.get("bankId", "her")
