...
her meet install                 # pip deps + Chromium (transcribe only)
her meet install --realtime      # + pulseaudio-utils / brew blackhole+ffmpeg
her meet auth                    # optional; skips guest-lobby wait
her meet setup                   # preflight checks
```

`her meet install --realtime` prompts before running `sudo apt-get` (Linux)
...
pip install playwright websockets && python -m playwright install chromium
her plugins enable google_meet
her meet node run --display-name my-mac    # persistent server
# copy the printed token

# on the gateway:
her meet node approve my-mac ws://<mac-ip>:18789 <token>
her meet node ping my-mac                   # confirm reachable
```

Run `her meet setup` to preflight local prereqs.
...
| `realtimeDevice` | Audio device name the bot is feeding (e.g. `her_meet_src`). |
...
Local:
```
$HER_HOME/workspace/meetings/<meeting-id>/transcript.txt
```
...
- Remote nodes use bearer-token auth; tokens are generated on the node (32 hex chars, persisted in `$HER_HOME/workspace/meetings/node_token.json`) and must be copied to the gateway via `her meet node approve`.
