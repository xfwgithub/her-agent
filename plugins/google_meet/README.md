...
Let the her agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where her runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `her meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `her meet` CLI |
...
| `cli.py` | `her meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$HER_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `her meet node {run,list,approve,remove,status,ping}` |
...
her meet install                                      # pip + Chromium
her meet setup                                        # preflight
her meet auth                                         # optional
her meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
her meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.her/.env
her meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
her meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
her meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.her/.env
her meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, her will **not** switch your system audio input automatically — the
...
her plugins enable google_meet
her meet node run --display-name my-mac --host 0.0.0.0 --port 18789
# prints the bearer token on first run; copy it
```

On the gateway:
```bash
her meet node approve my-mac ws://<mac-ip>:18789 <token>
her meet node ping my-mac
