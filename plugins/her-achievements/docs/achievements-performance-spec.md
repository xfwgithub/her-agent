# her Achievements Performance Spec (Post-Hackathon)

Status: Draft (no code changes yet)
Owner: her-achievements plugin
...
### `GET /api/plugins/her-achievements/achievements`
...
### `POST /api/plugins/her-achievements/rescan` (optional)
Manual refresh trigger.
Prefer async trigger + immediate status response.

### `GET /api/plugins/her-achievements/scan-status` (optional new)
Reports scan state for UX/ops.

### Removed
- `GET /api/plugins/her-achievements/overview`
...
- Persist plugin data under:
  - `~/.her/plugins/her-achievements/scan_snapshot.json`

### Phase 2
- Incremental scan checkpoints with per-session fingerprints.
- Persist checkpoint data under:
  - `~/.her/plugins/her-achievements/scan_checkpoint.json`
