/**
 * her Kanban — Dashboard Plugin
 *
 * Board view for the multi-agent collaboration board backed by
 * ~/.her/kanban.db. Calls the plugin's backend at /api/plugins/kanban/
 * and tails task_events over a WebSocket for live updates.
 *
 * Plain IIFE, no build step. Uses window.__HER_PLUGIN_SDK__ for React +
...
(function () {
  ...
  const COLUMN_DOT = {
    triage: "her-kanban-dot-triage",
    todo: "her-kanban-dot-todo",
    ready: "her-kanban-dot-ready",
    running: "her-kanban-dot-running",
    blocked: "her-kanban-dot-blocked",
    done: "her-kanban-dot-done",
    archived: "her-kanban-dot-archived",
  };
  ...
  const MIME_TASK = "text/x-her-task";
  ...
  const DOCS_URL = "https://her-agent.nousresearch.com/docs/user-guide/features/kanban";
  const DOCS_TUTORIAL_URL = "https://her-agent.nousresearch.com/docs/user-guide/features/kanban-tutorial";
  ...
  const LS_BOARD_KEY = "her.kanban.selectedBoard";
  ...
  function renderMarkdown(src) {
    ...
    html = html.replace(/\u0000CODE(\d+)\u0000/g, (_m, i) =>
      `<pre class="her-kanban-md-code"><code>${escapeHtml(blocks[Number(i)])}</code></pre>`,
    );
    return html;
  }
  ...
  function MarkdownBlock(props) {
    ...
    if (!enabled) {
      return h("pre", { className: "her-kanban-pre" }, props.source || "");
    }
    return h("div", {
      className: "her-kanban-md",
  ...
  // standard `drop` event and our `her-kanban:drop` event.
  // -------------------------------------------------------------------------
  ...
  function attachTouchDrag(el, taskId) {
    ...
    function onDown(e) {
      ...
      proxy.classList.add("her-kanban-touch-proxy");
      ...
      function move(ev) {
        ...
        if (target !== lastTarget) {
          if (lastTarget) lastTarget.classList.remove("her-kanban-column--drop");
          if (target) target.classList.add("her-kanban-column--drop");
          lastTarget = target;
        }
      }
      function up() {
        ...
        if (lastTarget) {
          lastTarget.classList.remove("her-kanban-column--drop");
          const status = lastTarget.getAttribute("data-kanban-column");
          const isTrash = lastTarget.hasAttribute("data-kanban-trash");
          if (isTrash) {
            lastTarget.dispatchEvent(new CustomEvent("her-kanban:delete", {
              detail: { taskId },
              bubbles: true,
            }));
          } else if (status) {
            lastTarget.dispatchEvent(new CustomEvent("her-kanban:drop", {
  ...
  function KanbanPage() {
    ...
    useEffect(function () {
      ...
      function openWs() {
        ...
        // loopback. Reading window.__HER_SESSION_TOKEN__ directly (the old
   ...
   const deleteTask = useCallback(function (taskId) {
    ...
    return h(ErrorBoundary, null,
      h("div", { className: "her-kanban flex flex-col gap-4" },
        h(BoardSwitcher, {
          board: board,
          boardList: boardList,
  ...
  function AttentionStrip(props) {
    ...
    return h("div", {
      className: cn(
        "her-kanban-attention",
        "her-kanban-attention--" + topSev,
      ),
    },
      h("div", { className: "her-kanban-attention-bar" },
        h("span", { className: "her-kanban-attention-icon" },
          topSev === "critical" ? "!!!" : topSev === "error" ? "!!" : "⚠"),
        h("span", { className: "her-kanban-attention-text" },
          diagTasks.length === 1
      ...
      expanded
        ? h("div", { className: "her-kanban-attention-list" },
            diagTasks.map(function (task) {
              ...
              return h("div", {
                key: task.id,
                className: cn(
                  "her-kanban-attention-row",
                  "her-kanban-attention-row--" + sev,
                ),
              },
                h("span", { className: "her-kanban-attention-row-sev" },
                  sev === "critical" ? "!!!" : sev === "error" ? "!!" : "⚠"),
                h("span", { className: "her-kanban-attention-row-id" }, task.id),
                h("span", { className: "her-kanban-attention-row-title" },
                  task.title || tx(t, "untitled", "(untitled)")),
                h("span", { className: "her-kanban-attention-row-meta" },
                  task.assignee ? "@" + task.assignee : tx(t, "unassigned", "unassigned"),
                  " \u00b7 ",
                  kinds.length > 0 ? kinds.join(", ") : tx(t, "diagnostic", "diagnostic"),
                ),
                h("button", {
                  className: "her-kanban-attention-row-btn",
  ...
  function DiagnosticActionButton(props) {
    ...
    const cls = cn(
      "her-kanban-diag-action-btn",
      action.suggested ? "her-kanban-diag-action-btn--suggested" : "",
    );
    ...
    return h("span", { className: cls + " her-kanban-diag-action-btn--unknown" },
      label);
  }
  ...
  function DiagnosticCard(props) {
    ...
    const execAction = function (action) {
      ...
      if (action.kind === "comment") {
        ...
        const ta = document.querySelector(".her-kanban-drawer-comment-row input, .her-kanban-drawer-comment-row textarea");
        if (ta) {
    ...
    const sevClass = "her-kanban-diag--" + (diag.severity || "warning");
    return h("div", { className: cn("her-kanban-diag", sevClass) },
      h("div", { className: "her-kanban-diag-header" },
        h("span", { className: "her-kanban-diag-sev" },
          diag.severity === "critical" ? "!!!" :
          diag.severity === "error" ? "!!" : "\u26a0"),
        h("span", { className: "her-kanban-diag-title" },
          diag.title),
      ),
      h("div", { className: "her-kanban-diag-detail" },
        diag.detail),
      diag.data && Object.keys(diag.data).length > 0
        ? h("div", { className: "her-kanban-diag-data" },
            Object.keys(diag.data).map(function (k) {
              const v = diag.data[k];
              if (Array.isArray(v) && v.length > 0 && typeof v[0] === "string" &&
                  v[0].indexOf("t_") === 0) {
                // Task-id list — render as chips.
                return h("div", { key: k, className: "her-kanban-diag-data-row" },
                  h("span", { className: "her-kanban-diag-data-key" }, k + ":"),
                  v.map(function (x) {
                    return h("code", {
                      key: x, className: "her-kanban-event-phantom-chip",
                    }, x);
                  }),
                );
              }
              return h("div", { key: k, className: "her-kanban-diag-data-row" },
                h("span", { className: "her-kanban-diag-data-key" }, k + ":"),
                h("span", { className: "her-kanban-diag-data-val" },
      ...
      reassignAction
        ? h("div", { className: "her-kanban-diag-reassign-row" },
            h("span", { className: "her-kanban-diag-reassign-label" },
              tx(t, "reassignTo", "Reassign to:")),
            h("select", {
              className: "her-kanban-recovery-select",
        ...
        : null,
      h("div", { className: "her-kanban-diag-actions" },
      ...
      msg
        ? h("div", {
            className: cn(
              "her-kanban-diag-msg",
              msg.ok ? "her-kanban-diag-msg--ok" : "her-kanban-diag-msg--err",
            ),
          }, msg.text)
        : null,
    );
  ...
  function DiagnosticsSection(props) {
    ...
    return h("div", { className: "her-kanban-section" },
      h("div", { className: "her-kanban-section-head-row" },
        h("span", { className: "her-kanban-section-head" },
          hasOpenDiags
            ? h("span", { className: "her-kanban-section-head-warning" },
                `\u26a0 ${tx(t, "diagnostics", "Diagnostics")} (${diags.length})`)
            : tx(t, "diagnostics", "Diagnostics"),
        ),
        h("button", {
          className: "her-kanban-section-toggle",
          onClick: function () { setOpen(function (x) { return !x; }); },
          type: "button",
        }, open ? tx(t, "hide", "Hide") : tx(t, "show", "Show")),
      ),
      open
        ? h("div", { className: "her-kanban-diag-list" },
  ...
  function DocsLink() {
    return h("a", {
      ...
      className: "her-kanban-docs-link",
      title: "Open her Kanban docs in a new tab",
      "aria-label": "her Kanban documentation",
    }, "?");
  }
  ...
  function OrchestrationPanel() {
    ...
    // each triage card (or runs `her kanban decompose <id>`) and tasks
    ...
    return h(Card, { className: "p-3" },
      h(CardContent, { className: "p-2 flex flex-col gap-3" },
        ...
        msg ? h("div", {
          className: msg.ok ? "her-kanban-msg-ok" : "her-kanban-msg-err",
        }, msg.text) : null,
  ...
  function BoardSwitcher(props) {
    ...
    if (!shouldShow) {
      return h("div", {
        className: "her-kanban-boardswitcher-compact",
    ...
    return h("div", { className: "her-kanban-boardswitcher" },
      h("div", { className: "her-kanban-boardswitcher-inner" },
  ...
  function NewBoardDialog(props) {
    ...
    return h("div", {
      className: "her-kanban-dialog-backdrop",
      onClick: function (e) { if (e.target === e.currentTarget) props.onCancel(); },
    },
      h("form", {
        className: "her-kanban-dialog",
        onSubmit: onSubmit,
      },
        h("div", { className: "her-kanban-dialog-title" },
        ...
        h("div", { className: "her-kanban-dialog-actions" },
          h(Button, {
            type: "button",
  ...
  function BoardToolbar(props) {
    ...
    return h("div", { className: "flex flex-wrap items-end gap-3" },
      ...
      h("div", { className: "flex flex-col gap-1",
                 title: "Filter by assigned her profile. Profiles are the named agent identities that claim and work on tasks." },
  ...
  function BulkActionBar(props) {
    ...
    return h("div", { className: "her-kanban-bulk" },
      h("span", { className: "her-kanban-bulk-count" },
      ...
      h("div", { className: "her-kanban-bulk-priority",
      ...
      h("div", { className: "her-kanban-bulk-reassign",
                 title: "Reassign selected tasks to a different her profile. Pick a profile (or unassign) and click Apply." },
        h(Select, Object.assign({
          value: assignee,
          className: "h-7 text-xs",
      ...
      h("label", { className: "her-kanban-bulk-reclaim-first", title: "Reclaim any active claims before reassigning" },
        h(Checkbox, {
  ...
  function TrashDropZone(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("her-kanban:delete", onTouchDelete);
      return function () { el.removeEventListener("her-kanban:delete", onTouchDelete); };
    }, [props.onDelete]);
    ...
    return h("div", {
      ...
      className: cn(
        "her-kanban-trash",
        dragOver ? "her-kanban-trash--drop" : "",
        props.draggingTaskId ? "her-kanban-trash--active" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("span", { className: "her-kanban-trash-icon" }, "🗑️"),
      h("span", { className: "her-kanban-trash-label" },
  ...
  function BoardColumns(props) {
    const handleDragStart = useCallback(function (e) {
      const card = e.target.closest && e.target.closest(".her-kanban-card");
      if (!card) return;
    ...
    return h("div", { className: "her-kanban-columns", onDragStart: handleDragStart, onDragEnd: handleDragEnd },
  ...
  function Column(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("her-kanban:drop", onTouchDrop);
      return function () { el.removeEventListener("her-kanban:drop", onTouchDrop); };
    ...
    return h("div", {
      ...
      className: cn(
        "her-kanban-column",
        dragOver ? "her-kanban-column--drop" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("div", { className: "her-kanban-column-header",
                 title: colHelp || "" },
        h(Checkbox, {
          className: "her-kanban-col-check",
        ...
        h("span", { className: cn("her-kanban-dot", COLUMN_DOT[props.column.name]) }),
        h("span", { className: "her-kanban-column-label" },
          colLabel || props.column.name),
        h("span", { className: "her-kanban-column-count",
                    title: `${props.column.tasks.length} task${props.column.tasks.length === 1 ? "" : "s"} in this column` },
          props.column.tasks.length),
        h("button", {
          type: "button",
          className: "her-kanban-column-add",
          title: tx(t, "createTask", "Create task in this column"),
          onClick: function () { setShowCreate(function (v) { return !v; }); },
        }, showCreate ? "×" : "+"),
      ),
      h("div", { className: "her-kanban-column-sub" },
        colHelp || ""),
      ...
      h("div", { className: "her-kanban-column-body" },
        props.column.tasks.length === 0
          ? h("div", { className: "her-kanban-empty" }, tx(t, "noTasks", "— no tasks —"))
          : lanes
            ? lanes.map(function (lane) {
                return h("div", { key: lane.assignee, className: "her-kanban-lane" },
                  h("div", { className: "her-kanban-lane-head" },
                    h("span", { className: "her-kanban-lane-name" }, lane.assignee),
                    h("span", { className: "her-kanban-lane-count" }, lane.tasks.length),
                  ),
  ...
  function stalenessClass(task) {
    ...
    if (age >= tier.red)   return "her-kanban-card--stale-red";
    if (age >= tier.amber) return "her-kanban-card--stale-amber";
    return "";
  }
  ...
  function TaskCard(props) {
    ...
    const handleDragStart = function (e) {
      ...
      const selectedCards = document.querySelectorAll(".her-kanban-card--selected");
      if (selectedCards.length > 1 && props.selected) {
        const ghost = document.createElement("div");
        ghost.className = "her-kanban-drag-ghost";
    ...
    return h("div", {
      ...
      className: cn(
        "her-kanban-card",
        props.selected ? "her-kanban-card--selected" : "",
        props.failed ? "her-kanban-card--failed" : "",
        props.draggingSource ? "her-kanban-card--dragging-source" : "",
        stalenessClass(t),
      ),
      draggable: true,
      tabIndex: 0,
      ...
      h(Card, null,
        h(CardContent, { className: "her-kanban-card-content" },
          h("div", { className: "her-kanban-card-row" },
            h("label", {
              className: "her-kanban-card-check-wrap",
              title: tx(i18n, "selectForBulk", "Select for bulk actions"),
              onClick: function (e) { e.stopPropagation(); },
            },
              h(Checkbox, {
                className: "her-kanban-card-check",
            ...
            h("span", { className: "her-kanban-card-id",
                        title: `Task id: ${t.id}. Use this id with kanban_show, /kanban show, or her kanban show.` }, t.id),
            t.warnings && t.warnings.count > 0
              ? h("span", {
                  className: cn(
                    "her-kanban-warning-badge",
                    "her-kanban-warning-badge--" + (t.warnings.highest_severity || "warning"),
                  ),
                  title: (
            ...
            progress
              ? h("span", {
                  className: cn(
                    "her-kanban-progress",
                    progress.done === progress.total ? "her-kanban-progress--full" : "",
                  ),
            ...
            needsAssignee
              ? h(Badge, {
                  variant: "outline",
                  className: "her-kanban-needs-assignee",
          ...
          h("div", { className: "her-kanban-card-title" },
            t.title || tx(i18n, "untitled", "(untitled)")),
          h("div", { className: "her-kanban-card-row her-kanban-card-meta" },
            t.assignee
              ? h("span", { className: "her-kanban-assignee",
                            title: `Assigned to her profile @${t.assignee}` }, "@", t.assignee)
              : h("span", { className: "her-kanban-unassigned",
                            title: needsAssignee
            ...
            t.comment_count > 0
              ? h("span", { className: "her-kanban-count",
            ...
            t.link_counts && (t.link_counts.parents + t.link_counts.children) > 0
              ...
              : null,
            h("span", { className: "her-kanban-ago",
  ...
  function InlineCreate(props) {
    ...
    return h("div", { className: "her-kanban-inline-create" },
      h("textarea", {
        value: title,
      ...
      h("div", { className: "flex gap-2" },
        h(Input, {
          ...
          title: props.columnName === "triage"
            ? "her profile that will spec this task (default: the dispatcher's configured specifier). Leave blank to let the dispatcher pick."
            : "her profile to assign. Leave blank and the dispatcher will pick from available profiles when the task is Ready.",
  ...
  function TaskDrawer(props) {
    ...
    return h("div", { className: "her-kanban-drawer-shade", onClick: props.onClose },
      h("div", {
        className: "her-kanban-drawer",
        onClick: function (e) { e.stopPropagation(); },
      },
        h("div", { className: "her-kanban-drawer-head" },
          h("span", { className: "text-xs text-muted-foreground" }, props.taskId),
          h("button", {
            type: "button",
            onClick: props.onClose,
            className: "her-kanban-drawer-close",
        ...
        data ? h("div", { className: "her-kanban-drawer-comment-row" },
          h(Input, {
            value: newComment,
  ...
  function AttachmentsSection(props) {
    ...
    return h("div", { className: "her-kanban-section" },
      h("div", { className: "her-kanban-section-head" },
      ...
      atts.length === 0
        ...
        : atts.map(function (a) {
            return h("div", {
              ...
              h("button", {
                type: "button",
                className: "her-kanban-attachment-link truncate",
                title: a.filename,
  ...
  function TaskDetail(props) {
    ...
    return h("div", { className: "her-kanban-drawer-body" },
      h("div", { className: "her-kanban-drawer-title" },
        h("span", { className: cn("her-kanban-dot", COLUMN_DOT[t.status]) }),
        props.editing
          ? h(TitleEditor, {
              initial: t.title || "",
      ...
      t.result ? h("div", { className: "her-kanban-section" },
        h("div", { className: "her-kanban-section-head" }, tx(i18n, "result", "Result")),
      ...
      h("div", { className: "her-kanban-section" },
        h("div", { className: "her-kanban-section-head" },
        ...
        comments.length === 0
          ...
          : comments.map(function (c) {
              return h("div", { key: c.id, className: "her-kanban-comment" },
                h("div", { className: "her-kanban-comment-head" },
                  h("span", { className: "her-kanban-comment-author" }, c.author || "anon"),
                  h("span", { className: "her-kanban-comment-ago" },
      ...
      h("div", { className: "her-kanban-section" },
        ...
        events.slice().reverse().slice(0, 20).map(function (e) {
          ...
          return h("div", {
            ...
            className: cn(
              "her-kanban-event",
              isDiag ? "her-kanban-event--hallucination" : "",
            ),
          },
            isDiag
              ? h("div", { className: "her-kanban-event-header" },
                  h("span", { className: "her-kanban-event-warning-icon" }, "⚠"),
                  h("span", { className: "her-kanban-event-warning-label" },
                    getDiagnosticEventLabel(i18n, e.kind) || e.kind),
                  h("span", { className: "her-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                )
              : h("div", { className: "her-kanban-event-header-plain" },
                  h("span", { className: "her-kanban-event-kind" }, e.kind),
                  h("span", { className: "her-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                ),
            isDiag && phantoms.length > 0
              ? h("div", { className: "her-kanban-event-phantom-row" },
                  h("span", { className: "her-kanban-event-phantom-label" },
                    tx(i18n, "phantomIds", "Phantom ids:")),
                  phantoms.map(function (pid) {
                    return h("code", {
                      key: pid,
                      className: "her-kanban-event-phantom-chip",
                    }, pid);
                  }),
                )
              : null,
            e.payload && !isDiag
              ? h("code", { className: "her-kanban-event-payload" },
    ...
    return h("div", { className: "her-kanban-section" },
      h("div", { className: "her-kanban-section-head-row" },
        ...
        !showAll
          ? h("button", {
              type: "button",
              onClick: function () { setExpanded(true); },
              className: "her-kanban-edit-link",
      ...
      visible.map(function (r) {
        const outcomeClass = r.ended_at
          ? `her-kanban-run--${r.outcome || r.status || "ended"}`
          : "her-kanban-run--active";
        return h("div", { key: r.id, className: cn("her-kanban-run", outcomeClass) },
          h("div", { className: "her-kanban-run-head" },
            h("span", { className: "her-kanban-run-outcome" },
              r.ended_at ? (r.outcome || r.status || tx(t, "ended", "ended")) : tx(t, "active", "active")),
            h("span", { className: "her-kanban-run-profile" },
              r.profile ? `@${r.profile}` : tx(t, "noProfile", "(no profile)")),
            h("span", { className: "her-kanban-run-elapsed" }, fmtElapsed(r)),
            h("span", { className: "her-kanban-run-ago" },
              timeAgo ? timeAgo(r.started_at) : ""),
          ),
          r.summary
            ? h("div", { className: "her-kanban-run-summary" }, r.summary)
            : null,
          r.error
            ? h("div", { className: "her-kanban-run-error" }, r.error)
            : null,
          (r.metadata && Object.keys(r.metadata).length > 0)
            ? (function () {
                ...
                return h("details", {
                    className: "her-kanban-run-meta-block",
                    open: !collapsed,
                  },
                  h("summary", { className: "her-kanban-run-meta-label" }, "Metadata"),
                  h("code", { className: "her-kanban-run-meta" }, json),
                );
              })()
            : null,
        );
  ...
  function WorkerLogSection(props) {
    ...
    if (state.loading) {
      ...
      body = h("pre", { className: "her-kanban-pre her-kanban-log" },
        data.content || "(empty)");
    }
  ...
  function MetaRow(props) {
    return h("div", { className: "her-kanban-meta-row" },
      h("span", { className: "her-kanban-meta-label" }, props.label),
      h("span", { className: "her-kanban-meta-value" }, props.value),
    );
  }
  ...
  function TitleEditor(props) {
    ...
    return h("div", { className: "her-kanban-edit-row" },
      h(Input, {
  ...
  function AssigneeEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "her-kanban-meta-row" },
        h("span", { className: "her-kanban-meta-label" }, tx(t, "assignee", "Assignee")),
        h("span", {
          className: "her-kanban-meta-value her-kanban-editable",
  ...
  function PriorityEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "her-kanban-meta-row" },
        h("span", { className: "her-kanban-meta-label" }, tx(t, "priority", "Priority")),
        h("span", {
          className: "her-kanban-meta-value her-kanban-editable",
          onClick: function () { setEditing(true); },
  ...
  function BodyEditor(props) {
    ...
    return h("div", { className: "her-kanban-section" },
      h("div", { className: "her-kanban-section-head-row" },
        h("span", { className: "her-kanban-section-head" }, tx(t, "description", "Description")),
        editing
  ...
  function DependencyEditor(props) {
    ...
    return h("div", { className: "her-kanban-section" },
      h("div", { className: "her-kanban-section-head" }, tx(t, "dependencies", "Dependencies")),
      h("div", { className: "her-kanban-deps-row" },
        h("span", { className: "her-kanban-deps-label" }, tx(t, "parents", "Parents:")),
        h("div", { className: "her-kanban-deps-chips" },
          (links.parents || []).length === 0
            ? h("span", { className: "her-kanban-deps-empty" }, tx(t, "none", "none"))
            : (links.parents || []).map(function (id) {
                return h("span", { key: id, className: "her-kanban-dep-chip" },
                  id,
                  h("button", {
                    type: "button",
                    className: "her-kanban-dep-chip-x",
      ...
      h("div", { className: "her-kanban-deps-row" },
        h("span", { className: "her-kanban-deps-label" }, tx(t, "children", "Children:")),
        h("div", { className: "her-kanban-deps-chips" },
  ...
  function StatusActions(props) {
    ...
    return h("div", null,
      h("div", { className: "her-kanban-actions" },
        specifyButton,
        decomposeButton,
  ...
  function HomeSubsSection(props) {
    ...
    return h("div", { className: "her-kanban-section" },
      ...
      h("div", { className: "her-kanban-home-subs" },
        channels.map(function (hc) {
          ...
          return h(Button, {
            ...
            className: hc.subscribed
              ? "her-kanban-home-sub her-kanban-home-sub--on"
              : "her-kanban-home-sub",
          }, label);
        })
      )
  ...
  if (window.__HER_PLUGINS__ && typeof window.__HER_PLUGINS__.register === "function") {
    window.__HER_PLUGINS__.register("kanban", KanbanPage);
  }
})();
