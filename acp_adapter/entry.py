"""CLI entry point for the her-agent ACP adapter.

Loads environment variables from ``~/.her/.env``, configures logging
...
def _load_env() -> None:
    """Load .env from HER_HOME (default ``~/.her``)."""
    from her_cli.env_loader import load_her_dotenv

    her_home = get_her_home()
    loaded = load_her_dotenv(her_home=her_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", her_home / ".env"
        )
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="her-acp",
        description="Run her Agent as an ACP stdio server.",
    )
    parser.add_argument("--version", action="store_true", help="Print her version and exit")
    parser.add_argument(
        "--check",
        action="store_true",
...
def _run_check() -> None:
    ...
    from acp_adapter.server import HerACPAgent  # noqa: F401

    print("her ACP check OK")


def _run_setup() -> None:
    from her_cli.main import main as her_main

    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "her", "model"]
        her_main()
    finally:
        sys.argv = old_argv
...
def _run_setup_browser(assume_yes: bool = False) -> int:
    ...
    with ``her postinstall`` and the runtime lazy installer.
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting her-agent ACP adapter")
    ...
    import acp
    from .server import HerACPAgent
