...
#   2. ~/.her/node/ from a prior her-managed install
...
#   if [ "$HER_NODE_AVAILABLE" = true ]; then ...; fi
#
# Env inputs (set before sourcing to override defaults):
#   HER_NODE_MIN_VERSION   (default: 20)   — accepted on PATH
#   HER_NODE_TARGET_MAJOR  (default: 22)   — installed when we install
#   HER_HOME               (default: $HOME/.her)
# ============================================================================

HER_NODE_MIN_VERSION="${HER_NODE_MIN_VERSION:-20}"
HER_NODE_TARGET_MAJOR="${HER_NODE_TARGET_MAJOR:-22}"
HER_HOME="${HER_HOME:-$HOME/.her}"
HER_NODE_AVAILABLE=false
...
_nb_have_modern_node() {
    ...
    [ "$(_nb_node_major)" -ge "$HER_NODE_MIN_VERSION" ]
}
...
_nb_try_fnm() {
    ...
    _nb_log "fnm detected — installing Node $HER_NODE_TARGET_MAJOR..."
    eval "$(fnm env 2>/dev/null)" || true
    fnm install "$HER_NODE_TARGET_MAJOR" >/dev/null 2>&1 || return 1
    fnm use     "$HER_NODE_TARGET_MAJOR" >/dev/null 2>&1 || return 1
...
_nb_try_proto() {
    ...
    _nb_log "proto detected — installing Node $HER_NODE_TARGET_MAJOR..."
    proto install node "$HER_NODE_TARGET_MAJOR" >/dev/null 2>&1 || return 1
    _nb_have_modern_node || return 1
...
_nb_try_nvm() {
    ...
    _nb_log "nvm detected — installing Node $HER_NODE_TARGET_MAJOR..."
    nvm install "$HER_NODE_TARGET_MAJOR" >/dev/null 2>&1 || return 1
    nvm use     "$HER_NODE_TARGET_MAJOR" >/dev/null 2>&1 || return 1
    _nb_have_modern_node || return 1
...
_nb_try_brew() {
    ...
    brew install "node@${HER_NODE_TARGET_MAJOR}" >/dev/null 2>&1 \
        || brew install node >/dev/null 2>&1 \
        || return 1
    brew link --overwrite --force "node@${HER_NODE_TARGET_MAJOR}" >/dev/null 2>&1 || true
    _nb_have_modern_node || return 1
...
_nb_install_bundled_node() {
    ...
    local index_url="https://nodejs.org/dist/latest-v${HER_NODE_TARGET_MAJOR}.x/"
    local tarball
    tarball=$(curl -fsSL "$index_url" \
        | grep -oE "node-v${HER_NODE_TARGET_MAJOR}\.[0-9]+\.[0-9]+-${node_os}-${node_arch}\.tar\.xz" \
        | head -1)
    if [ -z "$tarball" ]; then
        tarball=$(curl -fsSL "$index_url" \
            | grep -oE "node-v${HER_NODE_TARGET_MAJOR}\.[0-9]+\.[0-9]+-${node_os}-${node_arch}\.tar\.gz" \
            | head -1)
    fi
    if [ -z "$tarball" ]; then
        _nb_warn "Could not resolve Node $HER_NODE_TARGET_MAJOR binary for $node_os-$node_arch"
        return 1
    fi
...
ensure_node() {
    ...
    if [ -x "$HER_HOME/node/bin/node" ]; then
        export PATH="$HER_HOME/node/bin:$PATH"
        if _nb_have_modern_node; then
            _nb_ok "Node $(node --version) found (her-managed)"
            HER_NODE_AVAILABLE=true
            return 0
        fi
    fi
    ...
    _nb_try_fnm   && { HER_NODE_AVAILABLE=true; return 0; }
    _nb_try_proto && { HER_NODE_AVAILABLE=true; return 0; }
    _nb_try_nvm   && { HER_NODE_AVAILABLE=true; return 0; }

    # Platform package managers.
    _nb_try_termux_pkg && { HER_NODE_AVAILABLE=true; return 0; }
    _nb_try_brew       && { HER_NODE_AVAILABLE=true; return 0; }

    # Last resort: pinned nodejs.org tarball.
    _nb_install_bundled_node && { HER_NODE_AVAILABLE=true; return 0; }

    _nb_warn "Node.js install failed — TUI and browser tools will be unavailable."
    _nb_warn "Install manually: https://nodejs.org/en/download/  (or: \`brew install node\`, \`fnm install $HER_NODE_TARGET_MAJOR\`, etc.)"
    return 1
}
