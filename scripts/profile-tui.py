...
~30 Hz (matching xterm key-repeat), summarizes ~/.her/perf.log on exit.

The --tui build must exist (run `npm run build` in ui-tui first). This script
launches `node dist/entry.js` directly with HER_TUI_RESUME set so it
bypasses the her_cli wrapper — we want repeatable timing, not the CLI's
session-picker flow.
...
def loop_mode(args: argparse.Namespace) -> int:
    ...
    pkg_root = tui_dir / "packages" / "her-ink" / "src"
