#!/usr/bin/env python3
"""Live test harness for her Agent's Tool Search feature.
...
# Force-isolate the test environment BEFORE any her imports.
ORIGINAL_HOME = os.environ.get("HER_HOME")
ORIGINAL_AUTH = Path.home() / ".her" / "auth.json"
...
def setup_isolated_home(enabled: bool) -> Path:
    """Create a fresh ~/.her/ for one test, copying minimal credentials.

    Also reads OPENROUTER_API_KEY from the user's real ``~/.her/.env`` so
    the agent can authenticate against OpenRouter inside the isolated home.
    """
    home_dir = Path(tempfile.mkdtemp(prefix="her_ts_live_"))
    her_home = home_dir / ".her"
    her_home.mkdir(parents=True)

    if ORIGINAL_AUTH.exists():
        shutil.copy(ORIGINAL_AUTH, her_home / "auth.json")

    # Copy .env so OPENROUTER_API_KEY (or others) are visible to the agent
    # running inside the isolated home.
    real_env_file = Path.home() / ".her" / ".env"
    if real_env_file.exists():
        shutil.copy(real_env_file, her_home / ".env")
        ...
        load_her_dotenv(her_home=str(Path.home() / ".her"))
    ...
    (her_home / "config.yaml").write_text(_yaml_dump(cfg), encoding="utf-8")
    return her_home
...
def reset_module_state():
    """Drop cached modules so the new HER_HOME takes effect."""
    keys = [k for k in sys.modules.keys()
            if k.startswith(("tools.", "model_tools", "toolsets",
                             "her_cli", "agent.", "run_agent"))]
    for k in keys:
        del sys.modules[k]
...
def main():
    ...
    # Restore original HER_HOME
    if ORIGINAL_HOME is not None:
        os.environ["HER_HOME"] = ORIGINAL_HOME
    else:
