...
VERSION_FILE = REPO_ROOT / "her_cli" / "__init__.py"
...
AUTHOR_MAP = {
    ...
    "274096618+her-agent-dhabibi@users.noreply.github.com": "dhabibi",
    ...
    "her@nousresearch.com": "NousResearch",
    ...
    "her@example.com": "shellybotmoyer",  # PR #18915 (bot-committed)
    ...
    "sylw3st3rr@gmail.com": "Sylw3ster",  # salvage of #23252 (HER_KANBAN_BOARD restore)
    ...
    "58116817+TurgutKural@users.noreply.github.com": "TurgutKural",  # salvage of #23356 (HER_HOME inject)
    ...
    "sauravsejal40@gmail.com": "Saurav0989",  # PR #27071 (docs: her-eval community link)
    ...
    "heragent26@gmail.com": "heragent26",  # PR #26438 (kimi model-name reasoning pad)
    "276067471+heragent26@users.noreply.github.com": "heragent26",
    ...
    "vigo@her": "hawknewton",  # PR #26294 (bedrock boto3 lazy_deps)
    ...
    "kdkcfp@gmail.com": "slowtokki0409",  # PR #29025 (ignore local her runtime files)
    ...
    "2663402852@qq.com": "x1am1",  # PR #35098 (chown root-owned top-level HER_HOME state files)
    ...
    "leonard@sellem.me": "leonardsellem",  # PR #37405 (WebSocket origin guard on remote/Tailscale binds)
...
def _update_acp_registry_versions(semver: str) -> None:
    ...
    if ACP_REGISTRY_MANIFEST.exists():
        ...
        if "package" in uvx:
            uvx["package"] = f"her-agent[acp]=={semver}"
...
def parse_coauthors(body: str) -> list:
    ...
    _ignored_emails = {"noreply@anthropic.com", "noreply@github.com",
                       "cursoragent@cursor.com", "her@nousresearch.com"}
...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/NousResearch/her-agent",
    ...
    lines.append(f"# her Agent v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for her Agent. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="her Agent Release Tool")
    ...
    print(f"{'='*60}")
    print(f"  her Agent Release Preview")
    print(f"{'='*60}")
    ...
    if args.publish:
        ...
        tag_result = git_result(
            "tag", "-a", tag_name, "-m",
            f"her Agent v{new_version} ({calver_date})\n\nWeekly release"
        )
        ...
        gh_cmd = [
            ...
            "--title", f"her Agent v{new_version} ({calver_date})",
        ...
        if result and result.returncode == 0:
            ...
            print(
                f"    gh release create {tag_name} --title 'her Agent v{new_version} ({calver_date})' "
