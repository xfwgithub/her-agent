#!/usr/bin/env bash
# Canonical test runner for her-agent. Run this instead of calling
...
#   * Proper venv activation (probes .venv, venv, then ~/.her/...)
#
# Usage:
...
# ── Activate venv ───────────────────────────────────────────────────────────
VENV=""
for candidate in "$REPO_ROOT/.venv" "$REPO_ROOT/venv" "$HOME/.her/her-agent/venv"; do
...
if [ -f "$HOME/.her/pytest_live_guard.py" ]; then
  EXTRA_PYTHONPATH="$HOME/.her"
