import type { RunExternalProcess } from '@her/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`her ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching her: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`her ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
