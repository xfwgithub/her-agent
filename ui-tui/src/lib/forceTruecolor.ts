...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.HER_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep her on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via HER_TUI_TRUECOLOR=1.
