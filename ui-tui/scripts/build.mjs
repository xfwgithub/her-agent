...
await build({
  ...
  // Skip the prebuilt @her/ink bundle — esbuild's __esm helper doesn't
  // await nested async init, which breaks lazy-initialized exports like
  // `render`. Bundling from source sidesteps that.
  alias: { '@her/ink': resolve(root, 'packages/her-ink/src/entry-exports.ts') },
...
// the `node` token, leaving a broken interpreter). The her_cli launcher
