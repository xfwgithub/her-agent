...
There are two ways to add a platform to the her gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.her/plugins/` (or under `plugins/platforms/`
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `her gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `her_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the her core.
...
And add it to the `her-gateway` composite:

```python
"her-gateway": {
    "includes": [..., "her-your-platform"]
}
```
...
## 12. Status Display (`her_cli/status.py`)
...
## 13. Gateway Setup Wizard (`her_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ her_cli/ toolsets.py \
  --include="*.py" -l | sort -u
