...
from her_constants import get_default_her_root, get_her_dir, get_her_home


GATEWAY_SECRET_CAPTURE_UNSUPPORTED_MESSAGE = (
    "Secure secret entry is not supported over messaging. "
    "Load this skill in the local CLI to be prompted, or add the key to ~/.her/.env manually."
)
...
# Default location: {HER_HOME}/cache/images/ (legacy: image_cache/)
IMAGE_CACHE_DIR = get_her_dir("cache/images", "image_cache")
...
AUDIO_CACHE_DIR = get_her_dir("cache/audio", "audio_cache")
...
VIDEO_CACHE_DIR = get_her_dir("cache/videos", "video_cache")
...
MEDIA_DELIVERY_SAFE_ROOTS = (
    ...
    _HER_HOME / "image_cache",
    _HER_HOME / "audio_cache",
    _HER_HOME / "video_cache",
    _HER_HOME / "document_cache",
    _HER_HOME / "browser_screenshots",
    ...
    _HER_HOME / "cache" / "images",
    _HER_HOME / "cache" / "audio",
    _HER_HOME / "cache" / "videos",
    _HER_HOME / "cache" / "documents",
    _HER_HOME / "cache" / "screenshots",
)
...
def _media_delivery_denied_paths() -> List[Path]:
    ...
    for her_root in (_HER_HOME, _HER_ROOT):
        ...
        denied.append(her_root / "config.yaml")
    return denied
...
def validate_media_delivery_path(path: str) -> Optional[str]:
    ...
    # The denylist still blocks /etc, /proc, ~/.ssh, ~/.aws, ~/.her/.env,
    # ~/.her/auth.json, etc. — so the obvious prompt-injection sites
...
_PLAINTEXT_GATEWAY_RESTART_PATTERNS: tuple[re.Pattern[str], ...] = (
    ...
    re.compile(r"^(?:please\s+)?restart\s+(?:the\s+)?her\s+gateway[.!?\s]*$", re.IGNORECASE),
    re.compile(r"^(?:please\s+)?restart\s+her[.!?\s]*$", re.IGNORECASE),
)
...
class BasePlatformAdapter(ABC):
    ...
    def _mask_json_string_media(content: str) -> str:
        ...
        Serialized tool results frequently embed a previous reply's text, e.g.::
            ...
            {"result": "MEDIA:/Users/x/.her/media/generated/stale.png"}
    ...
    async def _process_message_background(self, event: MessageEvent, session_key: str) -> None:
        ...
        try:
            ...
            # not at the top of this task.  _her_run_generation is set on
