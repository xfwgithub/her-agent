...
Usage:
    ...
    python cli.py --skills her-agent-dev,github-auth
...
os.environ["HER_QUIET"] = "1"  # Our own modules

import yaml

from her_cli.fallback_config import get_fallback_chain
...
try:
    from her_cli.pt_input_extras import (
...
from her_cli.banner import _format_context_length, format_banner_version_label
...
load_her_dotenv(her_home=_her_home, project_env=_project_env)
...
def load_cli_config() -> Dict[str, Any]:
    ...
    Config lookup order:
    1. ~/.her/config.yaml (user config - preferred)
    ...
    """
    # Check user config first ({HER_HOME}/config.yaml)
    user_config_path = _her_home / 'config.yaml'
    ...
    defaults = {
        ...
        "display": {
            "compact": False,
            "resume_display": "full",
            # Recap tuning for /resume — see her_cli/config.py DEFAULT_CONFIG.
            "resume_exchanges": 10,
    ...
    if config_path.exists():
        try:
            with open(config_path, "r", encoding="utf-8") as f:
                from her_cli.config import _normalize_root_model_keys
    ...
    # Normalize config key: the new config system (her_cli/config.py) and all
    ...
    # Local backend: always os.getcwd(). Use `cd /dir && her` to control it.
...
# Initialize centralized logging early — agent.log + errors.log in ~/.her/logs/.
...
try:
    from her_cli.skin_engine import init_skin_from_config
...
try:
    ...
    class _AsyncHttpxDelNeuter:
        ...
        ``her --help`` paths inside the chat command flow).  See
...
def get_tool_definitions(*args, **kwargs):
    from her_cli.mcp_startup import wait_for_mcp_discovery
...
from her_cli.banner import build_welcome_banner
from her_cli.commands import SlashCommandCompleter, SlashCommandAutoSuggest
...
from her_cli.callbacks import prompt_for_secret
...
def _setup_worktree(repo_root: str = None) -> Optional[Dict[str, str]]:
    ...
    if not repo_root:
        ...
        print("  cd into your project repo first, then run her -w")
        return None

    short_id = uuid.uuid4().hex[:8]
    wt_name = f"her-{short_id}"
    branch_name = f"her/{wt_name}"
...
def _run_state_db_auto_maintenance(session_db) -> None:
    ...
    :func:`her_cli.config.load_config` (the authoritative loader that
    ...
    if session_db is None:
        return
    try:
        from her_cli.config import load_config as _load_full_config
        from her_constants import get_her_home as _get_her_home
        _her_home_maint = _get_her_home()
        ...
        try:
            if not session_db.get_meta("ghost_session_prune_v1"):
                pruned = session_db.prune_empty_ghost_sessions(
                    sessions_dir=_her_home_maint / "sessions"
                )
...
def _run_checkpoint_auto_maintenance() -> None:
    ...
    :func:`her_cli.config.load_config`. Honours ``auto_prune`` /
...
def _prune_stale_worktrees(repo_root: str, max_age_hours: int = 24) -> None:
    ...
    Also prunes orphaned ``her/*`` and ``pr-*`` local branches that
    ...
    for entry in worktrees_dir.iterdir():
        if not entry.is_dir() or not entry.name.startswith("her-"):
            continue
...
def _prune_orphaned_branches(repo_root: str) -> None:
    """Delete local ``her/her-*`` and ``pr-*`` branches with no worktree.

    These are auto-generated by ``her -w`` sessions and PR review
    ...
    orphaned = [
        ...
        and (b.startswith("her/her-") or b.startswith("pr-"))
    ]
    ...
    try:
        ...
        for var in ("HER_LIGHT", "HER_TUI_LIGHT"):
            ...
            if _FALSE_RE.match(v):
                ...
                return result
        # 2. Theme hint
        theme = (os.environ.get("HER_TUI_THEME") or "").strip().lower()
        if theme == "light":
            result = True
            _LIGHT_MODE_CACHE = result
            return result
        if theme == "dark":
            ...
            return result
        # 3. Explicit bg hex
        bg_hint = os.environ.get("HER_TUI_BACKGROUND") or ""
...
def _install_skin_light_mode_hook() -> None:
    ...
    try:
        from her_cli.skin_engine import SkinConfig  # type: ignore[import]
    except Exception:
        return
    if getattr(SkinConfig, "_her_light_mode_hook_installed", False):
        return
    ...
    SkinConfig._her_light_mode_hook_installed = True  # type: ignore[attr-defined]
...
from her_constants import is_termux as _is_termux_environment
...
def _apply_bracketed_paste_timeout_patch() -> None:
    ...
    ``_her_bp_timeout_patched`` sentinel on the module.
    """
    try:
        ...
        if getattr(_vt100_mod, "_her_bp_timeout_patched", False):
            return
        ...
        def _patched_vt100_feed(self_parser, data: str) -> None:
            if self_parser._in_bracketed_paste:
                ...
                if end_mark in self_parser._paste_buffer:
                    ...
                    self_parser._her_bp_start = None
                    if remaining:
                        _patched_vt100_feed(self_parser, remaining)
                else:
                    bp_start = getattr(self_parser, "_her_bp_start", None)
                    now = time.monotonic()
                    if bp_start is None:
                        self_parser._her_bp_start = now
        ...
        _vt100_mod._her_bp_timeout_patched = True
...
def save_config_value(key_path: str, value: any) -> bool:
    ...
    1. ~/.her/config.yaml (user config - preferred, used if it exists)
    2. ./cli-config.yaml (project config - fallback)
    ...
    try:
        # Ensure parent directory exists (for ~/.her/config.yaml on first use)
...
class HerCLI:
    ...
    def __init__(
        ...
        # checkpoint repos under ~/.her/checkpoints/.  Opt-in via
        ...
        self._history_file = _her_home / ".her_history"
    ...
    def set_voice_record_key_cache(self, raw_key: object) -> None:
        ...
        """
        try:
            from her_cli.voice import format_voice_record_key_for_status
    ...
    def _normalize_model_for_provider(self, resolved_provider: str) -> bool:
        ...
        if resolved_provider == "copilot":
            try:
                from her_cli.models import copilot_model_api_mode, normalize_copilot_model_id
        ...
        if resolved_provider in {"opencode-zen", "opencode-go"}:
            try:
                from her_cli.models import normalize_opencode_model_id, opencode_model_api_mode
    ...
    def _ensure_runtime_credentials(self) -> bool:
        ...
        if not _is_callable_provider and (not isinstance(api_key, str) or not api_key):
            ...
            if _has_custom_base:
                ...
                print("\n⚠️  Provider resolver returned an empty API key. "
                      "Set OPENROUTER_API_KEY or run: her setup")
                return False
        if not isinstance(base_url, str) or not base_url:
            print("\n⚠️  Provider resolver returned an empty base URL. "
                  "Check your provider config or run: her setup")
            return False
        ...
        # `her chat --model <provider-name>` sends the provider name
        ...
        # If model is still empty (e.g. user ran `her auth add openai-codex`
        # without `her model`), fall back to the provider's first catalog
        ...
        if self._resumed and self._session_db and not self.conversation_history:
            ...
            if not session_meta:
                if _quiet_mode:
                    ...
                    print(
                        "Use a session ID from a previous CLI run (her sessions list).",
                        file=sys.stderr,
                    )
                else:
                    _cprint(f"\033[1;31mSession not found: {self.session_id}{_RST}")
                    _cprint(f"{_DIM}Use a session ID from a previous CLI run (her sessions list).{_RST}")
                return False
    ...
    def _show_security_advisories(self):
        ...
        clean) listing the worst hit and pointing at ``her doctor``.
        Banner-cache rate-limits this to once per 24h per advisory; full
        remediation lives behind ``her doctor`` so the banner stays
        small.
        """
        try:
            from her_cli.security_advisories import (
                detect_compromised,
                startup_banner,
            )
    ...
    def _try_attach_clipboard_image(self) -> bool:
        ...
        Saves the image to ~/.her/images/ and appends the path to
        ``_attached_images``.  Returns True if an image was attached.
        """
        from her_cli.clipboard import save_clipboard_image

        img_dir = get_her_home() / "images"
    ...
    def _handle_rollback_command(self, command: str):
        ...
        if not mgr.enabled:
            ...
            print("  Enable with: her --checkpoints")
    ...
    def _handle_snapshot_command(self, command: str):
        ...
        if subcmd in {"list", "ls"}:
            ...
            if not snaps:
                ...
                return
            print(f"  State snapshots ({display_her_home()}/state-snapshots/):\n")
    ...
    def _handle_paste_command(self):
        ...
        from her_cli.clipboard import has_clipboard_image
    ...
    def _handle_image_command(self, cmd_original: str):
        ...
        if _remainder:
            ...
            _cprint(f"  {_DIM}Tip: type your next message, or run her chat -q --image {_termux_example_image_path(image_path.name)} \"What do you see?\"{_RST}")
    ...
    def _show_tool_availability_warnings(self):
        ...
        try:
            ...
            if api_key_missing:
                ...
                self._console_print("[dim]   Run 'her setup' to configure[/]")
        except Exception:
    ...
    def show_help(self):
        ...
        from her_cli.commands import COMMANDS_BY_CATEGORY

        try:
            from her_cli.skin_engine import get_active_help_header
    ...
    def _handle_tools_command(self, cmd: str):
        ...
        def _run_capture(ns: Namespace) -> None:
            ...
            # Buffer reports isatty()=True so color() in her_cli/colors.py
    ...
    def _show_recent_sessions(self, *, reason: str = "history", limit: int = 10) -> bool:
        ...
        from her_cli.main import _relative_time
    ...
    def _handle_handoff_command(self, cmd_original: str) -> bool:
        ...
        try:
            ...
            pass
        _cprint("  Timed out waiting for the gateway. Is `her gateway` running?")
    ...
    def _handle_resume_command(self, cmd_original: str) -> None:
        ...
        if not target:
            ...
            if self._show_recent_sessions(reason="resume"):
                ...
                return
            _cprint("  Tip:   Use /history or `her sessions list` to find sessions.")
            return
        ...
        if target.isdigit():
            ...
            from her_cli.main import _resolve_session_by_name_or_id
        ...
        if not session_meta:
            ...
            _cprint("  Use /history or `her sessions list` to see available sessions.")
            return
    ...
    def save_conversation(self):
        """Save the current conversation to a JSON snapshot under ~/.her/sessions/saved/.
        ...
        DB, so the live session remains resumable via ``her --resume <id>``
        ...
        try:
            ...
            return
        path = saved_dir / f"her_conversation_{timestamp}.json"
        ...
        try:
            ...
            if self.session_id:
                print(f"       Resume the live session with: her --resume {self.session_id}")
        except Exception as e:
    ...
    def _run_curses_picker(self, title: str, items: list[str], default_index: int = 0) -> int | None:
        ...
        import threading
        from her_cli.curses_ui import curses_single_select
    ...
    def _apply_model_switch_result(self, result, persist_global: bool) -> None:
        ...
        try:
            from her_cli.model_switch import resolve_display_context_length
    ...
    def _handle_model_picker_selection(self, persist_global: bool = False) -> None:
        ...
        if stage == "provider":
            ...
            # (same lists as `her model` and gateway pickers).
            # Only fall back to the live provider catalog when the curated
            # list is empty (e.g. user-defined endpoints with no curated list).
            model_list = provider_data.get("models", [])
            if not model_list:
                try:
                    from her_cli.models import provider_model_ids
        ...
        if stage == "model":
            ...
            if selected < len(model_list):
                from her_cli.model_switch import switch_model
    ...
    def _handle_model_switch(self, cmd_original: str):
        ...
        """
        from her_cli.model_switch import switch_model, parse_model_flags
    ...
    def _handle_codex_runtime(self, cmd_original: str) -> None:
        ...
        """
        from her_cli import codex_runtime_switch as crs
    ...
    def _handle_curator_command(self, cmd: str):
        ...
        Delegates to her_cli.curator so the CLI and the `her curator`
        ...
        try:
            from her_cli.curator import cli_main
            cli_main(tokens)
        except SystemExit:
    ...
    def _handle_kanban_command(self, cmd: str):
        ...
        """
        from her_cli.kanban import run_slash
    ...
    def _handle_skills_command(self, cmd: str):
        """Handle /skills slash command — delegates to her_cli.skills_hub."""
        from her_cli.skills_hub import handle_skills_slash
    ...
    def _show_gateway_status(self):
        ...
        try:
            ...
            print()
            print(f"  Configuration file: {display_her_home()}/config.yaml")
            print()
            ...
            print(f"    2. Or configure settings in {display_her_home()}/config.yaml")
            print()
    ...
    def process_command(self, command: str) -> bool:
        ...
        # change in her_cli/commands.py instead of touching every dispatch site.
        from her_cli.commands import resolve_command as _resolve_cmd
        ...
        if canonical in {"quit", "exit"}:
            ...
            from her_cli.main import _print_version_info
            ...
            from her_cli.config import reload_env
            count = reload_env()
            ...
            try:
                ...
                if not plugins:
                    ...
                    print(f"Drop plugin directories into {display_her_home()}/plugins/ to get started.")
                else:
            ...
            # Check for skill slash commands (/gif-search, /axolotl, etc.)
                ...
                from her_cli.commands import COMMANDS
    ...
    def _handle_bundles_command(self, cmd: str) -> None:
        ...
        Mirrors ``her bundles list`` but renders inside the running
        ...
        if not bundles:
            ...
            _cprint(
                f"  {_DIM}Create one with: her bundles create "
        ...
        _cprint(
            ...
            f"Manage with `her bundles`.{_RST}"
        )
    ...
    def _get_goal_manager(self):
        ...
        """
        try:
            from her_cli.goals import GoalManager
            from her_cli.config import load_config
        except Exception as exc:
    ...
    def _handle_skin_command(self, cmd: str):
        ...
        try:
            from her_cli.skin_engine import list_skins, set_active_skin, get_active_skin_name
        except ImportError:
        ...
        if len(parts) < 2 or not parts[1].strip():
            ...
            print("\n  Usage: /skin <name>")
            print(f"  Custom skins: drop a YAML file in {display_her_home()}/skins/\n")
            return
    ...
    def _handle_footer_command(self, cmd_original: str) -> None:
        ...
        """
        from her_cli.config import load_config
        from her_cli.colors import Colors as _Colors
    ...
    def _handle_fast_command(self, cmd: str):
        ...
        try:
            from her_cli.models import _is_anthropic_fast_model
            agent = getattr(self, "agent", None)
            model = getattr(agent, "model", None) or getattr(self, "model", None)
    ...
    def _manual_compress(self, cmd_original: str = ""):
        ...
        * ``/compress here [N]`` — boundary-aware compression. Summarize
        ...
        from her_cli.partial_compress import (
    ...
    def _handle_debug_command(self):
        ...
        from her_cli.debug import run_debug_share
    ...
    def _show_usage(self):
        ...
        if self.verbose:
            ...
            # Console quietness is enforced by her_logging not
    ...
    def _check_config_mcp_changes(self) -> None:
        ...
        from her_cli.config import get_config_path as _get_config_path
    ...
    def _reload_skills(self) -> None:
        """Reload skills: rescan ~/.her/skills/ and queue a note for the
        next user turn.
                ...
                try:
                    if (
                        ...
                        if not is_seen(CLI_CONFIG, TOOL_PROGRESS_FLAG):
                            ...
                            mark_seen(_her_home / "config.yaml", TOOL_PROGRESS_FLAG)
    ...
    def _voice_speak_response(self, text: str):
        ...
        try:
            ...
            os.makedirs(os.path.join(tempfile.gettempdir(), "her_voice"), exist_ok=True)
            mp3_path = os.path.join(
                tempfile.gettempdir(), "her_voice",
                ...
                if hasattr(self, '_interrupt_queue'):
                    try:
                        ...
                        if interrupt_msg:
                            ...
                            try:
                                _dbg = _her_home / "interrupt_debug.log"
    ...
    def _print_exit_summary(self):
        ...
        if msg_count > 0:
            ...
            # profile names use the standard HER_HOME, so no -p needed.
            try:
                from her_cli.profiles import get_active_profile_name
            ...
            print(f"  her --resume {self.session_id}{profile_flag}")
            if session_title:
                print(f"  her -c \"{session_title}\"{profile_flag}")
            print()
            ...
            try:
                from her_cli.skin_engine import get_active_goodbye
    ...
    def _get_tui_prompt_symbols(self) -> tuple[str, str]:
        ...
        """
        try:
            from her_cli.skin_engine import get_active_prompt_symbol
    ...
    def _build_tui_style_dict(self) -> dict[str, str]:
        ...
        try:
            from her_cli.skin_engine import get_prompt_toolkit_style_overrides
    ...
    def run(self):
        ...
        try:
            from her_cli.model_switch import prewarm_picker_cache_async
        ...
        try:
            ...
            if not is_seen(self.config, OPENCLAW_RESIDUE_FLAG) and detect_openclaw_residue():
                ...
                try:
                    from her_cli.config import get_config_path as _get_cfg_path_resid
        ...
        def handle_ignored_terminal_sequence(event):
            ...
            install_ignored_terminal_sequences() in her_cli.pt_input_extras
                ...
                if self._agent_running and not (text and _looks_like_slash_command(text)):
                    ...
                    try:
                        ...
                        if not is_seen(CLI_CONFIG, BUSY_INPUT_FLAG):
                            ...
                            mark_seen(_her_home / "config.yaml", BUSY_INPUT_FLAG)
        ...
        try:
            ...
            if not getattr(_pt_renderer, "_her_osd_patched", False):
                ...
                _pt_renderer._her_osd_patched = True
        except Exception:
            pass
        ...
        try:
            ...
            # delivers spurious CTRL_C_EVENT to the her process when
        ...
        try:
            ...
            print(
                ...
                "Try reinstalling Python via pyenv or Homebrew, then re-run: her setup"
            )
            _run_cleanup()
...
def _run_kanban_goal_loop_q(cli: "HerCLI", first_response: str) -> None:
    ...
    task_id = (_os.environ.get("HER_KANBAN_TASK") or "").strip()
    if not task_id:
        return

    from her_cli import kanban_db as _kb
    from her_cli.goals import run_kanban_goal_loop as _run_loop, DEFAULT_MAX_TURNS as _DEF_TURNS
...
def main(
    ...
    # Default to her-cli toolset which includes cronjob management tools
    toolsets_list = None
    if toolsets:
    ...
    if query or image:
        ...
        # Kanban workers spawn with ``her chat -q "work kanban task <id>"``;
        ...
        _kanban_task_id = os.environ.get("HER_KANBAN_TASK", "").strip()
        if _kanban_task_id:
            try:
                from her_cli import kanban_db as _kb
        ...
        if quiet:
            ...
            if cli._ensure_runtime_credentials():
                ...
                if cli._init_agent(
                    ...
                    if isinstance(result, dict) and result.get("failed"):
                        _exit_code = 1
                        if os.environ.get("HER_KANBAN_TASK") and result.get(
                            "failure_reason"
                        ) in ("rate_limit", "billing"):
                            try:
                                from her_cli.kanban_db import (
            ...
            # Single-query mode (`her chat -q "…"`): skip the welcome
