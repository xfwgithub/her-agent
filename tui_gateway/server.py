...
from her_constants import (
    get_her_home,
    get_her_home_override,
    reset_her_home_override,
    set_her_home_override,
)
...
load_her_dotenv(
    her_home=_her_home, project_env=Path(__file__).parent.parent / ".env"
)
...
# appends every unhandled exception to ~/.her/logs/tui_gateway_crash.log
...
_CRASH_LOG = os.path.join(_her_home, "logs", "tui_gateway_crash.log")
...
try:
    _slash_timeout = float(os.environ.get("HER_TUI_SLASH_TIMEOUT_S") or "45")
...
try:
    _ws_orphan_reap_grace = float(
        os.environ.get("HER_TUI_WS_ORPHAN_REAP_GRACE_S") or "20"
    )
except (ValueError, TypeError):
...
try:
    _rpc_pool_workers = max(
        2, int(os.environ.get("HER_TUI_RPC_POOL_WORKERS") or "4")
    )
except (ValueError, TypeError):
    _rpc_pool_workers = 4
...
# ``profile`` on those calls; we open that profile's db and bind its HER_HOME
...
def _profile_home(profile: str | None) -> Path | None:
    ...
    if home.resolve() == Path(_her_home).resolve():
        return None
...
def _start_agent_build(sid: str, session: dict) -> None:
    ...
    Classic `her` shows the prompt before constructing AIAgent; the TUI used
    ...
    def _build() -> None:
        ...
        try:
            ...
            # HER_HOME so config/skills/model resolve to it, and hand the
            # agent that profile's db so turns persist to the right state.db.
            session_db = None
            if profile_home:
                home_token = set_her_home_override(profile_home)
                try:
                    from her_state import SessionDB
            ...
            if home_token is not None:
                reset_her_home_override(home_token)
            with _sessions_lock:
...
def _load_cfg() -> dict:
    ...
    try:
        ...
        # launch profile's _her_home. Cache is keyed on the resolved path, so
        # profiles don't clobber each other.
        override = get_her_home_override()
        home = override if isinstance(override, str) and override else _her_home
...
def resolve_skin() -> dict:
    try:
        from her_cli.skin_engine import init_skin_from_config, get_active_skin
...
def _resolve_startup_runtime() -> tuple[str, str | None]:
    model = _resolve_model()
    explicit_provider = os.environ.get("HER_TUI_PROVIDER", "").strip()
    ...
    try:
        from her_cli.models import detect_static_provider_for_model
...
def _load_tool_progress_mode() -> str:
    env = os.environ.get("HER_TUI_TOOL_PROGRESS", "").strip().lower()
...
def _load_enabled_toolsets() -> list[str] | None:
    explicit = [
        item.strip()
        for item in os.environ.get("HER_TUI_TOOLSETS", "").split(",")
        if item.strip()
    ]
    cfg = None
    ...
    if explicit and validate_toolset is not None:
        ...
        if any(name in {"all", "*"} for name in built_in):
            ...
            if ignored:
                print(
                    "[tui] HER_TUI_TOOLSETS=all enables every toolset; "
        ...
        try:
            ...
            from her_cli.tools_config import _parse_enabled_flag
        ...
        if unknown:
            print(
                f"[tui] ignoring unknown HER_TUI_TOOLSETS entries: {', '.join(unknown)}",
                file=sys.stderr,
                flush=True,
            )
        if disabled:
            print(
                "[tui] ignoring disabled MCP servers in HER_TUI_TOOLSETS "
        ...
        fallback_notice = (
            "[tui] no valid HER_TUI_TOOLSETS entries; using configured CLI toolsets"
        )
    ...
    try:
        ...
        if fallback_notice is not None:
            print(
                "[tui] no valid HER_TUI_TOOLSETS entries and configured CLI toolsets could not be loaded; enabling all toolsets",
                file=sys.stderr,
                flush=True,
            )
        return None
...
def _apply_model_switch(sid: str, session: dict, raw_input: str) -> dict:
    from her_cli.model_switch import parse_model_flags, switch_model
    ...
    # in config or env.
    #
    # HER_TUI_PROVIDER is the canonical "explicit-this-process" carrier
...
def _session_info(agent, session: dict | None = None) -> dict:
    ...
    try:
        ...
        pass
    try:
        from her_cli.banner import get_available_skills
    ...
    try:
        ...
        pass
    try:
        from her_cli.banner import get_update_result
...
def _available_personalities(cfg: dict | None = None) -> dict:
    try:
        ...
        try:
            from her_cli.config import load_config as _load_full_cfg
...
def _cfg_max_turns(cfg: dict, default: int) -> int:
    try:
        env_max = int(os.environ.get("HER_TUI_MAX_TURNS", "") or 0)
        if env_max > 0:
            return env_max
    except (TypeError, ValueError):
        pass
...
def _parse_tui_skills_env() -> list[str]:
    raw = os.environ.get("HER_TUI_SKILLS", "")
    skills: list[str] = []
    seen: set[str] = set()
...
def _(rid, params: dict) -> dict:
    ...
    # and each turn re-bind HER_HOME. None/own profile → launch (unchanged).
...
def _(rid, params: dict) -> dict:
    ...
    _enable_gateway_prompts()
    home_token = (
        set_her_home_override(str(profile_home)) if profile_home is not None else None
    )
    try:
    ...
    with _session_resume_lock:
        ...
        try:
            ...
            if sid in _sessions:
                ...
                # Remember the profile home so each turn re-binds HER_HOME (the
...
#
# Layout:  $HER_HOME/spawn-trees/<session_id>/<timestamp>.json
...
def _run_prompt_submit(rid, sid: str, session: dict, text: Any) -> None:
    ...
    def run():
        approval_token = None
        session_tokens = []
        home_token = None  # per-turn HER_HOME override for a resumed remote profile
        ...
        try:
            ...
            if _profile_home_str:
                home_token = set_her_home_override(_profile_home_str)
            ...
            if images:
                try:
                    ...
                    from her_cli.config import load_config as _tui_load_config
            ...
            if (
                ...
                try:
                    from her_cli.voice import speak_text
                    ...
                    logger.warning("voice TTS skipped: her_cli.voice unavailable")
                except Exception as e:
...
def _(rid, params: dict) -> dict:
    ...
    if err:
        return err
    try:
        from her_cli.clipboard import has_clipboard_image, save_clipboard_image
    except Exception as e:
        return _err(rid, 5027, f"clipboard unavailable: {e}")

    session["image_counter"] = session.get("image_counter", 0) + 1
    img_dir = _her_home / "images"
...
def _(rid, params: dict) -> dict:
    ...
    if key == "provider":
        try:
            from her_cli.models import list_available_providers, normalize_provider
    ...
    if key == "profile":
        ...
        return _ok(rid, {"home": str(_her_home), "display": display_her_home()})
    if key == "project":
...
def _(rid, params: dict) -> dict:
    try:
        from her_cli.main import _has_any_provider_configured
...
@method("reload.env")
def _(rid, params: dict) -> dict:
    """Re-read ``~/.her/.env`` into the gateway process via
    ``her_cli.config.reload_env``, matching classic CLI's ``/reload``
...
def _cli_exec_blocked(argv: list[str]) -> str | None:
    ...
    if not argv:
        return "bare `her` is interactive — use `/her chat -q …` or run `her` in another terminal"
    a0 = argv[0].lower()
    if a0 == "setup":
        return "`her setup` needs a full terminal — run it outside the TUI"
    if a0 == "gateway":
        return "`her gateway` is long-running — run it in another terminal"
    if a0 == "sessions" and len(argv) > 1 and argv[1].lower() == "browse":
        return "`her sessions browse` is interactive — use /resume here, or run browse in another terminal"
    if a0 == "config" and len(argv) > 1 and argv[1].lower() == "edit":
        return "`her config edit` needs $EDITOR in a real terminal"
    return None


@method("cli.exec")
def _(rid, params: dict) -> dict:
    """Run `python -m her_cli.main` with argv; capture stdout/stderr (non-interactive only)."""
    ...
    try:
        r = subprocess.run(
            [sys.executable, "-m", "her_cli.main", *argv],
            capture_output=True,
            text=True,
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from her_cli.commands import SlashCommandCompleter
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        from her_cli.config import is_managed, save_env_value
        from her_cli.inventory import build_models_payload, load_picker_context
        ...
        if pconfig.auth_type != "api_key":
            return _err(
                ...
                f"run `her model` to configure",
            )
        if not pconfig.api_key_env_vars:
            return _err(rid, 4004, f"no env var defined for {pconfig.name}")

        # Save the key to ~/.her/.env
...
def _(rid, params: dict) -> dict:
    ...
    """
    try:
        from her_cli.auth import PROVIDER_REGISTRY, clear_provider_auth
    ...
    * ``tts`` — toggle speech-output of agent replies. Requires mode on
    ...
    if action in {"on", "off"}:
        ...
        if not enabled:
            ...
            try:
                from her_cli.voice import stop_continuous
...
def _(rid, params: dict) -> dict:
    ...
    try:
        if action == "start":
            ...
            from her_cli.voice import start_continuous
...
def _failure_messages(url: str, port: int, system: str) -> list[str]:
    from her_cli.browser_connect import manual_chrome_debug_command
...
def _browser_connect(rid, params: dict) -> dict:
    ...
    from her_cli.browser_connect import DEFAULT_BROWSER_CDP_URL
    ...
    try:
        ...
        if parsed.scheme in {"ws", "wss"} and parsed.path.startswith(
            ...
            if not ok and _is_default_local_cdp(parsed):
                from her_cli.browser_connect import try_launch_chrome_debug
        ...
        sections = [
            ...
            },
            {
                ...
                "rows": [
                    ...
                    ["Config File", str(_her_home / "config.yaml")],
                ],
            },
        ]
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if action == "install":
            from her_cli.skills_hub import do_install
        ...
        if action == "browse":
            from her_cli.skills_hub import browse_skills
        ...
        if action == "inspect":
            from her_cli.skills_hub import inspect_skill
