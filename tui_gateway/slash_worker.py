"""Persistent slash-command worker — one HerCLI per TUI session.
...
def _run(cli: HerCLI, command: str) -> str:
...
def main():
    ...
    os.environ["HER_SESSION_KEY"] = args.session_key
    os.environ["HER_INTERACTIVE"] = "1"

    with contextlib.redirect_stdout(io.StringIO()), contextlib.redirect_stderr(io.StringIO()):
        cli = HerCLI(model=args.model or None, compact=True, resume=args.session_key, verbose=False)
