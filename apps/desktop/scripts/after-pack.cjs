 ...
 *
 * Stamps the her icon + identity onto the packed Windows her.exe via
 * rcedit (delegated to set-exe-identity.cjs). This runs for EVERY packed build
 * — first install, `her desktop`, the installer's --update rebuild, and a
 ...
 *   - packager.appInfo.productFilename: the exe basename (e.g. 'her')
 */
...
exports.default = async function afterPack(context) {
  ...
  const productName = context.packager?.appInfo?.productFilename || 'her'
  ...
  try {
    ...
    console.warn(`[after-pack] exe identity stamp failed (${err.message}); her.exe keeps the stock Electron icon`)
  }
}
