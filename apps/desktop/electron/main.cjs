...
if (REMOTE_DISPLAY_REASON) {
  ...
  console.log(
    `[her] remote display detected (${REMOTE_DISPLAY_REASON}); disabling GPU hardware acceleration to prevent flicker`
  )
}
    ...
    try {
      ...
      if (parsed && typeof parsed === 'object' && typeof parsed.commit === 'string' && parsed.commit.length >= 7) {
        if (parsed.schemaVersion !== INSTALL_STAMP_SCHEMA_VERSION) {
          console.warn(
            `[her] install-stamp.json schemaVersion ${parsed.schemaVersion} != expected ${INSTALL_STAMP_SCHEMA_VERSION}; ignoring`
          )
          continue
        }
...
if (INSTALL_STAMP) {
  console.log(
    `[her] install stamp: ${INSTALL_STAMP.commit.slice(0, 12)}${INSTALL_STAMP.branch ? ` (${INSTALL_STAMP.branch})` : ''}${INSTALL_STAMP.dirty ? ' [DIRTY]' : ''} from ${INSTALL_STAMP.source || 'unknown'}`
  )
} else if (IS_PACKAGED) {
  ...
  console.error(
    '[her] WARNING: no install-stamp.json found in packaged build. First-launch bootstrap will not have a pinned ref to install.'
  )
}
...
//   macOS / Linux: ~/.her (matches install.sh)
//
// Special case for Windows: if the user has a legacy ~/.her directory
...
function resolveherHome() {
  ...
  if (IS_WINDOWS && process.env.LOCALAPPDATA) {
    const localappdata = path.join(process.env.LOCALAPPDATA, 'her')
    const legacy = path.join(app.getPath('home'), '.her')
    // Migrate transparently to LOCALAPPDATA, but honour an existing legacy
    // ~/.her setup (no LOCALAPPDATA install yet) so users don't lose state.
...
// Mirrors her_cli.profiles._PROFILE_ID_RE so we never hand the backend a
// value its profile resolver would reject and exit on.
const PROFILE_NAME_RE = /^[a-z0-9][a-z0-9_-]{0,63}$/
// Branch we track for self-update. The GUI work has merged to main, so this
// tracks main. User can also override at runtime via
// herDesktop.updates.setBranch().
const DEFAULT_UPDATE_BRANCH = 'main'
// desktop.log lives under HER_HOME/logs/ so it sits next to agent.log,
// errors.log, gateway.log produced by her_logging.setup_logging — one log
// directory per user, regardless of which UI surface produced the line.
const DESKTOP_LOG_PATH = path.join(HER_HOME, 'logs', 'desktop.log')
...
//
// Mirror the Python logs (her_logging.py RotatingFileHandler, maxBytes x
...
const MEDIA_PROTOCOL = 'her-media'
// Only audio/video may be streamed. Without this the handler would read any
// non-blocklisted local file (no size cap) for any `fetch(her-media://…)`.
...
function rememberLog(chunk) {
  ...
  herLog.push(...lines)
  if (herLog.length > 300) {
    herLog.splice(0, herLog.length - 300)
  }
...
function ensureWslWindowsFonts() {
  ...
  try {
    ...
    const confPath = path.join(confDir, '99-her-wsl-windows-fonts.conf')
    let existing = ''
    try {
...
function broadcastBootProgress() {
  ...
  webContents.send('her:boot-progress', bootProgressState)
}
...
//
// The snapshot is queryable via the her:bootstrap:get IPC handler so a
...
function broadcastBootstrapEvent(ev) {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('her:bootstrap:event', ev)
}
...
function findGitBash() {
  ...
  // install.ps1 drops PortableGit at %LOCALAPPDATA%\her\git\... — checked
  // first so users who installed via install.ps1 are detected before we
  // start probing system-wide locations.
  const localAppData = process.env.LOCALAPPDATA || ''
  const candidates = []
  if (localAppData) {
    candidates.push(path.join(localAppData, 'her', 'git', 'bin', 'bash.exe'))
    candidates.push(path.join(localAppData, 'her', 'git', 'usr', 'bin', 'bash.exe'))
  }
...
// install only has PortableGit under %LOCALAPPDATA%\her\git (never on
...
function resolveGitBinary() {
  ...
  const candidates = []
  if (localAppData) {
    candidates.push(path.join(localAppData, 'her', 'git', 'cmd', 'git.exe'))
    candidates.push(path.join(localAppData, 'her', 'git', 'bin', 'git.exe'))
  }
...
// ─── Self-update (git-pull against the running backend's her root) ──────
...
function emitUpdateProgress(payload) {
  ...
  for (const window of BrowserWindow.getAllWindows()) {
    window.webContents.send('her:updates:progress', merged)
  }
}
...
// HER_HOME/her-setup.exe on a successful install (see
// apps/bootstrap-installer paths::copy_self_to_her_home). That binary owns
// ALL repo mutation — running `her update` + rebuilding the desktop — so
...
function resolveUpdaterBinary() {
  const name = IS_WINDOWS ? 'her-setup.exe' : 'her-setup'
  const candidate = path.join(HER_HOME, name)
...
// only signals the direct child, so on Windows a backend `her.exe` that
// spawned its own grandchildren (a `her` REPL, a pty terminal session, the
...
// `herProcess.kill('SIGTERM')` + `app.quit()` fire-and-forget: SIGTERM on
// Windows doesn't reap the backend's grandchildren, and quit didn't wait for
// teardown, so the updater raced a still-locked `her.exe`, the quarantine
...
// problem: the desktop's backend (and the grandchildren IT spawned — a her
// REPL, a pty terminal, the gateway) keep `her.exe` and other files in the
...
async function releaseBackendLock(updateRoot, tag) {
  ...
  const pids = []
  if (herProcess && Number.isInteger(herProcess.pid)) pids.push(herProcess.pid)
...
async function applyUpdates(opts = {}) {
  ...
  try {
    ...
    if (!updater && !IS_WINDOWS) {
      // macOS/Linux drag-install: no staged Tauri her-setup. Unlike Windows
      ...
      // whole update itself: `her update` (backend) + `her desktop
    ...
    if (!updater) {
      ...
      // `her desktop`, never the Tauri installer that self-copies
      // her-setup.exe into HER_HOME). They DO have a working `her`
      ...
      // checkout they're on — bare `her update` defaults to main and would
      ...
      const updateRoot = resolveUpdateRoot()
      let command = 'her update'
      try {
        const head = await runGit(['rev-parse', '--abbrev-ref', 'HEAD'], { cwd: updateRoot })
        const current = (head.stdout || '').trim()
        if (head.code === 0 && current && current !== 'HEAD') {
          const branch = await resolveHealedBranch(updateRoot, current)
          if (branch !== 'main') command = `her update --branch ${branch}`
        }
      } catch {
        // Best-effort: fall back to bare `her update` if branch detection fails.
      }
    ...
    // her.exe (held by the backend child / its grandchildren) and the update
    // bricks. See releaseBackendLockForUpdate for the full failure analysis.
    await releaseBackendLockForUpdate(updateRoot)

    // Detached so the updater outlives this process — it needs us GONE before
    // `her update` will run (the venv shim is locked while we live).
    const child = spawn(updater, updaterArgs, {
      cwd: HER_HOME,
      env: {
        ...process.env,
        HER_HOME,
        PATH: [path.join(HER_HOME, 'node', 'bin'), venvBin, process.env.PATH].filter(Boolean).join(path.delimiter)
      },
      detached: true,
      stdio: 'ignore',
      windowsHide: false
...
// macOS/Linux in-app update: backend (`her update`) + OS-aware GUI rebuild
// (`her desktop --build-only`), then atomically swap the running .app bundle
...
async function applyUpdatesPosixInApp() {
  ...
  // `her update` reaps stale `her dashboard` backends (a code update
  ...
  if (herProcess && Number.isInteger(herProcess.pid)) {
    desktopChildPids.push(herProcess.pid)
  }
  for (const entry of backendPool.values()) {
  ...
  if (updated.code !== 0) {
    emitUpdateProgress({ stage: 'error', message: 'her update failed.', error: updated.error || 'update-failed' })
    return { ok: false, error: 'her update failed' }
  }

  emitUpdateProgress({ stage: 'rebuild', message: 'Rebuilding the desktop app…', percent: 60 })
  const rebuilt = await runStreamedUpdate(her, ['desktop', '--build-only'], {
    cwd: updateRoot,
    env,
    stage: 'rebuild'
  })
...
if [ "$SRC" != "$DST" ]; then
  if /usr/bin/ditto "$SRC" "$DST.her-update-new"; then
    rm -rf "$DST.her-update-old" 2>/dev/null || true
    mv "$DST" "$DST.her-update-old" 2>/dev/null || rm -rf "$DST"
    mv "$DST.her-update-new" "$DST"
    rm -rf "$DST.her-update-old" 2>/dev/null || true
  fi
fi
...
/usr/bin/open "$DST"
`
  const scriptPath = path.join(app.getPath('temp'), `her-desktop-update-${Date.now()}.sh`)
  try {
...
function isBootstrapComplete() {
  ...
  // pinned commit -- users update via the in-app update path or `her
...
function resolveherBackend(dashboardArgs) {
  ...
  //    to spawning her. Updates flow through the in-app update path
  //    (applyUpdates -> git pull) or `her update` from the CLI.
  if (isBootstrapComplete()) {
    return createActiveBackend(dashboardArgs)
  }

  // 4. Existing `her` on PATH -- installed via install.ps1 / install.sh from
  ...
  if (process.env.HER_DESKTOP_IGNORE_EXISTING !== '1') {
    ...
    if (herCommand) {
      // Smoke-test the candidate before trusting it. A `her` shim
  ...
  // 5. Last-ditch: pip-installed her_cli module via system Python.
  ...
  if (python) {
    ...
    // having her_cli installed -- common on dev boxes that have
    ...
    rememberLog(`Ignoring system Python ${python}: her_cli is not importable; falling through to bootstrap.`)
  }
...
async function ensureRuntime(backend) {
  ...
  if (backend.kind === 'bootstrap-needed') {
    ...
    const bootstrapResult = await runBootstrap({
      ...
      herHome: HER_HOME,
      logRoot: path.join(HER_HOME, 'logs'),
...
function getLinkTitleSession() {
  ...
  linkTitleSession = session.fromPartition('her:link-titles', { cache: false })
...
function sendPreviewFileChanged(payload) {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('her:preview-file-changed', payload)
}
...
function sendBackendExit(payload) {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('her:backend-exit', payload)
}
...
function sendClosePreviewRequested() {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('her:close-preview-requested')
}
...
function sendPowerResume() {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('her:power-resume')
}
...
function sendOpenUpdatesRequested() {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('her:open-updates')
...
function sendWindowStateChanged(nextIsFullscreen) {
  ...
  webContents.send('her:window-state-changed', state)
}
...
//
//   * REST is authed by HttpOnly session cookies (``her_session_at``),
...
//     alongside the ~15-min access token (Portal NAS #293 / her #37247).
//     Both are set as HttpOnly cookies (``her_session_at`` ~15 min,
//     ``her_session_rt`` 24h). When the AT cookie lapses but the RT cookie
...
const OAUTH_SESSION_PARTITION = 'persist:her-remote-oauth'
...
// a valid stored value (pins the root HER_HOME explicitly); null means "no
...
async function buildRemoteConnection(rawUrl, authMode, token, source) {
  ...
  if (authMode === 'oauth') {
    ...
    // Portal issues a 24h rotating refresh token (her #37247), and the
...
async function starther() {
  ...
  connectionPromise = (async () => {
    ...
    // deterministic (it wins over the sticky ~/.her/active_profile file) and
    // resolves HER_HOME the same way `her -p <name>` does on the CLI. An
    ...
    herProcess = spawn(backend.command, backend.args, {
      ...
      env: {
        ...
        // directories. install.ps1 sets HER_HOME via setx; the desktop
    ...
    herProcess.stdout.on('data', rememberLog)
    herProcess.stderr.on('data', rememberLog)
...
ipcMain.handle('her:connection', async (_event, profile) => ensureBackend(profile))
ipcMain.handle('her:backend:touch', async (_event, profile) => {
...
ipcMain.handle('her:boot-progress:get', async () => bootProgressState)
ipcMain.handle('her:bootstrap:get', async () => getBootstrapState())
ipcMain.handle('her:connection-config:get', async (_event, profile) =>
  sanitizeDesktopConnectionConfig(readDesktopConnectionConfig(), profile)
)
ipcMain.handle('her:connection-config:test', async (_event, payload) => testDesktopConnectionConfig(payload))
ipcMain.handle('her:connection-config:probe', async (_event, rawUrl) => probeRemoteAuthMode(rawUrl))
ipcMain.handle('her:connection-config:oauth-login', async (_event, rawUrl) => {
...
ipcMain.handle('her:connection-config:oauth-logout', async (_event, rawUrl) => {
...
ipcMain.handle('her:connection-config:save', async (_event, payload) => {
  const config = coerceDesktopConnectionConfig(payload)
  writeDesktopConnectionConfig(config)

  return sanitizeDesktopConnectionConfig(config, payload?.profile)
})
ipcMain.handle('her:connection-config:apply', async (_event, payload) => {
  const config = coerceDesktopConnectionConfig(payload)
  writeDesktopConnectionConfig(config)
...
ipcMain.handle('her:profile:get', async () => ({ profile: readActiveDesktopProfile() }))
ipcMain.handle('her:profile:set', async (_event, name) => {
  const next = writeActiveDesktopProfile(name)

  // Switching profiles is a backend re-home: relaunch the dashboard under the
  // new HER_HOME. Pool backends keep their own homes, so only the primary
  // is torn down.
  await teardownPrimaryBackendAndWait()
  mainWindow?.reload()
...
ipcMain.on('her:previewShortcutActive', (_event, active) => {
  previewShortcutActive = Boolean(active)
})

ipcMain.handle('her:requestMicrophoneAccess', async () => {
...
ipcMain.handle('her:api', async (_event, request) => {
...
ipcMain.handle('her:readFileDataUrl', async (_event, filePath) => {
...
ipcMain.handle('her:readFileText', async (_event, filePath) => {
...
ipcMain.handle('her:selectPaths', async (_event, options = {}) => {
...
ipcMain.handle('her:writeClipboard', (_event, text) => {
  clipboard.writeText(String(text || ''))
  return true
})

ipcMain.handle('her:saveImageFromUrl', (_event, url) => saveImageFromUrl(String(url || '')))

ipcMain.handle('her:saveImageBuffer', async (_event, payload) => {
...
ipcMain.handle('her:saveClipboardImage', async () => {
...
ipcMain.handle('her:normalizePreviewTarget', (_event, target, baseDir) =>
  normalizePreviewTarget(String(target || ''), baseDir ? String(baseDir) : '')
)

ipcMain.handle('her:watchPreviewFile', (_event, url) => watchPreviewFile(String(url || '')))

ipcMain.handle('her:stopPreviewFileWatch', (_event, id) => stopPreviewFileWatch(String(id || '')))

ipcMain.on('her:titlebar-theme', (_event, payload) => {
...
ipcMain.handle('her:openExternal', (_event, url) => {
...
ipcMain.handle('her:setting:defaultProjectDir:get', async () => ({
  ...
  defaultLabel: path.join(app.getPath('home'), 'her-projects')
}))

ipcMain.handle('her:setting:defaultProjectDir:set', async (_event, dir) => {
...
ipcMain.handle('her:setting:defaultProjectDir:pick', async () => {
...
ipcMain.handle('her:fetchLinkTitle', (_event, url) => fetchLinkTitle(url))

ipcMain.handle('her:logs:reveal', async () => {
  try {
    await fs.promises.mkdir(path.dirname(DESKTOP_LOG_PATH), { recursive: true })
...
ipcMain.handle('her:logs:recent', async () => ({ path: DESKTOP_LOG_PATH, lines: herLog.slice(-200) }))
...
ipcMain.handle('her:fs:readDir', async (_event, dirPath) => {
...
ipcMain.handle('her:fs:gitRoot', async (_event, startPath) => {
...
ipcMain.handle('her:terminal:write', (_event, id, data) => {
...
ipcMain.handle('her:terminal:resize', (_event, id, size = {}) => {
  const sessionInfo = terminalSessions.get(String(id || ''))
...
ipcMain.handle('her:terminal:dispose', (_event, id) => disposeTerminalSession(String(id || '')))

ipcMain.handle('her:updates:check', async () =>
...
ipcMain.handle('her:updates:apply', async (_event, payload) =>
...
ipcMain.handle('her:updates:branch:get', async () => readDesktopUpdateConfig())

ipcMain.handle('her:updates:branch:set', async (_event, name) => {
...
// the actual removal via `her uninstall …` so the cross-platform PATH /
// registry / service / node-symlink cleanup all lives in one place
// (her_cli/uninstall.py + her_cli/gui_uninstall.py).
//
...
async function getUninstallSummary() {
  ...
  return new Promise(resolve => {
    ...
    try {
      const child = spawn(py, ['-m', 'her_cli.main', 'uninstall', '--gui-summary'], {
        cwd: agentRoot,
        env: { ...process.env, HER_HOME, NO_COLOR: '1' },
        stdio: ['ignore', 'pipe', 'ignore']
      })
      child.stdout.on('data', chunk => {
...
async function runDesktopUninstall(mode) {
  ...
  // Python with PYTHONPATH=<agentRoot> so `import her_cli` resolves from
  ...
  try {
    if (IS_WINDOWS) {
      scriptPath = path.join(app.getPath('temp'), `her-uninstall-${Date.now()}.cmd`)
      ...
      scriptPath = path.join(app.getPath('temp'), `her-uninstall-${Date.now()}.sh`)
...
ipcMain.handle('her:uninstall:summary', async () => getUninstallSummary())
ipcMain.handle('her:uninstall:run', async (_event, payload) => {
