...
const UPDATE_TOAST_SNOOZE_KEY = 'her:update-toast-snooze-until'
...
export function reportBackendContract(contract: number | undefined): void {
  ...
  notify({
    action: { label: translateNow('notifications.updateher'), onClick: () => void applyUpdates() },
    durationMs: 0,
    id: SKEW_TOAST_ID,
    kind: 'warning',
...
export async function refreshDesktopVersion(): Promise<DesktopVersionInfo | null> {
  ...
  try {
    const next = await window.herDesktop?.getVersion?.()
...
export async function checkUpdates(): Promise<DesktopUpdateStatus | null> {
  const bridge = window.herDesktop?.updates
  ...
  try {
    ...
    // The update check pulls the latest her_cli + bundled package metadata
...
export async function applyUpdates(opts: DesktopUpdateApplyOptions = {}): Promise<DesktopUpdateApplyResult> {
  ...
  try {
    ...
    // `her update` themselves. Land on a dedicated manual state so the
    ...
    if (result?.manual) {
      $updateApply.set({
        ...
        applying: false,
        stage: 'manual',
        message: result.command ?? 'her update',
        command: result.command ?? 'her update'
      })
    }
...
function onFocus() {
  ...
  // updated her from another window/CLI between focuses, and About should
