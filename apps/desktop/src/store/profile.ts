...
import { getProfiles, setApiRequestProfile } from '@/her'
...
import type { ProfileInfo } from '@/types/her'
...
// /api/profiles/active `current`). "default" is the root ~/.her. This is the
...
const PROFILE_ORDER_STORAGE_KEY = 'her.desktop.profileOrder'
...
const PROFILE_COLORS_STORAGE_KEY = 'her.desktop.profileColors'
...
export async function refreshActiveProfile(): Promise<void> {
  try {
    const res = await window.herDesktop.api<ActiveProfileResponse>({ path: '/api/profiles/active' })
...
// Persist the choice and relaunch the backend under the new HER_HOME. The
...
export async function switchProfile(name: string): Promise<void> {
  ...
  await window.herDesktop.profile.set(name)
}
...
const SHOW_ALL_PROFILES_STORAGE_KEY = 'her.desktop.showAllProfiles'
...
// Switch to the default (root ~/.her) profile — bound to ⌘1.
...
export function touchActiveGatewayBackend(): void {
  ...
  void window.herDesktop?.touchBackend?.(target).catch(() => undefined)
}
