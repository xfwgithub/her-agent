...
const HER_REF_TYPES = ['file', 'folder', 'url', 'image', 'tool', 'line', 'terminal', 'session'] as const
type HerRefType = (typeof HER_REF_TYPES)[number]
...
const ICON_PATHS: Record<HerRefType, string[]> = {
  file: [
...
const iconPathsFor = (type: string) => ICON_PATHS[type as HerRefType] ?? ICON_FALLBACK
...
const HER_DIRECTIVE_RE = new RegExp(
...
function parseDirectiveText(text: string): Unstable_DirectiveSegment[] {
  const matches = [
    ...
    ...Array.from(text.matchAll(HER_DIRECTIVE_RE)).map(match => {
      ...
      return {
        ...
        type: match[1] || 'file',
        label: shortLabel(match[1] as HerRefType, id),
        id
      }
    })
  ]
...
function shortLabel(type: HerRefType, id: string): string {
  if (type === 'terminal') {
    return id || 'terminal'
  }
...
/**
 * Renders text containing her directives (`@file:...`, `@image:...`) as
