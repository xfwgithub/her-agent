...
export const EMPTY_SELECT_VALUE = '__her_empty__'
...
export const PROVIDER_GROUPS: ProviderPrefix[] = [
  {
    prefix: 'NOUS_',
    name: 'Nous Portal',
    description: 'Hosted her & Nous-trained models',
  ...
  { prefix: 'HER_GEMINI_', name: 'Gemini', priority: 4 },
  {
    prefix: 'DEEPSEEK_',
    name: 'DeepSeek',
  ...
  { prefix: 'HER_QWEN_', name: 'DashScope (Qwen)', priority: 6 },
  {
    prefix: 'GLM_',
    name: 'GLM / Z.AI',
...
export const FIELD_DESCRIPTIONS: Record<string, string> = defineFieldCopy({
  ...
  timezone: 'Used when her needs local time context. Blank uses the system timezone.',
  agent: {
    imageInputMode: 'Controls how image attachments are sent to the model.',
    maxTurns: 'Upper bound for tool-calling turns before her stops a run.'
  },
  terminal: {
  ...
  fileReadMaxChars: 'Maximum characters her can read from one file request.',
  approvals: {
    mode: 'How her handles commands that need explicit approval.',
  ...
  updates: {
    nonInteractiveLocalChanges:
      'When her updates itself from the app (no terminal prompt), keep local source edits (stash) or throw them away (discard). Terminal updates always ask.'
  }
})
