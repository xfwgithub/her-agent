...
export function GatewaySettings() {
  ...
  const save = async (apply: boolean) => {
    ...
    try {
      const next = apply
        ? await window.herDesktop.applyConnectionConfig(payload())
        : await window.herDesktop.saveConnectionConfig(payload())
  ...
  const signIn = async () => {
    ...
    try {
      ...
      const saved = await window.herDesktop.saveConnectionConfig({
        mode: state.mode,
        profile: scope ?? undefined,
      ...
      const result = await window.herDesktop.oauthLoginConnectionConfig(trimmedUrl)

      if (result.connected) {
        const refreshed = await window.herDesktop.getConnectionConfig(scope)
        setState(refreshed)
  ...
  const signOut = async () => {
    ...
    try {
      await window.herDesktop.oauthLogoutConnectionConfig(trimmedUrl || undefined)
      const refreshed = await window.herDesktop.getConnectionConfig(scope)
      setState(refreshed)
  ...
  const testRemote = async () => {
    ...
    try {
      const result = await window.herDesktop.testConnectionConfig({
        mode: 'remote',
        profile: scope ?? undefined,
        remoteAuthMode: authMode,
        remoteToken: authMode === 'token' ? remoteToken.trim() || undefined : undefined,
  ...
  if (!window.herDesktop?.getConnectionConfig) {
    return (
      <EmptyState
  ...
  return (
    <SettingsContent>
        ...
        <ListRow
          action={
            <Input
              ...
              placeholder="https://gateway.example.com/her"
      ...
      <div className="mt-6 grid gap-1">
        <ListRow
          action={
            <Button onClick={() => void window.herDesktop?.revealLogs()} size="sm" variant="textStrong">
              <FileText />
              {g.openLogs}
            </Button>
          }
