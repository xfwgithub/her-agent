...
import { getherConfigRecord, type HerConfigRecord, saveherConfig } from '@/her'
...
export interface I18nConfigClient {
  getConfig: () => Promise<HerConfigRecord>
  saveConfig: (config: HerConfigRecord) => Promise<{ ok: boolean }>
}

const defaultConfigClient: I18nConfigClient = {
  getConfig: () => {
    if (typeof window === 'undefined' || !window.herDesktop?.api) {
      return Promise.resolve({})
    }

    return getherConfigRecord()
  },
  saveConfig: config => {
    if (typeof window === 'undefined' || !window.herDesktop?.api) {
...
export function getConfigDisplayLanguage(config: HerConfigRecord): unknown {
  return isRecord(config.display) ? config.display.language : undefined
}

export function withConfigDisplayLanguage(config: HerConfigRecord, locale: Locale): HerConfigRecord {
