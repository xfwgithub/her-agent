...
## 5. GODMODE FAST — her 4 405B (Zero Refusal)

**Model:** `nousresearch/her-4-405b`
**Strategy:** her models are already uncensored, so this combo is about speed — instant token streaming with zero refusal checking needed. Uses the same Pliny Love divider format as a formality.
...
## Using with her Agent

### As ephemeral system prompt (config.yaml)

Pick a template above and set it in `~/.her/config.yaml`:
