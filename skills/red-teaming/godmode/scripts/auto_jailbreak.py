...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("HER_HOME", os.path.expanduser("~/.her")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("HER_HOME", Path.home() / ".her")) / "skills" / "red-teaming" / "godmode"
...
HER_HOME = Path(os.getenv("HER_HOME", Path.home() / ".her"))
CONFIG_PATH = HER_HOME / "config.yaml"
PREFILL_PATH = HER_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/her models — already uncensored, just needs clean prompt
    "her": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "her" in model_lower or "nous" in model_lower:
        return "her"
...
def _get_current_model() -> tuple:
    """Read current model and provider from her config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.her/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart her for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart her for changes to take effect.")
