...
HER_HOME_DIR="${HER_HOME:-$HOME/.her}"
HER_CFG="${HER_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure her config has twozero_td MCP entry ──
if [[ ! -f "$HER_CFG" ]]; then
    echo -e " ${FAIL} her config not found at ${HER_CFG}"
    manual_steps+=("Create ${HER_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$HER_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in her config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to her config..."
    python3 -c "
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${HER_CFG} manually"); }
    manual_steps+=("Restart her session to pick up config change")
fi
