...
SSE streaming. By exposing this endpoint, her-agent becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  her-agent   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.her/.env`
2. Run `her gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with her-agent through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns her-agent as a model) |
...
Authorization: Bearer her-api-key-here
...
But her-agent sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "her-agent"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured her-agent model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of her-agent's core prompt
...
Once implemented, her-agent works as a drop-in backend for:
