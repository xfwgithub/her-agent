"""Anthropic Messages API adapter for her Agent.

Translates between her's internal OpenAI-style message format and
...
# her effort → Anthropic adaptive-thinking effort (output_config.effort).
...
def _prefer_refreshable_claude_code_token(env_token: str, creds: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    her historically persisted setup tokens into ANTHROPIC_TOKEN. That makes
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    Priority:
      1. ANTHROPIC_TOKEN env var (OAuth/setup token saved by her)
    ...
    # 1. her-managed OAuth/setup token env var
    ...
    # This remains as a compatibility fallback for pre-migration her configs.
...
# ── her-native PKCE OAuth flow ────────────────────────────────────────
# Mirrors the flow used by Claude Code, pi-ai, and OpenCode.
# Stores credentials in ~/.her/.anthropic_oauth.json (our own file).

_OAUTH_CLIENT_ID = "9d1c250a-e61b-44d9-88ed-5944d1962f5e"
_OAUTH_TOKEN_URL = "https://console.anthropic.com/v1/oauth/token"
_OAUTH_REDIRECT_URI = "https://console.anthropic.com/oauth/code/callback"
_OAUTH_SCOPES = "org:create_api_key user:profile user:inference"
_HER_OAUTH_FILE = get_her_home() / ".anthropic_oauth.json"
...
def run_her_oauth_login_pure() -> Optional[Dict[str, Any]]:
    """Run her-native OAuth PKCE flow and return credential state."""
    import secrets
    import time
    import webbrowser
    ...
    print()
    print("Authorize her with your Claude Pro/Max subscription.")
    print()
    ...
    try:
        from her_cli.auth import _can_open_graphical_browser as _can_open_gui
    except Exception:
...
def read_her_oauth_credentials() -> Optional[Dict[str, Any]]:
    """Read her-managed OAuth credentials from ~/.her/.anthropic_oauth.json."""
    if _HER_OAUTH_FILE.exists():
        try:
            data = json.loads(_HER_OAUTH_FILE.read_text(encoding="utf-8"))
            if data.get("accessToken"):
                return data
        except (json.JSONDecodeError, OSError, IOError) as e:
            logger.debug("Failed to read her OAuth credentials: %s", e)
    return None
...
def _convert_assistant_message(m: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # can validate the message history.  See her-agent#13848.
    #
...
def _strip_orphaned_tool_blocks(result: List[Dict[str, Any]]) -> None:
    ...
    for m in result:
        if m["role"] == "assistant" and isinstance(m["content"], list):
            ...
            # replaying it verbatim.  See her-agent: extended-thinking + parallel
...
def _manage_thinking_signatures(
    ...
    replayed assistant tool-call messages.  See her-agent#13848 (Kimi) and
    her-agent#16748 (DeepSeek).
...
def build_anthropic_kwargs(
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                text = block.get("text", "")
                text = text.replace("her Agent", "Claude Code")
                text = text.replace("her agent", "Claude Code")
                text = text.replace("her-agent", "claude-code")
    ...
    # silently hides reasoning text that her surfaces in its CLI. We
