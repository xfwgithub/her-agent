...
Every her request that hits the Nous Portal — main agent loop, auxiliary
client (compression / titles / vision / web_extract / session_search / etc.),
and any future code path — must carry the same product-attribution tags so
Nous can attribute usage to her Agent and bucket it by client release.

Tag shape (sent in OpenAI-compatible ``extra_body['tags']``):

    [
        "product=her-agent",
        "client=her-client-v<__version__>",
    ]

The version is sourced live from ``her_cli.__version__`` so it auto-aligns
...
``her_cli.__version__`` is the canonical source of truth.
"""
...
def _her_version() -> str:
    """Return the current her release version, e.g. ``"0.13.0"``.

    Falls back to ``"unknown"`` if ``her_cli`` cannot be imported (should
...
def her_client_tag() -> str:
    """Return the ``client=...`` tag for Nous Portal requests.

    Format: ``client=her-client-v<MAJOR>.<MINOR>.<PATCH>``.
    """
    return f"client=her-client-v{_her_version()}"
...
def nous_portal_tags() -> List[str]:
    ...
    """
    return ["product=her-agent", her_client_tag()]
