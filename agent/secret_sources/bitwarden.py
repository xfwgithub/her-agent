...
her pulls API keys from Bitwarden Secrets Manager at process startup
so they don't have to live in plaintext in ``~/.her/.env``.

Design summary
--------------

* The ``bws`` binary is auto-installed into ``<her_home>/bin/bws`` on
  first use.  her pins one version (``_BWS_VERSION``) and downloads
...
* The access token is stored in ``~/.her/.env`` as
...
* Pulling secrets is a single ``bws secret list <project_id>
  ...
  ``cache_ttl_seconds`` so back-to-back ``her`` invocations don't
  hammer the API.
* Failures NEVER block her startup.  Missing binary, no network,
...
# In-process cache so repeated load_her_dotenv() calls (CLI startup,
# gateway hot-reload, test suites) don't re-fetch from BSM.
_CacheKey = Tuple[str, str, str]  # (access_token_fingerprint, project_id, server_url)
_CACHE: Dict[_CacheKey, "_CachedFetch"] = {}

# Disk-persisted cache so back-to-back CLI invocations (e.g. `her chat -q ...`
...
# <her_home>/cache/bws_cache.json. The file holds only the secret VALUES,
# never the access token. It's plaintext-equivalent to ~/.her/.env (which
...
def _disk_cache_path(home_path: Optional[Path] = None) -> Path:
    """Return the disk cache path under her_home/cache/.

    `home_path` is what `load_her_dotenv()` already resolved; falling back
    to `$HER_HOME` / `~/.her` keeps direct callers working too.
    """
    if home_path is None:
        home_path = Path(os.getenv("HER_HOME", Path.home() / ".her"))
...
def _her_bin_dir() -> Path:
    """Where her stores its managed binaries.  Profile-aware."""
    from her_constants import get_her_home

    return get_her_home() / "bin"
...
def find_bws(*, install_if_missing: bool = False) -> Optional[Path]:
    ...
    Resolution order:
      1. ``<her_home>/bin/bws``  (our managed copy — preferred)
    ...
    """
    managed = _her_bin_dir() / _platform_binary_name()
...
def install_bws(*, force: bool = False) -> Path:
    ...
    path catch these; the user-facing ``her secrets bitwarden setup``
    surface lets them propagate so the wizard can show a clear error.
    """
    bin_dir = _her_bin_dir()
    ...
    with tempfile.TemporaryDirectory(prefix="her-bws-") as tmpdir:
        tmp = Path(tmpdir)
...
def _http_download(url: str, dest: Path) -> None:
    req = urllib.request.Request(url, headers={"User-Agent": "her-agent"})
    try:
...
def fetch_bitwarden_secrets(
    *,
    ...
    ``<her_home>/cache/bws_cache.json`` (for back-to-back CLI invocations).
...
# ---------------------------------------------------------------------------
# Public entry point — called from her_cli.env_loader
# ---------------------------------------------------------------------------
...
def apply_bitwarden_secrets(
    *,
    ...
    This is the function ``load_her_dotenv()`` calls after the .env
    ...
    if not access_token:
        result.error = (
            ...
            "not set.  Run `her secrets bitwarden setup`."
        )
        return result
    ...
    if not project_id:
        result.error = (
            ...
            "Run `her secrets bitwarden setup`."
        )
        return result
    ...
    if binary is None:
        result.error = (
            ...
            "Run `her secrets bitwarden setup` to install."
        )
        return result
