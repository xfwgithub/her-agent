...
def _her_home_path() -> Path:
    """Resolve the active HER_HOME (profile-aware) without circular imports."""
    try:
        from her_constants import get_her_home  # local import to avoid cycles
        return get_her_home()
    except Exception:
        return Path(os.path.expanduser("~/.her"))


def _her_root_path() -> Path:
    """Resolve the her root dir (always the parent of any profile, never per-profile)."""
    try:
        from her_constants import get_default_her_root  # local import to avoid cycles
...
def build_write_denied_paths(home: str) -> set[str]:
    ...
    her_home = _her_home_path()
    her_root = _her_root_path()
    return {
        os.path.realpath(p)
        for p in [
            ...
            str(her_home / ".anthropic_oauth.json"),
            # Top-level Anthropic PKCE credential store remains sensitive even
            # when a profile is active; default/non-profile sessions still read it.
            str(her_root / ".anthropic_oauth.json"),
...
def get_safe_write_root() -> Optional[str]:
    """Return the resolved HER_WRITE_SAFE_ROOT path, or None if unset."""
    root = os.getenv("HER_WRITE_SAFE_ROOT", "")
    if not root:
        return None
    try:
...
def is_write_denied(path: str) -> bool:
    ...
    # her control-plane files: block both the ACTIVE profile's view
    # (her_home) AND the global root view. Without the root pass, a
    ...
    her_dirs = []
    for base in (_her_home_path(), _her_root_path()):
        try:
            real = os.path.realpath(base)
            if real not in her_dirs:
                her_dirs.append(real)
        except Exception:
            continue

    for base_real in her_dirs:
...
def get_read_block_error(path: str) -> Optional[str]:
    """Return an error message when a read targets a denied her path.

    Three categories are blocked:

      * Internal her cache files under ``HER_HOME/skills/.hub`` —
        readable metadata that an attacker could use as a prompt-injection
        carrier.
      * Credential / secret stores under HER_HOME and the global her
    ...
    **This is NOT a security boundary.** The terminal tool runs as the
    ...
    or ``cat ~/.her/.env`` and exfiltrate the file. The read-deny exists
    ...
    # Resolve BOTH the active HER_HOME (profile-aware) AND the global
    # her root so credential stores at <root>/auth.json etc. are also
    # blocked when running under a profile (HER_HOME points at
    ...
    for hd in her_dirs:
        ...
        for blocked in blocked_dirs:
            try:
                ...
                continue
            return (
                f"Access denied: {path} is an internal her cache file "
    ...
    for hd in her_dirs:
        for name in credential_file_names:
            ...
            if resolved == blocked:
                return (
                    f"Access denied: {path} is a her credential store "
    ...
    for hd in her_dirs:
        ...
        if resolved == mcp_tokens:
            return (
                f"Access denied: {path} is the her MCP token directory "
        ...
        try:
            ...
            continue
        return (
            f"Access denied: {path} is a her MCP token file "
            "and cannot be read directly. (Defense-in-depth — not a "
            "security boundary; the terminal tool can still bypass.)"
        )
...
#
# her profiles are separate HER_HOME dirs under
...
#
# Reference: May 2026 incident where a her-security profile session
# edited skills under both ``~/.her/profiles/her-security/skills/``
# AND ``~/.her/skills/`` (the default profile's skills) without realizing
# the second path belonged to a different profile.
# ---------------------------------------------------------------------------

# Profile-scoped directories under HER_HOME / <root> / <root>/profiles/<X>/
...
def _resolve_active_profile_name() -> str:
    """Return the active profile name derived from HER_HOME.
    ...
    """
    try:
        home_real = _her_home_path().resolve()
        root_real = _her_root_path().resolve()
...
def classify_cross_profile_target(path: str) -> Optional[dict]:
    ...
    Returns ``None`` when the target is outside her scope, or is inside
...
def get_cross_profile_warning(path: str) -> Optional[str]:
    ...
    her entirely. Caller is expected to surface the warning to the
    ...
    return (
        ...
        f"belongs to her profile {info['target_profile']!r}, but the "
...
#
#   <HER_HOME>/profiles/<name>/sandboxes/<backend>/<task>/home/.her/...
#
...
# ``…/sandboxes/<backend>/<task>/home/.her/…`` segment and warns
# regardless of which her profile is active. It does NOT cover the
# inner-container case where the bind mount strips the ``sandboxes/`` prefix
# (the agent's view inside the container is plain ``/root/.her/...``);
...
def _find_sandbox_mirror_segments(parts: tuple) -> Optional[int]:
    """Return the index of the inner ``.her`` part in a sandbox-mirror path.

    Matches ``…/sandboxes/<backend>/<task>/home/.her/…`` and returns the
    index where the inner her-state portion starts. Returns ``None`` for
    paths that do not contain the sandbox-mirror shape.
    """
    for i, part in enumerate(parts):
        if part != "sandboxes":
            continue
        # Need at least: sandboxes / <backend> / <task> / home / .her / <thing>
        if i + 5 >= len(parts):
            continue
        if parts[i + 3] == "home" and parts[i + 4] == ".her":
            return i + 4
    return None


def classify_sandbox_mirror_target(path: str) -> Optional[dict]:
    """Classify a write target as a sandbox-mirror of authoritative her state.

    Returns ``None`` when the path does not match the sandbox-mirror shape.
    Otherwise returns a dict with:

      * ``target_path``: the resolved path string
      * ``mirror_root``: the ``…/sandboxes/<backend>/<task>/home/.her``
        prefix (so callers can show users which sandbox owns the mirror)
      * ``inner_path``: the portion under the mirror's ``.her`` (what the
        agent likely meant to address on the host)

    Detection is path-shape-only — does not require any her resolver to
    succeed, so it works correctly even when called from contexts where
    HER_HOME resolution would be ambiguous.
    """
    try:
        target = Path(os.path.expanduser(str(path))).resolve()
    except (OSError, RuntimeError):
...
def get_sandbox_mirror_warning(path: str) -> Optional[str]:
    ...
    return (
        ...
        f"Writes here land on a copy that the host her process never "
        f"reads — the authoritative file is likely {info['inner_path']!r} "
        f"under the real HER_HOME. Use the host-side tool for "
...
# full ``…/sandboxes/<backend>/<task>/home/.her/…`` prefix on the host.
# But when file tools execute *inside* the container the bind-mount strips
# that prefix: the agent sees plain ``/root/.her/…``.  The root:root
...
def classify_container_mirror_target(
    ...
    under the mirror prefix. Otherwise returns:
      ...
      * ``inner_path``: portion under the mirror root (what the agent
        likely meant to address in the host HER_HOME)
    """
...
def get_container_mirror_warning(
    ...
    sandbox mirror of authoritative her state.
    ...
    return (
        ...
        f"bind-mounted home — a per-task mirror that the host her "
        f"process never reads. The authoritative file is "
        f"{info['inner_path']!r} under the real HER_HOME. Use the "
