...
HERMES_AGENT_HELP_GUIDANCE = (
    ...
    "information. Load the `her-agent` skill with skill_view(name='her-agent') "
...
KANBAN_GUIDANCE = (
    ...
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `her kanban <verb>` for board operations. Use "
...
def _skills_prompt_snapshot_path() -> Path:
    return get_her_home() / ".skills_prompt_snapshot.json"
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.her/skills/`` directory.  External dirs
...
def build_nous_subscription_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    lines.extend(
        [
            ...
            "Useful commands: her setup, her setup tools, her setup terminal, her status.",
        ]
    )
    return "\n".join(lines)
...
def load_soul_md() -> Optional[str]:
    ...
    """
    try:
        from her_cli.config import ensure_her_home
        ensure_her_home()
    except Exception as e:
        logger.debug("Could not ensure HER_HOME before loading SOUL.md: %s", e)
...
def _load_her_md(cwd_path: Path) -> str:
    ...
    if not her_md_path:
        return ""
    try:
        content = her_md_path.read_text(encoding="utf-8").strip()
        if not content:
            return ""
        content = _strip_yaml_frontmatter(content)
        rel = her_md_path.name
        try:
            rel = str(her_md_path.relative_to(cwd_path))
        except ValueError:
            pass
        content = _scan_context_content(content, rel)
        result = f"## {rel}\n\n{content}"
        return _truncate_content(result, ".her.md")
    except Exception as e:
        logger.debug("Could not read %s: %s", her_md_path, e)
        return ""
...
def build_context_files_prompt(cwd: Optional[str] = None, skip_soul: bool = False) -> str:
    ...
    SOUL.md from HER_HOME is independent and always included when present.
    ...
    # SOUL.md from HER_HOME only — skip when already loaded as identity
    if not skip_soul:
