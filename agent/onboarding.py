...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time her starts and finds ``~/.openclaw/``.

    Points users at ``her claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``her claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to her, run "
        "`her claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `her claw cleanup` (renames it to ~/.openclaw.pre-migration — "
