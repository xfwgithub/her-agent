...
from her_cli.timeouts import get_provider_request_timeout
...
def init_agent(
    ...
    Args:
        ...
        load_soul_identity (bool): If True, still use ~/.her/SOUL.md as the primary
    ...
    agent._cache_ttl = "5m"
    try:
        from her_cli.config import load_config as _load_pc_cfg
    ...
    # both live under ~/.her/logs/.  Idempotent, so gateway mode
    # (which creates a new AIAgent per message) won't duplicate handlers.
    from her_logging import setup_logging, setup_verbose_logging
    setup_logging(her_home=_ra()._her_home)
    ...
    if agent.verbose_logging:
        ...
        # on the fact that her_logging.setup_logging() does not
        ...
        # handler level inside her_logging.py, not here.
        pass
    ...
    if agent.api_mode == "anthropic_messages":
        ...
        if _is_bedrock_anthropic:
            ...
            if agent.provider == "minimax-oauth" and isinstance(effective_key, str) and effective_key:
                try:
                    from her_cli.auth import build_minimax_oauth_token_provider
        ...
        if api_key and base_url:
            ...
            if _routed_client is not None:
                ...
                if not getattr(agent, "_fallback_activated", False):
                    ...
                    raise RuntimeError(
                        "No LLM provider configured. Run `her model` to "
                        "select a provider, or run `her setup` for first-time "
                        "configuration."
                    )
    ...
    # worker (kanban_show tool is present iff HER_KANBAN_TASK is set).
    ...
    agent.logs_dir = her_home / "sessions"
    agent.logs_dir.mkdir(parents=True, exist_ok=True)
    # Per-session JSON snapshot writer (~/.her/sessions/session_{sid}.json)
    ...
    try:
        from her_cli.config import load_config as _load_sess_cfg
        ...
        try:
            ...
            if agent._memory_enabled or agent._user_profile_enabled:
                ...
                agent._memory_store = MemoryStore(
                    ...
                    user_char_limit=mem_config.get("user_char_limit", 256),
                )
            ...
            if _mem_provider_name and _mem_provider_name.strip():
                ...
                if agent._memory_manager.providers:
                    ...
                    try:
                        ...
                        _init_kwargs["agent_workspace"] = "her"
                    except Exception:
                        pass
    ...
    if _config_context_length is None and _custom_providers:
        try:
            from her_cli.config import get_custom_provider_context_length
