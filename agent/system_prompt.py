...
upstream prefix cache warm.  See ``her-agent-dev``'s
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    AIAgent.  her never re-renders parts of this string mid-
    ...
    # Some execution modes (cron) still want HER_HOME persona while keeping
    ...
    # Pointer to the her-agent skill + docs for user questions about her itself.
    stable_parts.append(HER_AGENT_HELP_GUIDANCE)
    ...
    # HER_KANBAN_TASK env var). Normal chat sessions never see
    ...
    # Active-profile hint — names the her profile the agent is running
    # under so it doesn't conflate ~/.her/skills/ (default profile) with
    # ~/.her/profiles/<active>/skills/ (this profile's). Deterministic
    ...
    if active_profile == "default":
        stable_parts.append(
            "Active her profile: default. Other profiles (if any) live "
            "under ~/.her/profiles/<name>/. Each profile has its own "
        ...
        stable_parts.append(
            f"Active her profile: {active_profile}. This session reads "
            f"and writes ~/.her/profiles/{active_profile}/. The default "
            f"profile's data lives at ~/.her/skills/, ~/.her/plugins/, "
            f"~/.her/cron/, ~/.her/memories/ — those belong to a "
    ...
    # Volatile-tier "Curated Context" block — produced by the background
    # ContextCurator (see agent/context_curator.py) when the active
    # conversation grows past ``context_curation.threshold_ratio`` of the
    # model's context window. The block lives on ``agent._curated_block``
    # and is refreshed asynchronously; it is *intentionally* read here
    # at volatile build time so the ``stable`` prefix stays untouched
    # and upstream prefix cache hits survive turn-to-turn.
    _curated_block = getattr(agent, "_curated_block", None)
    if isinstance(_curated_block, str) and _curated_block.strip():
        volatile_parts.append(_curated_block.strip())
...
def build_system_prompt(agent: Any, system_message: Optional[str] = None) -> str:
    ...
    one cached block — her never rebuilds or reinjects parts of it
