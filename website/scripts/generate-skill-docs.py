...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        "",
        "Hermes ships with a large built-in skill library copied into `~/.her/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Hermes also syncs bundled skills on `her update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.her/skills/` tree, it is still shipped with Hermes; restore it with `her skills reset <name> --restore`.",
        "",
