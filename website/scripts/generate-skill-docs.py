...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/NousResearch/her-agent/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    her_meta = (fm.get("metadata") or {}).get("her") or {}
    tags = her_meta.get("tags") or []
    related = her_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `her skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with her Agent"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "her ships with a large built-in skill library copied into `~/.her/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "her also syncs bundled skills on `her update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.her/skills/` tree, it is still shipped with her; restore it with `her skills reset <name> --restore`.",
        "",
