...
**For non-NixOS users**, this only changes the install step. Everything after (`her setup`, `her gateway install`, config editing) works identically to the standard install.

**For NixOS module users**, the entire lifecycle is different: configuration lives in `configuration.nix`, secrets go through sops-nix/agenix, the service is a systemd unit, and CLI config commands are blocked. You manage her the same way you manage any other NixOS service.
:::
...
nix run github:NousResearch/her-agent -- setup
nix run github:NousResearch/her-agent -- chat

# Or install persistently
nix profile install github:NousResearch/her-agent
her setup
her chat
```

After `nix profile install`, `her`, `her-agent`, and `her-acp` are on your PATH. From here, the workflow is identical to the [standard installation](./installation.md) — `her setup` walks you through provider selection, `her gateway install` sets up a launchd (macOS) or systemd user service, and config lives in `~/.her/`.
...
```bash
nix profile install github:NousResearch/her-agent#messaging
```

For all optional extras (voice, all providers, all platforms):

```bash
nix profile install github:NousResearch/her-agent#full
```
...
# /etc/nixos/flake.nix (or your system flake)
{
  inputs = {
    ...
    her-agent.url = "github:NousResearch/her-agent";
  };

  outputs = { nixpkgs, her-agent, ... }: {
    nixosConfigurations.your-host = nixpkgs.lib.nixosSystem {
      system = "x86_64-linux";
      modules = [
        her-agent.nixosModules.default
        ./configuration.nix
      ];
    };
  };
...
{ config, ... }: {
  services.her-agent = {
    ...
    environmentFiles = [ config.sops.secrets."her-env".path ];
    addToSystemPackages = true;
  };
}
```

That's it. `nixos-rebuild switch` creates the `her` user, generates `config.yaml`, wires up secrets, and starts the gateway — a long-running service that connects the agent to messaging platforms (Telegram, Discord, etc.) and listens for incoming messages.
...
```bash
echo "OPENROUTER_API_KEY=sk-or-your-key" | sudo install -m 0600 -o her /dev/stdin /var/lib/her/env
```

```nix
services.her-agent.environmentFiles = [ "/var/lib/her/env" ];
```
:::

:::tip addToSystemPackages
Setting `addToSystemPackages = true` does two things: puts the `her` CLI on your system PATH **and** sets `HER_HOME` system-wide so the interactive CLI shares state (sessions, skills, cron) with the gateway service. Without it, running `her` in your shell creates a separate `~/.her/` directory.
:::

### Container-aware CLI

:::info
When `container.enable = true` and `addToSystemPackages = true`, **every** `her` command on the host automatically routes into the managed container. This means your interactive CLI session runs inside the same environment as the gateway service — with access to all container-installed packages and tools.
...
Users listed in `hostUsers` are automatically added to the `her` group for file permission access.
...
The CLI auto-detects when sudo is needed and uses it transparently. Without this, you'll need to run `sudo her chat` manually.
:::
...
journalctl -u her-agent -f

# If addToSystemPackages is true, test the CLI
her version
her config       # shows the generated config
```
...
```nix
services.her-agent.configFile = /etc/her/config.yaml;
```

This bypasses `settings` entirely — no merge, no generation. The file is copied as-is to `$HER_HOME/config.yaml` on each activation.
...
| Add API keys | `environmentFiles` | `[ config.sops.secrets."her-env".path ]` |
| Give the agent a personality | `${services.her-agent.stateDir}/.her/SOUL.md` | manage the file directly |
...
| Override the her package | `package` | `inputs.her-agent.packages.${system}.default.override { ... }` |
| Change state directory | `stateDir` | `"/opt/her"` |
...
{
  sops = {
    defaultSopsFile = ./secrets/her.yaml;
    age.keyFile = "/home/user/.config/sops/age/keys.txt";
    secrets."her-env" = { format = "yaml"; };
  };

  services.her-agent.environmentFiles = [
    config.sops.secrets."her-env".path
  ];
}
```
...
```yaml
# secrets/her.yaml (encrypted with sops)
her-env: |
...
```nix
{
  age.secrets.her-env.file = ./secrets/her-env.age;

  services.her-agent.environmentFiles = [
    config.age.secrets.her-env.path
  ];
}
```
...
{
  services.her-agent = {
    authFile = config.sops.secrets."her/auth.json".path;
...
:::tip
Environment variables in `env` values are resolved from `$HER_HOME/.env` at runtime. Use `environmentFiles` to inject secrets — never put tokens directly in Nix config.
:::
...
```nix
{
  services.her-agent.mcpServers.remote-api = {
...
```nix
{
  services.her-agent.mcpServers.my-oauth-server = {
...
Tokens are stored in `$HER_HOME/mcp-tokens/<server-name>.json` and persist across restarts and rebuilds.
...
```bash
# Container mode
docker exec -it her-agent \
  her mcp add my-oauth-server --url https://mcp.example.com/mcp --auth oauth

# Native mode
sudo -u her HER_HOME=/var/lib/her/.her \
  her mcp add my-oauth-server --url https://mcp.example.com/mcp --auth oauth
```
...
```bash
her mcp add my-oauth-server --url https://mcp.example.com/mcp --auth oauth
scp ~/.her/mcp-tokens/my-oauth-server{,.client}.json \
    server:/var/lib/her/.her/mcp-tokens/
# Ensure: chown her:her, chmod 0600
```
...
```nix
{
  services.her-agent.mcpServers.analysis = {
    command = "npx";
...
When her runs via the NixOS module, the following CLI commands are **blocked** with a descriptive error pointing you to `configuration.nix`:

| Blocked command | Why |
|---|---|
| `her setup` | Config is declarative — edit `settings` in your Nix config |
| `her config edit` | Config is generated from `settings` |
| `her config set <key> <value>` | Config is generated from `settings` |
| `her gateway install` | The systemd service is managed by NixOS |
| `her gateway uninstall` | The systemd service is managed by NixOS |
...
When container mode is enabled, her runs inside a persistent Ubuntu container with the Nix-built binary bind-mounted read-only from the host:

```
Host                                    Container
────                                    ─────────
/nix/store/...-her-agent-0.1.0  ──►  /nix/store/... (ro)
~/.her -> /var/lib/her/.her       (symlink bridge, per hostUsers)
/var/lib/her/                    ──►  /data/          (rw)
...
The Nix-built binary works inside the Ubuntu container because `/nix/store` is bind-mounted — it brings its own interpreter and all dependencies, so there's no reliance on the container's system libraries. The container entrypoint resolves through a `current-package` symlink: `/data/current-package/bin/her gateway run --replace`. On `nixos-rebuild switch`, only the symlink is updated — the container keeps running.

### What Persists Across What

| Event | Container recreated? | `/data` (state) | `/home/her` | Writable layer (`apt`/`pip`/`npm`) |
|---|---|---|---|---|
| `systemctl restart her-agent` | No | Persists | Persists | Persists |
...
The container is only recreated when its **identity hash** changes. The hash covers: schema version, image, `extraVolumes`, `extraOptions`, and the entrypoint script. Changes to environment variables, settings, documents, or the her package itself do **not** trigger recreation.

:::warning Writable layer loss
When the identity hash changes (image upgrade, new volumes, new container options), the container is destroyed and recreated from a fresh pull of `container.image`. Any `apt install`, `pip install`, or `npm install` packages in the writable layer are lost. State in `/data` and `/home/her` is preserved (these are bind mounts).

If the agent relies on specific packages, consider baking them into a custom image (`container.image = "my-registry/her-base:latest"`) or scripting their installation in the agent's SOUL.md.
:::

### GC Root Protection

The `preStart` script creates a GC root at `${stateDir}/.gc-root` pointing to the current her package. This prevents `nix-collect-garbage` from removing the running binary. If the GC root somehow breaks, restarting the service recreates it.

---

## Plugins

The NixOS module supports declarative plugin installation — no imperative `her plugins install` needed.

### Directory Plugins (`extraPlugins`)

For plugins that are just a source tree with `plugin.yaml` + `__init__.py` (e.g., [her-lcm](https://github.com/stephenschoettler/her-lcm)):

```nix
services.her-agent.extraPlugins = [
...
For pip-packaged plugins that register via `[project.entry-points."her_agent.plugins"]` (e.g., [rtk-her](https://github.com/ogallotti/rtk-her)):

```nix
services.her-agent.extraPythonPackages = [
...
The package's `site-packages` is added to PYTHONPATH in the her wrapper. `importlib.metadata` discovers the entry point at session start.

### Optional Dependency Groups (`extraDependencyGroups`)

For optional extras declared in her-agent's `pyproject.toml`, use `extraDependencyGroups` to include them in the sealed venv at build time. This is required for any extra not in the default `[all]` set — on Nix, runtime installation into the read-only store is not possible.

```nix
# Enable Discord, Telegram, Slack
services.her-agent.extraDependencyGroups = [ "messaging" ];
```
...
```nix
{
  inputs.her-agent.url = "github:NousResearch/her-agent";
  outputs = { her-agent, nixpkgs, ... }: {
    nixpkgs.overlays = [ her-agent.overlays.default ];
    # Then:
...
```nix
services.her-agent.settings.plugins.enabled = [
  "her-lcm"
  "rtk-rewrite"
];
```
...
:::note
A build-time collision check prevents plugin packages from shadowing core her dependencies. If a plugin provides a package already in the sealed venv, `nixos-rebuild` fails with a clear error.
:::
...
|---|---|---|---|
| `enable` | `bool` | `false` | Enable the her-agent service |
| `package` | `package` | `her-agent` | The her-agent package to use |
| `user` | `str` | `"her"` | System user |
| `group` | `str` | `"her"` | System group |
| `createUser` | `bool` | `true` | Auto-create user/group |
| `stateDir` | `str` | `"/var/lib/her"` | State directory (`HER_HOME` parent) |
| `workingDirectory` | `str` | `"${stateDir}/workspace"` | Agent working directory |
| `addToSystemPackages` | `bool` | `false` | Add `her` CLI to system PATH and set `HER_HOME` system-wide |
...
| Option | Type | Default | Description |
|---|---|---|---|
| `environmentFiles` | `listOf str` | `[]` | Paths to env files with secrets. Merged into `$HER_HOME/.env` at activation time |
...
| Option | Type | Default | Description |
|---|---|---|---|
| `extraArgs` | `listOf str` | `[]` | Extra args for `her gateway` |
| `extraPackages` | `listOf package` | `[]` | Extra packages available to the agent. Added to the her user's per-user profile so terminal commands, skills, and cron jobs all see them |
| `extraPlugins` | `listOf package` | `[]` | Directory plugin packages to symlink into `$HER_HOME/plugins/`. Each must contain `plugin.yaml` |
...
| `container.hostUsers` | `listOf str` | `[]` | Interactive users who get a `~/.her` symlink to the service stateDir and are auto-added to the `her` group |
...
```
/var/lib/her/                     # stateDir (owned by her:her, 0750)
├── .her/                         # HER_HOME
...
cd /etc/nixos && nix flake update her-agent
...
```bash
systemctl status her-agent
docker ps -a --filter name=her-agent
docker inspect her-agent --format='{{.State.Status}}'
docker exec -it her-agent bash
docker exec her-agent readlink /data/current-package
docker exec her-agent cat /data/.container-identity
```
...
```bash
sudo systemctl stop her-agent
docker rm -f her-agent
sudo rm /var/lib/her/.container-identity
sudo systemctl start her-agent
```
...
```bash
# Native mode
sudo -u her cat /var/lib/her/.her/.env

# Container mode
docker exec her-agent cat /data/.her/.env
```

### GC Root Verification

```bash
nix-store --query --roots $(docker exec her-agent readlink /data/current-package)
```
...
| `No adapter available for discord` (or telegram/slack) | Messaging deps missing from the sealed Nix venv | Install `#messaging` variant: `nix profile install ...#messaging`. For NixOS module: `extraDependencyGroups = [ "messaging" ]`. Check `journalctl -u her-agent` for `FeatureUnavailable` or `requirements not met` for the underlying error. |
| Container recreated unexpectedly | `extraVolumes`, `extraOptions`, or `image` changed | Expected — writable layer resets. Reinstall packages or use a custom image |
| `her version` shows old version | Container not restarted | `systemctl restart her-agent` |
| Permission denied on `/var/lib/her` | State dir is `0750 her:her` | Use `docker exec` or `sudo -u her` |
| `nix-collect-garbage` removed her | GC root missing | Restart the service (preStart recreates the GC root) |
| `no container with name or ID "her-agent"` (Podman) | Podman rootful container not visible to regular user | Add passwordless sudo for podman (see [Container Mode](#container-mode) section) |
| `unable to find user her` | Container still starting (entrypoint hasn't created user yet) | Wait a few seconds and retry — the CLI retries automatically |
| Tool added via `extraPackages` not found in terminal | Requires `nixos-rebuild switch` to update the per-user profile | Rebuild and restart: `nixos-rebuild switch && systemctl restart her-agent` |
