...
2. **her spins up** a fresh agent session with your prompt
...
her will search the web, read through results, and produce something like:
...
---
3 stories • Sources searched: 8 • Generated by her Agent
```
...
Before creating cron jobs, ensure her has a default model and provider configured globally. If you want a specific job to use different values, set explicit per-job model/provider overrides when creating it.
...
her will create the cron job for you using the unified `cronjob` tool.
...
For faster briefings, tell her to delegate each topic to a sub-agent:
...
her will use `cronjob(action="list")` to find it and `cronjob(action="remove")` to delete it.
