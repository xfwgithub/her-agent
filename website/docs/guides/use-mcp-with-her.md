---
sidebar_position: 6
title: "Use MCP with her"
description: "A practical guide to connecting MCP servers to her Agent, filtering their tools, and using them safely in real workflows"
---

# Use MCP with her

This guide shows how to actually use MCP with her Agent in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native her tool
- you want her to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect her to internal APIs, databases, or company systems without modifying her core

Do not use MCP when:
- a built-in her tool already solves the job well
...
- her remains the agent
- MCP servers contribute tools
- her discovers those tools at startup or reload time
...
If you installed her with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- her banner/status should show MCP integration when configured
- ask her what tools it has available
...
## WSL2: bridge her in WSL to Windows Chrome

This is the practical setup when:

- her runs inside WSL2
...
In this setup, her does **not** connect to Chrome directly. Instead:

- her runs in WSL
- her starts a local stdio MCP server
...
```text
her (WSL) -> MCP stdio bridge -> Windows Chrome
```

### Why this mode is useful

- you keep your real Windows browser profile, cookies, and logins
- her stays in its supported Unix environment (WSL2)
- browser control is exposed as MCP tools instead of relying on her core browser transport
...
```bash
her mcp add chrome-devtools-win --command cmd.exe --args /c npx -y chrome-devtools-mcp@latest --autoConnect --no-usage-statistics
```

After saving the server:

```bash
her mcp test chrome-devtools-win
```

Then start a fresh her session or run:
...
Once loaded, her can use the MCP-prefixed browser tools directly. For example:
...
If her runs in WSL and Chrome runs on Windows, `/browser connect` may fail even though Chrome is open and debuggable.
...
- Start her from a Windows-mounted path like `/mnt/c/Users/<you>` or `/mnt/c/workspace/...` when using Windows stdio executables through MCP.
- If you start her from `/root` or `/home/...`, Windows may emit a `UNC` current-directory warning before the MCP server starts.
...
There are two categories of MCP-exposed functionality in her:
...
So her will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want her to reason over a bounded workspace.
...
Now her can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing her core.
...
Because her now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
