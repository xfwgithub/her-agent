...
sidebar_position: 8
title: "Context Files"
description: "Project context files — .her.md, AGENTS.md, CLAUDE.md, global SOUL.md, and .cursorrules — automatically injected into every conversation"
---
...
Only **one** project context type is loaded per session (first match wins): `.her.md` → `AGENTS.md` → `CLAUDE.md` → `.cursorrules`. **SOUL.md** is always loaded independently as the agent identity (slot #1).
:::
...
1. **Scan working directory** — checks for `.her.md` → `AGENTS.md` → `CLAUDE.md` → `.cursorrules` (first match wins)
