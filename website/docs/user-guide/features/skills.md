...
By default every profile is seeded with the bundled skill catalog, and each `her update` adds any newly bundled skills. If you want a profile with **no bundled skills** — and that stays empty across updates — you have two paths:

**At install time** (applies to the default `~/.her` profile):

```bash
curl -fsSL https://her-agent.nousresearch.com/install.sh | bash -s -- --no-skills
```

**At profile-create time** (named profiles):

```bash
her profile create research --no-skills
```

**On an already-installed profile** (default or named), toggle it at runtime:

```bash
her skills opt-out            # stop future seeding — nothing on disk is touched
her skills opt-out --remove   # also delete UNMODIFIED bundled skills (confirms first)
her skills opt-in --sync      # undo: remove the marker and re-seed now
```
...
Settings are stored under `skills.config` in your config.yaml. `her config migrate` prompts for unconfigured settings, and `her config show` displays them. When a skill loads, its resolved config values are injected into the context so the agent knows the configured values automatically.
...
Bundles live in **`~/.her/skill-bundles/<slug>.yaml`** and look like this:
...
- `description` (optional) — short text shown in `/bundles` and `her bundles list`.
...
# Re-scan ~/.her/skill-bundles/ and report changes
her bundles reload
```
...
- You want to ship a team-wide "task profile" by checking the bundle YAML into a shared dotfiles repo and symlinking it into `~/.her/skill-bundles/`.

A bundle is just a YAML alias — it doesn't install skills for you. The skills themselves must already be present (in `~/.her/skills/` or an external skill directory). Otherwise the bundle invocation just skips the missing ones.
...
Trust level is always `community` — the same security scan runs as for every other source. The URL is stored as the install identifier, so `her skills update` re-fetches from the same URL automatically when you want to refresh.
...
`her skills inspect ...` now also surfaces upstream metadata when available:
- repo URL
...
- Each skill directory must contain a `SKILL.md` with standard [SKILL.md frontmatter](#skillmd-format) (`name`, `description`, plus optional `metadata.her.tags`, `version`, `author`, `platforms`, `metadata.her.config`).
...
If your skills don't live under `skills/` (common when you're adding a `skills/` subtree to an existing project), edit the tap entry in `~/.her/.hub/taps.json`:
...
The `her skills tap add` CLI defaults new taps to `path: "skills/"`; edit the file directly if you need a different path. `her skills tap list` shows the effective path per tap.
...
Taps are stored in `~/.her/.hub/taps.json` (created on demand).
...
:::note Profiles
Each profile has its own `.bundled_manifest` under its own `HER_HOME`, so `her -p coder skills reset <name>` only affects that profile.
:::
