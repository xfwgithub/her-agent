...
her runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. her
...
1. her captures a baseline of current diagnostics for the file.
...
…). her auto-detects the binary on PATH or in
`<HER_HOME>/lsp/bin/`.
...
`node_modules` tree — her installs both packages together when you
...
When `install_strategy: auto`, her installs binaries into
...
shared location — the staging dir is fully her-owned and is
...
Servers are kept alive for the life of the her process. There's
