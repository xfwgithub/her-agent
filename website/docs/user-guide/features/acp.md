...
Hermes runs with a curated `her-acp` toolset designed for editor workflows. It includes:
...
- Configure Hermes provider credentials first with `her model`, or set them in `~/.her/.env` / `~/.her/config.yaml`.
...
or by editing `~/.her/.env`. Registry clients can also trigger Hermes' terminal auth flow, which runs the same interactive provider/model setup.
