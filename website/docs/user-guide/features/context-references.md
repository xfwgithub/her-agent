...
Type `@` followed by a reference to inject content directly into your message. her expands the reference inline and appends the content under an `--- Attached Context ---` section.
...
- her env: `$HER_HOME/.env`

These directories are fully blocked (any file inside):
- `~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.kube/`, `$HER_HOME/skills/.hub/`
