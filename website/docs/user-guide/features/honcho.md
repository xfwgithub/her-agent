...
[Honcho](https://github.com/plastic-labs/honcho) is an AI-native memory backend that adds dialectic reasoning and deep user modeling on top of her's built-in memory system. Instead of simple key-value storage, Honcho maintains a running model of who the user is — their preferences, communication style, goals, and patterns — by reasoning about conversations after they happen.
...
**Multi-agent profiles**: When multiple her instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
her memory setup    # select "honcho" from the provider list
```
...
- `per-session` — each `her` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — her syncs them back at session init.
...
The `her honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). On a fresh install, configure Honcho directly with `her memory setup honcho` (or run `her memory setup` and pick it from the list); the `her honcho` subcommand then appears on the next invocation.

```bash
her memory setup honcho    # Configure Honcho directly (works before activation)
her honcho status          # Connection status, config, and key settings
her honcho setup           # Redirects to `her memory setup` (post-activation alias)
her honcho strategy        # Show or set session strategy (per-session/per-directory/per-repo/global)
her honcho peer            # Show or update peer names + dialectic reasoning level
her honcho mode            # Show or set recall mode (hybrid/context/tools)
her honcho tokens          # Show or set token budget for context and dialectic
her honcho identity        # Seed or show the AI peer's Honcho identity
her honcho sync            # Sync Honcho config to all existing profiles
her honcho peers           # Show peer identities across all profiles
her honcho sessions        # List known Honcho session mappings
her honcho map             # Map current directory to a Honcho session name
her honcho enable          # Enable Honcho for the active profile
her honcho disable         # Disable Honcho for the active profile
her honcho migrate         # Step-by-step migration guide from openclaw-honcho
```

## Migrating from `her honcho`

If you previously used the standalone `her honcho setup`:
...
No re-login or re-setup needed. Run `her memory setup` and select "honcho" — the wizard detects your existing config.
