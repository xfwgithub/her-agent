...
Or set the default skin in `~/.her/config.yaml`:
...
Create YAML files under `~/.her/skins/`. User skins inherit missing values from the built-in `default` skin, so you only need to specify the keys you want to change.
...
- Activates a skin by updating `~/.her/config.yaml`
...
- Built-in skins load from `her_cli/skin_engine.py`.
- Unknown skins automatically fall back to `default`.
- `/skin` updates the active CLI theme immediately for the current session.
- User skins in `~/.her/skins/` take precedence over built-in skins with the same name.
