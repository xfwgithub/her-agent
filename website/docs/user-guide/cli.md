...
sidebar_position: 1
title: "CLI Interface"
description: "Master the her Agent terminal interface — commands, keybindings, personalities, and more"
---

# CLI Interface

her Agent's CLI is a full terminal user interface (TUI) — not a web UI. It features multiline editing, slash-command autocomplete, conversation history, interrupt-and-redirect, and streaming tool output. Built for people who live in the terminal.

:::tip First-time setup
One command — `her setup --portal` — and you're ready to `her chat`. See [Nous Portal](/integrations/nous-portal).
:::

:::tip
her also ships a modern TUI with modal overlays, mouse selection, and non-blocking input. Launch it with `her --tui` — see the [TUI](tui.md) guide.
:::
...
her chat --model "anthropic/claude-sonnet-4"

# With a specific provider
her chat --provider nous        # Use Nous Portal
her chat --provider openrouter  # Force OpenRouter
...
her chat -s github-pr-workflow -q "open a draft PR"

# Resume previous sessions
her --continue             # Resume the most recent CLI session (-c)
her --resume <session_id>  # Resume a specific session by ID (-r)

# Verbose mode (debug output)
her chat --verbose

# Isolated git worktree (for running multiple agents in parallel)
her -w                         # Interactive mode in worktree
her -w -q "Fix issue #123"     # Single query in worktree
```

## Interface Layout

<img className="docs-terminal-figure" src="/docs/img/docs/cli-layout.svg" alt="Stylized preview of the her CLI layout showing the banner, conversation area, and fixed input prompt." />
<p className="docs-figure-caption">The her CLI banner, conversation stream, and fixed input prompt rendered as a stable docs figure instead of fragile text art.</p>
...
| ⚠ YOLO | **YOLO mode warning** — shown whenever `HER_YOLO_MODE` is on (either `her --yolo` at launch or `/yolo` toggled mid-session). Mirrors the banner-line warning so you can't forget you're in auto-approve mode. |
...
When resuming a previous session (`her -c` or `her --resume <id>`), a "Previous Conversation" panel appears between the banner and the input prompt, showing a compact recap of the conversation history. See [Sessions — Conversation Recap on Resume](sessions.md#conversation-recap-on-resume) for details and configuration.
...
| `Ctrl+D` | Exit |
| `Ctrl+Z` | Suspend her to background (Unix only). Run `fg` in the shell to resume. |
...
Type `/` to see the autocomplete dropdown. her supports a large set of CLI slash commands, dynamic skill commands, and user-defined quick commands.
...
| `/voice tts` | Toggle spoken playback for her replies |
...
```bash
her -s her-agent-dev,github-auth
her chat -s github-pr-workflow -s github-auth
```

her loads each named skill into the session prompt before the first turn. The same flag works in interactive mode and single-query mode.

## Skill Slash Commands

Every installed skill in `~/.her/skills/` is automatically registered as a slash command. The skill name becomes the command:
...
You can also define custom personalities in `~/.her/config.yaml`:

```yaml
personalities:
  helpful: "You are a helpful, friendly AI assistant."
  kawaii: "You are a kawaii assistant! Use cute expressions..."
  pirate: "Arrr! Ye be talkin' to Captain her..."
  # Add your own!
```
...
Most terminals send the same byte sequence for `Enter` and `Shift+Enter` by default, so applications cannot distinguish them. her recognises `Shift+Enter` only when the terminal sends a distinct sequence via the [Kitty keyboard protocol](https://sw.kovidgoyal.net/kitty/keyboard-protocol/) or xterm's `modifyOtherKeys` mode.
...
The very first time you press Enter while her is working, her prints a one-line reminder explaining the `/busy` knob (`"(tip) Your message interrupted the current run…"`). It only fires once per install — a flag in `config.yaml` under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
:::

### Suspending to Background

On Unix systems, press **`Ctrl+Z`** to suspend her to the background — just like any terminal process. The shell prints a confirmation:
...
Resume this session with:
  her --resume 20260225_143052_a1b2c3
...
```bash
her --continue                          # Resume the most recent CLI session
her -c                                  # Short form
her -c "my project"                     # Resume a named session (latest in lineage)
her --resume 20260225_143052_a1b2c3     # Resume a specific session by ID
her --resume "refactoring auth"         # Resume by title
her -r 20260225_143052_a1b2c3           # Short form
```

Resuming restores the full conversation history from SQLite. The agent sees all previous messages, tool calls, and responses — just as if you never left.

Use `/title My Session Name` inside a chat to name the current session, or `her sessions rename <id> <title>` from the command line. Use `her sessions list` to browse past sessions.

### Session Storage

CLI sessions are stored in her's SQLite state database under `~/.her/state.db`. The database keeps:
...
her immediately confirms the task and gives you back the prompt:
