...
:::tip
Hermes also ships a modern TUI with modal overlays, mouse selection, and non-blocking input. Launch it with `her --tui` — see the [TUI](tui.md) guide.
:::
...
| ⚠ YOLO | **YOLO mode warning** — shown whenever `HERMES_YOLO_MODE` is on (either `her --yolo` at launch or `/yolo` toggled mid-session). Mirrors the banner-line warning so you can't forget you're in auto-approve mode. |
...
CLI sessions are stored in Hermes's SQLite state database under `~/.her/state.db`. The database keeps:
