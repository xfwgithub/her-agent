...
title: "Git Worktrees"
description: "Run multiple her agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

her Agent is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with her so each session has a clean, isolated working directory.

## Why Use Worktrees with her?

her treats the **current working directory** as the project root:

- CLI: the directory where you run `her` or `her chat`
- Messaging gateways: the directory set by `terminal.cwd` in `~/.her/config.yaml`
...
git worktree add ../repo-feature feature/her-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/her-experiment` checked out in that directory

Now you can `cd` into the new worktree and run her there:

```bash
cd ../repo-feature

# Start her in the worktree
her
```
...
git worktree add ../repo-experiment-a feature/her-a
git worktree add ../repo-experiment-b feature/her-b
```
...
- Works on its own branch (`feature/her-a` vs `feature/her-b`).
...
- her checkpoint data under `~/.her/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per her experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/her-checkpoints-docs`, `feature/her-refactor-tests`.
- **Commit frequently**
...
- **Avoid running her from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `her -w` (Automatic Worktree Mode)

her has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `her/her-<hash>`).
...
```bash
her -w -q "Fix issue #123"
```

For parallel agents, open multiple terminals and run `her -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each her session its own clean checkout.
