...
- **[Web Dashboard](./features/web-dashboard.md)** (`her dashboard`) — a browser admin panel; its optional **Chat** tab embeds the TUI through a pseudo-terminal.
...
The [manual update process](https://her-agent.nousresearch.com/docs/getting-started/updating) also works with the GUI.
...
You can do the same from the terminal — `her uninstall --gui` for the GUI alone, or `her uninstall` / `her uninstall --full` for the agent too.

:::note
Running `her uninstall --gui` from a **source checkout** (a `her desktop` dev build) also removes the workspace `node_modules` and `apps/desktop/{dist,release}` build output, since those are GUI build artifacts. They're recoverable with `her desktop` (or `npm install` + a rebuild) — but if you're actively hacking on the desktop app, expect to reinstall dependencies afterward.
:::

## CLI reference: `her desktop`

To launch via the CLI, simply run `her desktop`. By default it installs workspace Node dependencies, builds the current OS's unpacked Electron app, then launches that packaged artifact.
...
Set a username and password, then start the dashboard bound to a reachable address. The credentials live in `~/.her/.env` (the secrets file, mode 0600):
...
Keep that `her dashboard` process running for as long as you want the desktop app to be able to connect — if it stops, the app can no longer reach the backend. Run it under `systemd`, `tmux`, or your process manager of choice so it survives logout and reboots.
...
Boot logs land in `HER_HOME/logs/desktop.log` (it includes backend output and recent Python tracebacks) — check it first if the app reports a boot failure. You can also tail it from the CLI:
