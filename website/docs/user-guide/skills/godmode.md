...
| `zero_refusal` | her 4 405B | Already uncensored — uses Pliny Love divider as formality |
...
The godmode skill integrates with two her Agent config mechanisms:
...
The `agent.system_prompt` is appended **after** her's own system prompt — it augments, not replaces.
...
```bash
# Load the skill in a her session
/godmode
...
5. **For hard refusals, switch models** — When all techniques fail, ULTRAPLINIAN (racing multiple models) is the practical fallback. her models and Grok are typically least filtered.
...
4. **her models don't need jailbreaking** — `nousresearch/her-3-*` and `her-4-*` are already uncensored. Use them directly.
...
6. **Restart her after auto-jailbreak** — The CLI reads config once at startup. Gateway sessions pick up changes immediately.
...
9. **Gray-area vs hard queries** — Jailbreak techniques work much better on dual-use queries (lock picking, security tools) than overtly harmful ones (phishing, malware). For hard queries, skip to ULTRAPLINIAN or use her/Grok.
