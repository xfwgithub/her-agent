---
title: "Openclaw Migration — Migrate a user's OpenClaw customization footprint into her Agent"
sidebar_label: "Openclaw Migration"
description: "Migrate a user's OpenClaw customization footprint into her Agent"
---
...
Migrate a user's OpenClaw customization footprint into her Agent. Imports her-compatible memories, SOUL.md, command allowlists, user skills, and selected workspace assets from ~/.openclaw, then reports exactly what could not be migrated and why.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `her skills install official/migration/openclaw-migration` |
...
| Related skills | [`her-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-her-agent) |
...
# OpenClaw -> her Migration

Use this skill when a user wants to move their OpenClaw setup into her Agent with minimal manual cleanup.
...
```bash
her claw migrate              # Full interactive migration
...
her claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `her setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_her.py` to:

- import `SOUL.md` into the her home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into her memory entries
- merge OpenClaw command approval patterns into her `command_allowlist`
- migrate her-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS`, and map OpenClaw workspace settings to her working-directory configuration
- copy OpenClaw skills into `~/.her/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen her workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.her/tts/`
- archive non-secret docs that do not have a direct her destination
...
- `scripts/openclaw_to_her.py`

When this skill is installed from the Skills Hub, the normal location is:

- `~/.her/skills/migration/openclaw-migration/scripts/openclaw_to_her.py`

Do not guess a shorter path like `~/.her/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.her/skills/migration/openclaw-migration/`.
...
With `--migrate-secrets`, it will also import a small allowlisted set of her-compatible secrets, currently:
...
her CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in her. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a her workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing her skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
```bash
python3 ~/.her/skills/migration/openclaw-migration/scripts/openclaw_to_her.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python3 /home/USER/.her/skills/migration/openclaw-migration/scripts/openclaw_to_her.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python3 ~/.her/skills/migration/openclaw-migration/scripts/openclaw_to_her.py --preset user-data
```

Execute a user-data migration:

```bash
python3 ~/.her/skills/migration/openclaw-migration/scripts/openclaw_to_her.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python3 ~/.her/skills/migration/openclaw-migration/scripts/openclaw_to_her.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python3 ~/.her/skills/migration/openclaw-migration/scripts/openclaw_to_her.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of her unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty her targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean her destination. Unsupported auth blobs must still be reported as skipped.
...
- her persona state imported
- her memory files populated with converted OpenClaw knowledge
- OpenClaw skills available under `~/.her/skills/openclaw-imports/`
