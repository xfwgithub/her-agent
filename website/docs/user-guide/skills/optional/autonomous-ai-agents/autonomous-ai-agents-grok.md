...
| | |
|---|---|
| Source | Optional — install with `her skills install official/autonomous-ai-agents/grok` |
...
| Related skills | [`codex`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-codex), [`claude-code`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-claude-code), [`her-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-her-agent) |
...
# Grok Build CLI — her Orchestration Guide

Delegate coding tasks to [Grok Build](https://docs.x.ai/build/overview) (xAI's
autonomous coding agent CLI, the `grok` command) via the her terminal. Grok
...
1. Prepare stable input files first with her tools (`read_file`,
...
2. **Don't conflate her' xAI auth with the `grok` CLI's auth.** her'
...
7. **Verify auth before relying on it** — check `~/.grok/auth.json` or run a
   cheap `grok -p "Say ok."` smoke test; don't assume her' xAI auth carries
   over.
