...
sidebar_position: 12
title: "Google Chat"
description: "Set up her Agent as a Google Chat bot using Cloud Pub/Sub"
---

# Google Chat Setup

Connect her Agent to Google Chat as a bot. The integration uses Cloud Pub/Sub
pull subscriptions for inbound events and the Chat REST API for outbound messages.
Equivalent ergonomics to Slack Socket Mode or Telegram long-polling: your her
...
download the file. Save it somewhere only her can read (e.g.,
...
Also grant `Pub/Sub Viewer` on the same subscription — her calls
...
- **App name**: whatever you want users to see ("her" is reasonable).
...
`ADDED_TO_SPACE` event that her uses to cache the bot's own `users/{id}` for
self-message filtering.

---

## Step 9: Configure her
...
Install the dependencies the Google Chat adapter needs (no her extra is currently published — install them directly):
...
Send "hola" in the test DM. The bot posts a "her is thinking…" marker, then
...
Thread support: when a user replies inside a thread, her detects the
`thread.name` and posts its reply in the same thread, so each thread gets a
separate her session.
...
3. Download the JSON. Move it onto the host that runs her.
4. Register the client with her (run under the profile you want it scoped to):
...
That writes the client secret into the active profile's her home (e.g.
...
1. Check the Pub/Sub subscription has undelivered messages in the console.
   If it does, her isn't authenticated — verify `GOOGLE_CHAT_SERVICE_ACCOUNT_JSON`
...
- **Attachment download protection**: her will only attach the SA bearer
