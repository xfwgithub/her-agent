...
description: "Chat with her from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Yuanbao, Microsoft Teams, LINE, Webhooks, or any OpenAI-compatible frontend via the API server — architecture and setup overview"
---

# Messaging Gateway

Chat with her from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Feishu/Lark, WeCom, Weixin, BlueBubbles (iMessage), QQ, Yuanbao, Microsoft Teams, LINE, ntfy, or your browser. The gateway is a single background process that connects to all your configured platforms, handles sessions, runs cron jobs, and delivers voice messages.

For the full voice feature set — including CLI microphone mode, spoken replies in messaging, and Discord voice-channel conversations — see [Voice Mode](/user-guide/features/voice-mode) and [Use Voice Mode with her](/guides/use-voice-mode-with-her).
...
```bash
her gateway setup        # Interactive setup for all messaging platforms
```
...
her gateway setup        # Configure messaging platforms interactively
her gateway install      # Install as a user service (Linux) / launchd service (macOS)
sudo her gateway install --system   # Linux only: install a boot-time system service
her gateway start        # Start the default service
her gateway stop         # Stop the default service
her gateway status       # Check default service status
her gateway status --system         # Linux only: inspect the system service explicitly
```
...
| `/update` | Update her Agent to the latest version |
...
Configure per-platform overrides in `~/.her/gateway.json`:
...
her pairing list          # View pending + approved users
her pairing revoke telegram 123456789  # Remove access
```
...
The first time you message a busy agent on any platform, her appends a one-line reminder to the busy-ack explaining the knob (`"💡 First-time tip — …"`). The reminder fires once per install — a flag under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
...
Control how much tool activity is displayed in `~/.her/config.yaml`:
...
When the agent running a background session uses `terminal(background=true)` to start long-running processes (servers, builds, etc.), the gateway can push status updates to your chat. Control this with `display.background_process_notifications` in `~/.her/config.yaml`:
...
```bash
HER_BACKGROUND_NOTIFICATIONS=result
```
...
```bash
her gateway install               # Install as user service
her gateway start                 # Start the service
her gateway stop                  # Stop the service
her gateway status                # Check status
journalctl --user -u her-gateway -f  # View logs
...
Avoid keeping both the user and system gateway units installed at once unless you really mean to. her will warn if it detects both because start/stop/status behavior gets ambiguous.

:::info Multiple installations
If you run multiple her installations on the same machine (with different `HER_HOME` directories), each gets its own systemd service name. The default `~/.her` uses `her-gateway`; other installations use `her-gateway-<hash>`. The `her gateway` commands automatically target the correct service for your current `HER_HOME`.
:::

### macOS (launchd)

```bash
her gateway install               # Install as launchd agent
her gateway start                 # Start the service
her gateway stop                  # Stop the service
her gateway status                # Check status
tail -f ~/.her/logs/gateway.log   # View logs
```

The generated plist lives at `~/Library/LaunchAgents/ai.her.gateway.plist`. It includes three environment variables:
...
- **HER_HOME** — scopes the gateway to your her installation.

:::tip PATH changes after install
launchd plists are static — if you install new tools (e.g. a new Node.js version via nvm, or ffmpeg via Homebrew) after setting up the gateway, run `her gateway install` again to capture the updated PATH. The gateway will detect the stale plist and reload automatically.
:::

:::info Multiple installations
Like the Linux systemd service, each `HER_HOME` directory gets its own launchd label. The default `~/.her` uses `ai.her.gateway`; other installations use `ai.her.gateway-<suffix>`.
:::
...
|----------|---------|--------------|
| CLI | `her-cli` | Full access |
| Telegram | `her-telegram` | Full tools including terminal |
| Discord | `her-discord` | Full tools including terminal |
| WhatsApp | `her-whatsapp` | Full tools including terminal |
| Slack | `her-slack` | Full tools including terminal |
| Google Chat | `her-google_chat` | Full tools including terminal |
| Signal | `her-signal` | Full tools including terminal |
| SMS | `her-sms` | Full tools including terminal |
| Email | `her-email` | Full tools including terminal |
| Home Assistant | `her-homeassistant` | Full tools + HA device control (ha_list_entities, ha_get_state, ha_call_service, ha_list_services) |
| Mattermost | `her-mattermost` | Full tools including terminal |
| Matrix | `her-matrix` | Full tools including terminal |
| DingTalk | `her-dingtalk` | Full tools including terminal |
| Feishu/Lark | `her-feishu` | Full tools including terminal |
| WeCom | `her-wecom` | Full tools including terminal |
| WeCom Callback | `her-wecom-callback` | Full tools including terminal |
| Weixin | `her-weixin` | Full tools including terminal |
| BlueBubbles | `her-bluebubbles` | Full tools including terminal |
| QQBot | `her-qqbot` | Full tools including terminal |
| Yuanbao | `her-yuanbao` | Full tools including terminal |
| Microsoft Teams | `her-teams` | Full tools including terminal |
| API Server | `her-api-server` | Full tools (drops `clarify`, `send_message`, `text_to_speech` — programmatic access doesn't have an interactive user) |
| Webhooks | `her-webhook` | Full tools including terminal |
...
1. **Gateway log** (`~/.her/logs/gateway.log` or the systemd / launchd unit log). Search for the platform name and `circuit breaker`, `paused`, or `disabled`. The trip event includes the failure count and the last error.
