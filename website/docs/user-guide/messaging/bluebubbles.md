...
Connect her to Apple iMessage via [BlueBubbles](https://bluebubbles.app/) — a free, open-source macOS server that bridges iMessage to any device.
...
- Network connectivity between her and the BlueBubbles server
...
By default, her responds to every authorized BlueBubbles/iMessage DM or group message. To make group chats opt-in, enable mention gating:
...
With `require_mention: true`, DMs still work normally, but group-chat messages are ignored unless they match a mention pattern. If you do not configure custom patterns, her uses conservative defaults for `her` and `@her agent` variants.
...
When someone messages your iMessage, her automatically sends them a pairing code. Approve it with:
```bash
...
her will connect to your BlueBubbles server, register a webhook, and start listening for iMessage messages.

## How It Works

```
iMessage → Messages.app → BlueBubbles Server → Webhook → her
her → BlueBubbles REST API → Messages.app → iMessage
```

- **Inbound:** BlueBubbles sends webhook events to a local listener when new messages arrive. No polling — instant delivery.
- **Outbound:** her sends messages via the BlueBubbles REST API.
...
| `BLUEBUBBLES_MENTION_PATTERNS` | No | her wake words | JSON array, newline-separated, or comma-separated regex patterns for group mention matching |
...
### Chat Addressing
You can address chats by email or phone number — her resolves them to BlueBubbles chat GUIDs automatically. No need to use raw GUID format.
