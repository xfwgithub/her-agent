...
The messaging gateway is the long-running process that connects her to 20+ external messaging platforms through a unified architecture.
...
1. `resolve_command()` from `her_cli/commands.py` maps input to canonical name (handles aliases, prefix matching)
...
|--------|-----------------|
| `~/.her/.env` | API keys, bot tokens, platform credentials |
| `~/.her/config.yaml` | Model settings, tool configuration, display options |
...
- **Explicit target delivery** — `send_message` tool specifying `telegram:-1001234567890`, or the [`her send` CLI](/guides/pipe-script-output) wrapping the same tool for shell scripts
...
Hooks are discovered from `gateway/builtin_hooks/` (an extension point — currently empty in the shipped distribution; `_register_builtin_hooks()` is a no-op stub) and `~/.her/hooks/` (user-installed). Each hook is a directory with a `HOOK.yaml` manifest and `handler.py`.
...
- `her gateway start` / `her gateway stop` — manual control
- `systemctl` (Linux) or `launchctl` (macOS) — service management
- PID file at `~/.her/gateway.pid` — profile-scoped process tracking

**Profile-scoped vs global**: `start_gateway()` uses profile-scoped PID files. `her gateway stop` stops only the current profile's gateway. `her gateway stop --all` uses global `ps aux` scanning to kill all gateway processes (used during updates).
