...
Source file: `her_state.py`


## Architecture Overview

```
~/.her/state.db (SQLite, WAL mode)
...
Multiple her processes (gateway + CLI sessions + worktree agents) share one
...
db = SessionDB()                           # Default: ~/.her/state.db
...
Default path: `~/.her/state.db`

This is derived from `her_constants.get_her_home()` which resolves to
`~/.her/` by default, or the value of `HER_HOME` environment variable.
