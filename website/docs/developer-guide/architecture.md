...
sidebar_position: 1
title: "Architecture"
description: "her Agent internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of her Agent internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
│ her_state.py   │              │ Browser (5 backends)  │
...
├── cli.py                    # HerCLI — interactive terminal UI (large file)
├── model_tools.py            # Tool discovery, schema collection, dispatch
├── toolsets.py               # Tool groupings and platform presets
├── her_state.py           # SQLite session/state database with FTS5
├── her_constants.py       # HER_HOME, profile-aware paths
...
│
├── her_cli/               # CLI subcommands and setup
│   ├── main.py               # Entry point — all `her` subcommands (large file)
...
│   ├── skills_config.py      # her skills — enable/disable per platform
│   ├── skills_hub.py         # /skills slash command
│   ├── tools_config.py       # her tools — enable/disable per platform
│   ├── plugins.py            # PluginManager — discovery, loading, hooks
│   ├── callbacks.py          # Terminal callbacks (clarify, sudo, approval)
│   └── gateway.py            # her gateway start/stop
│
...
```text
User input → HerCLI.process_input()
...
Three discovery sources: `~/.her/plugins/` (user), `.her/plugins/` (project), and pip entry points. Plugins register tools, hooks, and CLI commands through a context API. Two specialized plugin types exist: memory providers (`plugins/memory/`) and context engines (`plugins/context_engine/`). Both are single-select — only one of each can be active at a time, configured via `her plugins` or `config.yaml`.

→ [Plugin Guide](/guides/build-a-her-plugin), [Memory Provider Plugin](./memory-provider-plugin.md)
...
Exposes her as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`her -p <name>`) gets its own HER_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
