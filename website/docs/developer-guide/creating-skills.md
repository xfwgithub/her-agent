...
description: "How to create skills for her Agent — SKILL.md format, guidelines, and publishing"
---

# Creating Skills

Skills are the preferred way to add new capabilities to her Agent. They're easier to create than tools, require no code changes to the agent, and can be shared with the community.
...
Use `required_environment_variables` when a skill needs an API key or token. Missing values do **not** hide the skill from discovery. Instead, her prompts for them securely when the skill is loaded in the local CLI.
...
The user can skip setup and keep loading the skill. her never exposes the raw secret value to the model. Gateway and messaging sessions show local setup guidance instead of collecting secrets in-band.
...
When loaded, her checks if these files exist. Missing files trigger `setup_needed`. Existing files are automatically:
...
Prefer stdlib Python, curl, and existing her tools (`web_extract`, `terminal`, `read_file`). If a dependency is needed, document installation steps in the skill.
...
| Token | Replaced with |
|---|---|
| `${HER_SKILL_DIR}` | Absolute path to the skill's directory |
| `${HER_SESSION_ID}` | The active session id (left in place if there is no session) |
...
her can now consume third-party skills from multiple external discovery models:
