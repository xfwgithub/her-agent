...
想让 Hermes 拥有稳定的默认风格？编辑 `~/.her/SOUL.md`（如果使用自定义 Hermes home，则为 `$HER_HOME/SOUL.md`）。Hermes 现在会自动生成一个初始 SOUL 文件，并将该全局文件作为实例级个性来源。

完整说明请参阅 [在 Hermes 中使用 SOUL.md](/guides/use-soul-with-her)。
