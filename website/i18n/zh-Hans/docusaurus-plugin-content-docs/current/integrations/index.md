...
Hermes 开箱即支持多个 AI 推理提供商。使用 `her model` 进行交互式配置，或在 `config.yaml` 中直接设置。
...
- **[插件系统](/user-guide/features/plugins)** — 无需修改核心代码，通过自定义工具、生命周期 hook（钩子）和 CLI 命令扩展 Hermes。插件从 `~/.her/plugins/`、项目本地 `.her/plugins/` 以及通过 pip 安装的入口点自动发现。
- **[构建插件](/guides/build-a-her-plugin)** — 创建包含工具、hook 和 CLI 命令的 Hermes 插件的分步指南。
