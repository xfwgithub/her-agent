...
| `--ignore-user-config` | 忽略 `~/.her/config.yaml`，回退到内置默认值。`.env` 中的凭据仍会加载。 |
| `--ignore-rules` | 跳过 `AGENTS.md`、`SOUL.md`、`.cursorrules`、memory（记忆）和预加载 skill 的自动注入。 |
| `--tui` | 启动 [TUI](../user-guide/tui.md) 而非经典 CLI。等同于 `HER_TUI=1`。 |
...
| 命令 | 用途 |
|---------|---------|
| `her chat` | 与 agent 进行交互式或单次聊天。 |
| `her model` | 交互式选择默认 provider 和模型。 |
| `her fallback` | 管理主模型出错时依次尝试的 fallback provider。 |
| `her gateway` | 运行或管理消息 gateway 服务。 |
| `her proxy` | 本地 OpenAI 兼容代理，附加 OAuth provider 凭据。参见 [订阅代理](../user-guide/features/subscription-proxy.md)。 |
| `her lsp` | 管理 Language Server Protocol 集成（为 write_file/patch 提供语义诊断）。 |
| `her setup` | 全部或部分配置的交互式设置向导。 |
| `her whatsapp` | 配置并配对 WhatsApp 桥接。 |
| `her slack` | Slack 辅助工具（当前功能：生成将每条命令注册为原生斜杠命令的 app manifest）。 |
| `her auth` | 管理凭据——添加、列出、删除、重置、设置策略。处理 Codex/Nous/Anthropic 的 OAuth 流程。 |
| `her login` / `logout` | **已弃用** — 请改用 `her auth`。 |
| `her status` | 显示 agent、auth 和平台状态。 |
| `her cron` | 检查并触发 cron 调度器。 |
| `her kanban` | 多 profile 协作看板（任务、链接、调度器）。 |
| `her webhook` | 管理用于事件驱动激活的动态 webhook 订阅。 |
| `her hooks` | 检查、审批或删除 `config.yaml` 中声明的 shell 脚本 hook。 |
| `her doctor` | 诊断配置和依赖问题。 |
...
| `her skills` | 浏览、安装、发布、审计和配置 skill。 |
...
| `her sessions` | 浏览、导出、修剪、重命名和删除会话。 |
...
| `--ignore-user-config` | 忽略 `~/.her/config.yaml`，使用内置默认值。`.env` 中的凭据仍会加载。适用于隔离的 CI 运行、可复现的 bug 报告和第三方集成。 |
...
### `her -z <prompt>` — 脚本化单次调用

对于程序化调用方（shell 脚本、CI、cron、通过管道传入 prompt 的父进程），`her -z` 是最纯粹的单次入口：**单个 prompt 输入，最终响应文本输出，stdout 和 stderr 上不输出任何其他内容。** 无横幅、无 spinner、无工具预览、无 `Session:` 行——只有 agent 的最终回复纯文本。
...
如果你只配置了 OpenRouter，`/model` 将只显示 OpenRouter 模型。要添加其他 provider（Anthropic、DeepSeek、Copilot 等），请退出会话并从终端运行 `her model`。
:::
...
| `--reset` | 在设置前将配置重置为默认值。 |
| `--reconfigure` | 向后兼容别名——在已有安装上裸运行 `her setup` 现在默认执行此操作。 |
...
关于 gateway 本身的配置，请参阅 [Tool Gateway](../user-guide/features/tool-gateway.md)。关于一键设置路径，请参阅上方的 `her setup --portal`。
...
```bash
her slack manifest              # 将 manifest 打印到 stdout
her slack manifest --write      # 写入 ~/.her/slack-manifest.json
her slack manifest --slashes-only  # 仅输出 features.slash_commands 数组
```
...
`her update` 后重新运行 `her slack manifest --write` 以获取新增命令。


## `her login` / `her logout` *（已弃用）*

:::caution
`her login` 已被移除。请使用 `her auth` 管理 OAuth 凭据，使用 `her model` 选择 provider，或使用 `her setup` 进行完整的交互式设置。
:::
...
her auth                                              # 交互式向导
her auth list                                         # 显示所有池
her auth list openrouter                              # 显示特定 provider
her auth add openrouter --api-key sk-or-v1-xxx        # 添加 API 密钥
her auth add anthropic --type oauth                   # 添加 OAuth 凭据
her auth remove openrouter 2                          # 按索引删除
...
多 profile、多项目协作看板。每个安装可托管多个看板（每个项目、仓库或领域一个）；每个看板是独立的队列，拥有自己的 SQLite 数据库和调度器作用域。新安装从名为 `default` 的单个看板开始，其数据库为 `~/.her/kanban.db`（向后兼容）；其他看板位于 `~/.her/kanban/boards/<slug>/kanban.db`。嵌入在 gateway 中的调度器每次 tick 扫描所有看板。

**全局标志（适用于以下所有操作）：**

| 标志 | 用途 |
|------|---------|
| `--board <slug>` | 操作特定看板。默认为当前看板（通过 `her kanban boards switch`、`HER_KANBAN_BOARD` 环境变量或 `default` 设置）。 |

**这是人工/脚本操作界面。** 调度器生成的 agent worker 通过专用的 `kanban_*` [toolset](/user-guide/features/kanban#how-workers-interact-with-the-board)（`kanban_show`、`kanban_complete`、`kanban_block`、`kanban_create`、`kanban_link`、`kanban_comment`、`kanban_heartbeat`；编排器 profile 还可使用 `kanban_list` 和 `kanban_unblock`）驱动看板，而非调用 `her kanban`。Worker 的环境中固定了 `HER_KANBAN_BOARD`，因此物理上无法看到其他看板。
...
| `boards switch <slug>` / `boards use` | 将 `<slug>` 持久化为活跃看板（写入 `~/.her/kanban/current`）。 |
...
her kanban list                  # 显示 atm10-server 的任务
...
看板解析顺序（优先级从高到低）：`--board <slug>` 标志 → `HER_KANBAN_BOARD` 环境变量 → `~/.her/kanban/current` 文件 → `default`。

所有操作也可作为 gateway 中的斜杠命令使用（`/kanban …`），参数界面相同——包括 `boards` 子命令和 `--board` 标志。

完整设计——与 Cline Kanban / Paperclip / NanoClaw / Gemini Enterprise 的对比、八种协作模式、四个用户故事、并发正确性证明——请参阅仓库中的 `docs/her-kanban-v1-spec.pdf` 或 [Kanban 用户指南](/user-guide/features/kanban)。
...
订阅持久化到 `~/.her/webhook_subscriptions.json`，webhook 适配器无需重启 gateway 即可热重载。
...
:::tip
`her dump` 专为分享而设计。交互式诊断请使用 `her doctor`。可视化概览请使用 `her status`。
:::
...
```bash
her debug share              # 上传调试报告，打印 URL
her debug share --lines 500  # 包含更多日志行
her debug share --expire 30  # 粘贴保留 30 天
her debug share --local      # 在终端打印报告（不上传）
```
...
| 选项 | 说明 |
|--------|-------------|
| `-o`, `--output <path>` | zip 文件的输出路径（默认：`~/her-backup-<timestamp>.zip`）。 |
...
- `her-agent` 代码本身（这是用户数据备份，不是仓库快照）。

### 示例

```bash
her backup                           # 完整备份到 ~/her-backup-*.zip
her backup -o /tmp/her.zip        # 完整备份到指定路径
her backup --quick                   # 仅状态快速快照
her backup --quick --label "pre-upgrade"  # 带标签的快速快照
```
...
检查和管理 `~/.her/checkpoints/` 处的影子 git 存储——会话内 `/rollback` 命令的存储层。可随时安全运行；不需要 agent 正在运行。

| 子命令 | 说明 |
|------------|-------------|
| `status`（默认） | 显示总大小、项目数量和每个项目的详情。裸 `her checkpoints` 等同于此。 |
| `list` | `status` 的别名。 |
...
```bash
her checkpoints                                  # 状态概览
her checkpoints prune --retention-days 3         # 激进清理
her checkpoints prune --max-size-mb 200          # 一次性收紧大小上限
her checkpoints clear-legacy -f                  # 删除 v1 归档目录
her checkpoints clear -f                         # 清除所有内容
```
...
### 示例
```bash
her import ~/her-backup-20260423.zip           # 覆盖现有配置前提示确认
her import ~/her-backup-20260423.zip --force   # 不提示直接覆盖
```
...
her skills install https://sharethis.chat/SKILL.md                     # 直接 URL（单文件 SKILL.md）
her skills install https://example.com/SKILL.md --name my-skill        # frontmatter 无名称时覆盖名称
her skills check
her skills update
her skills config
...
Skill bundle 将多个 skill 归组到一个 `/<bundle-name>` 斜杠命令下。调用 bundle 会将每个引用的 skill 加载到单个合并的用户消息中。存储位置：`~/.her/skill-bundles/<slug>.yaml`。YAML schema 和行为请参阅 [Skill Bundles](../user-guide/features/skills.md#skill-bundles)。
...
| `reload` | 重新扫描 `~/.her/skill-bundles/` 并报告新增/删除的 bundle |
...
| `backup` | 手动对 `~/.her/skills/` 进行 tar.gz 快照（curator 在每次真实运行前也会自动快照） |
| `rollback` | 从快照恢复 `~/.her/skills/`（默认使用最新快照） |
...
在全新安装时，第一次计划运行会延迟一个完整的 `interval_hours`（默认 7 天）——gateway 不会在 `her update` 后的第一次 tick 时立即执行 curator。使用 `her curator run --dry-run` 在此之前预览。
...
| `add` | 选择 provider + 模型（与 `her model` 相同的选择器）并追加到链末尾 |
...
检查 `~/.her/config.yaml` 中声明的 shell 脚本 hook，针对合成 payload 测试它们，并管理 `~/.her/shell-hooks-allowlist.json` 处的首次使用同意许可名单。
...
:::info Provider 特定子命令
当外部 memory provider 处于活跃状态时，它可能会注册自己的顶级 `her <provider>` 命令用于 provider 特定管理（例如 Honcho 激活时的 `her honcho`）。未激活的 provider 不暴露其子命令。运行 `her --help` 查看当前已连接的命令。
:::
...
统一的 plugin 管理——通用 plugin、memory provider 和 context engine 集于一处。不带子命令运行 `her plugins` 会打开包含两个部分的复合交互界面：
...
启动 Web 控制台——基于浏览器的界面，用于管理配置、API 密钥和监控会话。需要 `pip install her-agent[web]`（FastAPI + Uvicorn）。内嵌浏览器 Chat 标签页始终可用，但额外需要 `pty` extra（`pip install 'her-agent[web,pty]'`）以及 POSIX PTY 环境（如 Linux、macOS 或 WSL2）。完整文档请参阅 [Web 控制台](/user-guide/features/web-dashboard)。
...
| `--stop` | — | 停止正在运行的 `her dashboard` 进程并退出。 |
| `--status` | — | 列出正在运行的 `her dashboard` 进程并退出。 |
...
```bash
her update [--check] [--backup] [--restart-gateway]
```

拉取最新的 `her-agent` 代码并在 venv 中重新安装依赖，然后重新运行安装后 hook（MCP 服务器、skill 同步、补全安装）。可在运行中的安装上安全执行。

**pip 安装：** `her update` 自动检测基于 pip 的安装——查询 PyPI 获取最新版本并运行 `pip install --upgrade her-agent`，而非 `git pull`。PyPI 发布跟踪标记版本（主要/次要版本），而非 `main` 上的每个 commit。使用 `--check` 查看是否有更新的 PyPI 版本可用，而不安装。

| 选项 | 说明 |
|--------|-------------|
| `--check` | 并排打印当前 commit 和最新 `origin/main` commit，同步时退出码为 0，落后时为 1。不拉取、不安装、不重启任何内容。 |
| `--backup` | 在拉取前创建 `HER_HOME` 的带标签预更新快照（config、auth、会话、skill、配对数据）。默认**关闭**——之前的始终备份行为在大型主目录上每次更新会增加数分钟。通过 `config.yaml` 中的 `update.backup: true` 永久开启。 |
