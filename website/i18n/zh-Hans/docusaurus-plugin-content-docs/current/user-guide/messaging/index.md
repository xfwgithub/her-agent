...
完整的语音功能集——包括 CLI 麦克风模式、消息中的语音回复以及 Discord 语音频道对话——请参阅 [Voice Mode](/user-guide/features/voice-mode) 和 [Use Voice Mode with Hermes](/guides/use-voice-mode-with-her)。
...
:::info 多个安装
如果你在同一台机器上运行多个 Hermes 安装（使用不同的 `HER_HOME` 目录），每个安装都有自己的 systemd 服务名称。默认的 `~/.her` 使用 `her-gateway`；其他安装使用 `her-gateway-<hash>`。`her gateway` 命令会自动针对当前 `HER_HOME` 对应的正确服务。
:::
