...
:::tip
Hermes 还提供了一个现代 TUI，支持模态覆盖层、鼠标选择和非阻塞输入。使用 `her --tui` 启动——参见 [TUI](tui.md) 指南。
:::
...
| 时长 | 会话已用时间 |
| ⚠ YOLO | **YOLO 模式警告**——当 `HERMES_YOLO_MODE` 开启时显示（通过启动时的 `her --yolo` 或会话中的 `/yolo` 切换）。与横幅行警告保持同步，确保你不会忘记自己处于自动批准模式。 |
...
CLI 会话存储在 Hermes 的 SQLite 状态数据库 `~/.her/state.db` 中。数据库保存：
