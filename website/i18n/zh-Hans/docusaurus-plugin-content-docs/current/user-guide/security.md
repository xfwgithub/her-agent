...
加载后，Hermes 会检查这些文件是否存在于活跃 profile 的 `HER_HOME` 中，并将其注册为挂载：
...
Hermes 内置了一个公告扫描器，用于标记活跃 venv 中与已知受损版本目录匹配的 Python 包（例如 2026 年 5 月的 `mistralai 2.4.6` 供应链投毒事件）。实现位于 `her_cli/security_advisories.py`。
...
许多功能（Mistral TTS、ElevenLabs、Honcho 记忆、Bedrock、Slack、Matrix 等）依赖并非每个用户都需要的 Python 包。Hermes 在首次使用时**懒加载**安装这些包，而非在 `her-agent[all]` 下急切安装。实现位于 `tools/lazy_deps.py`。
