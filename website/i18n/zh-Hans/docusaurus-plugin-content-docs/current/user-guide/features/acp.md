...
Hermes 使用专为编辑器工作流设计的精选 `her-acp` 工具集运行，包括：
...
- 先通过 `her model` 配置 Hermes provider 凭据，或在 `~/.her/.env` / `~/.her/config.yaml` 中设置。
...
或编辑 `~/.her/.env`。Registry 客户端也可以触发 Hermes 的终端认证流程，该流程运行相同的交互式 provider/模型设置。
