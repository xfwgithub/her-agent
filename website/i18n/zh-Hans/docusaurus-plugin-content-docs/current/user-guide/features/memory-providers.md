...
| **AI peer**（`aiPeer`） | 每个 Hermes profile 一个。host key `her` → 默认；其他 profile 使用 `her.<profile>`。 |
...
扫描所有 Hermes profile，为没有 host 块的 profile 创建 host 块，从默认 `her` 块继承设置，并提前创建新的 AI peer。幂等操作——跳过已有 host 块的 profile。
...
- **Profile 范围容器**——在 `container_tag` 中使用 `{identity}`（例如 `her-{identity}` → `her-coder`），按 Hermes profile 隔离记忆
