...
使用 `her computer-use status` 验证安装结果。

**方式二：通过交互式界面启用工具集。**

1. 运行 `her tools`，选择 `🖱️ Computer Use (macOS)` → `cua-driver (background)`。
...
4. 启动启用了该工具集的会话：
   ...
   ```
   或在 `~/.her/config.yaml` 中将 `computer_use` 添加到已启用的工具集列表。
...
如需对每个操作进行确认，可在 `~/.her/config.yaml` 中配置 `approvals.mode: manual`。
...
**`computer_use backend unavailable: cua-driver is not installed`** — 运行 `her computer-use install` 获取 cua-driver 二进制文件，或运行 `her tools` 并启用 Computer Use 工具集。
...
- [通用技能：`macos-computer-use`](https://github.com/NousResearch/her-agent/blob/main/skills/apple/macos-computer-use/SKILL.md)
