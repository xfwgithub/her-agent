...
2. Hermes 将该单一令牌以 `BWS_ACCESS_TOKEN` 的形式存储在 `~/.her/.env` 中。
3. 每次 `her`（或 gateway，或 cron 任务）启动时，在加载 `~/.her/.env` 之后，Hermes 会调用 `bws secret list <project_id>` 并将返回的密钥写入 `os.environ`。
